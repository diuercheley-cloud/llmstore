...
async def _run_once() -> None:
    ...
    if (
        settings.commercial_qos_priority_queue_enabled
        and settings.commercial_qos_priority_queue_mode == "active"
    ):
        ...
        item = await redis_client.blpop(
            settings.async_job_queue_name, timeout=settings.async_worker_block_seconds
        )
        if item is None:
        ...
        try:
            ...
            logger.warning(
                "discarding invalid job id from redis queue",
                extra={"extra_data": {"job_id": raw_job_id}},
            )
            return
    ...
    async with SessionLocal() as session:
        result = await process_generation_job(
            session, job_id, get_inference_proxy(), get_backend_slot_manager()
        )
        if result == "requeue":
            ...
            if settings.commercial_qos_priority_queue_enabled:
                ...
                if job:
                    await redis_client.zadd(
                        ...
                        {str(job_id): float(job.effective_priority or 0)},
                    )
            else:
