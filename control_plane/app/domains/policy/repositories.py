...
class SqlAlchemyPolicyRepository:
    ...
    async def get_policy_by_id(self, policy_id: str) -> PolicyData | None:
        result = await self.db.execute(
            select(DeterministicPolicy).where(DeterministicPolicy.id == policy_id)
        )
    ...
    async def list_policies_by_client(self, client_id: str) -> list[PolicyData]:
        ...
        result = await self.db.execute(
            select(DeterministicPolicy).where(DeterministicPolicy.client_id == client_uuid)
        )
    ...
    async def list_all_policies(self) -> list[PolicyData]:
        ...
        result = await self.db.execute(
            select(DeterministicPolicy).order_by(desc(DeterministicPolicy.created_at))
        )
        policies = result.scalars().all()
        return [self._map_policy(p) for p in policies]
    ...
    async def delete_policy(self, policy_id: str) -> None:
        await self.db.execute(
            delete(DeterministicPolicy).where(DeterministicPolicy.id == policy_id)
        )
        await self.db.flush()
