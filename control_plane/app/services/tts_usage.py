...
async def check_tts_feature_blocked(
...
async def ensure_tts_quota(session: AsyncSession, client: Client, requested_chars: int):
    ...
    if requested_chars > limits["max_chars_per_request"]:
        ...
        raise HTTPException(
            status_code=413,
            detail=f"Request exceeds maximum characters per request ({limits['max_chars_per_request']})",
        )
...
async def get_admin_tts_usage(session: AsyncSession):
    ...
    daily_query = select(func.sum(QuotaCounter.used_tts_chars)).where(
        QuotaCounter.period_start == today, QuotaCounter.period_type == "daily"
    )
    ...
    monthly_query = select(func.sum(QuotaCounter.used_tts_chars)).where(
        QuotaCounter.period_start == this_month, QuotaCounter.period_type == "monthly"
    )
    monthly_total = _result_scalar(await session.execute(monthly_query), 0)

    # Usage by client
    client_usage_query = (
        select(Client.id, Client.name, QuotaCounter.used_tts_chars, QuotaCounter.period_type)
        .join(QuotaCounter, Client.id == QuotaCounter.client_id)
        .where(QuotaCounter.period_start == this_month, QuotaCounter.period_type == "monthly")
    )
    ...
    usage_by_client = [
        {"client_id": str(r.id), "client_name": r.name, "monthly_chars": r.used_tts_chars}
        for r in client_usage_results
    ]

    # Total files and storage (approx from events)
    stats_query = select(
        func.count(TtsUsageEvent.id), func.sum(TtsUsageEvent.audio_size_bytes)
    ).where(
        TtsUsageEvent.created_at >= datetime.combine(this_month, datetime.min.time(), tzinfo=UTC)
    )
