...
class STTStreamService:
    ...
    async def _mock_transcribe(self, audio_chunk: bytes) -> Optional[str]:
    ...
    async def _local_transcribe(self, audio_chunk: bytes) -> Optional[str]:
        """Placeholder for local whisper.cpp."""
        logger.debug(f"Local STT: chunk size {len(audio_chunk)}")
        return None

    async def _deepgram_transcribe(self, audio_chunk: bytes) -> Optional[str]:
        """Placeholder for Deepgram streaming STT."""
        logger.debug(f"Deepgram STT: chunk size {len(audio_chunk)}")
        return None

    async def _whisper_transcribe(self, audio_chunk: bytes) -> Optional[str]:
