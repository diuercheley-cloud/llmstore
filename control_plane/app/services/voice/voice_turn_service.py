...
class VoiceTurnService:
    ...
    async def process_turn(
        ...
        await self._emit_audit_event(voice_session.id, "voice_turn_started", {
        ...
        try:
            ...
            while run.status not in ("completed", "failed", "cancelled"):
                ...
                if (utc_now() - start_time).total_seconds() > timeout:
                    logger.warning(f"Agent run {run.id} timed out for voice session {voice_session.id}")
                    break
            ...
            if run.status == "completed":
                ...
                await self._emit_audit_event(voice_session.id, "voice_turn_completed", {
                    "turn_number": turn_number,
                    "run_id": str(run.id),
                ...
                logger.error(f"Agent run {run.id} failed for voice session {voice_session.id}: {error_msg}")
            ...
            await self._emit_audit_event(voice_session.id, "voice_turn_fallback_used", {
    ...
    async def _get_text_from_memory(self, run_id: uuid.UUID) -> Optional[str]:
        stmt = select(AgentMemoryItem).where(
            AgentMemoryItem.source_run_id == run_id,
            AgentMemoryItem.memory_type == "short_term"
        ).order_by(AgentMemoryItem.created_at.desc()).limit(1)
        res = await self.db.execute(stmt)
        item = res.scalar_one_or_none()
        return item.raw_content if item else None

    async def _get_text_from_thread(self, run_id: uuid.UUID) -> Optional[str]:
        stmt = select(AgentThreadMessage).where(
            AgentThreadMessage.run_id == run_id,
            AgentThreadMessage.role == "assistant"
        ).order_by(AgentThreadMessage.created_at.desc()).limit(1)
        res = await self.db.execute(stmt)
