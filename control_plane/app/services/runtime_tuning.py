...
class RuntimeTuningService:
    ...
    async def get_current_profile(self) -> Optional[RuntimeTuningProfile]:
    ...
    async def generate_recommendations(self, benchmark: RuntimeBenchmarkRun):
        ...
        for rec_data in recommendations:
            rec = RuntimeTuningRecommendation(
    ...
    async def apply_profile(self, profile_name: str, operator_id: str = "system") -> Dict[str, Any]:
        ...
        await self.db.execute(
            update(RuntimeTuningProfile).values(is_active=False)
        )
        ...
        event = RuntimeTuningEvent(
            ...
            metadata_json={"advisory": not os.getenv("RUNTIME_TUNING_APPLY_ENABLED", "false").lower() == "true"}
        )
        self.db.add(event)
    ...
    async def get_recommendations(self) -> List[RuntimeTuningRecommendation]:
        result = await self.db.execute(
            select(RuntimeTuningRecommendation).where(RuntimeTuningRecommendation.status == "pending")
        )
        return result.scalars().all()
