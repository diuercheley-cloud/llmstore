...
class AnomalyDetectionService:
    ...
    async def detect_cost_spike(
        self, agent_id: uuid.UUID, threshold_multiplier: float = 2.0
    ) -> bool:
        now = utc_now()
        ...
        stmt_curr = select(func.sum(AgentRunCosts.estimated_cost_brl)).where(
            AgentRunCosts.agent_id == agent_id, AgentRunCosts.created_at >= last_hour
        )
        ...
        if avg_cost > 0 and curr_cost > (avg_cost * threshold_multiplier):
            logger.warning(
                f"Cost spike detected for agent {agent_id}: {curr_cost} vs avg {avg_cost}"
            )
            return True
    ...
    async def detect_failure_spike(self, agent_id: uuid.UUID, threshold: float = 1.5) -> bool:
        ...
        if avg_failures > 0 and curr_failures > (avg_failures * threshold):
            logger.warning(
                f"Failure spike detected for agent {agent_id}: {curr_failures} vs avg {avg_failures}"
            )
            return True

        return False

    async def detect_latency_degradation(
        self, agent_id: uuid.UUID, threshold_ms: float = 5000
    ) -> bool:
        now = utc_now()
        last_hour = now - timedelta(hours=1)
        ...
        if avg_latency > threshold_ms:
            logger.warning(
                f"Latency degradation for agent {agent_id}: avg {avg_latency}ms > {threshold_ms}ms"
            )
            return True

        return False

    async def detect_token_usage_anomaly(
        self, agent_id: uuid.UUID, threshold_multiplier: float = 3.0
    ) -> bool:
        now = utc_now()
        last_hour = now - timedelta(hours=1)
        prev_24h = now - timedelta(hours=24)
        ...
        if avg_tokens > 0 and curr_tokens > (avg_tokens * threshold_multiplier):
            logger.warning(
                f"Token usage anomaly for agent {agent_id}: {curr_tokens} vs avg {avg_tokens}"
            )
            return True
