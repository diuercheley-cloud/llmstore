...
from app.schemas.backup import BackupManifest, BackupVerificationResult, BackupVerificationEntry
...
class BackupVerificationService:
    ...
    async def verify(
        ...
        from .errors import BackupManifestError, BackupSignatureError, BackupKeyError, BackupValidationError
        ...
        if manifest.key_id and manifest.key_id != self.crypto.key_id:
            details["key_id_mismatch"] = f"Expected '{manifest.key_id}', current is '{self.crypto.key_id}'"
            status = "corrupted"
            ...
            for component in manifest.components:
                ...
                if actual_content is not None:
                    if isinstance(actual_content, dict):
                        ...
                        import json
                        actual_hash = _sha256_bytes(json.dumps(actual_content, sort_keys=True).encode("utf-8"))
                    else:
                ...
                if component_status != "ok":
                    ...
                    BACKUP_VERIFICATION_FAILURE_TOTAL.labels(reason=f"component_mismatch:{component.name}").inc()
