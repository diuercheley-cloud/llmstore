...
from app.schemas.backup import BackupCreateRequest, BackupRestoreRequest, BackupRestoreResult
from sqlalchemy.ext.asyncio import AsyncSession, async_sessionmaker, create_async_engine

from .backup_service import BackupService
from .contracts import PromotionProvider, RollbackProvider, StagingProvider
...
class RestoreStagingService:
    def __init__(
        self,
        db: AsyncSession,
        staging_provider: StagingProvider | None = None,
        promotion_provider: PromotionProvider | None = None,
        ...
        self.promotion_service = promotion_provider or RestorePromotionService(
            db, self.backup_service.repo_root
        )
    ...
    def _write_restore_state(self, payload: dict[str, Any]) -> None:
        ...
        self.restore_state_path.write_text(
    ...
    async def restore_with_staging(
        self, backup_id: str, request: BackupRestoreRequest | None = None, actor: str = "system"
    ) -> BackupRestoreResult:
        start_time = time.time()
        self._log(
            logging.INFO,
            f"Starting restore for backup {backup_id}",
            backup_id=backup_id,
            actor=actor,
        )
        try:
            ...
            if result.status == "restored":
                ...
                self._log(
                    logging.INFO,
                    f"Restore successful for backup {backup_id}",
                    backup_id=backup_id,
                    duration=duration,
                )
                ...
                self._log(
                    logging.WARNING,
                    f"Restore {result.status} for backup {backup_id}",
                    backup_id=backup_id,
            ...
            self._log(
                logging.ERROR,
                f"Restore failed for backup {backup_id}: {e}",
                backup_id=backup_id,
                error_code=error_code,
            )
            raise e

    async def _restore_with_staging_impl(
        self, backup_id: str, request: BackupRestoreRequest | None = None, actor: str = "system"
    ) -> BackupRestoreResult:
        ...
        try:
            staging_db_url, pg_dbname, sqlite_file = await self.staging_logic.setup_staging_db(
                staging_dbname
            )
        ...
        try:
            ...
            staging_session_maker = async_sessionmaker(
                staging_engine, expire_on_commit=False, class_=AsyncSession
            )
                    ...
                    if isinstance(db_part, dict):
                        ...
                        for model in models:
                            for row in db_part.get("tables", {}).get(model.__tablename__, []):
                                await staging_session.merge(
                                    model(**self.backup_service._deserialize_row(model, row))
                                )
            ...
            for entry in parts.get("configs.json", {}).get("files", []):
                ...
                staging_target = staging_config_dir / target.relative_to(
                    self.backup_service.repo_root.resolve()
                )
            ...
            safety_backup = await self.backup_service.create_backup(
                BackupCreateRequest(scope=manifest.scope, backup_type="pre-restore-safety-backup"),
                actor=actor,
            )
            ...
            try:
                await self.promotion_service.promote_database(
                    manifest.scope, parts, sqlite_file, self.backup_service._get_database_provider
                )
                if "configs.json" in parts:
                    self.promotion_service.promote_configs(
                        parts["configs.json"], staging_config_dir
                    )
                ...
                try:
                    ...
                    return await self._fail_restore(
                        ...
                        safety_id,
                        "success",
                        error_code=RestorePromotionError.error_code,
                    )
                    ...
                    return await self._fail_restore(
                        ...
                        plan,
                        validation_report,
                        f"Promotion failed: {e}. Rollback ALSO FAILED: {rb_err}",
                        actor,
                        safety_id,
                        "failed",
                        error_code=RestoreRollbackError.error_code,
                    )
