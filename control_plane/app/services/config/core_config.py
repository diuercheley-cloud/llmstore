...
class CoreConfig(BaseSettings):
    ...
    public_plausible_src: str = Field(
        default="https://plausible.io/js/script.js", alias="PUBLIC_PLAUSIBLE_SRC"
    )
    public_base_url: str = Field(default="", alias="PUBLIC_BASE_URL")
    public_api_enabled: bool = Field(default=False, alias="PUBLIC_API_ENABLED")
    public_signup_rate_limit_per_minute: int = Field(
        default=5, ge=1, le=60, alias="PUBLIC_SIGNUP_RATE_LIMIT_PER_MINUTE"
    )
    ...
    observability_profile: str = Field(
        default="basic", alias="OBSERVABILITY_PROFILE"
    )  # off|basic|full
    commercial_profile: str = Field(
        default="off", alias="COMMERCIAL_PROFILE"
    )  # off|billing|billing_payments
    security_profile: str = Field(
        default="local", alias="SECURITY_PROFILE"
    ...
    lmstudio_base_url: str = Field(
        default="http://192.168.101.1:1234/v1", alias="LMSTUDIO_BASE_URL"
    )
    lmstudio_api_key: str = Field(default="lm-studio", alias="LMSTUDIO_API_KEY")
    lmstudio_chat_model: str = Field(
        default="nvidia/nemotron-3-nano-4b", alias="LMSTUDIO_CHAT_MODEL"
    )
    ...
    managed_control_plane_enabled: bool = Field(
        default=False, alias="MANAGED_CONTROL_PLANE_ENABLED"
    )
    ...
    routing_test_force_local_failure: bool = Field(
        default=False, alias="ROUTING_TEST_FORCE_LOCAL_FAILURE"
    )
    ...
    circuit_breaker_recovery_seconds: int = Field(
        default=20, alias="CIRCUIT_BREAKER_RECOVERY_SECONDS"
    )
    ...
    agent_runtime_service_url: str = Field(
        default="http://agent-runtime:8080", alias="AGENT_RUNTIME_SERVICE_URL"
    )
    agent_runtime_service_token: str = Field(
        default="agent-runtime-secret-token", alias="AGENT_RUNTIME_SERVICE_TOKEN"
    )
    ...
    token_counting_fallback_allowed: bool = Field(
        default=True, alias="TOKEN_COUNTING_FALLBACK_ALLOWED"
    )

    max_request_body_size_bytes: int = Field(
        default=1024 * 1024 * 5, alias="MAX_REQUEST_BODY_SIZE_BYTES"
    )
    ...
    inference_max_completion_tokens: int = Field(
        default=512, alias="INFERENCE_MAX_COMPLETION_TOKENS"
    )
    ...
    real_provider_validation_enabled: bool = Field(
        default=False, alias="REAL_PROVIDER_VALIDATION_ENABLED"
    )
    provider_timeout_seconds: int = Field(
        default=30, ge=1, le=300, alias="PROVIDER_TIMEOUT_SECONDS"
    )
    ...
    real_provider_store_responses: bool = Field(
        default=False, alias="REAL_PROVIDER_STORE_RESPONSES"
    )
    max_global_provider_cost_per_day_brl: float = Field(
        default=0.0, alias="MAX_GLOBAL_PROVIDER_COST_PER_DAY_BRL"
    )
    max_provider_cost_per_day_brl: float = Field(default=0.0, alias="MAX_PROVIDER_COST_PER_DAY_BRL")
    max_client_provider_cost_per_day_brl: float = Field(
        default=0.0, alias="MAX_CLIENT_PROVIDER_COST_PER_DAY_BRL"
    )
    margin_warning_percent: float = Field(default=20.0, alias="MARGIN_WARNING_PERCENT")
    negative_margin_block_mode: str = Field(
        default="report_only", alias="NEGATIVE_MARGIN_BLOCK_MODE"
    )
    ...
    deployment_autoscale_window_sec: int = Field(
        default=60, alias="DEPLOYMENT_AUTOSCALE_WINDOW_SEC"
    )
    ...
    opsgenie_api_url: str = Field(
        default="https://api.opsgenie.com/v2/alerts", alias="OPSGENIE_API_URL"
    )

    content_moderation_enabled: bool = Field(default=True, alias="CONTENT_MODERATION_ENABLED")
    content_moderation_toxicity_threshold: float = Field(
        default=0.7, alias="CONTENT_MODERATION_TOXICITY_THRESHOLD"
    )
    ...
    collab_chat_websocket_enabled: bool = Field(
        default=False, alias="COLLAB_CHAT_WEBSOCKET_ENABLED"
    )
    ...
    web_ide_workspaces_dir: str = Field(
        default="./data/ide_workspaces", alias="WEB_IDE_WORKSPACES_DIR"
    )
