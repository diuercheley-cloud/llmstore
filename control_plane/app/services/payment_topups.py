...
def serialize_topup(intent: WalletTopUpIntent, *, include_payment_data: bool = False) -> dict:
    ...
    if include_payment_data:
        payload["payment_data"] = json.loads(intent.payment_data_json) if intent.payment_data_json else {}
    return payload
...
async def create_topup_intent(
    ...
    *,
    ...
    if amount_brl <= 0:
        raise HTTPException(status_code=400, detail={"code": "invalid_topup_amount", "message": "amount_brl must be positive"})
    ...
    intent = WalletTopUpIntent(
        ...
        payment_data_json=json.dumps(_sanitize_payment_data(result.payment_data), ensure_ascii=True),
    )
    session.add(intent)
    await session.flush()
    return intent
...
async def process_payment_webhook(
    ...
    *,
    ...
    if configured_provider == "disabled":
        raise HTTPException(status_code=503, detail={"code": "payment_provider_disabled", "message": "payment provider is disabled"})
    if configured_provider != provider:
        raise HTTPException(status_code=404, detail={"code": "payment_provider_mismatch", "message": "webhook provider is not configured"})
    body = await request.body()
    _validate_webhook_signature(body, request)
    try:
        raw_payload = json.loads(body.decode("utf-8")) if body else {}
    except json.JSONDecodeError as exc:
        raise HTTPException(status_code=400, detail={"code": "invalid_webhook_json", "message": "invalid JSON payload"}) from exc
    if not isinstance(raw_payload, dict):
        raise HTTPException(status_code=400, detail={"code": "invalid_webhook_payload", "message": "payload must be an object"})
    ...
    existing_event = await _find_existing_webhook_event(session, provider=provider, external_id=webhook.external_id, idempotency_key=webhook.idempotency_key)
    if existing_event is not None:
        return {"status": "already_processed", "event_id": str(existing_event.id)}

    intent = await _find_topup_intent(session, provider=provider, external_id=webhook.external_id, idempotency_key=webhook.idempotency_key)
    if intent is None:
        raise HTTPException(status_code=404, detail={"code": "topup_intent_not_found", "message": "topup intent not found"})

    paid_at = _parse_paid_at(webhook.paid_at) or utc_now() if webhook.status in PAID_STATUSES else None
    ...
    if webhook.status in PAID_STATUSES:
        if webhook.amount_brl != intent.amount_brl:
            raise HTTPException(status_code=400, detail={"code": "topup_amount_mismatch", "message": "webhook amount does not match topup intent"})
        intent.status = "paid"
...
async def _find_existing_webhook_event(
    ...
    *,
    ...
    clauses = [PaymentWebhookEvent.provider == provider, PaymentWebhookEvent.idempotency_key == idempotency_key]
    if external_id:
        clauses = [
            PaymentWebhookEvent.provider == provider,
            or_(PaymentWebhookEvent.external_id == external_id, PaymentWebhookEvent.idempotency_key == idempotency_key),
        ]
    ...
    if external_id:
        return (
            await session.execute(
                select(WalletTopUpIntent).where(
                    WalletTopUpIntent.provider == provider,
                    or_(WalletTopUpIntent.external_id == external_id, WalletTopUpIntent.idempotency_key == idempotency_key),
                )
            )
        ).scalar_one_or_none()
    return (
...
def _validate_webhook_signature(body: bytes, request: Request) -> None:
    ...
    if not hmac.compare_digest(signature, expected):
        raise HTTPException(status_code=401, detail={"code": "invalid_payment_signature", "message": "invalid payment webhook signature"})
