...
class EvaluationService:
    ...
    async def create_dataset(self, name: str, version: str, cases: List[Dict[str, Any]]) -> EvalDataset:
    ...
    async def run_evaluation(self, dataset_id: uuid.UUID, model_name: str, tenant_id: str) -> EvalRun:
        run = EvalRun(
    ...
    async def record_match(self, dataset_id: uuid.UUID, model_a: str, model_b: str, score_a: float, score_b: float):
        winner = None
        if score_a > score_b: winner = model_a
        elif score_b > score_a: winner = model_b
        else: winner = "draw"
    ...
    async def _update_elo(self, model_a: str, model_b: str, score_a: float, score_b: float, k_factor: int = 32):
