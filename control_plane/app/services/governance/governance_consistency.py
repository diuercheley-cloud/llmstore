...
class GovernanceConsistencyService:
    ...
    async def compare_active_policies_across_clusters(
        ...
        for local in local_active:
            sync_result = await db.execute(
                ...
                .order_by(CommercialFederatedPolicySync.created_at.desc())
                .limit(1)
            )
    ...
    async def check_compliance_consistency(
        ...
        for peer in peers:
            ...
            comparisons = await self.compare_active_policies_across_clusters(
                db, peer.peer_cluster_id
            )
        ...
        return {
            ...
            if drift_count == 0 and offline_count == 0
            else "inconsistent",
        }
