...
class DataResidencyService:
    ...
    def configure_policy(self, policy: ResidencyPolicy):
        ...
        logger.info("Residency policy configured for tenant %s: %s", policy.tenant_id, policy.allowed_regions)

    def get_policy(self, tenant_id: str) -> Optional[ResidencyPolicy]:
        return self._policies.get(tenant_id)

    def validate_storage(self, tenant_id: str, data_classification: str, target_region: Optional[str] = None) -> bool:
        ...
        if data_classification == "restricted" and policy.require_sovereign_processing:
            if region != self._current_region:
                logger.warning("Restricted data for tenant %s requires sovereign processing in %s", tenant_id, region)
                return False

        if not policy.is_allowed(region):
            logger.warning("Region %s not allowed for tenant %s data (allowed: %s)", region, tenant_id, policy.allowed_regions)
            return False
    ...
    def validate_backup(self, tenant_id: str, target_region: str) -> bool:
        ...
        if policy.backup_region and target_region != policy.backup_region:
            logger.warning("Backup region %s != configured backup region %s for tenant %s", target_region, policy.backup_region, tenant_id)
            return False
    ...
    def get_restricted_tenants(self) -> List[str]:
        return [tid for tid, p in self._policies.items() if p.data_classification in ("confidential", "restricted")]
...
class DataResidencyMiddleware:
    ...
    async def check_request(self, tenant_id: str, request_region: Optional[str] = None) -> bool:
        region = request_region or self.service._current_region
        return self.service.validate_storage(tenant_id, "internal", region)

    async def check_agent_memory(self, tenant_id: str, memory_data_classification: str = "internal") -> bool:
