...
class InferenceProxy:
    ...
    async def health_backend(self, backend: InferenceBackend) -> dict:
        ...
        if (
            not ok
            and backend.provider in ("openai_compatible", "vllm")
            and health_path == "/health"
        ):
        ...
        BACKEND_LATENCY.labels(backend_name=backend.name, endpoint=health_path).observe(
    ...
    async def list_models(self, base_url: str | None = None, api_key: str | None = None) -> dict:
        ...
        try:
            ...
            if self._should_trip_circuit_breaker(exc):
                ...
                raise HTTPException(
                    status_code=status.HTTP_503_SERVICE_UNAVAILABLE, detail="backend unavailable"
                ) from exc
            if isinstance(exc, httpx.HTTPStatusError):
                raise HTTPException(
                    status_code=exc.response.status_code, detail=self._backend_error_detail(exc)
                ) from exc
            raise HTTPException(
                status_code=status.HTTP_503_SERVICE_UNAVAILABLE, detail="backend unavailable"
            ) from exc
    ...
    def _client_for_backend(self, backend: str, backend_url: str) -> httpx.AsyncClient:
        if backend not in {
            ...
            raise HTTPException(
                status_code=status.HTTP_503_SERVICE_UNAVAILABLE, detail="unsupported model backend"
            )
    ...
    def _sanitize_logged_payload(self, payload: dict) -> dict:
        ...
        if "tool_choice" in logged_payload and isinstance(logged_payload["tool_choice"], dict):
            logged_payload["tool_choice"] = {
                ...
                "function": {
                    "name": (logged_payload["tool_choice"].get("function") or {}).get("name")
                },
            }
    ...
    async def _forward(
        ...
        try:
            await self.circuit_breaker.before_call()
            if manage_slot:
                async with self.queue_manager.slot(
                    plan_code=plan_code, is_admin=is_admin, backend_id=backend_id
                ):
                    if stream:
    ...
    async def _json_forward(
        ...
        if not target_endpoint.startswith("/v1/") and any(
            t in target_endpoint for t in ["chat/completions", "completions", "embeddings"]
        ):
        ...
        if backend == "ollama":
            target_endpoint, request_payload = self._translate_ollama_request(
                endpoint, payload, stream=False
            )
        ...
        for attempt in range(1, self.settings.retry_attempts + 2):
            try:
                ...
                logger.info(
                    f"Forwarding to {client.base_url}{target_endpoint} with headers keys: {list(headers.keys())}"
                )
                ...
                BACKEND_LATENCY.labels(
                    backend_name=backend_name or backend, endpoint=endpoint
                ).observe(elapsed)
                record_inference_latency(
                ...
                if backend == "ollama":
                    response_payload = self._translate_ollama_response(
                        response_payload, endpoint, payload.get("model", "")
                    )
                ...
                try:
                    counting_res = token_counter.count_tokens(
                        "", completion_text, model_name, provider=backend
                    )
                ...
                if "usage" not in response_payload or not isinstance(
                    response_payload["usage"], dict
                ):
                ...
                try:
                    ...
                    if content_to_watermark:
                        wm_res = await watermarker.apply_watermark(
                            content_to_watermark, model_name, run_id
                        )
                ...
                return ForwardResult(
                    response=JSONResponse(
                        status_code=response.status_code, content=response_payload
                    ),
                    backend_name=backend_name,
                    attempts=1,
                    fallback_used=False,
                ...
                retriable = (
                    ...
                    or getattr(exc.response, "status_code", 500) >= 500
                )
        ...
        logger.warning(
            ...
            extra={"extra_data": {"endpoint": endpoint, "error": str(last_error)}},
        )
        if isinstance(last_error, httpx.HTTPStatusError) and last_error.response.status_code < 500:
            raise HTTPException(
                status_code=last_error.response.status_code,
                detail=self._backend_error_detail(last_error),
            )
        raise HTTPException(
            status_code=status.HTTP_503_SERVICE_UNAVAILABLE, detail="data plane unavailable"
        )
    ...
    async def _streaming_forward(
        ...
        request = stream_client.build_request(
            "POST", target_endpoint, json=request_payload, headers=headers
        )
        ...
        try:
            ...
            response.raise_for_status()
            await self.circuit_breaker.record_success()
            BACKEND_LATENCY.labels(backend_name=backend_name or backend, endpoint=endpoint).observe(
                perf_counter() - started
            )
        except (httpx.HTTPStatusError, httpx.TransportError) as exc:
            ...
            try:
                async for line in response.aiter_lines():
                    ...
                    if loop_detected:
                        ...
                        break
                    if backend == "ollama":
                        translated_line = self._translate_ollama_stream_line(
                            line, endpoint, payload.get("model", "")
                        )
                    ...
                    if line.startswith("data: ") and line != "data: [DONE]":
                        try:
                            ...
                            if len(completion_fragments) % 5 == 0 and detect_repetition(
                                accumulated, prompt_template=prompt_template
                            ):
                                loop_detected = True
                ...
                if self._should_trip_circuit_breaker(exc):
                    await self.circuit_breaker.record_failure()
                error_event = {
                    "error": {"message": "stream interrupted", "type": exc.__class__.__name__}
                }
                ...
                if corr_id:
                    try:
                        ...
                        async with SessionLocal() as db_session:
                            stmt_log = select(RequestLog).where(
                                RequestLog.correlation_id == corr_id
                            )
                            ...
                            if req_log:
                                ...
                                stmt_fin = select(RequestFinancial).where(
                                    RequestFinancial.request_log_id == req_log.id
                                )
    ...
    def _translate_ollama_stream_line(
        self, line: str, endpoint: str, model_name: str
    ) -> str | None:
        try:
