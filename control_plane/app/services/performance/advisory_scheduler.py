...
class AdvisoryScheduler:
    ...
    async def get_scheduling_recommendation(
        self, 
        model_id: str, 
        backends: List[BackendPerformanceProfile]
    ) -> SchedulingDecision:
        """
        ...
        vram_available = sum(gpu.total_vram_gb - gpu.used_vram_gb for gpu in best_backend.gpu_resources)
        ...
        for b in backends:
            ...
            if b.throughput_tokens_sec < 10.0 and b.backend_type != "cpu":
                 recommendations.append(OptimizationRecommendation(
                    ...
                    priority="low",
                    target_backend_id=b.backend_id,
                    suggested_config={"speculative_decoding": {"draft_model": "tiny-llama-1.1b"}}
                ))
    ...
    async def simulate_load(
        ...
        if (req.concurrent_requests * vram_per_request) > sum(gpu.total_vram_gb for gpu in target.gpu_resources):
