...
class PluginSBOMService:
    ...
    def generate_sbom(
        ...
        if not plugin_path.exists() or not plugin_path.is_dir():
            raise SBOMGenerationError(
                f"Plugin path does not exist or is not a directory: {plugin_path}"
            )
        ...
        if expected_hash and expected_hash != package_hash:
            raise SBOMGenerationError(
                f"Integrity check failed: expected hash {expected_hash}, got {package_hash}"
            )
        ...
        if not has_python and not has_node:
            raise SBOMGenerationError(
                "Neither Python (pyproject.toml/requirements.txt) nor Node (package.json/package-lock.json) files were found."
            )
        ...
        if package_lock_path.exists():
            try:
                with open(package_lock_path, encoding="utf-8") as f:
                    data = json.load(f)
        ...
        for dep_name, dep_version in sorted(dep_map.items()):
            ...
            components.append({"type": "library", "name": dep_name, "version": clean_ver})
        ...
        cyclonedx_sbom = {
            ...
            "metadata": {
                "component": {
                    ...
                    "licenses": [{"license": {"id": lic}} for lic in licenses] if licenses else [],
                }
            },
    ...
    def validate_sbom(
        ...
        package_hash = None
        try:
            package_hash = sbom_record.dependency_summary_json["metadata"]["component"]["hashes"][
                0
            ]["content"]
