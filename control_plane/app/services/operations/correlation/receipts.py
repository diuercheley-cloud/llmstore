...
def _build_receipt(
    receipt_type: str, immutable_hash: str, payload: Any, version: str = "v1"
) -> dict[str, Any]:
...
def build_graph_summary_receipt(summary: dict[str, Any]) -> dict[str, Any]:
    return _build_receipt(
        ...
        immutable_hash=_sha256(
            _canonical_json(summary.get("nodes", []))
            + _sha256(_canonical_json(summary.get("edges", [])))
        ),
        payload={
