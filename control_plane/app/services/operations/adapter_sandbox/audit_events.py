...
async def log_adapter_manifest_registered(
    session: AsyncSession, client_id: uuid.UUID, manifest_id: uuid.UUID, adapter_name: str
):
    return await log_adapter_sandbox_audit_event(
        session,
        "adapter_manifest_registered",
        client_id,
        {"manifest_id": str(manifest_id), "adapter_name": adapter_name},
    )


async def log_adapter_manifest_blocked(
    session: AsyncSession, client_id: uuid.UUID, manifest_id: uuid.UUID, reason: str
):
    return await log_adapter_sandbox_audit_event(
        session,
        "adapter_manifest_blocked",
        client_id,
        {"manifest_id": str(manifest_id), "reason": reason},
        status="blocked",
    )


async def log_adapter_sandbox_run_prepared(
...
async def log_adapter_sandbox_run_completed(
    session: AsyncSession, client_id: uuid.UUID, run_id: uuid.UUID, status: str
):
    return await log_adapter_sandbox_audit_event(
        session,
        "adapter_sandbox_run_completed",
        client_id,
        {"run_id": str(run_id), "status": status},
    )


async def log_adapter_policy_violation_detected(
    session: AsyncSession, client_id: uuid.UUID, violation_id: uuid.UUID, v_type: str
):
    return await log_adapter_sandbox_audit_event(
        session,
        "adapter_policy_violation_detected",
        client_id,
        {"violation_id": str(violation_id), "type": v_type},
        status="violation",
    )
