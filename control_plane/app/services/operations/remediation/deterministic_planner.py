...
class DeterministicRemediationPlanner:
    ...
    def compute_input_hash(self, inputs: Dict[str, Any]) -> str:
        ...
        return hashlib.sha256(f"{self.version}:{raw}".encode("utf-8")).hexdigest()
    ...
    def explain_plan(self, plan: Dict[str, Any], steps: List[Dict[str, Any]]) -> str:
        ...
        for step in steps:
            explanation.append(f"{step['step_order']}. [{step['action_type'].upper()}] {step['description']}")
        
        explanation.append("")
        explanation.append("Note: This plan is advisory-only and for dry-run purposes. No actual changes will be performed.")
