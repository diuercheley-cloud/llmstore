...
async def log_remediation_plan_proposed(
    session: AsyncSession, client_id: uuid.UUID, plan_id: uuid.UUID, plan_type: str
):
...
async def log_remediation_step_proposed(
    session: AsyncSession, client_id: uuid.UUID, step_id: uuid.UUID, action_type: str
):
    return await log_remediation_audit_event(
        session,
...
async def log_remediation_approval_required(
    session: AsyncSession, client_id: uuid.UUID, plan_id: uuid.UUID, approval_scope: str
):
    return await log_remediation_audit_event(
        session,
...
async def log_remediation_plan_receipt_created(
    session: AsyncSession, client_id: uuid.UUID, receipt_id: uuid.UUID, plan_id: uuid.UUID
):
    return await log_remediation_audit_event(
        session,
