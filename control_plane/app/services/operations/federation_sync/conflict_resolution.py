...
class FederationConflictResolutionService:
    ...
    def resolve_conflict(self, conflict: dict[str, Any], strategy: str) -> FederationConflictResolution:
        conflict_type = conflict["conflict_type"]
        if conflict_type == "lineage_conflict" and strategy != "reject":
            status = "blocked"
            replay_safe = False
        elif conflict_type == "replay_conflict":
            status = "blocked"
            replay_safe = False
            ...
            status = "resolved" if strategy in {"reject", "deterministic_merge", "keep_source", "keep_target"} else "proposed"
            replay_safe = strategy == "deterministic_merge" and conflict_type not in {"lineage_conflict", "replay_conflict"}
        return FederationConflictResolution(
            id=sha256_hex({"kind": "federation_conflict_id", "conflict_type": conflict_type, "strategy": strategy}),
            ...
            immutable_hash=sha256_hex({"kind": "federation_conflict_immutable", "conflict_type": conflict_type, "strategy": strategy, "status": status}),
        )
    ...
    def validate_resolution(self, resolution: FederationConflictResolution) -> dict[str, Any]:
        ...
        return {
            ...
            "blocked": blocked,
            "finalize_blocked": blocked or resolution.resolution_strategy == "manual_review_required",
