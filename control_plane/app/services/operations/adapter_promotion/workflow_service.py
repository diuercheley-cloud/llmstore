...
class AdapterPromotionWorkflowService:
    ...
    async def create_workflow(
        self, registry_entry: SignedAdapterRegistryEntry, target_stage: str
    ) -> AdapterPromotionWorkflow:
        payload = {
    ...
    async def record_gate_results(
        self, workflow: AdapterPromotionWorkflow, gate_results: list[dict[str, Any]]
    ...
    async def propose_transition(
        self, workflow: AdapterPromotionWorkflow, to_stage: str, reason: str = None
    ) -> AdapterPromotionStageTransition:
    ...
    async def promote(
        self, workflow: AdapterPromotionWorkflow, transition: AdapterPromotionStageTransition
    ) -> None:
        ...
        workflow.promotion_status = (
            "promoted" if transition.to_stage == workflow.target_stage else "approved"
        )
        transition.transition_status = "completed"
        await self.session.flush()

    async def rollback(
        self, workflow: AdapterPromotionWorkflow, rollback_to_stage: str, reason: str
    ) -> AdapterPromotionRollback:
        if not reason:
            raise ValueError("Reason is mandatory for rollback")

        immutable_hash = sha256_hex(
            f"rollback_{workflow.id}_{workflow.current_stage}_{rollback_to_stage}"
        )
