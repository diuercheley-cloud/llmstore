...
class AdapterPromotionGateService:
    def evaluate_gates(
        ...
        results.append({
            ...
            "reason": "Entry is in approved status" if is_approved else f"Entry status is {registry_entry.registry_status}",
        ...
        # 4. not_blocklisted
        is_blocked = registry_entry.registry_status == "blocked" or context.get("is_blocklisted", False)
        results.append({
        ...
        results.append({
            ...
            "reason": "Sandbox manifest found" if is_manifest_valid else "Sandbox manifest missing",
            "required": True,
            "blocking": True
        })
        ...
        results.append({
            ...
            "reason": "Sandbox simulation passed" if sandbox_passed else "Sandbox simulation failed or missing",
            "required": True,
            "blocking": True
        })
        ...
        results.append({
            ...
            "reason": "No policy violations found" if no_violations else "Policy violations detected",
            "required": True,
            "blocking": True
        })
        ...
        results.append({
            ...
            "gate_status": "passed" if staging_passed else ("skipped" if target_stage != "production_eligible" else "failed"),
            "reason": "Staging simulation passed" if staging_passed else "Staging simulation required for production eligibility",
        ...
        results.append({
            ...
            "gate_status": "passed" if (target_stage != "production_eligible" or context.get("production_approval_granted", False)) else "failed",
            "reason": "Production approval granted" if context.get("production_approval_granted") else "Approval required for production eligibility",
            "required": target_stage == "production_eligible",
            "blocking": target_stage == "production_eligible"
        })
