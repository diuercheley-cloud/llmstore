...
class ApprovalGatedRemediationExecutor:
    ...
    async def prepare_execution(
        ...
        if idempotency_key:
            stmt = select(RemediationExecution).where(
                RemediationExecution.idempotency_key == idempotency_key
            )
        ...
        execution = RemediationExecution(
            ...
            immutable_hash=compute_deterministic_hash(
                fields={**input_data, "client_id": str(plan.client_id), "idemp": idempotency_key}
            ),
        )
        db.add(execution)
        await db.flush()
        ...
        rb_data = self.rollback_service.build_rollback_plan(
            plan.__dict__, [s.__dict__ for s in steps]
        )
        rollback = RemediationRollbackPlan(
            ...
            dry_run=dry_run,
            immutable_hash=compute_deterministic_hash(
                fields={"execution_id": str(execution.id), "strategy": rb_data["rollback_strategy"]}
            ),
        )
        db.add(rollback)
    ...
    async def execute(
        ...
        if not execution.dry_run:
            await log_remediation_approved(
                db, execution.client_id, execution.id, execution.approved_by or "unknown"
            )
        ...
        for p_step in steps:
            ...
            exec_step = RemediationExecutionStep(
                ...
                immutable_hash=compute_deterministic_hash(
                    fields={
                        ...
                        "plan_step_id": str(p_step.id),
                        "order": p_step.step_order,
                    }
                ),
            )
            db.add(exec_step)
            ...
            if execution.dry_run:
                ...
                await log_remediation_execution_step_simulated(
                    db, execution.client_id, exec_step.id, "success"
                )
            else:
                # Real Execution
                result = await RemediationActionRegistry.execute(
                    p_step.action_type, p_step.__dict__
                )
                ...
                if exec_step.execution_status == "failed":
                    ...
                    exec_step.completed_at = datetime.now(UTC)
                    await log_remediation_failed(
                        db, execution.client_id, execution.id, execution.error_code
                    )
                    await db.commit()
                    await log_remediation_execution_step_executed(
                        db, execution.client_id, exec_step.id, "failed"
                    )
                    return {
                        "execution_id": execution.id,
                        "status": "failed",
