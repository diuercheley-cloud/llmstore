...
def build_pre_execution_receipt(execution: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "immutable_hash": hashlib.sha256(f"{client_id}:pre:{payload_hash}".encode()).hexdigest(),
...
def build_post_execution_receipt(
    execution: dict[str, Any], results: list[dict[str, Any]]
) -> dict[str, Any]:
    """Builds a receipt after execution completes."""
    payload = {"execution": execution, "results": results}
    ...
    return {
        ...
        "client_id": client_id,
        "subject_id": execution.get("id"),
        "immutable_hash": hashlib.sha256(f"{client_id}:post:{payload_hash}".encode()).hexdigest(),
        "payload_hash": payload_hash,
        "deterministic_version": execution.get("deterministic_version", "v1"),
        "dry_run": execution.get("dry_run", True),
...
def build_kill_switch_receipt(state: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "immutable_hash": hashlib.sha256(f"{client_id}:ks:{payload_hash}".encode()).hexdigest(),
        "payload_hash": payload_hash,
...
def build_rollback_plan_receipt(rollback_plan: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "immutable_hash": hashlib.sha256(f"{client_id}:rb:{payload_hash}".encode()).hexdigest(),
        "payload_hash": payload_hash,
        "deterministic_version": "v1",
        "dry_run": True,
