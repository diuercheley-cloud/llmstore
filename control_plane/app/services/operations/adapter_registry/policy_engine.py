...
class AdapterRegistryPolicyEngine:
    def evaluate_manifest(
        self, manifest: AdapterManifest, policy: AdapterRegistryPolicy
    ) -> tuple[bool, str]:
    ...
    def evaluate_registry_entry(
        self, entry: SignedAdapterRegistryEntry, policy: AdapterRegistryPolicy
    ) -> tuple[bool, str]:
        """Evaluates a registry entry against a policy."""
        if policy.require_approval and entry.registry_status not in [
            "approved",
            "revoked",
            "blocked",
            "deprecated",
