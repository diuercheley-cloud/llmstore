...
class SignedAdapterRegistryService:
    ...
    async def submit_entry(self, entry: SignedAdapterRegistryEntry, decided_by: str) -> AdapterRegistryDecision:
        if entry.registry_status != "draft":
            raise ValueError(f"Cannot submit entry in status '{entry.registry_status}'. Must be 'draft'.")
        
        entry.registry_status = "submitted"
        return await self._create_decision(entry, "submit", "accepted", "Submitted for approval", decided_by)

    async def approve_entry(self, entry: SignedAdapterRegistryEntry, approved_by: str) -> AdapterRegistryDecision:
        if entry.registry_status not in ["submitted", "rejected"]:
            raise ValueError(f"Cannot approve entry in status '{entry.registry_status}'. Must be 'submitted' or 'rejected'.")
        
        entry.registry_status = "approved"
        entry.approved_by = approved_by
        entry.approved_at = utc_now()
        return await self._create_decision(entry, "approve", "accepted", "Approved by admin", approved_by)

    async def reject_entry(self, entry: SignedAdapterRegistryEntry, reason: str, decided_by: str) -> AdapterRegistryDecision:
        if not reason:
            raise ValueError("Reason is mandatory for rejection")
        if entry.registry_status != "submitted":
            raise ValueError(f"Cannot reject entry in status '{entry.registry_status}'. Must be 'submitted'.")
            
        entry.registry_status = "rejected"
        return await self._create_decision(entry, "reject", "denied", reason, decided_by)

    async def revoke_entry(self, entry: SignedAdapterRegistryEntry, reason: str, decided_by: str) -> AdapterRegistryDecision:
        if not reason:
            raise ValueError("Reason is mandatory for revocation")
        if entry.registry_status != "approved":
            raise ValueError(f"Cannot revoke entry in status '{entry.registry_status}'. Must be 'approved'.")
    ...
    async def block_entry(self, entry: SignedAdapterRegistryEntry, reason: str, decided_by: str) -> AdapterRegistryDecision:
        if not reason:
    ...
    async def deprecate_entry(self, entry: SignedAdapterRegistryEntry, reason: str, decided_by: str) -> AdapterRegistryDecision:
        if not reason:
            raise ValueError("Reason is mandatory for deprecation")
        if entry.registry_status != "approved":
            raise ValueError(f"Cannot deprecate entry in status '{entry.registry_status}'. Must be 'approved'.")
