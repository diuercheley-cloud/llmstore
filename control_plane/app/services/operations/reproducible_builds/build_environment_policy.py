...
class BuildEnvironmentPolicyService:
    def validate_environment_constraints(
        ...
        logical_payload = {
            ...
            "external_dependency_resolution_allowed": payload.get(
                "external_dependency_resolution_allowed", False
            ),
        }
        constraint = BuildEnvironmentConstraint(
            ...
            external_dependency_resolution_allowed=logical_payload[
                "external_dependency_resolution_allowed"
            ],
            immutable_hash=sha256_hex(
                {"kind": "build_environment_constraint_immutable", **logical_payload}
            ),
        )
    ...
    def enforce_offline_constraints(self, constraint: BuildEnvironmentConstraint) -> dict[str, Any]:
        blocked = (
            constraint.external_network_allowed
            or constraint.external_dependency_resolution_allowed
            or not constraint.offline_only
        )
        return {
            "allowed": not blocked,
            "verification_status": "blocked" if blocked else "passed",
            "reason": "external network or dependency resolution denied"
            if blocked
            else "offline-first constraints enforced",
        }

    def enforce_determinism_constraints(
        self,
        constraint: BuildEnvironmentConstraint,
        ...
        return {
            "allowed": not blocked,
            "verification_status": "blocked" if blocked else "passed",
            "reason": "determinism constraints violated"
            if blocked
            else "determinism constraints satisfied",
