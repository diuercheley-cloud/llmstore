...
def _build_model_receipt(receipt_type: str, manifest: Any, subject_id: str, payload_hash: str) -> ReproducibleBuildReceipt:
    ...
    receipt = ReproducibleBuildReceipt(
        id=sha256_hex({"kind": "reproducible_build_receipt_id", "receipt_type": receipt_type, "subject_id": subject_id, "payload_hash": payload_hash}),
...
def build_manifest_receipt(manifest: Any) -> ReproducibleBuildReceipt:
    return _build_model_receipt("build_manifest_receipt", manifest, manifest.id, manifest.build_manifest_hash)


def build_artifact_receipt(manifest: Any, artifact_record: Any) -> ReproducibleBuildReceipt:
    return _build_model_receipt("artifact_verification_receipt", manifest, artifact_record.id, artifact_record.artifact_hash)
...
def build_reproducibility_receipt(manifest: Any, verification_result: Any) -> ReproducibleBuildReceipt:
    payload_hash = sha256_hex({"kind": "reproducibility_result_payload", "verification_result_id": verification_result.id, "immutable_hash": verification_result.immutable_hash})
    return _build_model_receipt("reproducibility_receipt", manifest, verification_result.id, payload_hash)
