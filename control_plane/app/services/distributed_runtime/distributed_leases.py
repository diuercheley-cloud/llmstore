...
class DistributedLeaseService:
    ...
    async def acquire_lease(
        self, job_id: uuid.UUID, node_id: uuid.UUID, duration_seconds: int = 60
    ) -> bool:
        result = await self.db.execute(
            select(DistributedJobLease).where(DistributedJobLease.job_id == job_id)
        )
    ...
    async def release_lease(self, job_id: uuid.UUID):
        await self.db.execute(
            delete(DistributedJobLease).where(DistributedJobLease.job_id == job_id)
        )
        await self.db.commit()
