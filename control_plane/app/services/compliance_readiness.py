...
class ComplianceReadinessService:
    ...
    async def get_readiness_report(self, framework_id: str) -> dict[str, Any]:
        framework = await self.db.get(
            ComplianceFramework, framework_id, options=[selectinload(ComplianceFramework.controls)]
        )
        if not framework:
        ...
        readiness_score = (
            (implemented + (partial * 0.5)) / total_controls if total_controls > 0 else 0
        )
    ...
    async def seed_frameworks(self):
        # SOC 2
        soc2 = await self.db.execute(
            select(ComplianceFramework).where(ComplianceFramework.name == "SOC2")
        )
        ...
        # ISO 27001
        iso = await self.db.execute(
            select(ComplianceFramework).where(ComplianceFramework.name == "ISO27001")
        )
