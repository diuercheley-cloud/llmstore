...
class ComplianceEvidenceService:
    ...
    async def _collect_soc2(self) -> list[EvidenceItem]:
        return [
            self._create_evidence(
                ...
                "RBAC Policy Logs",
                {"entries": ["User A accessed Node X", "User B denied"]},
            ),
            self._create_evidence(
                ComplianceFramework.SOC2,
                "CC7.1",
                ...
                "System Audit Trail",
                {"events": ["Config changed by admin", "Model Y deployed"]},
            ),
        ]
    ...
    async def _collect_iso27001(self) -> list[EvidenceItem]:
        return [
            self._create_evidence(
                ...
                {"last_verified": datetime.now(UTC).isoformat(), "status": "success"},
            ),
            self._create_evidence(
                ComplianceFramework.ISO27001,
                "A.14.2",
    ...
    async def _collect_gdpr(self) -> list[EvidenceItem]:
        return [
            self._create_evidence(
                ...
                "Data Encryption Attestation",
                {"method": "AES-256-GCM", "scope": "All PII"},
            ),
            self._create_evidence(
                ComplianceFramework.GDPR,
                "Art. 35",
                EvidenceType.EVAL_REPORT,
                "DPIA Summary (Draft)",
    ...
    def format_as_markdown(self, items: list[EvidenceItem]) -> str:
        ...
        for item in sorted(items, key=lambda x: x.framework):
            fw_name = (
                item.framework.value if hasattr(item.framework, "value") else str(item.framework)
            )
