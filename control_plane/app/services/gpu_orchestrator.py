...
class GpuOrchestrator:
    ...
    async def sync_local_gpus(self, node_id: uuid.UUID) -> list[GpuDevice]:
        ...
        for gpu_data in gpus:
            ...
            query = select(GpuDevice).where(
                GpuDevice.runtime_node_id == node_id, GpuDevice.gpu_index == gpu_data["index"]
            )
            ...
            LLM_GPU_MEMORY_USED_BYTES.labels(node_id=node_id_str, gpu_index=gpu_idx_str).set(
                gpu_data["memory_used"] * 1024 * 1024
            )
            LLM_GPU_MEMORY_TOTAL_BYTES.labels(node_id=node_id_str, gpu_index=gpu_idx_str).set(
                gpu_data["memory_total"] * 1024 * 1024
            )
            LLM_GPU_UTILIZATION_RATIO.labels(node_id=node_id_str, gpu_index=gpu_idx_str).set(
                gpu_data["utilization"] / 100.0
            )
            LLM_GPU_TEMPERATURE_CELSIUS.labels(node_id=node_id_str, gpu_index=gpu_idx_str).set(
                gpu_data["temperature"]
            )

            if gpu_data["memory_total"] > 0:
                pressure = gpu_data["memory_used"] / gpu_data["memory_total"]
                LLM_GPU_MEMORY_PRESSURE_RATIO.labels(node_id=node_id_str, gpu_id=gpu_idx_str).set(
                    pressure
                )
    ...
    async def allocate_gpu_for_model(
        self, model_id: uuid.UUID, memory_required_mb: int
    ) -> GpuDevice | None:
        # Find a GPU with enough free memory
        query = (
            select(GpuDevice)
            .where(GpuDevice.status == "active")
            .order_by(GpuDevice.memory_used_mb.asc())
        )
        result = await self.db.execute(query)
        devices = result.scalars().all()
    ...
    async def evaluate_autoscaling(self):
        ...
        for policy in policies:
            ...
            LLM_AUTOSCALING_REPLICAS_DESIRED.labels(policy_id=policy_id_str).set(
                decision["replicas_after"]
            )
            LLM_AUTOSCALING_REPLICAS_CURRENT.labels(policy_id=policy_id_str).set(
                decision["replicas_before"]
            )
            ...
            if decision["action"] != "no_op":
                ...
                self.db.add(event)
                LLM_AUTOSCALING_DECISIONS_TOTAL.labels(
                    policy_id=policy_id_str, action=decision["action"]
                ).inc()
                logger.info(
                    f"Autoscaling Decision for {policy.name}: {decision['action']} - {decision['reason']}"
                )
    ...
    async def _check_policy(self, policy: AutoscalingPolicy) -> dict[str, Any]:
        ...
        metrics = {"queue_depth": 0, "latency_p95": 0.0, "gpu_pressure": 0.0}
