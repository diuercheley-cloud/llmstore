...
class SigningService:
    ...
    def get_signing_profile(self, profile_id: uuid.UUID) -> CommercialSigningProfile:
        profile = (
            self.db.query(CommercialSigningProfile)
            .filter(CommercialSigningProfile.id == profile_id)
            .first()
        )
        if not profile:
    ...
    async def sign_payload(
        self, profile_id: uuid.UUID, payload: bytes, context: dict[str, Any] | None = None
    ) -> bytes:
        try:
            ...
            signature = await provider.sign(
                payload, key_material.encrypted_key_blob, profile.algorithm
            )

            self.kms_runtime.log_operation(
                profile.key_id, CryptoOperationType.SIGN, "success", context or {}
            )
            return signature
    ...
    async def verify_signature(
        ...
        try:
            ...
            is_valid = await provider.verify(
                payload, signature, key_material.encrypted_key_blob, profile.algorithm
            )
