...
class PKIService:
    ...
    async def initialize_ca(self):
        ...
        subject = issuer = x509.Name(
            [
                x509.NameAttribute(NameOID.ORGANIZATION_NAME, "LLM Inference Stack"),
                x509.NameAttribute(NameOID.COMMON_NAME, "Local Root CA"),
            ]
        )
        ...
        cert = (
            ...
            .not_valid_after(now + timedelta(days=self.settings.pki_ca_rotation_days))
            .add_extension(
    ...
    async def issue_certificate(self, common_name: str) -> tuple[str, str]:
        ...
        cert = (
            ...
            .not_valid_before(now)
            .not_valid_after(now + timedelta(days=self.settings.pki_cert_rotation_days))
            .add_extension(
    ...
    async def revoke_certificate(self, serial_number: str):
        ...
        result = await self.db.execute(
            select(CertificateInventory).where(CertificateInventory.serial_number == serial_number)
        )
    ...
    async def verify_certificate(self, cert_pem: str) -> bool:
        ...
        try:
            ...
            result = await self.db.execute(
                select(CertificateInventory).where(CertificateInventory.serial_number == serial_str)
            )
            db_cert = result.scalars().first()
    ...
    async def _update_crl(self):
        result = await self.db.execute(
            select(CertificateInventory).where(CertificateInventory.is_revoked == True)
        )
    ...
    async def _generate_crl(self, revoked_certs: list[CertificateInventory]):
        ...
        for r_cert in revoked_certs:
            revoked_cert = (
                ...
                .serial_number(int(r_cert.serial_number))
                .revocation_date(r_cert.revoked_at or now)
                .build()
            )
