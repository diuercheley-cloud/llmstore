...
class KMSRuntime:
    ...
    def get_provider_config(self, provider_id: uuid.UUID) -> CommercialKMSProvider:
        provider = self.db.query(CommercialKMSProvider).filter(CommercialKMSProvider.id == provider_id).first()
        if not provider:
    ...
    def get_key_material(self, key_id: uuid.UUID) -> CommercialKeyMaterial:
        key = self.db.query(CommercialKeyMaterial).filter(CommercialKeyMaterial.id == key_id).first()
        if not key:
    ...
    def log_operation(self, key_id: uuid.UUID, op_type: CryptoOperationType, status: str, audit_context: dict):
    ...
    async def encrypt(self, key_id: uuid.UUID, plaintext: bytes, context: Optional[Dict[str, Any]] = None) -> bytes:
        try:
            ...
            self.log_operation(key_id, CryptoOperationType.ENCRYPT, "failed", {"error": str(e), **(context or {})})
            raise KMSRuntimeError(f"Encryption failed: {e}")

    async def decrypt(self, key_id: uuid.UUID, ciphertext: bytes, context: Optional[Dict[str, Any]] = None) -> bytes:
        try:
            key_material = self.get_key_material(key_id)
            provider_config = self.get_provider_config(key_material.provider_id)
            ...
            self.log_operation(key_id, CryptoOperationType.DECRYPT, "failed", {"error": str(e), **(context or {})})
