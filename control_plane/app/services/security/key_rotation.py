...
class KeyRotationService:
    ...
    def get_keys_pending_rotation(self) -> list[CommercialKeyRotationSchedule]:
        now = utc_now()
        schedules = (
            self.db.query(CommercialKeyRotationSchedule)
            .filter(CommercialKeyRotationSchedule.next_rotation_at <= now)
            .all()
        )
        return schedules
    ...
    async def rotate_key(self, schedule_id: uuid.UUID) -> CommercialKeyMaterial:
        schedule = (
            self.db.query(CommercialKeyRotationSchedule)
            .filter(CommercialKeyRotationSchedule.id == schedule_id)
            .first()
        )
        ...
        schedule.next_rotation_at = schedule.last_rotated_at + timedelta(
    ...
    async def process_rotations(self):
        ...
        for schedule in schedules:
            try:
                ...
                results.append(
                    {"schedule_id": str(schedule.id), "status": "failed", "error": str(e)}
                )
        return results
