...
class AudioStreamService:
    ...
    async def process_audio_chunk(self, session_id: uuid.UUID, audio_data: bytes) -> str | None:
        ...
        if transcript:
            ...
            if await self.vad.is_turn_complete(audio_data):
                voice_transcript = VoiceTranscript(
                    session_id=session_id, speaker="user", text=transcript
                )
    ...
    async def generate_response_audio(self, session_id: uuid.UUID, text: str):
        ...
        event = VoiceStreamEvent(
            session_id=session_id, event_type="tts_start", payload={"text_length": len(text)}
        )
        self.db.add(event)

        # 3. Log transcript for agent
        voice_transcript = VoiceTranscript(session_id=session_id, speaker="agent", text=text)
        self.db.add(voice_transcript)
        await self.db.flush()
