...
class WorkflowPolicyEnforcementService:
    ...
    async def _resolve_policy_bundle(
        ...
        *,
        ...
        if bundle_uuid:
            candidate = (
                await db.execute(stmt.where(CommercialPolicyBundle.id == bundle_uuid))
            ).scalar_one_or_none()
    ...
    async def bind_stage_policy(
        ...
        *,
        ...
        bundle = await self._resolve_policy_bundle(
            db, tenant_id=execution.tenant_id, bundle_ref=str(bundle_ref) if bundle_ref else None
        )
        ...
        snapshot_hash = sha256_hex(
            {"policy": policy_json, "runtime": runtime_context, "stage_key": stage.stage_key}
        )
        binding = CommercialWorkflowPolicyBinding(
            ...
            enforcement_mode=stage_policy.get("enforcement_mode")
            or (bundle.mode if bundle else "enforce"),
        ...
        if bundle is not None:
            snapshot = CommercialWorkflowPolicySnapshot(
                ...
                detached_signature=sign_governance_payload(
                    {"snapshot_hash": snapshot_hash}, scope="workflow_policy_snapshot"
                ),
                signature_algorithm="ed25519",
                immutable_hash=sha256_hex(
                    {"snapshot_hash": snapshot_hash, "policy_hash": sha256_hex(policy_json)}
                ),
        ...
        *,
        ...
        existing = (
            await db.execute(
                ...
                .order_by(
                    desc(CommercialWorkflowPolicyBinding.created_at),
                    desc(CommercialWorkflowPolicyBinding.id),
                )
                .limit(1)
            )
        ).scalar_one_or_none()
        if existing is not None:
            ...
            if stage.active_policy_snapshot_id:
                snapshot = await db.get(
                    CommercialWorkflowPolicySnapshot, stage.active_policy_snapshot_id
                )
    ...
    async def evaluate_stage(
        ...
        *,
        ...
        approval_required = bool(
            stage_policy.get("approval_required") or snapshot.policy_json.get("approval_required")
        )
        if drift_detected:
        ...
        if mode == "dry_run":
            ...
            denied = bool(
                snapshot.policy_json.get("deny_execution") or stage_policy.get("deny_execution")
            )
        ...
        stage.governance_decision_signature = sign_governance_payload(
            signed_payload, scope="workflow_stage_decision"
        )
        return {
            "decision": decision,
            "reason": reason,
    ...
    async def rollback_stage_policy(
        ...
        *,
        ...
        current_binding, snapshot = await self.ensure_stage_binding(
            db, execution=execution, stage=stage
        )
        restored_policy = (
            snapshot.policy_json
            if snapshot
            else (((stage.metadata_json or {}).get("policy")) or {})
        )
        rollback_binding = CommercialWorkflowPolicyBinding(
            ...
            immutable_hash=sha256_hex(
                {"rollback_from_binding_id": str(current_binding.id), "policy": restored_policy}
            ),
        ...
        await self.ledger.append_event(
            ...
            event_payload={
                "binding_id": str(rollback_binding.id),
                "rollback_from_binding_id": str(current_binding.id),
            },
        )
        ...
        return [
            {
                ...
                "metadata": sanitize_report_payload(
                    redact_sensitive_payload(row.metadata_json or {})
                ),
            }
            for row in rows
        ]
