...
class MeshFailoverService:
    ...
    def orchestrate_failover(self, failed_node_id: str, term: int):
        ...
        node = self.db.query(CommercialMeshNode).filter(CommercialMeshNode.id == failed_node_id).first()
        if node:
    ...
    def detect_partition(self, partition_id: str, isolated_node_ids: List[str]) -> CommercialMeshPartitionEvent:
        ...
        # Update node modes
        nodes = self.db.query(CommercialMeshNode).filter(CommercialMeshNode.id.in_(isolated_node_ids)).all()
        for node in nodes:
