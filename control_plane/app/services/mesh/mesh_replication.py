...
class MeshReplicationService:
    ...
    def get_logs_since(self, log_index: int) -> list[CommercialMeshReplicationLog]:
        return (
            self.db.query(CommercialMeshReplicationLog)
            .filter(CommercialMeshReplicationLog.log_index > log_index)
            .order_by(CommercialMeshReplicationLog.log_index)
            .all()
        )
    ...
    def mark_log_applied(self, log_id: str):
        log = (
            self.db.query(CommercialMeshReplicationLog)
            .filter(CommercialMeshReplicationLog.id == log_id)
            .first()
        )
        if log:
            log.applied = True
    ...
    def reconcile_state(self, peer_logs: list[dict]):
        ...
        for log_data in peer_logs:
            existing_log = (
                self.db.query(CommercialMeshReplicationLog)
                .filter(CommercialMeshReplicationLog.log_index == log_data.get("log_index"))
                .first()
            )
            if not existing_log:
                self.append_log(
