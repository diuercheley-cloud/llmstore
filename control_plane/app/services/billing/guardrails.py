...
async def is_cloud_blocked_by_guardrails(session: AsyncSession, client_id: UUID) -> tuple[bool, str | None]:
    if settings.deployment_mode != "saas":
        return False, None
    
    # 1. Check global limit
    global_cost = await get_global_provider_cost_today(session)
    if settings.max_global_provider_cost_per_day_brl > 0 and global_cost >= settings.max_global_provider_cost_per_day_brl:
        return True, f"Global SaaS provider cost limit reached (R$ {global_cost:.2f} >= R$ {settings.max_global_provider_cost_per_day_brl:.2f})"
    
    # 2. Check client limit
    client_cost = await get_client_provider_cost_today(session, client_id)
    if settings.max_client_provider_cost_per_day_brl > 0 and client_cost >= settings.max_client_provider_cost_per_day_brl:
        return True, f"Client daily provider cost limit reached (R$ {client_cost:.2f} >= R$ {settings.max_client_provider_cost_per_day_brl:.2f})"
