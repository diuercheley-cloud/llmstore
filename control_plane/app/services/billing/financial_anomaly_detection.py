...
class FinancialAnomalyDetectionService:
    ...
    async def _detect_anomalies(self, metric_type: str) -> List[CommercialFinancialAnomaly]:
        ...
        historical_series = await self._get_historical_series(metric_type, baseline_start, baseline_end)
        ...
        if abs(z_score) >= self.settings.commercial_financial_anomaly_zscore_threshold:
            is_anomaly = True
        elif abs(deviation_pct) >= self.settings.commercial_financial_anomaly_percent_threshold:
            is_anomaly = True
    ...
    async def _get_historical_series(self, metric_type: str, start: datetime, end: datetime) -> List[float]:
        # Implementation similar to forecasting series but for baseline
        if metric_type == "revenue":
            stmt = select(func.sum(RequestFinancial.customer_price_brl)).where(RequestFinancial.created_at >= start, RequestFinancial.created_at <= end).group_by(func.date_trunc('day', RequestFinancial.created_at))
        elif metric_type == "cost":
            stmt = select(func.sum(RequestFinancial.provider_cost_brl)).where(RequestFinancial.created_at >= start, RequestFinancial.created_at <= end).group_by(func.date_trunc('day', RequestFinancial.created_at))
        elif metric_type == "margin":
            stmt = select(func.sum(RequestFinancial.gross_profit_brl)).where(RequestFinancial.created_at >= start, RequestFinancial.created_at <= end).group_by(func.date_trunc('day', RequestFinancial.created_at))
        elif metric_type == "wallet_debit":
            stmt = select(func.sum(AiWalletTransaction.amount_brl)).where(AiWalletTransaction.type == "debit", AiWalletTransaction.created_at >= start, AiWalletTransaction.created_at <= end).group_by(func.date_trunc('day', AiWalletTransaction.created_at))
        elif metric_type == "qos_billing":
            stmt = select(func.sum(CommercialQoSBillingRecord.billable_amount_brl)).where(CommercialQoSBillingRecord.created_at >= start, CommercialQoSBillingRecord.created_at <= end).group_by(func.date_trunc('day', CommercialQoSBillingRecord.created_at))
        elif metric_type == "dispute":
            stmt = select(func.count(CommercialBillingDispute.id)).where(CommercialBillingDispute.created_at >= start, CommercialBillingDispute.created_at <= end).group_by(func.date_trunc('day', CommercialBillingDispute.created_at))
        else:
            return []
    ...
    async def _get_recent_value(self, metric_type: str, start: datetime, end: datetime) -> float:
        if metric_type == "revenue":
            stmt = select(func.sum(RequestFinancial.customer_price_brl)).where(RequestFinancial.created_at >= start, RequestFinancial.created_at <= end)
        elif metric_type == "cost":
            stmt = select(func.sum(RequestFinancial.provider_cost_brl)).where(RequestFinancial.created_at >= start, RequestFinancial.created_at <= end)
        elif metric_type == "margin":
            stmt = select(func.sum(RequestFinancial.gross_profit_brl)).where(RequestFinancial.created_at >= start, RequestFinancial.created_at <= end)
        elif metric_type == "wallet_debit":
            stmt = select(func.sum(AiWalletTransaction.amount_brl)).where(AiWalletTransaction.type == "debit", AiWalletTransaction.created_at >= start, AiWalletTransaction.created_at <= end)
        elif metric_type == "qos_billing":
            stmt = select(func.sum(CommercialQoSBillingRecord.billable_amount_brl)).where(CommercialQoSBillingRecord.created_at >= start, CommercialQoSBillingRecord.created_at <= end)
        elif metric_type == "dispute":
            stmt = select(func.count(CommercialBillingDispute.id)).where(CommercialBillingDispute.created_at >= start, CommercialBillingDispute.created_at <= end)
        else:
            return 0.0
    ...
    async def summarize_anomalies(self) -> Dict[str, Any]:
        stmt = select(
            CommercialFinancialAnomaly.status,
            func.count(CommercialFinancialAnomaly.id)
        ).group_by(CommercialFinancialAnomaly.status)
        
        result = await self.session.execute(stmt)
        summary = {row.status: row[1] for row in result.all()}
        
        stmt_crit = select(func.count(CommercialFinancialAnomaly.id)).where(CommercialFinancialAnomaly.severity == "critical", CommercialFinancialAnomaly.status == "open")
