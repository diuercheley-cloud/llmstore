...
class RevenueForecastingService:
    ...
    async def forecast_revenue(
        self, window_days: int = 30, method: str = None
    ...
    async def forecast_qos_billing(
        self, window_days: int = 30, method: str = None
    ) -> CommercialRevenueForecast:
    ...
    async def forecast_by_client(
        self, client_id: uuid.UUID, window_days: int = 30, method: str = None
    ) -> CommercialRevenueForecast:
    ...
    async def _get_historical_series(
        ...
        if forecast_type == "qos_billing":
            ...
            stmt = select(
                ...
                RequestFinancial.created_at >= start_date, RequestFinancial.created_at <= end_date
            )
            if client_id:
