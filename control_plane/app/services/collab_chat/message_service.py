...
class MessageService:
    ...
    async def get_messages(self, channel_id: uuid.UUID, limit: int = 50) -> List[ChatMessage]:
        stmt = select(ChatMessage).where(ChatMessage.channel_id == channel_id).order_by(ChatMessage.created_at.desc()).limit(limit)
        ...
        if mentions:
            ...
            for agent_id_str in mentions:
                try:
                    ...
                    await agent_svc.trigger_agent_run(message.channel_id, agent_id, message.user_id, message.content)
                except ValueError:
                    continue
