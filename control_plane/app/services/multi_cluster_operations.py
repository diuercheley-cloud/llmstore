...
class MultiClusterOperationsService:
    ...
    async def create_cluster(
        self, name: str, cluster_type: str, base_url: str, location: str = None
    ) -> Cluster:
        cluster = Cluster(
            name=name,
    ...
    async def set_cluster_status(
        self, cluster_id: str, status: str, reason: str = None, operator_id: str = None
    ) -> Cluster:
    ...
    async def sync_cluster_config(
        ...
        if any(key in str(payload).lower() for key in sensitive_keys):
            ...
            await self.db.commit()
            raise ValueError(
                "Security Violation: Multi-cluster sync cannot carry sensitive data (prompts/docs)."
            )
    ...
    async def list_sync_events(self, cluster_id: str) -> list[ClusterSyncEvent]:
        result = await self.db.execute(
            ...
            .where(
                (ClusterSyncEvent.source_cluster_id == cluster_id)
                | (ClusterSyncEvent.target_cluster_id == cluster_id)
            )
