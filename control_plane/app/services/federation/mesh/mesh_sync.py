...
from app.models.core.federation_mesh import (
    ClusterNode,
    ConflictRecord,
    SyncCommit,
)
...
class MeshSyncService:
    ...
    async def create_commit(
        self, payload: dict[str, Any], parent_hash: str | None = None
    ) -> SyncCommit:
        """Generates a git-like commit for a set of changes."""
        # 1. Increment logical clock
        stmt = (
            update(ClusterNode)
            .where(ClusterNode.id == self.node_id)
            .values(logical_clock=ClusterNode.logical_clock + 1)
        ...
        # 2. Compute hash
        content = (
            f"{parent_hash}|{self.node_id}|{logical_clock}|{json.dumps(payload, sort_keys=True)}"
        )
    ...
    async def export_bundle(self, since_hash: str | None = None) -> dict[str, Any]:
    ...
    async def _check_conflict(self, commit_data: dict[str, Any]) -> bool:
        ...
        stmt = select(SyncCommit).where(
            SyncCommit.parent_hash == parent, SyncCommit.hash != commit_data["hash"]
        )
        res = await self.db.execute(stmt)
