...
async def create_vault(
    db: AsyncSession, tenant_id: str, vault_name: str, retention_days: int = 30
) -> CommercialRAGVault:
...
async def add_document_to_vault(
    db: AsyncSession, vault_id: uuid.UUID, content: str, classification: str = "confidential"
) -> CommercialRAGDocument:
    ...
    chunk = CommercialRAGChunk(document_id=doc.id, chunk_hash=chunk_hash, chunk_index=0)
    db.add(chunk)
    await db.commit()
...
async def log_policy_violation(
    db: AsyncSession, vault_id: uuid.UUID, session_id: str, query_hash: str, violation_type: str
) -> CommercialRetrievalPolicyViolation:
