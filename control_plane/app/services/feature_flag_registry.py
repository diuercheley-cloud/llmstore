...
class FeatureFlagRegistryService:
        ...
        for f in flags:
            ...
            if status == "deprecated":
                ...
                if not replacement or str(replacement).strip() == "":
                    errors.append(
                        f"Deprecated flag '{name}' must have a 'replacement' flag or 'none'."
                    )
            ...
            if status == "experimental":
                ...
                if default_val is not False:
                    errors.append(
                        f"Experimental flag '{name}' must be opt-in (default must be false)."
                    )
            ...
            if risk == "high":
                ...
                if default_val is not False:
                    errors.append(
                        f"High-risk flag '{name}' must be disabled by default (default must be false)."
                    )
    ...
    def scan_orphans(self) -> dict[str, Any]:
        ...
        model_fields = getattr(settings_model, "model_fields", None) or getattr(
            settings_model, "__fields__", {}
        )
        settings_keys = set()
        ...
        for d_key in sorted(doc_keys):
            if d_key not in registered_keys and d_key not in missing_registration:
                ...
                if (
                    d_key in settings_keys
                    or d_key.startswith("AGENT_")
                    or d_key.startswith("COMMERCIAL_")
                ):
