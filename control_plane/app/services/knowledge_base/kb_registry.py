...
class KBRegistry:
    ...
    async def create_kb(
        self, tenant_id: str, name: str, description: str = None, provider_config: dict = None
    ) -> KnowledgeBase:
        kb = KnowledgeBase(
            tenant_id=tenant_id,
            name=name,
    ...
    async def create_document_version(
        self, doc_id: uuid.UUID, version_tag: str, content_hash: str, storage_path: str = None
    ) -> KBDocumentVersion:
    ...
    async def get_document_versions(self, doc_id: uuid.UUID) -> list[KBDocumentVersion]:
        stmt = (
            ...
            .where(KBDocumentVersion.document_id == doc_id)
            .order_by(KBDocumentVersion.created_at.desc())
        )
        res = await self.db.execute(stmt)
        return list(res.scalars().all())
