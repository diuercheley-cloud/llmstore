...
async def request_witness_signature(
    db: AsyncSession, timeline_id: uuid.UUID, witness_id: uuid.UUID
) -> CommercialWitnessSignature | None:
    ...
    witness_result = await db.execute(
        select(CommercialWitness).where(CommercialWitness.id == witness_id)
    )
...
async def verify_witness_signature(db: AsyncSession, signature_id: uuid.UUID) -> bool:
    ...
    witness_result = await db.execute(
        select(CommercialWitness).where(CommercialWitness.id == sig.witness_id)
    )
    witness = witness_result.scalar_one_or_none()
    if not witness:
        return False
    ...
    timeline_result = await db.execute(
        select(CommercialMerkleTimeline).where(CommercialMerkleTimeline.id == sig.timeline_id)
    )
    timeline = timeline_result.scalar_one_or_none()
    if not timeline:
        return False
...
async def evaluate_witness_quorum(db: AsyncSession, timeline_id: uuid.UUID) -> dict:
    # 1. Get timeline
    timeline_result = await db.execute(
        select(CommercialMerkleTimeline).where(CommercialMerkleTimeline.id == timeline_id)
    )
    ...
    req_sigs = (
        policy.required_signatures if policy else get_settings().commercial_witness_min_signatures
    )
    req_ext = (
        policy.require_external_witness
        if policy
        else get_settings().commercial_witness_require_external
    )
    ...
    for s in sigs:
        w_result = await db.execute(
            select(CommercialWitness).where(CommercialWitness.id == s.witness_id)
        )
...
async def verify_witness_audit_event(db: AsyncSession, event_id: uuid.UUID) -> bool:
