...
async def ensure_appliance_profile(db: AsyncSession) -> CommercialApplianceProfile:
    ...
    res = await db.execute(
        select(CommercialApplianceProfile).where(
            CommercialApplianceProfile.appliance_id == appliance_id
        )
    )
...
async def create_offline_sync_manifest(
    db: AsyncSession, sync_direction: str, payload_type: str, media_uuid: str | None = None
) -> CommercialOfflineSyncManifest:
    profile = await ensure_appliance_profile(db)

    manifest_hash = hashlib.sha256(
        f"{profile.appliance_id}-{datetime.now(UTC).timestamp()}".encode()
    ).hexdigest()
...
async def stage_offline_model_bundle(
    db: AsyncSession, model_name: str, manifest_id: uuid.UUID
) -> CommercialOfflineModelBundle:
...
async def verify_removable_media(db: AsyncSession, manifest_id: uuid.UUID, media_uuid: str) -> bool:
    profile = await ensure_appliance_profile(db)
    res = await db.execute(
        select(CommercialOfflineSyncManifest).where(CommercialOfflineSyncManifest.id == manifest_id)
    )
