...
) -> CommercialConfidentialRuntimeProfile | None:
...
async def create_confidential_session(
    ...
    client_id: str | None,
    request_id: str | None,
    profile: CommercialConfidentialRuntimeProfile | None,
) -> CommercialConfidentialInferenceSession:
    ...
    await log_confidential_audit(
        db, session.id, "confidential_session_started", f"Session started for request {request_id}"
    )
    return session
...
async def validate_confidential_request(
    ...
    if profile.require_encrypted_input and not payload.get("encrypted_input"):
        await log_confidential_audit(
            db, session.id, "validation_failed", "Encrypted input required but not provided"
        )
...
async def enforce_no_plaintext_logging(
    ...
    if (
        data_type == "prompt"
        and profile.prohibit_prompt_logging
        or data_type == "response"
        and profile.prohibit_response_logging
    ):
        prohibit = True

    if prohibit and get_settings().commercial_confidential_prohibit_plaintext_logging:
        await log_confidential_audit(
            db, session.id, "plaintext_logging_blocked", f"{data_type} logging blocked by policy"
        )
...
async def apply_retention_policy(
    ...
    retention = (
        profile.max_retention_seconds
        or get_settings().commercial_confidential_default_retention_seconds
    )
...
async def log_confidential_audit(
    db: AsyncSession, session_id: uuid.UUID | None, event_type: str, summary: str
) -> CommercialConfidentialRuntimeAuditEvent:
...
async def verify_confidential_audit_event(db: AsyncSession, event_id: uuid.UUID) -> bool:
    res = await db.execute(
        select(CommercialConfidentialRuntimeAuditEvent).where(
            CommercialConfidentialRuntimeAuditEvent.id == event_id
        )
    )
    ...
    payload = (
        f"{event.session_id}:{event.event_type}:{event.summary}:{event.created_at.isoformat()}"
    )
