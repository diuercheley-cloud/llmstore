...
async def authorize_tool_execution(
    db: AsyncSession, execution_id: uuid.UUID, tool_name: str, tool_input: Any
) -> tuple[bool, str]:
    # 1. Resolve execution and profile
    res = await db.execute(
        select(CommercialAgentExecution, CommercialAgentProfile)
        .join(
            CommercialAgentProfile, CommercialAgentExecution.agent_id == CommercialAgentProfile.id
        )
...
async def check_delegation_allowed(
    db: AsyncSession, source_agent_id: uuid.UUID, target_agent_id: uuid.UUID
) -> bool:
    ...
    source_res = await db.execute(
        select(CommercialAgentProfile).where(CommercialAgentProfile.id == source_agent_id)
    )
...
async def enforce_memory_boundary(
    db: AsyncSession, execution_id: uuid.UUID, tenant_id: str, boundary_type: str = "session"
) -> CommercialAgentMemoryBoundary:
    boundary = CommercialAgentMemoryBoundary(
        execution_id=execution_id, tenant_id=tenant_id, boundary_type=boundary_type
    )
    db.add(boundary)
    await db.commit()
    return boundary
