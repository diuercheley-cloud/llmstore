...
from app.models.agents.agent_service import AgentServiceUsage
...
class AgentServiceAPI:
    ...
    async def invoke_agent(
        ...
        if mode == "sync":
            ...
            await self._record_usage(
                agent_id, tenant_id, uuid.UUID(result["run_id"]), tier.id, "sync"
            )
            return result
        else:
    ...
    async def _record_usage(
        self, agent_id: uuid.UUID, tenant_id: str, run_id: uuid.UUID, tier_id: uuid.UUID, mode: str
    ):
