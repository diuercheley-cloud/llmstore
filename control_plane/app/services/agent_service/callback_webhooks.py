...
class CallbackWebhookService:
    ...
    async def register_webhook(self, agent_id: uuid.UUID, tenant_id: str, url: str) -> AgentCallbackWebhook:
    ...
    async def trigger_callback(self, agent_id: uuid.UUID, payload: Dict[str, Any]):
        ...
        stmt = select(AgentCallbackWebhook).where(
            AgentCallbackWebhook.agent_id == agent_id,
            AgentCallbackWebhook.is_active == True
        )
