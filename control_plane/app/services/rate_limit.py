...
async def enforce_ip_rate_limit(redis: Redis, source_ip: str, limit_per_minute: int = 60) -> None:
    ...
    if current > limit_per_minute:
        raise RateLimitExceeded(
            f"IP address {source_ip} exceeded {limit_per_minute} requests per minute"
        )
...
async def enforce_tenant_rate_limit(
    redis: Redis, tenant_id: str, limit_per_minute: int = 500
) -> None:
    current_minute = int(time.time() // 60)
    ...
    if current > limit_per_minute:
        raise RateLimitExceeded(
            f"Tenant {tenant_id} rate limit exceeded ({limit_per_minute} req/min)"
        )


async def enforce_client_rate_limit(
    redis: Redis, client_id: str, limit_per_minute: int = 1000
) -> None:
    current_minute = int(time.time() // 60)
    key = f"ratelimit:client:{client_id}:{current_minute}"
    current = await redis.incr(key)
