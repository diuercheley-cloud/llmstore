...
class DeterminismRepairService:
    ...
    async def report_drift(self, workflow_id: str, step_index: int, expected_hash: str, actual_hash: str, drift_details: dict):
    ...
    async def mark_repaired(self, drift_id: str, repair_plan_id: str):
        stmt = select(CommercialRuntimeDeterminismDrift).filter(CommercialRuntimeDeterminismDrift.id == drift_id)
