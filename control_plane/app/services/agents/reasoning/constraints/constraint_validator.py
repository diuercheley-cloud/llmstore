...
class ConstraintValidator:
    ...
    def validate(self, model: ConstraintModel, plan_data: Dict[str, Any]) -> List[Dict[str, str]]:
        ...
        for c in model.constraints:
            ...
            if not self._check_constraint(c, field_value):
                errors.append({
                    "id": c.id,
                    "message": c.message or f"Constraint {c.id} violated on field {c.target_field}"
                })
        return errors
    ...
    def _check_constraint(self, c: Constraint, value: Any) -> bool:
        try:
            ...
            if c.operator == "ge": return value >= c.value
            if c.operator == "le": return value <= c.value
            if c.operator == "neq": return value != c.value
            if c.operator == "in": return value in c.value
