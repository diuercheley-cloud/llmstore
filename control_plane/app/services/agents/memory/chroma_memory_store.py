...
class ChromaMemoryStore(VectorStore):
    ...
    async def add_item(
        ...
        collection.upsert(ids=[str(memory_id)], embeddings=[embedding], metadatas=[full_metadata])
    ...
    async def search(
        ...
        results = collection.query(
            query_embeddings=[query_embedding], n_results=top_k, where={"agent_id": str(agent_id)}
        )
