...
class MockMemoryStore(VectorStore):
    ...
    async def add_item(
        ...
        # Remove if exists
        self._store[tenant_id] = [item for item in self._store[tenant_id] if item["memory_id"] != memory_id]
        ...
        logger.info(f"Mock store INDEX: added item {memory_id} for tenant {tenant_id}, agent {agent_id}. Store size for tenant: {len(self._store[tenant_id])}")
    ...
    async def search(
        ...
        logger.info(f"Mock store SEARCH: tenant {tenant_id}, agent {agent_id}. Tenants in store: {list(self._store.keys())}")
        if tenant_id not in self._store:
            return []
        ...
        for item in self._store[tenant_id]:
            ...
            logger.info(f"Mock store EVAL: memory_id={item['memory_id']} score={score} threshold={score_threshold}")
