...
class PGVectorMemoryStore(VectorStore):
    ...
    async def add_item(
        ...
        stmt = select(AgentMemoryIndex).where(
            AgentMemoryIndex.tenant_id == tenant_id, AgentMemoryIndex.memory_item_id == memory_id
        )
    ...
    async def search(
        ...
        try:
            ...
            for row in res:
                results.append(
                    {"memory_id": row[0], "score": float(row[1]), "provider": "pgvector"}
                )
            return results
            ...
            return await self._manual_fallback_search(
                tenant_id, agent_id, query_embedding, top_k, score_threshold
            )

    async def _manual_fallback_search(
        self, tenant_id, agent_id, query_embedding, top_k, score_threshold
    ):
        import json
        import math
        ...
        for it in items:
            ...
            if score >= score_threshold:
                scored.append(
                    {
                        "memory_id": it.memory_item_id,
                        "score": score,
                        "provider": "pgvector-fallback",
                    }
                )
