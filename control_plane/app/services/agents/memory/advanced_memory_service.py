...
class AdvancedMemoryService:
    ...
    async def access_memory(self, event_id: uuid.UUID):
        ...
        stmt = (
            ...
            .values(
                access_count=MemoryEvent.access_count + 1,
    ...
    async def list_memories(self, agent_id: uuid.UUID, scope: Optional[MemoryScope] = None) -> List[MemoryEvent]:
    ...
    async def redact_memory(self, event_id: uuid.UUID, reason: str):
        ...
        if event:
            await self.append_event(
                ...
                payload={"target_event_id": str(event_id), "reason": reason, "original_type": event.event_type}
            )
    ...
    async def get_memory_events(self, agent_id: uuid.UUID) -> List[MemoryEvent]:
        stmt = select(MemoryEvent).where(MemoryEvent.agent_id == agent_id).order_by(MemoryEvent.logical_counter.asc())
        result = await self.db.execute(stmt)
        return result.scalars().all()
