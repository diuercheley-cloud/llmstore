...
def _verify_runtime_enabled():
    ...
    if not settings.agent_execution_plane_enabled:
        raise RuntimeDisabledError(
            "Agent Execution Plane is disabled. Set AGENT_EXECUTION_PLANE_ENABLED=true to enable it."
        )
    if not settings.agent_runtime_enabled:
        raise RuntimeDisabledError(
            "Agent runtime is disabled. Set AGENT_RUNTIME_ENABLED=true to enable it."
        )
...
async def pause_run(db: AsyncSession, run_id: uuid.UUID) -> Any | None:
    _verify_runtime_enabled()
    run = await agent_state.get_agent_run(db, run_id)
    if not run:
...
async def resume_run(
    ...
    if run.status not in ("paused", "waiting_approval"):
        raise ValueError(
            f"Cannot resume a run that is not paused or waiting approval. Status: {run.status}"
        )
    ...
    if run.status == "waiting_approval":
        ...
        stmt = select(AgentApprovalRequest).where(
            AgentApprovalRequest.agent_run_id == run_id, AgentApprovalRequest.status == "approved"
        )
...
async def resume_run_internal(
    ...
    if settings.agent_async_execution_enabled:
        ...
        if job:
            ...
            queue_mgr = AgentQueueManager(db)
            await queue_mgr.enqueue_job(
                agent_run_id=run_id, agent_id=run.agent_id, tenant_id=run.tenant_id, max_attempts=3
            )
...
async def cancel_run(db: AsyncSession, run_id: uuid.UUID) -> Any | None:
    _verify_runtime_enabled()
    run = await agent_state.get_agent_run(db, run_id)
    if not run:
        raise ValueError(f"Agent run not found: {run_id}")
...
async def replay_run(db: AsyncSession, run_id: uuid.UUID) -> dict[str, Any]:
    ...
    if not settings.agent_replay_enabled:
        raise ReplayDisabledError(
            "Replay features are disabled. Set AGENT_REPLAY_ENABLED=true to enable them."
        )
