...
class AgentExecutor:
    ...
    async def _check_output_guardrails(self, run, content: str) -> tuple[bool, str]:
        ...
        decision, reason, sanitized = await self.guardrails.check_output(
            run.id, run.tenant_id, content
        )
        if decision == "block":
    ...
    async def execute_step(self) -> bool:
        ...
        if self.settings.agent_debugger_enabled:
            ...
            stepper = LiveStepper(self.db, DebugSessionManager(self.db), BreakpointManager(self.db))
            await stepper.check_and_pause(
                run.id,
                run.tenant_id,
                run.total_steps + 1,
                "step_start",
                {"run_status": run.status, "total_steps": run.total_steps},
            )
        ...
        if approved_req:
            res = await self._execute_tool_and_process(
                run,
                approved_req.sanitized_context.get("tool_name"),
                approved_req.raw_tool_input,
                step_number,
            )
            await self.db.commit()
            return res
    ...
    async def _fail_run(self, reason: str):
        await agent_state.update_run(
            self.db, self.run_id, status="failed", failure_reason=reason, completed_at=utc_now()
        )
        run = await agent_state.get_agent_run(self.db, self.run_id)
    ...
    async def _check_limits(self, run, agent_def) -> bool:
        ...
        if (
            utc_now().replace(tzinfo=None) - started_at
        ).total_seconds() > agent_def.max_runtime_seconds:
    ...
    async def _get_approved_request(self, run):
        ...
        for r in res.scalars().all():
            if (r.sanitized_context or {}).get("step_number") == run.total_steps:
                return r
        return None

    async def _get_active_plan(self):
        stmt = select(AgentPlan).where(
            AgentPlan.agent_run_id == self.run_id, AgentPlan.status == "executing"
        )
        res = await self.db.execute(stmt)
    ...
    async def _continue_plan_execution(self, run, plan):
        ...
        if plan.status == "completed":
            await agent_state.update_run(
                self.db, self.run_id, status="completed", completed_at=utc_now()
            )
            await self.obs.record_run_completion(run.agent_id, run.id)
            return False
        return True

    async def _get_llm_decision(self, run, agent_def, step_number: int) -> dict | None:
        ...
        if self.settings.agent_memory_enabled:
            mem = await self.memory.build_memory_context(
                tenant_id=run.tenant_id, agent_id=run.agent_id, query=run.input_text or ""
            )
        ...
        if run.session_id:
            ...
            session_context = await context_builder.build_context_for_llm(
                session_id=run.session_id, max_messages=50, include_summary=True
            )
            if session_context:
                ...
                logger.info(
                    f"Injected {len(session_context)} messages from session {run.session_id} into context"
                )
        ...
        try:
            ...
            try:
                decision = await self.reasoning_loop.execute(
                    agent_def=agent_def, run=run, allowed_tools=agent_def.allowed_tools or []
                )
            ...
            await self.obs.record_model_call(
                self.run_id, "completed", latency_ms, decision.get("usage")
            )
    ...
    async def _handle_tool_call_decision(self, run, decision, step_number):
        ...
        await agent_state.log_run_step(
            ...
            "model_call",
            {"input_hash": run.input_hash},
            {"tool_name": tool_name, "tool_input_hash": i_hash},
        )
        ...
        req, risk, reason, role = await check_approval_required(
            self.db, self.run_id, tool_name, tool_input
        )
        if req:
            await self.obs.record_approval_request(self.run_id, tool_name, reason)
            await agent_state.update_run(self.db, self.run_id, status="waiting_approval")
            await create_approval_request(
                self.db, self.run_id, tool_name, tool_input, risk, reason, role, step_number
            )
    ...
    async def _execute_tool_and_process(self, run, tool_name, tool_input, step_number):
        ...
        try:
            ...
            if exec_mode == "mock":
                ...
                category = SimulationRuntime.classify_tool(
                    tool_name, tool.category if tool else None
                )
        ...
        if not error and output:
            ...
            try:
                ...
                output, error = (
                    {"error": f"DLP Egress Blocked: {str(e)}"},
                    f"DLP Egress Blocked: {str(e)}",
                )
        ...
        if not error:
            ...
            if is_blocked:
                ...
                output, error = (
                    {"error": "Tool output blocked by safety guardrails"},
                    "Guardrail violation",
                )
            else:
        ...
        await self.obs.record_tool_call_result(
            self.run_id, tool_name, "failed" if error else "completed", latency, error
        )
        ...
        await self.receipts.create_receipt(
            self.run_id,
            step_number,
            "tool_execution",
            i_hash,
    ...
    def _build_simulated_output(
        self, *, mode: str, tool_name: str, reason: str, policy_decision_id: str | None
    ) -> dict[str, Any]:
        return {
            "status": mode,
    ...
    async def _handle_memory_op(self, run, decision, step_number, op):
        if self.is_replay:
            return True
        await self.obs.record_memory_op_detailed(
            self.run_id, op, decision.get("memory_type", "short_term"), True
        )
        ...
        await agent_state.log_run_step(
            ...
            f"memory_{op}",
            {"input_hash": run.input_hash},
            {"op": op, "memory_type": decision.get("memory_type")},
        )
        return True
    ...
    async def _handle_handoff(self, run, decision, step_number):
        ...
        await self.handoff.initiate_handoff(
            self.run_id,
        ...
        await agent_state.update_run(
            self.db, self.run_id, status="completed", failure_reason=f"Handed off to {target}"
        )
        await self.obs.record_run_completion(run.agent_id, run.id)
        return False
    ...
    async def _handle_final_decision(self, run, decision, step_number):
        ...
        await agent_state.update_run(
            self.db, self.run_id, status="completed", output_hash=o_hash, completed_at=utc_now()
        )
        await self.obs.record_run_completion(run.agent_id, run.id)
        return False
    ...
    async def _resolve_prompt_template(self, agent_def, run) -> Any:
        try:
            ...
            ver_stmt = select(PromptTemplateVersion).where(PromptTemplateVersion.id == version_id)
            ...
            declared = await svc.get_declared_variables(agent_def.prompt_template_id)
            declared_list = [
                {
                    "name": v.name,
                    "type": v.var_type,
