...
class MemoryConsentService:
    ...
    async def get_consent(
        self, tenant_id: str, user_id: str, memory_type: str, agent_id: uuid.UUID | None = None
    ) -> AgentMemoryConsent | None:
    ...
    async def revoke_consent(self, tenant_id: str, consent_id: uuid.UUID):
        stmt = select(AgentMemoryConsent).where(
            AgentMemoryConsent.id == consent_id, AgentMemoryConsent.tenant_id == tenant_id
        )
        res = await self.db.execute(stmt)
