        ...
        try:
            ...
            for spec in specialists:
                ...
                allowed, reason = await self.policy.validate_delegation(
                    run.id, manager.agent_id, spec.agent_id
                )
                if not allowed:
                ...
                task_description = (
                    spec.metadata_json.get("task_description")
                    or f"Analyze subproblem for goal: {goal}"
                )
            ...
            for index, spec in enumerate(specialists, start=1):
                ...
                max_retries = 60
                while (
                    sub_run.status not in ("completed", "failed", "cancelled") and max_retries > 0
                ):
                ...
                if sub_run.status != "completed":
                    ...
                    specialist_result = (
                        f"Error: Specialist failed to complete task. Status: {sub_run.status}"
                    )
                else:
            ...
            arbitration_res = await self.arbitrator.arbitrate(
                specialist_outputs,
                {"goal": goal, "topology": "hierarchical", "tenant_id": team.tenant_id},
            )
