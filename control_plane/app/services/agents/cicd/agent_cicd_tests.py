    ...
    async def _run_case(self, case, tenant_id: str) -> dict[str, Any]:
        ...
        if isinstance(case.expected_output, str):
            assertions = (
                ...
                if case.expected_output.startswith("[")
                else [{"type": "exact_match", "value": case.expected_output}]
            )
            ...
            if a_type == "exact_match":
                if instructions.strip() != a_value.strip():
                    return {
                        ...
                        "case_name": case.name,
                        "passed": False,
                        "error": f"Expected '{a_value}', got '{instructions.strip()}'",
                    }
                ...
                if a_value not in instructions:
                    return {
                        ...
                        "case_name": case.name,
                        "passed": False,
                        "error": f"Expected '{a_value}' not found in instructions",
                    }
                ...
                if a_value in instructions:
                    return {
                        ...
                        "case_name": case.name,
                        "passed": False,
                        "error": f"Unexpected '{a_value}' found in instructions",
                    }
            elif a_type == "regex":
                import re
                ...
                if not re.search(a_value, instructions):
                    return {
                        ...
                        "case_name": case.name,
                        "passed": False,
                        "error": f"Regex '{a_value}' did not match",
                    }
