...
class EnvironmentPolicyService:
    ...
    async def validate_promotion(
        ...
        version_id: str,
        to_environment: str,
        promotion_req_id: str | None = None,
    ) -> tuple[bool, str | None]:
        """
        ...
        stmt_def = select(AgentDefinition).where(
            AgentDefinition.id == version_uuid, AgentDefinition.tenant_id == tenant_id
        )
        ...
        if to_env_lower == "dev":
            ...
            stmt_eval = select(AgentEvalGateResult).where(
                AgentEvalGateResult.agent_id == reg.id, AgentEvalGateResult.passed == True
            )
            ...
            if not evals_passed:
                return (
                    False,
                    "Evals check failed: No successful evaluation gate results found for this agent.",
                )
            return True, None
            ...
            # Production requires:
            # 1. Eval baseline
            stmt_baseline = select(AgentEvalBaseline).where(AgentEvalBaseline.agent_id == reg.id)
            ...
            for pattern in secret_patterns:
                if re.search(pattern, instructions):
                    return (
                        False,
                        "Security check failed: Agent instructions contain sensitive credentials or private keys.",
                    )
            ...
            if promotion_req_id:
                ...
                if not approved:
                    return (
                        False,
                        "Production deployment denied: Promotion request has not been approved.",
                    )
            else:
                ...
                if not approved:
                    return (
                        False,
                        "Production deployment denied: Requires an approved promotion request.",
                    )
            ...
            if "mock" in model_id or "dummy" in model_id:
                return (
                    False,
                    "Production policy violation: Mock model providers are prohibited in production.",
                )
