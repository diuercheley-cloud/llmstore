...
class PromptBaselineRegistry:
    ...
    async def get_latest_baseline(self, agent_id: uuid.UUID) -> AgentPromptBaseline | None:
        stmt = (
            select(AgentPromptBaseline)
            .where(AgentPromptBaseline.agent_id == agent_id, AgentPromptBaseline.status == "active")
    ...
    async def create_baseline(
        ...
        stmt = select(AgentPromptBaseline).where(
            AgentPromptBaseline.agent_id == agent_id, AgentPromptBaseline.status == "active"
        )
        res = await self.db.execute(stmt)
    ...
    async def validate_prompt_against_baseline(
        self, agent_id: uuid.UUID, current_instructions: str
    ) -> bool:
