...
class ToolValidatorNodeVisitor(ast.NodeVisitor):
        ...
        for alias in node.names:
            ...
            if base_module in self.blocked_imports:
                raise SecurityException(
                    f"Importing '{base_module}' is not allowed for security reasons."
                )
    ...
    def visit_ImportFrom(self, node):
        if node.module:
            ...
            if base_module in self.blocked_imports:
                raise SecurityException(
                    f"Importing from '{base_module}' is not allowed for security reasons."
                )
        self.generic_visit(node)
    ...
    def visit_Call(self, node):
        if isinstance(node.func, ast.Name):
            if node.func.id in self.blocked_functions:
                raise SecurityException(
                    f"Function '{node.func.id}' is blocked for security reasons."
                )
        self.generic_visit(node)
