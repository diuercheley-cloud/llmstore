...
class GeneratedToolRegistry:
    ...
    def register_tool(
        self, name: str, description: str, author_id: uuid.UUID = None
    ) -> AgentGeneratedTool:
    ...
    def add_tool_version(
        self, tool_id: uuid.UUID, version_tag: str, code: str, schema_json: dict = None
    ) -> AgentGeneratedToolVersion:
        version = AgentGeneratedToolVersion(
            tool_id=tool_id, version_tag=version_tag, code=code, schema_json=schema_json
        )
        self.db.add(version)
        self.db.commit()
    ...
    def approve_version(self, version_id: uuid.UUID, approver: str):
        version = (
            self.db.query(AgentGeneratedToolVersion)
            .filter(AgentGeneratedToolVersion.id == version_id)
            .first()
        )
        if version:
