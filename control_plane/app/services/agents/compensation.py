...
class CompensationService:
    ...
    async def register_compensation(
        self, task_id: uuid.UUID, action_type: str, payload: dict
    ) -> AgentCompensationAction:
        action = AgentCompensationAction(
            task_id=task_id, action_type=action_type, payload=payload, status="pending"
        )
        self.db.add(action)
    ...
    async def trigger_compensation(self, task_id: uuid.UUID):
        stmt = select(AgentCompensationAction).where(
            AgentCompensationAction.task_id == task_id, AgentCompensationAction.status == "pending"
        )
