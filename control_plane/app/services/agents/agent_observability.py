...
class AgentObservabilityService:
    ...
    async def record_run_start(self, agent_id: uuid.UUID, run_id: uuid.UUID, tenant_id: str):
        ...
        await agent_state.log_run_event(
            ...
            run_id,
            "run_started",
            {"tenant_hash": hashed_tenant, "started_at": utc_now().isoformat()},
        )
    ...
    async def record_run_completion(self, agent_id: uuid.UUID, run_id: uuid.UUID):
        ...
        await agent_state.log_run_event(
            self.db, run_id, "run_completed", {"completed_at": utc_now().isoformat()}
        )
    ...
    async def record_step_start(self, run_id: uuid.UUID, step_number: int, step_type: str):
        await agent_state.log_run_event(
            self.db, run_id, "step_started", {"step_number": step_number, "step_type": step_type}
        )
    ...
    async def record_tool_call_start(self, run_id: uuid.UUID, tool_name: str):
        await agent_state.log_run_event(
            self.db, run_id, "tool_call_started", {"tool_name": tool_name}
        )
    ...
    async def record_tool_call_result(
        ...
        if run:
            ...
            metrics.LLM_AGENT_TOOL_LATENCY_SECONDS.labels(
                agent_id=agent_id_str, tool_name=tool_name
            ).observe(latency_ms / 1000.0)
            metrics.LLM_AGENT_TOOL_FAILURE_RATE.labels(
                agent_id=agent_id_str, tool_name=tool_name
            ).set(1.0 if status == "failed" else 0.0)
        ...
        await agent_state.log_run_event(
            ...
            run_id,
            event_type,
            {"tool_name": tool_name, "latency_ms": latency_ms, "error": error},
        )

    async def record_memory_op_detailed(
        self, run_id: uuid.UUID, op: str, memory_type: str, success: bool
    ):
        if op == "read":
            run = await agent_state.get_agent_run(self.db, run_id)
            if run:
                metrics.LLM_AGENT_MEMORY_HIT_RATE.labels(
                    agent_id=str(run.agent_id), memory_type=memory_type
                ).set(1.0 if success else 0.0)
                await agent_state.increment_run_metric(self.db, run_id, "memory_reads_count", 1.0)

        await agent_state.log_run_event(
            self.db, run_id, f"memory_{op}", {"memory_type": memory_type, "success": success}
        )

    async def record_policy_decision(self, run_id: uuid.UUID, decision: str, reason: str):
        await agent_state.log_run_event(
            self.db, run_id, "policy_decision", {"decision": decision, "reason": reason}
        )
    ...
    async def record_replan(self, run_id: uuid.UUID, reason: str):
        ...
        await agent_state.log_run_event(
            self.db, run_id, "replan_requested", {"replan_reason": reason}
        )

    async def record_approval_request(self, run_id: uuid.UUID, tool_name: str, reason: str):
        await agent_state.log_run_event(
            self.db, run_id, "approval_required", {"tool_name": tool_name, "reason": reason}
        )
    ...
    def get_trace_attributes(self, run: Any, step: Any = None) -> dict[str, Any]:
        tenant_val = (
            self._hash_tenant(run.tenant_id)
            if self.settings.agent_trace_export_enabled
            else run.tenant_id
        )
        if step is None:
            attrs = {
    ...
    async def record_step(
        self, agent_id: uuid.UUID, run_id: uuid.UUID, step_type: str, duration_ms: float
    ):
        metrics.LLM_AGENT_STEP_LATENCY_SECONDS.labels(
            agent_id=str(agent_id), step_type=step_type
    ...
    async def record_tool_call(
        ...
        metrics.LLM_AGENT_TOOL_DURATION_SECONDS.labels(
            agent_id=str(agent_id), tool_name=tool_name
        ).observe(duration_ms / 1000.0)
    ...
    async def record_memory_op(
        self, agent_id: uuid.UUID, run_id: uuid.UUID, operation: str, latency_ms: float
    ):
        metrics.LLM_AGENT_MEMORY_LATENCY_SECONDS.labels(
            agent_id=str(agent_id), operation=operation
        ).observe(latency_ms / 1000.0)
        await self._record_timeline_event(run_id, f"memory.{operation}", {"latency_ms": latency_ms})

    async def record_handoff(
        self, agent_id: uuid.UUID, target_agent_id: uuid.UUID, run_id: uuid.UUID
    ):
        metrics.LLM_AGENT_HANDOFF_COUNT.labels(
            agent_id=str(agent_id), target_agent_id=str(target_agent_id)
        ).inc()
        ...
        if run:
            ...
            metrics.LLM_AGENT_HANDOFF_DEPTH.labels(agent_id=str(agent_id), run_id=str(run_id)).set(
                depth
            )

        await self._record_timeline_event(
            run_id, "handoff.started", {"target_agent_id": str(target_agent_id)}
        )

    async def record_policy_denial(self, agent_id: uuid.UUID, run_id: uuid.UUID, tool_name: str):
        metrics.LLM_AGENT_POLICY_DENIALS_TOTAL.labels(
            agent_id=str(agent_id), tool_name=tool_name
        ).inc()
        await self._record_timeline_event(run_id, "policy.denied", {"tool_name": tool_name})

    async def record_approval_wait(
        self, agent_id: uuid.UUID, run_id: uuid.UUID, tool_name: str, wait_time_seconds: float
    ):
        metrics.LLM_AGENT_APPROVAL_WAIT_SECONDS.labels(
            agent_id=str(agent_id), tool_name=tool_name
        ).observe(wait_time_seconds)
        await agent_state.increment_run_metric(
            self.db, run_id, "approval_wait_seconds", wait_time_seconds
        )
    ...
    async def record_cost(
        ...
        metrics.LLM_AGENT_TOKENS_TOTAL.labels(agent_id=str(agent_id), token_type="prompt").inc(
            prompt_tokens
        )
        metrics.LLM_AGENT_TOKENS_TOTAL.labels(agent_id=str(agent_id), token_type="completion").inc(
            completion_tokens
        )

        # Update budget usage
        metrics.LLM_AGENT_TOKEN_BUDGET_USED.labels(agent_id=str(agent_id)).set(
            prompt_tokens + completion_tokens
        )
        metrics.LLM_AGENT_COST_BUDGET_USED_BRL.labels(agent_id=str(agent_id)).set(cost_brl)

    async def _record_timeline_event(
        self, run_id: uuid.UUID, event_type: str, details: dict[str, Any]
    ):
        # Redact secrets
