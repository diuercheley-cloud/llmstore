...
class MarketplaceAnalyticsService:
    ...
    async def record_download(
        self, item_id: uuid.UUID, tenant_id: str, version: str
    ) -> MarketplaceDownloadEvent:
        event = MarketplaceDownloadEvent(item_id=item_id, tenant_id=tenant_id, version=version)
        self.db.add(event)
    ...
    async def get_download_stats(self, item_id: uuid.UUID):
        stmt = select(func.count(MarketplaceDownloadEvent.id)).where(
            MarketplaceDownloadEvent.item_id == item_id
        )
        res = await self.db.execute(stmt)
