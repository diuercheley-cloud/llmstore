...
class CostQualityPolicy:
    ...
    async def get_policy(self, name: str) -> AgentRoutingPolicy | None:
        result = await self.db.execute(
            select(AgentRoutingPolicy).filter(
                AgentRoutingPolicy.name == name, AgentRoutingPolicy.is_active == True
            )
        )
        return result.scalars().first()

    async def get_profile(self, name: str) -> AgentCostQualityProfile | None:
        result = await self.db.execute(
            select(AgentCostQualityProfile).filter(AgentCostQualityProfile.name == name)
        )
        return result.scalars().first()

    async def apply_policy(
        self, models: list[AgentModelCapability], policy_name: str, profile_name: str | None = None
        ...
        if profile:
            if profile.max_cost_input is not None:
                filtered_models = [
                    m for m in filtered_models if m.cost_input <= profile.max_cost_input
                ]
            if profile.max_cost_output is not None:
                filtered_models = [
                    m for m in filtered_models if m.cost_output <= profile.max_cost_output
                ]
            filtered_models = [
                m for m in filtered_models if m.quality_tier >= profile.min_quality_tier
            ]

        # Apply policy logic
        if policy_name == PolicyType.LOWEST_COST:
            return sorted(
                filtered_models, key=lambda m: (m.cost_input + m.cost_output, -m.quality_tier)
            )

        if policy_name == PolicyType.HIGH_QUALITY:
            return sorted(
                filtered_models, key=lambda m: (-m.quality_tier, m.cost_input + m.cost_output)
            )
        ...
        if policy_name == PolicyType.BALANCED:
            ...
            return sorted(
                filtered_models,
                key=lambda m: -(m.quality_tier / (m.cost_input + m.cost_output + 0.0001)),
            )
