...
class MemoryErasureService:
    ...
    async def _propagate_deletion(self, item: AgentMemoryItem):
        try:
            ...
            elif hasattr(store, "remove_item") and callable(store.remove_item):
                await store.remove_item(str(item.id))
            else:
                logger.info(
                    f"Vector store {type(store).__name__} does not support deletion; memory {item.id} tombstoned in DB only"
                )
    ...
    async def get_deletion_proof(self, tenant_id: str, item_id: uuid.UUID) -> dict | None:
