...
class AgentGraphDryRunRunner:
    ...
    async def run_dry_run(
        ...
        result = await self.engine.execute(
            spec, run_id=run_id, global_input=global_input
        )
    ...
    async def _node_runner(self, node: AgentNode, context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            if ntype in ("agent", "llm_call", "model_reasoning"):
                ...
                events.append({"event": "tool_call_simulated", "tool": tool_name, "category": category})
            ...
            status = "failed"
            output = {"error": str(e)}
            events.append({"event": "node_failed", "error": str(e), "timestamp": utc_now().isoformat()})
