    ...
    def validate(self, version: AgentFlowVersion) -> list[dict[str, str]]:
        ...
        if self._has_cycles(nodes, edges):
            errors.append(
                {
                    "code": "cyclic_graph",
                    "message": "Flow graph contains cycles, which are not allowed in this runtime.",
                }
            )
            ...
            errors.append(
                {
                    ...
                    "message": "Flow must end with a final_response or handoff node.",
                }
            )
        ...
        for node in nodes:
            if node.get("node_type") == "tool_call":
                ...
                if config.get("requires_approval") is False and self._is_high_risk(
