...
class AgentReadinessService:
    ...
    async def check_readiness(self) -> dict[str, Any]:
        ...
        executor_simulation_mode = bool(
            getattr(self.settings, "agent_executor_allow_simulation", False)
        )
        ...
        if llm_provider not in VALID_LLM_PROVIDERS:
            ...
            elif llm_provider in ("gateway", "real") and deployment_mode in (
                "production",
                "enterprise_managed",
            ):
                if not require_real:
            ...
            results["checks"].append(
                {
                    ...
                    if messages
                    else f"LLM provider '{llm_provider}' is valid for '{deployment_mode}' mode.",
                }
            )
        ...
        res_queue = await self.db.execute(
            select(func.count(AgentExecutionJob.id)).where(
        ...
        if stuck_runs > 0:
            ...
            results["checks"].append(
                {"id": "stuck_runs", "name": "Stuck Agent Runs", "status": "pass", "value": 0}
            )

        # 5. Orphan Leases and DLQ
        res_orphans = await self.db.execute(
            select(func.count(AgentExecutionLease.id)).where(
                AgentExecutionLease.expires_at <= utc_now()
            )
        )
        ...
        res_approvals = await self.db.execute(
        ...
        res_incidents = await self.db.execute(
            select(func.count(AgentIncident.id)).where(AgentIncident.status == "open")
        )
        ...
        if results["blockers"] and results["status"] not in [
