...
class SovereigntyPolicy:
    ...
    async def validate_sync(
        self, peer_id: uuid.UUID, data_type: str, is_raw: bool
    ) -> tuple[bool, str]:
        """
        ...
        if (
            data_type in prohibited_regions
            and peer.data_residency_region in prohibited_regions[data_type]
        ):
            return (
                False,
                f"Data residency violation: {data_type} cannot reside in {peer.data_residency_region}.",
            )
