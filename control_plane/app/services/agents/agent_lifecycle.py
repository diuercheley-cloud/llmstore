...
async def submit_review(
    db: AsyncSession, entry_id: uuid.UUID, performed_by: str = "system"
) -> AgentRegistryEntry:
    ...
    if entry.status != "draft":
        raise ValueError(
            f"Cannot submit review: agent is in '{entry.status}' status, must be 'draft'"
        )
    ...
    if settings.agent_evals_enabled:
        ...
        res_suites = await db.execute(
            select(AgentEvalSuite.id).where(AgentEvalSuite.agent_id == entry.id)
        )
        suite_ids = res_suites.scalars().all()
        if not suite_ids:
            raise ValueError(
                "Cannot submit review: At least one completed evaluation dry-run is required."
            )
...
async def approve_agent(
    ...
    if entry.status != "review":
        raise ValueError(
            f"Cannot approve agent: status must be 'review', currently '{entry.status}'"
        )
    ...
    if entry.risk_level in ("high", "critical"):
        if not approved_by or not approved_by.strip():
            raise ValueError(
                "Approval signature ('approved_by') is strictly required for high/critical risk level agents."
            )
    ...
    if settings.agent_production_requires_eval_baseline:
        ...
        if not baseline:
            raise ValueError(
                "Cannot approve agent: Evaluation baseline is missing. Baseline must be set before approval."
            )
        if getattr(baseline, "is_stale", False):
            raise ValueError(
                "Cannot approve agent: Evaluation baseline is stale. A new baseline must be set before approval."
            )
...
async def activate_agent(
    ...
    if entry.status not in ("approved", "paused"):
        raise ValueError(
            f"Cannot activate agent: status must be 'approved' or 'paused', currently '{entry.status}'"
        )

    # 1. Enforce Owner requirement
    if not entry.owner or not entry.owner.strip():
        raise ValueError(
            "Cannot activate agent: Owner is missing. Owner must be set before activation."
        )
    ...
    if settings.agent_production_requires_eval_baseline or settings.agent_promotion_requires_evals:
        ...
        if not baseline:
            ...
            if not entry.eval_baseline or not entry.eval_baseline.strip():
                raise ValueError(
                    "Cannot activate agent: Evaluation baseline is missing. Eval baseline must be defined and run."
                )
        else:
            if getattr(baseline, "is_stale", False):
                raise ValueError(
                    "Cannot activate agent: Evaluation baseline is stale. A new evaluation is required before activation."
                )
            ...
            if not gate_res:
                raise ValueError(
                    "Cannot activate agent: Promotion gate verification has not been run."
                )
            if not gate_res.passed and not gate_res.audit_override:
                raise ValueError(
                    "Cannot activate agent: Promotion gate verification failed. Override required to proceed."
                )
            ...
            if run_id_to_check:
                ...
                if eval_run and eval_run.metadata_json:
                    ...
                    if eval_provider == "mock" and not settings.agent_eval_allow_mock_for_promotion:
                        raise ValueError(
                            "Cannot activate agent: Production-ready agents require evaluation via gateway or real provider."
                        )
    ...
    if entry.risk_level in ("high", "critical"):
        ...
        if not has_approval:
            raise ValueError(
                "Cannot activate agent: High/critical risk agents require prior approval signature."
            )
...
async def deprecate_agent(
    ...
    if entry.status != "active":
        raise ValueError(
            f"Cannot deprecate agent: status must be 'active', currently '{entry.status}'"
        )
...
async def archive_agent(
    ...
    if entry.status != "deprecated":
        raise ValueError(
            f"Cannot archive agent: status must be 'deprecated', currently '{entry.status}'"
        )
