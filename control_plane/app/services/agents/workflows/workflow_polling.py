...
class WorkflowPollingService:
    ...
    async def process_ready_polls(self):
        ...
        """
        stmt = (
            select(AgentWorkflowPollingJob)
            .where(
    ...
    async def _execute_poll(self, job: AgentWorkflowPollingJob):
        ...
        try:
            ...
            if (
                mock_response.get(condition_path) == target_value
                or job.attempts_count >= job.max_attempts
            ):
                job.status = (
                    "completed"
                    if mock_response.get(condition_path) == target_value
                    else "timed_out"
                )
                ...
                delay = int(
                    job.interval_seconds * (job.backoff_multiplier ** (job.attempts_count - 1))
                )
