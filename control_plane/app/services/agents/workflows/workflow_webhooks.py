...
class WorkflowWebhookService:
    ...
    async def create_subscription(
        ...
        sub = AgentWorkflowWebhookSubscription(
            run_id=run_id, tenant_id=tenant_id, secret_token=str(uuid.uuid4()), status="active"
        )
        self.db.add(sub)
        await self.db.flush()
        return sub
    ...
    async def handle_incoming(
        self, subscription_id: uuid.UUID, payload: dict[str, Any], secret_token: str
    ):
        ...
        if not sub or sub.status != "active":
            raise HTTPException(
                status_code=404, detail="Webhook subscription not found or inactive"
            )
        ...
        await self.signals.send_signal(
            run_id=sub.run_id, signal_name=f"webhook_received_{subscription_id}", payload=payload
        )
