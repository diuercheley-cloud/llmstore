...
class SubworkflowRuntime:
    ...
    async def start_subworkflow(
        self, node_key: str, definition_id: uuid.UUID, input_data: dict[str, Any]
    ) -> AgentSubworkflowRun:
        """
    ...
    async def update_subworkflow_status(
        self, sub_run_id: uuid.UUID, status: str, output_data: dict[str, Any] | None = None
    ):
    ...
    async def get_subworkflow_results(self, node_key: str) -> dict[str, Any] | None:
        stmt = (
            select(AgentSubworkflowRun)
            .where(
    ...
    async def is_subworkflow_complete(self, node_key: str) -> bool:
        stmt = (
            select(AgentSubworkflowRun.status)
            .where(
                AgentSubworkflowRun.parent_run_id == self.parent_run.id,
                AgentSubworkflowRun.node_key == node_key,
            )
