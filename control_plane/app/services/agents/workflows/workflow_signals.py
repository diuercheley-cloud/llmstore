...
class WorkflowSignalManager:
    ...
    async def send_signal(self, run_id: uuid.UUID, signal_name: str, payload: dict[str, Any]):
        signal = AgentWorkflowSignal(
            run_id=run_id, signal_name=signal_name, payload=payload, status="pending"
        )
        self.db.add(signal)
    ...
    async def get_pending_signals(self, run_id: uuid.UUID):
        stmt = (
            select(AgentWorkflowSignal)
            .where(AgentWorkflowSignal.run_id == run_id, AgentWorkflowSignal.status == "pending")
