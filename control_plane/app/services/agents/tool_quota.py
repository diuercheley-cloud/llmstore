...
async def check_and_increment_quota(
    ...
    targets.append(
        (
            ...
            DEFAULT_LIMIT_TENANT,
            f"Tenant {tenant_id} daily quota",
        )
    )
    ...
    if side_effect_level:
        ...
        targets.append(
            (
                ...
                limit,
                f"Side-effect level '{side_effect_level}' daily quota",
            )
        )
    ...
    for filters, default_limit, desc in targets:
        ...
        if counter.invocation_count >= counter.max_limit:
            logger.warning(
                f"Quota exceeded: {desc} ({counter.invocation_count}/{counter.max_limit})"
            )
