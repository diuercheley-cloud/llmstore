    ...
    async def run(
        ...
        if fc_binary is None:
            if self.settings.agent_code_sandbox_microvm_required:
                raise RuntimeError(
                    "Firecracker provider is required but firecracker binary is not available"
                )
            return {
        ...
        if not os.path.exists(kernel_path) or not os.path.exists(rootfs_path):
            ...
            if (
                ...
                or self.settings.agent_code_sandbox_microvm_required
            ):
                raise RuntimeError(
                    f"Firecracker assets missing: kernel={kernel_path}, rootfs={rootfs_path}"
                )
