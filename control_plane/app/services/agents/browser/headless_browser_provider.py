...
async def browser_click_element(session: HeadlessBrowserSession, selector: str) -> dict:
    ...
    tag_match = re.search(rf'<[a-zA-Z0-9]+\s+[^>]*?(?:id|class)=["\']{clean_selector}["\'][^>]*?>', session.html, re.IGNORECASE)
    if tag_match:
        tag_str = tag_match.group(0)
        onclick_match = re.search(r'onclick="([^"]*)"', tag_str, re.IGNORECASE) or re.search(r"onclick='([^']*)'", tag_str, re.IGNORECASE)
        if onclick_match:
            ...
            if match_mutation:
                ...
                return {"status": "success", "message": f"Clicked element '{selector}' and triggered action."}
...
def browser_extract_page_text(session: HeadlessBrowserSession) -> dict:
    html_clean = re.sub(r"<script.*?>.*?</script>", "", session.html, flags=re.DOTALL | re.IGNORECASE)
