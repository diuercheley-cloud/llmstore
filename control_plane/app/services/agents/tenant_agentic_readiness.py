...
class TenantAgenticReadinessService:
    ...
    async def get_readiness_report(self, tenant_id: str) -> dict[str, Any]:
        ...
        """
        report = {"tenant_id": tenant_id, "overall_status": "ready", "checks": {}}
    ...
    async def _check_allowed_tools(self, tenant_id: str) -> dict[str, Any]:
        ...
        return {
            ...
            if status == "ready"
            else "Tooling is available, but execution guardrails are incomplete.",
            "details": {
    ...
    async def _check_memory_readiness(self, tenant_id: str) -> dict[str, Any]:
        ...
        return {
            ...
            "details": {"has_consent": has_consent, "has_policy": has_policy},
        }
    ...
    async def _check_budget_policy(self, tenant_id: str) -> dict[str, Any]:
        ...
        res = await self.db.execute(
            select(AgentDefinition).where(
                AgentDefinition.tenant_id == tenant_id, AgentDefinition.max_cost_brl.isnot(None)
            )
        )
    ...
    async def _check_observability_isolation(self, tenant_id: str) -> dict[str, Any]:
        ...
        return {
            ...
            if enabled
            else "Observability isolation is not fully enabled.",
    ...
    async def _check_approval_reviewers(self, tenant_id: str) -> dict[str, Any]:
        ...
        return {
            ...
            if enabled
            else "Human approval flow is disabled.",
    ...
    async def _check_data_boundary(self, tenant_id: str) -> dict[str, Any]:
        ...
        return {"status": "ready", "message": "Data boundary integrity verified.", "details": {}}
    ...
    async def _check_runtime_posture(self, tenant_id: str) -> dict[str, Any]:
        ...
        if active_non_real_modes:
            if production_like:
                blockers.append(
                    f"Agent executor non-real modes active: {', '.join(active_non_real_modes)}."
                )
            else:
                warnings.append(
                    f"Agent executor non-real modes active: {', '.join(active_non_real_modes)}."
        ...
        return {
            "status": status,
            "message": "Runtime posture validated."
            if status == "ready"
            else "Runtime posture requires attention.",
            "details": {
    ...
    async def _check_integrations_posture(self, tenant_id: str) -> dict[str, Any]:
        ...
        vector_provider = (
            str(getattr(self.settings, "agent_memory_vector_provider", "mock")).strip().lower()
        )
        embeddings_provider = (
            str(getattr(self.settings, "agent_memory_embeddings_provider", "mock")).strip().lower()
        )
        ...
        if memory_enabled and production_like and embeddings_provider == "mock":
            findings.append(
                "Semantic memory is enabled with AGENT_MEMORY_EMBEDDINGS_PROVIDER=mock."
            )
        ...
        if connector_active and connector_mode != "real":
            if production_like:
                findings.append(
                    f"Connectors are enabled with AGENT_CONNECTOR_MODE={connector_mode}."
                )
            else:
                warnings.append(
                    f"Connectors are enabled with AGENT_CONNECTOR_MODE={connector_mode}."
        ...
        if connector_active and connector_mode == "real" and not connector_real_http:
            if production_like:
                findings.append(
                    "Connector mode is real but AGENT_CONNECTOR_REAL_HTTP_ENABLED is false."
                )
            else:
        ...
        code_sandbox_provider = (
            str(getattr(self.settings, "agent_code_sandbox_provider", "mock")).strip().lower()
        )
        ...
        return {
            "status": status,
            "message": "Integration posture validated."
            if status == "ready"
            else "Integration dependencies require hardening.",
            "details": {
                "deployment_mode": deployment_mode,
    ...
    async def _check_operational_evidence(self, tenant_id: str) -> dict[str, Any]:
        ...
        production_like = deployment_mode in {"pilot", "production", "enterprise_managed"}
        required_artifacts = {
            "real_execution_readiness": self._base_dir
            / "artifacts/runtime/real-execution-readiness.json",
            "production_agentic_e2e": self._base_dir
            / "artifacts/e2e/production-agentic/summary.md",
