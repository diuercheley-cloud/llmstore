...
async def evaluate_trigger(
    db: AsyncSession, trigger_id: uuid.UUID, payload: dict[str, Any]
) -> bool:
    """
...
async def fire_trigger(db: AsyncSession, trigger_id: uuid.UUID, payload: dict[str, Any]) -> bool:
    ...
    input_text = sanitized_payload.get("input_text") or trigger.config.get(
        "default_input", "Event triggered execution"
    )
...
async def _run_agent_in_background(
    agent_id: uuid.UUID, tenant_id: str, input_text: str, delivery_id: uuid.UUID
):
    ...
    async with SessionLocal() as db:
        try:
            ...
            await db.commit()
            logger.info(
                f"Background AgentRun started successfully: run={run.id} delivery={delivery_id}"
            )
