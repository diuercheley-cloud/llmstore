...
async def evaluate_schedules():
    ...
    if not settings.agent_event_driven_enabled or not settings.agent_cron_triggers_enabled:
        logger.info(
            "Cron triggers or Event-driven execution is disabled. Scheduler loop will not run."
        )
        return
...
async def check_and_fire_schedules(db: AsyncSession):
    ...
    stmt = (
        ...
        .where(AgentEventTrigger.is_paused == False, AgentScheduledTrigger.next_run_at <= now)
    ...
    for schedule in schedules:
        logger.info(
            f"Firing scheduled trigger {schedule.trigger_id} (next_run_at: {schedule.next_run_at})"
        )
...
def calculate_next_run(
    cron_expression: str, start_time: datetime, timezone_str: str = "UTC"
) -> datetime:
    """
