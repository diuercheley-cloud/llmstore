...
class SafetyInterlock:
    ...
    async def check_command(self, twin_id: uuid.UUID, command: str, params: Dict[str, Any]) -> Tuple[bool, str]:
        """
        ...
        if command in dangerous_commands:
            event = DigitalTwinSafetyEvent(
                ...
                severity="critical",
                details={"command": command, "reason": "Attempted override of critical safety command."}
            )
            self.db.add(event)
