...
class ArtifactReviewManager:
    ...
    @staticmethod
    async def check_promotion_allowed(
        db: AsyncSession, artifact_id: uuid.UUID, version_id: uuid.UUID
    ) -> None:
        ...
        if not approvals:
            raise PermissionError(
                "Artifact must have an approved review for the current version before promotion."
            )
    ...
    async def promote_artifact(
        db: AsyncSession, artifact: AgentSharedArtifact, actor_id: str, actor_type: str
    ) -> None:
        ...
        await ArtifactReviewManager.check_promotion_allowed(
            db, artifact.id, artifact.current_version_id
        )
