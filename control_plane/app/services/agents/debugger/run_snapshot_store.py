...
class RunSnapshotStore:
    ...
    async def capture_step(
        self, run_id: uuid.UUID, step_number: int, data: dict[str, Any]
    ) -> AgentRunSnapshot:
        """
        ...
        context_hash = hashlib.sha256(
            json.dumps(data.get("context", {}), sort_keys=True).encode()
        ).hexdigest()
    ...
    async def get_snapshots(self, run_id: uuid.UUID) -> list[AgentRunSnapshot]:
        stmt = (
            ...
            .order_by(AgentRunSnapshot.step_number)
        )
        res = await self.db.execute(stmt)
        return list(res.scalars().all())

    async def get_snapshot_at_step(
        self, run_id: uuid.UUID, step_number: int
