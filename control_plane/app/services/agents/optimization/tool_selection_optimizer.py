...
class ToolSelectionOptimizer:
    def optimize_tool_selection(self, current_tools: List[str], failures: List[AgentEvalFailure]) -> List[str]:
        """
        ...
        if "*" in optimized_tools:
            ...
            optimized_tools = ["confluence", "github", "jira", "microsoft365", "salesforce", "slack"]
        ...
        for f in failures:
            if f.failure_type == "tool_error":
                ...
                if offending_tool and offending_tool in optimized_tools:
                    ...
                    logger.info(f"Removing failing tool '{offending_tool}' from candidate allowed list.")
