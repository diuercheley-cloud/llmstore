...
class AnalyticsQueries:
    ...
    async def get_run_success_rate(
        self, tenant_id: str, start_time: datetime, end_time: datetime
    ) -> dict[str, Any]:
        stmt = (
            select(AgentRun.status, func.count(AgentRun.id).label("count"))
            .where(
                and_(
    ...
    async def get_cost_and_tokens(
        self, tenant_id: str, start_time: datetime, end_time: datetime
    ) -> dict[str, Any]:
        stmt = select(
            func.sum(AgentRunCosts.estimated_cost_brl).label("total_cost"),
            func.sum(AgentRunCosts.prompt_tokens + AgentRunCosts.completion_tokens).label(
                "total_tokens"
            ),
        ).where(
            and_(
    ...
    async def get_tool_metrics(
        ...
        stmt = (
            ...
            .join(AgentRun, AgentRunMetrics.run_id == AgentRun.id)
            .where(
                and_(
                    AgentRun.tenant_id == tenant_id,
                    AgentRun.started_at >= start_time,
    ...
    async def get_eval_pass_rate(
        self, tenant_id: str, start_time: datetime, end_time: datetime
    ) -> float:
        stmt = (
            select(func.avg(func.cast(AgentEvalResult.passed, func.Integer)))
            .join(AgentRun, AgentEvalResult.run_id_ref == AgentRun.id)
            .where(
                and_(
                    AgentRun.tenant_id == tenant_id,
