...
class AgentBundleVerifierService:
    ...
    async def verify_bundle(self, version_id: uuid.UUID, content: bytes, is_internal: bool = False) -> Dict[str, Any]:
        # 1. Verify Checksum
        ...
        internal_sig_required_in_prod = getattr(self.settings, "agent_internal_bundle_signature_required_in_production", True)
        allow_unsigned_internal = getattr(self.settings, "allow_unsigned_internal_bundles", False)

        if not is_signed:
            if not is_internal:
                return {"passed": False, "reason": "Signature required for external/marketplace packages"}
            
            if is_prod and internal_sig_required_in_prod:
                return {"passed": False, "reason": "Signature required for internal packages in production"}
