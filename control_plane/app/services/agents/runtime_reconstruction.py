...
class RuntimeReconstructionService:
    ...
    async def reconstruct_run_state(self, run_id: uuid.UUID) -> Dict[str, Any]:
        ...
        receipt_stmt = select(AgentRunReceipt).where(AgentRunReceipt.run_id == run_id).order_by(AgentRunReceipt.step_number.asc())
        ...
        logger.info(f"Reconstructing run {run_id}: found {expected_steps} receipts. Current run total_steps is {run.total_steps}")

        # Reconcile database state with receipts
        if run.total_steps != expected_steps:
            logger.warning(f"Run total_steps ({run.total_steps}) mismatch with receipts ({expected_steps}). Reconciling...")
        ...
        if run.status == "running":
            ...
            job_stmt = select(AgentExecutionJob).where(AgentExecutionJob.agent_run_id == run_id, AgentExecutionJob.queue_status == "running")
            ...
            for job in active_jobs:
                ...
                if time_elapsed > 300: # 5 minutes lease expiry
                    logger.warning(f"Lease expired for execution job {job.id} (run {run_id}). Reclaiming lease.")
