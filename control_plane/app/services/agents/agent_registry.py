...
def load_registry_config() -> dict[str, Any]:
    ...
    if not os.path.exists(config_path):
        ...
        config_path = os.path.abspath(
            os.path.join(this_dir, "..", "..", "..", "..", "config", "agent-registry.yaml")
        )
...
async def get_registry_entry(db: AsyncSession, entry_id: uuid.UUID) -> AgentRegistryEntry | None:
...
async def get_registry_entry_by_agent_id(
    db: AsyncSession, agent_id: uuid.UUID
) -> AgentRegistryEntry | None:
    """Fetches a single Agent Registry entry by public agent UUID."""
    result = await db.execute(
        select(AgentRegistryEntry).where(AgentRegistryEntry.agent_id == agent_id)
    )
    return result.scalars().first()
...
async def create_registry_entry(
    db: AsyncSession, data: dict[str, Any], performed_by: str = "system"
) -> AgentRegistryEntry:
    ...
    if risk_level not in config.get("risk_levels", []):
        raise ValueError(
            f"Invalid risk_level: '{risk_level}'. Must be one of {config.get('risk_levels')}"
        )
...
async def update_registry_entry(
    db: AsyncSession, entry_id: uuid.UUID, update_data: dict[str, Any], performed_by: str = "system"
) -> AgentRegistryEntry:
    ...
    if "supported_surface_status" in update_data:
        if (
            update_data["supported_surface_status"] == "supported"
            and entry.supported_surface_status == "experimental"
        ):
    ...
    inst_changed = (
        "instructions" in update_data and update_data["instructions"] != entry.instructions
    )
    tools_changed = (
        "allowed_tools" in update_data and update_data["allowed_tools"] != entry.allowed_tools
    )
    memory_changed = (
        "memory_enabled" in update_data and update_data["memory_enabled"] != entry.memory_enabled
    )
