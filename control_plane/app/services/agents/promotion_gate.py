...
class AgentPromotionService:
    ...
    async def run_promotion_check(self, agent_id: uuid.UUID, target_status: str) -> dict[str, Any]:
        ...
        registry_res = await self.db.execute(
            select(AgentRegistryEntry).where(AgentRegistryEntry.agent_id == agent.id)
        )
        ...
        if latest_run:
            is_ready, reasons = await scoring_manager.evaluate_promotion_readiness(
                agent_id, latest_run.id
            )
        ...
        checks["security_check"] = (
            True  # Simplified for now, in practice we'd check red-team run results
        )
        ...
        # 3. Prompt Freshness
        checks["prompt_freshness"] = await self.baseline_registry.validate_prompt_against_baseline(
            agent_id, agent.instructions
        )
    ...
    async def promote_agent(
        self, agent_id: uuid.UUID, target_status: str, approved_by: str
    ) -> dict[str, Any]:
        """
