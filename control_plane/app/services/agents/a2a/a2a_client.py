...
class A2AClientService:
    ...
    async def send_message(
        ...
        if not sender_reg:
            raise HTTPException(
                status_code=404, detail=f"Sender agent {sender_agent_id} registration not found."
            )
        ...
        if not recipient_reg:
            raise HTTPException(
                status_code=404,
                detail=f"Recipient agent {recipient_agent_id} registration not found.",
            )

        # Tenant isolation check
        if sender_reg.tenant_id != tenant_id or recipient_reg.tenant_id != tenant_id:
            raise HTTPException(
                status_code=403, detail="Cross-tenant communication is strictly blocked."
            )
        ...
        if (
            recipient_reg.target_url
            and not recipient_reg.target_url.startswith("mock://")
            and not recipient_reg.target_url.startswith("internal://")
        ):
            try:
                ...
                logger.error(
                    f"HTTP request failed delivering A2A message to {recipient_reg.target_url}: {exc}"
                )
                raise HTTPException(
                    status_code=502,
                    detail=f"Network error communicating with external agent: {exc}",
                )
        else:
    ...
    async def delegate_task(
        ...
        if not delegator_reg:
            raise HTTPException(
                status_code=404,
                detail=f"Delegator agent {delegator_agent_id} registration not found.",
            )
        ...
        if not delegatee_reg:
            raise HTTPException(
                status_code=404,
                detail=f"Delegatee agent {delegatee_agent_id} registration not found.",
            )

        # Tenant isolation check
        if delegator_reg.tenant_id != tenant_id or delegatee_reg.tenant_id != tenant_id:
            raise HTTPException(
                status_code=403, detail="Cross-tenant delegation is strictly blocked."
            )
        ...
        if (
            delegatee_reg.target_url
            and not delegatee_reg.target_url.startswith("mock://")
            and not delegatee_reg.target_url.startswith("internal://")
        ):
            try:
                async with httpx.AsyncClient(timeout=10.0) as client:
                ...
                logger.error(
                    f"HTTP request failed delegating task to {delegatee_reg.target_url}: {exc}"
                )
                raise HTTPException(
                    status_code=502,
                    detail=f"Network error communicating with external agent: {exc}",
