...
class CommercialCrossClusterForwarder:
    ...
    def is_circuit_open(self, cluster_id: str) -> bool:
        ...
        if cb["state"] == "open":
            ...
            if (
                now - cb["last_failure"]
                > cfg.commercial_cross_cluster_forwarding_circuit_breaker_reset_seconds
            ):
                cb["state"] = "half_open"
                return False
            return True
    ...
    def mark_failure(self, cluster_id: str):
        ...
        if (
            cb["failures"]
            >= cfg.commercial_cross_cluster_forwarding_circuit_breaker_failure_threshold
        ):
            cb["state"] = "open"

    def should_forward_request(
        self, target_cluster: CommercialClusterRegistry, qos_tier: Any | None = None
    ) -> bool:
    ...
    def generate_internal_jwt(self, audience: str | None) -> str:
    ...
    def build_forward_headers(
        ...
        target_cluster: CommercialClusterRegistry,
        correlation_id: str,
    ) -> dict:
        headers = {}
    ...
    async def forward_request(
        self, request: Request, target_cluster: CommercialClusterRegistry, body: bytes
    ) -> Response | None:
        """
    ...
    async def stream_sse_forward(
        ...
        try:
            ...
            return StreamingResponse(
                _stream_generator(), status_code=resp.status_code, headers=dict(resp.headers)
            )
    ...
    async def _record_event_safe(
        ...
        try:
            await self._record_event(
                target_cluster_id, correlation_id, mode, result, latency, bytes_out, bytes_in
            )
        except Exception:
