...
def rank_commercial_routes(
    ...
    for cand in candidates:
        ...
        if dynamic_configs:
            ...
            def sort_key(c):
                ...
                if st is None and isinstance(c, dict):
                    st = c.get("scope_type", "")
                return {"provider_model": 1, "model": 2, "provider": 3, "global": 4}.get(
                    st or "", 5
                )
            ...
            if matches:
                matches.sort(
                    key=lambda x: (
                        sort_key(x),
                        -getattr(x, "canary_enabled", False),
                        -getattr(x, "created_at", datetime.now(UTC)).timestamp(),
                    )
                )
                ...
                canary_config = next(
                    (c for c in configs_for_scope if getattr(c, "canary_enabled", False)), None
                )
                stable_config = next(
                    (c for c in configs_for_scope if not getattr(c, "canary_enabled", False)), None
                )
                ...
                if canary_config:
                    ...
                    if bucket < getattr(canary_config, "canary_percent", 0):
                        ...
                        best = next(
                            (c for c in matches if not getattr(c, "canary_enabled", False)), None
                        )
                    ...
                    eff_config.update(
                        {
                            ...
                            "variant": variant,
                            "cost_multiplier": get_attr(best, "cost_multiplier")
                            if get_attr(best, "cost_multiplier") is not None
                            else 1.0,
                            "margin_weight": get_attr(best, "margin_weight")
                            if get_attr(best, "margin_weight") is not None
                            else 0.6,
                            "latency_weight": get_attr(best, "latency_weight")
                            if get_attr(best, "latency_weight") is not None
                            else 0.2,
                            "quality_weight": get_attr(best, "quality_weight")
                            if get_attr(best, "quality_weight") is not None
                            else 0.2,
                            "local_route_bonus": get_attr(best, "local_route_bonus")
                            if get_attr(best, "local_route_bonus") is not None
                            else 20.0,
                            "min_margin_percent": get_attr(best, "min_margin_percent")
                            if get_attr(best, "min_margin_percent") is not None
                            else 10.0,
        ...
        price_res = calculate_customer_price(
            client_plan, estimated_input_tokens, estimated_output_tokens
        )
        ...
        if (
            is_cloud
            and margin_res.margin_percent is not None
            and margin_res.margin_percent < min_margin
        ):
        ...
        if is_cloud and wallet_balance is not None and wallet_balance < adjusted_cost_brl:
            rejection_reasons.append(
                f"insufficient_wallet_balance_{wallet_balance:.2f}_needed_{adjusted_cost_brl:.2f}"
            )
        ...
        weighted_score = (
            (margin_score * w_margin) + (quality_score * w_quality) - (latency_penalty * w_latency)
        )
        ...
        if rejection_reasons:
            ...
            logger.debug(
                "commercial_route_ranked: provider=%s score=%.2f", provider, explanation.score
            )
    ...
    if ranked:
        ...
        logger.info(
            ...
            top.model,
            top.score,
            top.estimated_margin_percent or 0,
        )
