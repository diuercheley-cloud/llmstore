...
async def validate_simulation_against_policy(
    ...
    if sla_impact.get("risk") == "critical" or sla_impact.get("sla_risk_increase_percent", 0) > policy.max_predicted_sla_violation_percent:
        return "blocked", "SLA risk exceeds policy limits"

    # 4. Blast radius check
    if requires_manual_approval(simulation, policy):
        return "requires_approval", f"Blast radius {simulation.blast_radius} requires manual approval"
...
async def validate_against_defaults(simulation: CommercialInfrastructureSimulation) -> Tuple[str, Optional[str]]:
    settings = get_settings()
...
def requires_manual_approval(simulation: CommercialInfrastructureSimulation, policy: CommercialSafetyPolicy) -> bool:
...
def classify_blast_radius(impact_factors: Dict[str, Any]) -> str:
    ...
    if clusters > 1 or nodes > 20: return "critical"
    if clusters == 1 or nodes > 10: return "high"
    if nodes > 2: return "medium"
    return "low"
