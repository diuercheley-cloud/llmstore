...
class CommercialQoSFairnessService:
    ...
    async def collect_queue_metrics(db: AsyncSession, redis: Any) -> list[CommercialQueueMetric]:
        ...
        interval_start = now - timedelta(
            seconds=settings.commercial_qos_fairness_collection_interval_seconds
        )
        ...
        for tier in tiers:
            ...
            depth_stmt = (
                ...
                .where(and_(GenerationJob.status == "queued", GenerationJob.qos_tier == tier))
            )
            ...
            stats_stmt = select(
                ...
                and_(GenerationJob.qos_tier == tier, GenerationJob.dequeued_at >= interval_start)
            )
            ...
            p95_stmt = (
                select(GenerationJob.queue_wait_ms)
                .where(
                    and_(
                        GenerationJob.qos_tier == tier, GenerationJob.dequeued_at >= interval_start
                    )
                )
            ...
            starvation_stmt = select(func.count()).where(
                and_(
                    ...
                    GenerationJob.qos_tier == tier,
                    (func.extract("epoch", now) - func.extract("epoch", GenerationJob.queued_at))
                    * 1000
    ...
    async def summarize_fairness(db: AsyncSession, hours: int = 24) -> dict[str, Any]:
        ...
        # Get latest metrics
        stmt = (
            select(CommercialQueueMetric)
            .where(CommercialQueueMetric.timestamp >= since)
            .order_by(CommercialQueueMetric.timestamp.desc())
        )
    ...
    async def detect_starvation(db: AsyncSession) -> list[dict[str, Any]]:
        ...
        stmt = select(GenerationJob).where(
            and_(
                GenerationJob.status == "queued",
                (func.extract("epoch", now) - func.extract("epoch", GenerationJob.queued_at)) * 1000
                > threshold_ms,
            )
        )
        result = await db.execute(stmt)
    ...
    async def detect_priority_inversion(db: AsyncSession, limit: int = 100) -> list[dict[str, Any]]:
        ...
        stmt = (
            ...
            .where(and_(GenerationJob.dequeued_at >= since, GenerationJob.qos_tier != None))
            .order_by(GenerationJob.dequeued_at.desc())
            .limit(limit)
        )
            ...
            inversion_stmt = (
                ...
                .where(
                    and_(
                        ...
                        (GenerationJob.dequeued_at > job.dequeued_at)
                        | (GenerationJob.status == "queued"),
                    )
                )
                .limit(1)
