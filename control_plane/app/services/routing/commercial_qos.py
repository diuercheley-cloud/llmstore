...
class CommercialQoSService:
    ...
    async def get_all_tiers(db: AsyncSession) -> list[CommercialQoSTier]:
        result = await db.execute(
            select(CommercialQoSTier).order_by(CommercialQoSTier.priority.desc())
        )
    ...
    @staticmethod
    async def resolve_qos_tier(
        db: AsyncSession, client_id: uuid.UUID | None, plan_code: str | None
    ) -> CommercialQoSTier:
        ...
        if not tier:
            ...
            result = await db.execute(
                select(CommercialQoSTier).where(CommercialQoSTier.enabled == True).limit(1)
            )
        ...
        constraints = get_active_revenue_protection_constraints(
            client_id=client_id, qos_tier=tier.name
        )
    ...
    def evaluate_route_against_qos(
        ...
        if is_cloud and estimated_margin_percent is not None:
            min_margin = (
                float(tier.min_margin_percent) if tier.min_margin_percent is not None else 0.0
            )
        ...
        # 3. Cost constraint
        max_cost = (
            float(tier.max_cost_per_request_brl)
            if tier.max_cost_per_request_brl is not None
            else 999.0
        )
