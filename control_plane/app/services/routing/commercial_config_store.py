...
class CommercialConfigStore:
    ...
    def validate_config_safety(
        ...
        if not force:
            if cost_multiplier > 3.0 or cost_multiplier < 0.3:
                raise ValueError(f"Cost multiplier {cost_multiplier} out of safe bounds (0.3-3.0). Use force=True to override.")
    ...
    async def deactivate_config(self, config_id: uuid.UUID, actor: str = "system") -> bool:
        ...
        audit = AdminActionLog(
            ...
            admin_role="admin",
            status="success",
            payload_json={"config_id": str(config.id), "scope": config.scope_type, "provider": config.provider, "model": config.model}
        )
        self.db.add(audit)
        await self.db.commit()
        return True

    async def rollback_config(self, scope_type: str, provider: Optional[str], model: Optional[str], actor: str = "system") -> Optional[CommercialRoutingConfig]:
        """
