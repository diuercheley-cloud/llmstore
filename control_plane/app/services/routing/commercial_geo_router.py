...
class CommercialGeoRouter:
    ...
    def detect_cross_ocean(
        self, cluster1: CommercialClusterRegistry, cluster2: CommercialClusterRegistry
    ) -> bool:
        ...
        dist = self.calculate_geo_distance_km(
            cluster1.latitude or 0,
            cluster1.longitude or 0,
            cluster2.latitude or 0,
            cluster2.longitude or 0,
        )
    ...
    def calculate_latency_penalty(self, distance_km: float, avg_latency: int) -> float:
        ...
        if distance_km > 0:
            ...
            penalty += min(
                1.0, distance_km / self.settings.commercial_geo_routing_max_region_distance_km
            )
    ...
    def score_geo_candidate(
        ...
        if source_cluster.latitude and candidate.latitude:
            dist_km = self.calculate_geo_distance_km(
                source_cluster.latitude,
                source_cluster.longitude,
                candidate.latitude,
                candidate.longitude,
            )
        ...
        latency_penalty = self.calculate_latency_penalty(
            dist_km, candidate.avg_public_latency_ms or 0
        )

        # 4. Same Region Group Bonus
        region_bonus = 0.0
        if (
            source_cluster.region_group
            and candidate.region_group
            and source_cluster.region_group == candidate.region_group
        ):
            region_bonus = 1.0
        ...
        score = (
            base
            - (latency_penalty * w_lat)
            + (norm_margin * w_margin)
            + (candidate_health * w_health)
            + (region_bonus * w_reg)
        )
    ...
    def rank_geo_clusters(
        ...
        for info in candidates_info:
            ...
            score_result = self.score_geo_candidate(
                source_cluster, candidate, margin, health, qos_tier=qos_tier
            )
