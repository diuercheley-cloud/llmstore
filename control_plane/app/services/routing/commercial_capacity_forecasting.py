...
async def forecast_capacity(
...
) -> CommercialCapacityForecast | None:
    """
    ...
    stmt = (
        ...
        .where(and_(*filters))
        .order_by(CommercialCapacitySnapshot.timestamp.asc())
    )
    ...
    if trend > 0.05:  # Growing more than 5% in lookback window
        ...
        limit_rpm = 1000.0
        minutes_to_limit = (
            (limit_rpm - recent_rpm) / (trend * recent_rpm / (lookback_window.total_seconds() / 60))
            if trend > 0
            else 0
        )
