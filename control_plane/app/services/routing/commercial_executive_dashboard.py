...
class CommercialExecutiveDashboardService:
    ...
    async def get_overview(
        ...
        clients = await self.summarize_clients_profitability(
            since, limit=10, client_id=client_id, provider=provider, model=model
        )

        # 4. Providers Profitability
        providers = await self.summarize_providers_profitability(
            since, limit=10, client_id=client_id, provider=provider, model=model
        )
    ...
    async def get_revenue_forecast_summary(self) -> dict[str, Any]:
        ...
        stmt = select(CommercialRevenueForecast).order_by(
            CommercialRevenueForecast.created_at.desc()
        )
    ...
    async def get_financial_anomalies_summary(self) -> dict[str, Any]:
        ...
        stmt = (
            select(CommercialFinancialAnomaly)
            .order_by(CommercialFinancialAnomaly.detected_at.desc())
            .limit(10)
        )
        result = await self.db.execute(stmt)
    ...
    async def get_infra_simulation_summary(self) -> dict[str, Any]:
        ...
        stmt = (
            select(CommercialInfrastructureSimulation)
            .order_by(CommercialInfrastructureSimulation.created_at.desc())
            .limit(10)
        )
        result = await self.db.execute(stmt)
        ...
        return {
            ...
            "health_badge": "SAFE"
            if not blocked and not requires_approval
            else "WARNING"
    ...
    async def get_qos_sla_summary(self, since: datetime) -> dict[str, Any]:
        ...
        stmt = (
            select(
                ...
                .filter(CommercialRoutingEvent.sla_pass == True)
                .label("passed"),
        ...
        return {
            ...
            "overall_sla_pass_rate": round(
                sum(t["sla_pass_rate"] for t in tier_summaries) / len(tier_summaries), 2
            )
            if tier_summaries
            else 100.0,
        }
    ...
    async def get_drift_overview(
        ...
        """
        current_end = date_to or datetime.now(UTC)
        ...
        return {
            ...
            "margin_drift_percent": round(
                calc_drift(current["avg_margin_pct"], previous["avg_margin_pct"]), 2
            ),
            "latency_drift_percent": round(
                calc_drift(current["avg_latency"], previous["avg_latency"]), 2
            ),
        }

    async def _query_basic_metrics(
        self, start: datetime, end: datetime, client_id, provider, model
    ) -> dict[str, float]:
        filters = [
    ...
    async def summarize_canary_health(self) -> dict[str, Any]:
        ...
        rollbacks_stmt = select(func.count(AdminActionLog.id)).where(
            ...
            >= datetime.now(UTC) - timedelta(hours=self.settings.commercial_anomaly_lookback_hours),
        )
        ...
        return {
            ...
            "pass_rate_percent": round(healthy_count / len(promotions) * 100, 2)
            if promotions
            else 100.0,
    ...
    async def detect_anomalies(
        self, since: datetime, profitability: dict[str, Any], drift: dict[str, Any]
    ) -> list[dict[str, Any]]:
        anomalies = []
    ...
    async def generate_executive_recommendations(
        self, anomalies: list[dict[str, Any]], canaries: dict[str, Any]
    ) -> list[dict[str, Any]]:
        recommendations = []
