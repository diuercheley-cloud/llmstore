...
class CommercialQoSChargebackService:
    @staticmethod
    async def calculate_chargeback(db: AsyncSession, period_hours: int = 24) -> List[CommercialQueueChargeback]:
        ...
        stmt = select(GenerationJob).where(
            and_(
                GenerationJob.completed_at >= period_start,
                ...
                if job.started_at:
                    opp_stmt = select(func.count()).where(
                        and_(
                            ...
                            (GenerationJob.dequeued_at > job.started_at) | (GenerationJob.status == "queued")
                        )
                    )
