...
class ProxmoxAdapter(BaseInfraAdapter):
    ...
    def _get_headers(self) -> Dict[str, str]:
        if not self.settings.commercial_proxmox_token_id or not self.settings.commercial_proxmox_token_secret:
            return {}
    ...
    async def execute_action(self, simulation: CommercialInfrastructureSimulation, dry_run: bool = True) -> Dict[str, Any]:
        ...
        # Check allowlist
        allowed_vms = [x.strip() for x in self.settings.commercial_proxmox_allowed_vm_ids.split(",") if x.strip()]
        allowed_cts = [x.strip() for x in self.settings.commercial_proxmox_allowed_ct_ids.split(",") if x.strip()]
    ...
    def get_capacity(self) -> Dict[str, Any]:
        ...
        node = self.settings.commercial_proxmox_node
        url = f"{self.settings.commercial_proxmox_api_url.rstrip('/')}/api2/json/nodes/{node}/status"
