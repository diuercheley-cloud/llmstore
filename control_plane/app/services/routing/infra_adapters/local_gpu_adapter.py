...
class LocalGPUAdapter(BaseInfraAdapter):
    ...
    async def execute_action(
        ...
        allowed_actions = [
            x.strip()
            for x in self.settings.commercial_local_gpu_allowed_actions.split(",")
            if x.strip()
        ]
    ...
    def apply_power_limit(self, limit_watts: int | None) -> dict[str, Any]:
        if not limit_watts:
    ...
    def kill_gpu_process(self, pid: int | None) -> dict[str, Any]:
