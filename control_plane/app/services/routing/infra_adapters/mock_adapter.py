...
class MockAdapter(BaseInfraAdapter):
    ...
    async def execute_action(
        ...
        logger.info(
            f"Mock execution: {simulation.simulation_type} on {simulation.target_identifier} (dry_run={dry_run})"
        )
        return {
    ...
    async def rollback_action(self, execution_record_id: str) -> dict[str, Any]:
        ...
        return {"status": "rolled_back", "message": "Successfully rolled back mock action"}

    async def get_status(self, external_operation_id: str) -> dict[str, Any]:
        return {"status": "completed", "external_id": external_operation_id}
