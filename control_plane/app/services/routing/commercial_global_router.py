...
async def resolve_routing_settings(db: AsyncSession, settings: Settings | None = None) -> Any:
    ...
    try:
        stmt = (
            ...
            .order_by(GlobalRoutingPolicyVersion.version.desc())
        )
    ...
    *,
    ...
    if region_preference and cluster_region == region_preference:
        ...
        if not cfg.commercial_global_routing_allow_cross_region:
            warnings.append(
                f"Cross-region not allowed (Cluster: {cluster_region}, Target: {region_preference})"
            )
...
async def rank_clusters(
    ...
    *,
    ...
    for cluster in clusters:
        ...
        if not allowed:
            rejected.append(
                {"cluster_id": cluster_id, "reason": "tenant_scope_rejected", "message": reason}
            )
            continue

        # Validation: Health
        if cfg.commercial_global_routing_require_healthy_cluster and cluster.get("status") not in {
            "active",
            "degraded",
        }:
            rejected.append(
        ...
        if (
            not cfg.commercial_global_routing_allow_cross_region
            and region_preference
            and cluster.get("region") != region_preference
        ):
            rejected.append(
                {
                    "cluster_id": cluster_id,
        ...
        score_info = score_cluster(
            cluster, region_preference=region_preference, settings=cfg, qos_tier=qos_tier
        )
...
async def simulate_global_route(
    ...
    *,
    ...
    cfg = await resolve_routing_settings(db, settings)
    ranking = await rank_clusters(
        db, tenant_id=tenant_id, region_preference=region_preference, settings=cfg
    )
    ...
    return sanitize_report_payload(
        {
            ...
            if recommended
            else ["No cluster available for routing"],
        }
    )
