...
class CommercialCanaryPromotionService:
    ...
    async def get_canary_metrics(
        self, config_id: uuid.UUID, window_minutes: int = 60
    ) -> dict[str, Any]:
        """
    ...
    async def _query_metrics(
        self, config_id: uuid.UUID | None, variant: str, since: datetime
    ) -> dict[str, Any]:
        filters = [
        ...
        stmt = select(
            ...
            .filter(CommercialRoutingEvent.error_type.is_not(None))
            .label("errors"),
            ...
            func.avg(
                func.abs(
                    func.coalesce(CommercialRoutingEvent.estimated_margin_percent, 0)
                    - func.coalesce(CommercialRoutingEvent.actual_margin_percent, 0)
                )
            ).label("avg_est_error"),
            func.count(CommercialRoutingEvent.id)
    ...
    async def evaluate_canary_slo(self, config_id: uuid.UUID) -> dict[str, Any]:
        ...
        window = (
            config.canary_observation_window_minutes
            or self.settings.commercial_canary_min_observation_minutes
        )
        ...
        observation_mins = (datetime.now(UTC) - started_at).total_seconds() / 60
        checks.append(
            {
        ...
        if stable.get("avg_latency_ms", 0) > 0:
            regression = (
                (canary["avg_latency_ms"] - stable["avg_latency_ms"]) / stable["avg_latency_ms"]
            ) * 100
        ...
        return {"pass": all_pass, "checks": checks, "metrics": metrics, "config_id": str(config_id)}
    ...
    async def promote_canary_step(
        self, config_id: uuid.UUID, actor: str = "system"
    ) -> dict[str, Any]:
        ...
        if recommendation["action"] == "promote":
            ...
            config.canary_last_promoted_at = datetime.now(UTC)
            ...
            await self._log_action(
                ...
                payload={
                    ...
                    "before": old_percent,
                    "after": new_percent,
                    "metrics": recommendation["slo"]["metrics"],
                },
                actor=actor,
            )
    ...
    async def complete_canary_to_stable(
        self, config_id: uuid.UUID, actor: str = "system"
    ) -> dict[str, Any]:
        config = await self.db.get(CommercialRoutingConfig, config_id)
        ...
        config.stable_promoted_at = datetime.now(UTC)
    ...
    async def auto_rollback_if_unhealthy(
        self, config_id: uuid.UUID, actor: str = "system"
    ) -> dict[str, Any]:
