...
async def calculate_estimation_error(
    ...
    stmt = (
        select(CommercialRoutingEvent.estimated_cost_brl, CommercialRoutingEvent.actual_cost_brl)
        .where(*filters)
        .order_by(desc(CommercialRoutingEvent.created_at))
    )
...
async def summarize_error_by_model(db: AsyncSession, days: int = 7) -> list[dict[str, Any]]:
    ...
    for p, m, count in models:
        if not p or not m:
            continue
        summary = await calculate_estimation_error(
            db, days=days, provider=p, model=m, min_samples=1
        )
        summary["provider"] = p
        summary["model"] = m
        summaries.append(summary)
...
def recommend_cost_multiplier(
    avg_error_pct: float, confidence: str, max_multiplier: float = 3.0, min_multiplier: float = 0.5
) -> float:
    """
...
async def recommend_weight_adjustments(
    db: AsyncSession, days: int = 7, settings: Any | None = None
) -> list[dict[str, Any]]:
    """
...
async def generate_calibration_report(
                ...
                multipliers.append(
                    {
                        ...
                        "reason": "actual cost is higher than estimated cost"
                        if mult > 1.0
                        else "actual cost is lower than estimated cost",
                    }
                )
    ...
    return {
        ...
        "warnings": ["Insufficient samples for some providers/models"]
        if any(s.get("confidence") == "low" for s in by_provider)
        else [],
        "mode": "recommend_only",
    }
