...
def hash_secret(secret: str, salt: str | None = None) -> str:
    ...
    digest = hashlib.pbkdf2_hmac("sha256", secret.encode("utf-8"), secret_salt.encode("utf-8"), 200_000)
    return f"{secret_salt}${digest.hex()}"


def verify_secret(secret: str, stored_hash: str) -> bool:
    salt, expected = stored_hash.split("$", 1)
    digest = hashlib.pbkdf2_hmac("sha256", secret.encode("utf-8"), salt.encode("utf-8"), 200_000).hex()
