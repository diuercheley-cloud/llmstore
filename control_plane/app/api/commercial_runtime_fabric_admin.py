...
async def report_heartbeat(
    report: HeartbeatReport, db: AsyncSession = Depends(get_db), admin=Depends(require_admin_user)
):
    service = RuntimeFabricService(db)
...
async def list_healing_events(
    db: AsyncSession = Depends(get_db), admin=Depends(require_admin_user)
):
    service = RuntimeFabricService(db)
...
async def execute_recovery_plan(
    plan_id: str, db: AsyncSession = Depends(get_db), admin=Depends(require_admin_user)
):
    service = RuntimeRecoveryService(db)
...
async def report_drift(
    report: DriftReport, db: AsyncSession = Depends(get_db), admin=Depends(require_admin_user)
):
    service = DeterminismRepairService(db)
    return await service.report_drift(
        report.workflow_id,
        report.step_index,
...
async def trigger_replay_repair(
    drift_id: str, db: AsyncSession = Depends(get_db), admin=Depends(require_admin_user)
):
