...
async def list_receipts(db: AsyncSession = Depends(get_db), admin: Any = Depends(get_admin_user)):
    result = await db.execute(
        select(CommercialRetrievalReceipt)
        .order_by(CommercialRetrievalReceipt.created_at.desc())
        .limit(100)
    )
...
async def list_violations(db: AsyncSession = Depends(get_db), admin: Any = Depends(get_admin_user)):
    result = await db.execute(
        select(CommercialRetrievalPolicyViolation)
        .order_by(CommercialRetrievalPolicyViolation.created_at.desc())
        .limit(100)
    )
...
async def get_lineage(
    chunk_hash: str, db: AsyncSession = Depends(get_db), admin: Any = Depends(get_admin_user)
):
