...
async def get_aiops_forecasts(limit: int = 50, db: AsyncSession = Depends(get_db), admin=Depends(require_admin_user)):
    service = PredictiveAIOpsService(db)
    return await service.get_latest_forecasts(limit)

@router.get("/anomalies")
async def get_aiops_anomalies(limit: int = 50, db: AsyncSession = Depends(get_db), admin=Depends(require_admin_user)):
    service = PredictiveAIOpsService(db)
    return await service.get_latest_anomalies(limit)

@router.get("/recommendations")
async def get_aiops_recommendations(limit: int = 50, db: AsyncSession = Depends(get_db), admin=Depends(require_admin_user)):
    service = PredictiveAIOpsService(db)
    return await service.get_latest_recommendations(limit)

@router.get("/risk-trends")
async def get_aiops_risk_trends(limit: int = 50, db: AsyncSession = Depends(get_db), admin=Depends(require_admin_user)):
    service = PredictiveAIOpsService(db)
