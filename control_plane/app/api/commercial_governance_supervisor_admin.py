...
async def get_supervisor_status(
    db: AsyncSession = Depends(get_db), admin: Any = Depends(get_current_admin)
):
    """
...
async def get_supervisor_incidents(
    ...
    return [
        {"id": str(i.id), "type": i.incident_type, "severity": i.severity, "status": i.status}
        for i in incidents
    ]
...
async def get_supervisor_decisions(
    ...
    return [
        {"id": str(d.id), "type": d.decision_type, "mode": d.mode_used, "approved": d.is_approved}
        for d in decisions
    ]
...
async def get_supervisor_risk(
    ...
    result = await db.execute(
        select(CommercialGovernanceSupervisorRiskScore)
        .order_by(CommercialGovernanceSupervisorRiskScore.calculated_at.desc())
        .limit(10)
    )
    ...
    return [
        {
            "id": str(s.id),
            "overall": s.overall_risk_score,
            "financial": s.financial_risk,
            "compliance": s.compliance_risk,
        }
        for s in scores
    ]
...
async def get_supervisor_explainability(
    decision_id: str, db: AsyncSession = Depends(get_db), admin: Any = Depends(get_current_admin)
):
    """
