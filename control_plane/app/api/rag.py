...
async def upload_client_rag_document(
    ...
    if (
        limits["max_storage_mb"] is not None
        and (float(usage["storage_mb"]) + file_size_mb) > limits["max_storage_mb"]
    ):
        return JSONResponse(
            status_code=429,
            content={
    ...
    await record_rag_event(
        session, client.id, "document_uploaded", document_id=file_id, storage_bytes=file_size
    )
...
async def delete_rag_file(
    ...
    await delete_rag_document(session, doc)
    await record_rag_event(
        session, client.id, "document_deleted", document_id=file_id, storage_bytes=-file_size
    )
    await session.commit()
...
async def query_rag(
    ...
    has_enterprise_fields = (
        payload.collection_ids is not None
        or payload.document_ids is not None
        or payload.abac_attributes is not None
        or payload.rerank
        or (payload.score_threshold and payload.score_threshold > 0.0)
    )
    if (
        has_enterprise_fields
        or effective_plan.code in ("enterprise", "scale")
        or getattr(effective_plan, "rag_max_documents", 0) > 5
    ):
    ...
    hits = await store.search(
        collection_name="rag_chunks", vector=query_embedding, limit=payload.top_k, filters=filters
    )
    ...
    documents = await backend.document_store.get_rag_documents_by_ids(
    ...
    for hit in hits:
        ...
        if doc is None:
            continue
        context_str += (
            f"[fonte: {doc.original_filename}, página {chunk.page_number}]\n{chunk.content}\n\n"
        )
        sources.append(
            RAGSource(
