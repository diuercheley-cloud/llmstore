...
class ModelCapabilitySchema(BaseModel):
    ...
    latency_class: str
    quality_tier: int
    recommended_step_classes: list[str] | None = None
...
async def get_capabilities(
    db: AsyncSession = Depends(get_db_session), admin=Depends(require_admin)
):
...
async def simulate_routing(
    ...
    try:
        ...
        result = await db.execute(
            select(AgentStepRoutingDecision).filter(AgentStepRoutingDecision.run_id == mock_run_id)
        )
