...
async def run_workflow(
    id: uuid.UUID, payload: WorkflowRunRequest, db: AsyncSession = Depends(get_db_session)
):
...
async def get_workflow_run(
    id: uuid.UUID, run_id: uuid.UUID, db: AsyncSession = Depends(get_db_session)
):
...
async def create_webhook_wait(
    ...
    return WebhookResponse(
        id=sub.id, secret_token=sub.secret_token, webhook_url=f"/agents/workflows/webhooks/{sub.id}"
    )
...
async def create_polling_job(
    id: uuid.UUID,
    run_id: uuid.UUID,
    payload: PollingJobCreate,
    db: AsyncSession = Depends(get_db_session),
):
    service = WorkflowPollingService(db)
    job = await service.create_polling_job(
        run_id, "default", payload.url, payload.stop_condition, payload.interval
    )
    await db.commit()
