...
async def get_opportunities(db: AsyncSession = Depends(get_db), _: dict = Depends(require_admin)):
    ...
    res = await db.execute(
        select(CommercialClusterRegistry).where(CommercialClusterRegistry.status == "active")
    )
...
async def simulate_live_balancing(payload: dict, _: dict = Depends(require_admin)):
    ...
    if recommendation["action"] != "none":
        is_flapping = balancer.detect_flapping(
            "target-cluster", recommendation.get("target_decrease_percent", 0), history
        )
        if is_flapping:
    ...
    return {"status": "success", "recommendation": recommendation, "simulation": simulation}
