...
router = APIRouter(
    prefix="/admin/onboarding", tags=["admin-onboarding"], dependencies=[Depends(require_admin)]
)
...
async def update_onboarding_status(
    payload: OnboardingStatusUpdate, redis: Redis = Depends(get_redis)
):
    status_dict = {
        "is_finished": payload.is_finished,
        "finished_at": datetime.now(UTC).isoformat() if payload.is_finished else None,
