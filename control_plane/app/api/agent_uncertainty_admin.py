...
async def list_uncertainty_events(agent_id: uuid.UUID, db: AsyncSession = Depends(get_db)):
...
async def update_uncertainty_policy(
    agent_id: uuid.UUID, config: dict[str, Any], db: AsyncSession = Depends(get_db)
):
    ...
    policy.min_confidence_threshold = config.get(
    ...
    policy.hitl_on_low_confidence = config.get(
        "hitl_on_low_confidence", policy.hitl_on_low_confidence
    )
