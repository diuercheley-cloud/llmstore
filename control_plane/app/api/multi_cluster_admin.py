...
async def drain_cluster(
    ...
    service = MultiClusterOperationsService(db)
    return await service.set_cluster_status(
        id, "degraded", reason="Draining traffic", operator_id=admin.id
    )
...
async def resume_cluster(
    ...
    service = MultiClusterOperationsService(db)
    return await service.set_cluster_status(
        id, "active", reason="Resuming normal operations", operator_id=admin.id
    )
