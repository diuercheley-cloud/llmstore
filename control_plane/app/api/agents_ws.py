...
async def ws_agent_run_stream(
    websocket: WebSocket, run_id: str, db: AsyncSession = Depends(get_db_session)
):
    # Verify feature flag
    settings = get_settings()
    if not settings.agent_websocket_streaming_enabled:
        await websocket.close(
            code=status.WS_1008_POLICY_VIOLATION, reason="WebSocket streaming is disabled"
        )
        return
    ...
    if not run or run.tenant_id != str(client.id):
        ...
        await websocket.close(
            code=status.WS_1008_POLICY_VIOLATION, reason="Run not found or tenant mismatch"
        )
        return
        ...
        while True:
            ...
            if command == "cancel":
                ...
                await websocket.send_json(
                    {"status": "command_processed", "command": "cancel", "success": success}
                )
            elif command == "pause":
                try:
