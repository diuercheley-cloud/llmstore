...
async def get_capacity_overview(
    db: AsyncSession = Depends(get_db), admin=Depends(get_admin_user), cluster_id: str | None = None
):
    ...
    stmt = (
        ...
        .where(CommercialCapacitySnapshot.cluster_id == cid)
...
async def get_capacity_forecast(
    ...
    stmt = (
        ...
        .where(CommercialCapacityForecast.cluster_id == cid)
    ...
    return {"cluster_id": cid, "forecasts": forecasts}
...
async def get_autoscaling_recommendations(
    ...
    stmt = (
        select(CommercialAutoscalingRecommendation)
        .where(CommercialAutoscalingRecommendation.cluster_id == cid)
        .order_by(CommercialAutoscalingRecommendation.created_at.desc())
        .limit(20)
    )
    ...
    return {"cluster_id": cid, "recommendations": recs}
