...
async def get_ops_overview(
    request: Request, admin: Any = Depends(get_current_admin)
) -> dict[str, Any]:
    return {
        "status": "healthy",
...
async def get_ops_risk(request: Request, admin: Any = Depends(get_current_admin)) -> dict[str, Any]:
    return {
        "overall_risk": "low",
        "heatmaps": {"compliance": "low", "security": "low", "operational": "medium"},
        "remediation_timeline": [],
    }


@commercial_ops_center_admin_router.get("/workflows")
async def get_ops_workflows(
    request: Request, admin: Any = Depends(get_current_admin)
) -> dict[str, Any]:
    return {
        "dags_active": 38,
