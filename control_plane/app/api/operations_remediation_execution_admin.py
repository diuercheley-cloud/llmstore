...
async def prepare_remediation_execution(
    ...
    stmt_steps = (
        select(RemediationStep)
        .where(RemediationStep.plan_id == payload.plan_id)
        .order_by(RemediationStep.step_order.asc())
    )
    ...
    return {"execution": execution, "receipt": receipt}
...
async def execute_remediation(
    ...
    stmt_steps = (
        select(RemediationStep)
        .where(RemediationStep.plan_id == plan.id)
        .order_by(RemediationStep.step_order.asc())
    )
    steps = (await db.execute(stmt_steps)).scalars().all()

    stmt_approvals = select(RemediationApprovalRequirement).where(
        RemediationApprovalRequirement.plan_id == plan.id
    )
    ...
    stmt_ks = select(RemediationKillSwitchState).where(
        RemediationKillSwitchState.client_id == payload.client_id
    )
    ...
    result = await EXECUTOR.execute(
        db, execution, plan, list(steps), [r.__dict__ for r in approvals_reqs], ks_state
    )
    ...
    return {"execution": execution, "step_results": result["step_results"], "receipt": receipt}
...
async def list_remediation_executions(
    ...
    stmt = (
        ...
        .where(RemediationExecution.client_id == client_id)
...
async def update_remediation_kill_switch(
    ...
    stmt = select(RemediationKillSwitchState).where(
        RemediationKillSwitchState.client_id == payload.client_id
    )
    ...
    if not state:
        state = RemediationKillSwitchState(
            ...
            immutable_hash=compute_deterministic_hash(
                fields={"client_id": str(payload.client_id), "op": "kill_switch_init"}
            ),
        )
        db.add(state)
    else:
        ...
        state.immutable_hash = compute_deterministic_hash(
            fields={
    ...
    await db.commit()
    await log_remediation_kill_switch_updated(
        db, payload.client_id, payload.enabled, payload.reason
    )
    await db.commit()
...
async def get_remediation_kill_switch(
    ...
    stmt = select(RemediationKillSwitchState).where(
        RemediationKillSwitchState.client_id == client_id
    )
    state = (await db.execute(stmt)).scalar_one_or_none()
    if not state:
...
async def get_remediation_rollback_plan(
    ...
    stmt = select(RemediationRollbackPlan).where(
        RemediationRollbackPlan.execution_id == execution_id
    )
