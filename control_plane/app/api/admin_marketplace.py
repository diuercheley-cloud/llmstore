...
async def approve_installation(
    request: ApproveInstallRequest,
    db: AsyncSession = Depends(get_db_session)
):
    # In a real system, this would finalize the download and register local agent
    return {"status": "success", "agent_id": str(uuid.uuid4()), "message": "Agent installed and sandbox provisioned."}
