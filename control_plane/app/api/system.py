    ...
    return {
        ...
        "summary": {
            ...
            "healthy": sum(
                ...
                if report.health.get("ok") is True
                or report.health.get("status") in {"healthy", "degraded"}
            ),
...
def _get_latest_artifact_info(base_dir: str, filename: str) -> dict | None:
    try:
        ...
        if report_file.exists():
            return {"timestamp": reports[0].name, "path": str(report_file.relative_to(Path.cwd()))}
        # If filename is empty, just return the directory info
        if not filename:
            return {"timestamp": reports[0].name, "path": str(reports[0].relative_to(Path.cwd()))}
    except Exception:
        pass
    return None
...
def _get_latest_release_info() -> dict | None:
    try:
        ...
        releases = sorted(
            [d for d in release_dir.iterdir() if d.is_dir() and d.name.startswith("v")],
            reverse=True,
        )
        if not releases:
            return None
        return {"version": releases[0].name, "path": str(releases[0].relative_to(Path.cwd()))}
    except Exception:
        pass
    return None
...
async def get_operational_readiness(
    ...
    if settings.agent_runtime_enabled:
        try:
            ...
            checks["agentic"] = (
                "ok"
                if agent_report["status"] in ["ready", "degraded", "disabled"]
                else f"failed: {agent_report['status']}"
            )
        except Exception as e:
    ...
    all_ok = all(
        ...
        if k not in ("modes", "deployment_mode")
    )
...
async def ready(
    ...
    try:
        ...
        if not res.scalar():
            logging.error(
                "Readiness check failed: migrations dependency not ready (missing version)"
            )
    ...
    if not settings.rag_enabled:
        ...
        logging.warning(
            "Readiness degraded reason: RAG component is disabled (opt-in provider disabled)"
        )
        if status != "not_ready":
            status = "degraded"
    else:
    ...
    if not settings.tts_enabled:
        ...
        logging.warning(
            "Readiness degraded reason: TTS component is disabled (opt-in provider disabled)"
        )
        if status != "not_ready":
            status = "degraded"
    else:
    ...
    if not settings.lmstudio_enabled:
        ...
        logging.warning(
            "Readiness degraded reason: LM Studio provider is disabled (opt-in provider disabled)"
        )
        if status != "not_ready":
            status = "degraded"
    else:
    ...
    try:
        ...
        if blockers:
            ...
            logging.error(
                f"Readiness check warning: deployment mode configuration incoherence. Blockers: {blockers}"
            )
        elif warnings:
            if status != "not_ready":
                status = "degraded"
            logging.warning(
                f"Readiness check warning: deployment mode configuration warning. Warnings: {warnings}"
            )
    except Exception as e:
...
async def system_status(
    ...
    return {
        ...
        "cors_configured": bool(
            settings.cors_allow_origins or settings.localhost_mode or settings.local_appliance_mode
        ),
        "components": {
            "api": "online",
...
async def admin_system_status(
    ...
    return {
        ...
        "inference": {"backends": backends, "active_models": models},
        "rag": rag_status,
        "cache": cache_info,
    }
...
async def health_deep(
    ...
    if db_detail["status"] == "online":
        try:
            ...
            job_snapshot = {
                ...
                "delayed": summary.get(
                    "delayed", 0
                ),  # Note: GenerationJob model might not have delayed, but schema expects it
            }
        except Exception as e:
    ...
    if db_detail["status"] == "online":
        try:
            backend_rows = (
                (
                    await session.execute(
                        select(InferenceBackend).where(InferenceBackend.is_active == True)
                    )
                )
                .scalars()
                .all()
                ...
                m_count = (
                    await session.execute(
                        select(func.count(ModelRegistry.id)).where(
                            ModelRegistry.inference_backend_id == b.id
                        )
                    )
                ).scalar() or 0
                backends.append(
    ...
    # TTS
    tts_status = {
        "enabled": current_settings.tts_enabled,
    ...
    if db_detail["status"] == "online":
        try:
            rows = (
                await session.execute(
                    select(Client.billing_status, func.count(Client.id)).group_by(
    ...
    security_info = {
        ...
        "cors_configured": bool(
            current_settings.cors_allow_origins
            or current_settings.localhost_mode
            or current_settings.local_appliance_mode
        ),
    ...
    response["dependencies"] = {
        ...
        "data_plane": {
            ...
            "latency_ms": next(
                (
                    backend.get("latency_ms", 0)
                    for backend in response["inference_backends"]
                    if "latency_ms" in backend
                ),
                0,
            ),
...
async def get_latest_reports(admin=Depends(require_admin)):
    return {
        "readiness": _get_latest_artifact_info("artifacts/production-readiness", "report.json"),
        "security": _get_latest_artifact_info("artifacts/security-reports", "security-report.json"),
        "validation": _get_latest_artifact_info(
            "artifacts/local-production-validation", "report.json"
        ),
    }
...
async def provider_settings_page():
    if settings.public_exposure:
        return Response(content='{"detail":"disabled"}', status_code=404)
    static_file = (
        Path(__file__).resolve().parents[1] / "static" / "provider-settings" / "index.html"
    )
    return FileResponse(static_file)
