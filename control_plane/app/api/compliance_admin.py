...
async def get_control_map(framework: str | None = None, admin: Any = Depends(get_current_admin)):
...
async def get_control_detail(
    framework: str, control_id: str, admin: Any = Depends(get_current_admin)
):
    service = ComplianceControlMapperService()
...
async def get_latest_evidence_index(admin: Any = Depends(get_current_admin)):
    index_path = "artifacts/compliance/latest/evidence-index.json"
    if not os.path.exists(index_path):
        raise HTTPException(
            status_code=404, detail="No evidence index found. Run collection first."
        )
...
async def get_isms_status(admin: Any = Depends(get_current_admin)):
    service = ISMSManagerService()
    return service.get_status()


@router.get("/isms/policies")
async def list_isms_policies(admin: Any = Depends(get_current_admin)):
    service = ISMSManagerService()
    return service.list_policies()


@router.get("/isms/risk-register")
async def get_risk_register(admin: Any = Depends(get_current_admin)):
    service = ISMSManagerService()
...
async def run_gap_analysis(admin: Any = Depends(get_current_admin)):
