...
async def create_draft(
    ...
    return {
        ...
        "status": draft.status,
        "previous_version_id": str(draft.previous_version_id) if draft.previous_version_id else None
    }
...
async def activate_policy(
    ...
    stmt_active = select(GlobalRoutingPolicyVersion).where(GlobalRoutingPolicyVersion.status == "active")
    ...
    return {
        ...
        "previous_version_id": str(policy.previous_version_id) if policy.previous_version_id else None
    }
...
async def rollback_policy(
    ...
    if current_active.previous_version_id:
        stmt_prev = select(GlobalRoutingPolicyVersion).where(GlobalRoutingPolicyVersion.id == current_active.previous_version_id)
        res_prev = await db.execute(stmt_prev)
        target_policy = res_prev.scalars().first()
        
    if not target_policy:
        stmt_fallback = select(GlobalRoutingPolicyVersion).where(GlobalRoutingPolicyVersion.version == current_active.version - 1)
        res_fallback = await db.execute(stmt_fallback)
        target_policy = res_fallback.scalars().first()
        
    if not target_policy:
        raise HTTPException(status_code=400, detail="No previous policy version found to rollback to")
    ...
    return {
        ...
        "previous_version_id": str(target_policy.previous_version_id) if target_policy.previous_version_id else None
    }
...
async def list_policies(
    ...
    for p in policies:
        ...
        out.append({
            ...
            "status": p.status,
            "previous_version_id": str(p.previous_version_id) if p.previous_version_id else None
        })
    return out
