...
async def create_assistant(
    ...
    await db.commit()
    return {"id": str(assistant.id), "object": "assistant", "created_at": int(assistant.created_at.timestamp())}
...
async def list_assistants(
    ...
    return {
        "object": "list",
        "data": [{"id": str(a.id), "name": a.name} for a in assistants]
    }
...
async def create_thread(
    ...
    await db.commit()
    return {"id": str(thread.id), "object": "thread", "created_at": int(thread.created_at.timestamp())}
...
async def add_message(
    ...
    await db.commit()
    return {"id": str(message.id), "object": "thread.message", "created_at": int(message.created_at.timestamp())}
...
async def create_run(
    ...
    await db.commit()
    return {"id": str(run.id), "object": "thread.run", "status": run.status, "assistant_id": str(data.assistant_id)}
