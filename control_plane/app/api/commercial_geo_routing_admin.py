...
async def get_overview(db: AsyncSession = Depends(get_db), _: dict = Depends(require_admin)):
    ...
    candidates_info = [{"cluster": c, "margin": 30.0, "health": 1.0} for c in all_clusters if c.status == "active"]
...
async def simulate_geo_routing(payload: dict, db: AsyncSession = Depends(get_db), _: dict = Depends(require_admin)):
    ...
    candidates_info = [{"cluster": c, "margin": payload.get("margins", {}).get(c.cluster_id, 30.0), "health": 1.0} for c in all_clusters if c.status == "active"]
