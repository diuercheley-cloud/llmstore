...
async def create_policy(payload: PolicyBundlePayload, db: AsyncSession = Depends(get_db_session)):
    ...
    bundle = await registry_service.create_policy_bundle(db=db, **sanitized)
    await db.commit()
...
async def publish_policy(
    bundle_id: uuid.UUID, payload: PublishPayload, db: AsyncSession = Depends(get_db_session)
):
    try:
        bundle = await registry_service.publish_policy_bundle(db, bundle_id, payload.published_by)

        # Sign the bundle on publish
        bundle.signature = engine_service.sign_policy_bundle(
            bundle.rules_json, bundle.immutable_hash
        )
...
async def activate_policy(
    bundle_id: uuid.UUID, payload: ActivatePayload, db: AsyncSession = Depends(get_db_session)
):
    try:
...
async def rollback_policy(
    bundle_id: uuid.UUID, payload: RollbackPayload, db: AsyncSession = Depends(get_db_session)
):
    try:
        bundle = await registry_service.rollback_policy_bundle(
            db, bundle_id, payload.rolled_back_by
        )
        await db.commit()
        await db.refresh(bundle)
        return bundle
...
async def simulate_policy(
    bundle_id: uuid.UUID, payload: SimulatePayload, db: AsyncSession = Depends(get_db_session)
):
    try:
        results = await engine_service.simulate_policy_bundle(
            db, bundle_id, payload.runtime_context
        )
        await db.commit()
        return results
    except ValueError as e:
...
async def get_drift_events(
    resolved: bool = Query(default=False), db: AsyncSession = Depends(get_db_session)
):
    stmt = (
        select(CommercialPolicyDriftEvent)
        .where(CommercialPolicyDriftEvent.resolved == resolved)
        .order_by(desc(CommercialPolicyDriftEvent.created_at))
    )
