...
@router.post(
    "/organizations", response_model=ManagedOrganization, dependencies=[Depends(require_admin)]
)
...
@router.get(
    "/organizations",
    response_model=list[ManagedOrganization],
...
@router.get(
    "/workspaces", response_model=list[ManagedWorkspace], dependencies=[Depends(require_admin)]
)
...
@router.post(
    "/appliances/enrollment-token",
    response_model=ApplianceEnrollmentToken,
    dependencies=[Depends(require_admin)],
)
async def create_enrollment_token(
    ...
    return ApplianceEnrollmentToken(
        enrollment_token=enrollment.enrollment_token, expires_at=enrollment.expires_at
    )
...
async def record_heartbeat(
    ...
    if not success:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN, detail="Appliance not found or revoked"
        )
...
@router.get(
    "/appliances", response_model=list[ManagedAppliance], dependencies=[Depends(require_admin)]
)
