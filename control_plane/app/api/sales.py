...
async def list_leads(
    ...
    query = (
        select(SalesLead)
        .options(selectinload(SalesLead.timeline_notes))
        .order_by(desc(SalesLead.created_at))
    )
    if status:
...
async def quote_preview(
    request: QuotePreviewRequest, _role: AdminRole = Depends(require_admin_role(AdminRole.READ))
):
    ...
    cmd = [
        ...
        str(request.models),
        "--responses",
        str(request.responses),
        "--support-hours",
        str(request.support_hours),
        "--custom-integration-hours",
        str(request.custom_integration_hours),
        "--discount-percent",
        str(request.discount_percent),
    ]
    if request.rag:
        cmd.append("--rag")
    if request.tts:
    ...
    try:
        ...
        if not json_path or not os.path.exists(json_path):
            raise HTTPException(
                status_code=500, detail="Quote JSON file not found in script output"
            )
...
async def monthly_report_preview(
    ...
    result = await session.execute(
        select(
            ...
            func.coalesce(func.sum(RequestLog.completion_tokens_estimated), 0).label(
                "completion_tokens"
            ),
    ...
    recommendations = []
    usage_pct = (
        (total_tokens / effective_plan.monthly_token_quota * 100)
        if effective_plan.monthly_token_quota > 0
        else 0
    )
    if usage_pct < 20:
