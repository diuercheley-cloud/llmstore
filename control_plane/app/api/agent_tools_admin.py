...
class AgentToolCreate(BaseModel):
    ...
    dry_run_supported: bool | None = False
    rollback_supported: bool | None = False
    docs_url: str | None = Field(None, max_length=256)
    data_boundary: str | None = Field(None, max_length=64)
...
class CredentialCreate(BaseModel):
    ...
    agent_tool_id: uuid.UUID | None = None
    agent_id: uuid.UUID | None = None
...
# Endpoints
@router.get(
    "", response_model=list[AgentToolResponse], dependencies=[Depends(verify_tool_registry_active)]
)
...
@router.post(
    ...
    dependencies=[Depends(verify_tool_registry_active)],
)
async def register_agent_tool(payload: AgentToolCreate, db: AsyncSession = Depends(get_db_session)):
...
@router.patch(
    "/{id}", response_model=AgentToolResponse, dependencies=[Depends(verify_tool_registry_active)]
)
async def update_agent_tool(
    id: uuid.UUID, payload: AgentToolUpdate, db: AsyncSession = Depends(get_db_session)
):
...
async def enable_agent_tool(id: uuid.UUID, db: AsyncSession = Depends(get_db_session)):
...
async def disable_agent_tool(id: uuid.UUID, db: AsyncSession = Depends(get_db_session)):
...
async def list_tool_invocations(
    id: uuid.UUID, limit: int = 100, offset: int = 0, db: AsyncSession = Depends(get_db_session)
):
...
async def rollback_invocation(
    id: uuid.UUID, tenant_id: str = "default", db: AsyncSession = Depends(get_db_session)
):
    ...
    try:
        success = await rollback_invocation_side_effects(
            db=db, tenant_id=tenant_id, invocation_id=id
        )
...
async def create_credential(payload: CredentialCreate, db: AsyncSession = Depends(get_db_session)):
...
async def revoke_tool_credential(
    ...
    try:
        ...
        if not success:
            raise HTTPException(
                status_code=404, detail="Credential not found or not owned by tenant."
            )
        await db.commit()
        return {"success": True}
...
@router.get(
    "/quotas",
    response_model=list[QuotaCounterResponse],
    dependencies=[Depends(verify_tool_registry_active)],
)
