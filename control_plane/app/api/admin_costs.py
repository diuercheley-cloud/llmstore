...
from app.schemas.costs import CostByAgent, CostByTenant, CostByTool, CostSummary
...
async def get_costs_summary(
    ...
    stmt = select(func.sum(CostEvent.estimated_cost), func.count(CostEvent.id))
    ...
    return CostSummary(total_cost=float(row[0] or 0.0), currency="BRL", event_count=row[1] or 0)
...
async def get_costs_by_tool(
    ...
    stmt = (
        ...
        .where(CostEvent.tool_name.isnot(None))
        .group_by(CostEvent.tool_name)
    )
    ...
    res = await db.execute(stmt)
    return [
        CostByTool(tool_name=row[0], total_cost=float(row[1] or 0.0), event_count=row[2] or 0)
        for row in res.all()
    ]
...
async def get_costs_by_tenant(
    ...
    res = await db.execute(stmt)
    return [
        CostByTenant(tenant_id=row[0], total_cost=float(row[1] or 0.0), event_count=row[2] or 0)
        for row in res.all()
    ]
