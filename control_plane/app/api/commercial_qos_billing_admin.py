...
async def attach_to_invoice(
    ...
    if not success:
        raise HTTPException(
            status_code=400, detail="Failed to attach to invoice. Check record status and mode."
        )
...
async def debit_wallet(
    ...
    decision = await evaluate_control_policy(
        ...
        before_state={
            ...
            "wallet_transaction_id": str(record.wallet_transaction_id)
    ...
    if not tx:
        raise HTTPException(
            status_code=400, detail=f"Failed to debit wallet: {record.error_message}"
        )
...
async def export_qos_billing_records(
    ...
    stmt = select(CommercialQoSBillingRecord).where(
        CommercialQoSBillingRecord.created_at >= utc_now() - timedelta(hours=hours)
    )
    result = await db.execute(stmt)
