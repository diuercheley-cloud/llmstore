...
async def get_operations_overview(
    admin: Any = Depends(require_admin_role(AdminRole.READ)), settings=Depends(get_settings)
) -> Any:
...
async def list_incidents(admin: Any = Depends(require_admin_role(AdminRole.READ))) -> Any:
    return [
        {
            "id": "INC-2024-001",
...
async def list_recommendations(admin: Any = Depends(require_admin_role(AdminRole.READ))) -> Any:
    return [
        {
            "id": "REC-001",
...
async def list_runtime_nodes(admin: Any = Depends(require_admin_role(AdminRole.READ))) -> Any:
    return [
        {
            "id": "node-01",
...
async def run_readiness_check(admin: Any = Depends(require_admin_role(AdminRole.WRITE))) -> Any:
...
async def reset_circuit_breaker(
    target: str | None = None, admin: Any = Depends(require_admin_role(AdminRole.WRITE))
) -> Any:
...
async def drain_node(
    node_id: str, admin: Any = Depends(require_admin_role(AdminRole.WRITE))
) -> Any:
...
async def rollback_model(
    model_id: str, admin: Any = Depends(require_admin_role(AdminRole.WRITE))
) -> Any:
    return {"status": "rollback_initiated", "model_id": model_id}


@router.get("/security-report")
async def download_security_report(admin: Any = Depends(require_admin_role(AdminRole.READ))) -> Any:
    return {
        "report_url": "/static/reports/security-posture-latest.pdf",
        "generated_at": utc_now().isoformat(),
    }


@router.get("/release-summary")
async def download_release_summary(admin: Any = Depends(require_admin_role(AdminRole.READ))) -> Any:
    return {"release_version": "v1.2.0", "summary_url": "/static/releases/v1.2.0-summary.md"}


@router.get("/readiness-report")
async def get_readiness_report(admin: Any = Depends(require_admin_role(AdminRole.READ))) -> Any:
    return {
