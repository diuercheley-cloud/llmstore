...
async def start_canary_assignment(
    agent_id: uuid.UUID, config: dict[str, Any], tenant_id: str, db: AsyncSession = Depends(get_db)
):
...
async def list_canary_comparisons(agent_id: uuid.UUID, db: AsyncSession = Depends(get_db)):
    stmt = (
        select(AgentCanaryComparison)
        .join(AgentCanaryComparison.shadow_run)
        .join(AgentShadowRun.assignment)
        .where(AgentCanaryAssignment.base_agent_id == agent_id)
    )
