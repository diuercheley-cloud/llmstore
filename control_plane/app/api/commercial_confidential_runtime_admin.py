...
router = APIRouter(
    prefix="/admin/inference/confidential-runtime", tags=["Confidential Runtime Admin"]
)
...
async def list_profiles(db: AsyncSession = Depends(get_db), admin: Any = Depends(get_admin_user)):
    result = await db.execute(
        select(CommercialConfidentialRuntimeProfile).order_by(
            CommercialConfidentialRuntimeProfile.created_at.desc()
        )
    )
...
async def list_sessions(db: AsyncSession = Depends(get_db), admin: Any = Depends(get_admin_user)):
    result = await db.execute(
        select(CommercialConfidentialInferenceSession)
        .order_by(CommercialConfidentialInferenceSession.created_at.desc())
        .limit(100)
    )
...
async def list_audit(db: AsyncSession = Depends(get_db), admin: Any = Depends(get_admin_user)):
    result = await db.execute(
        select(CommercialConfidentialRuntimeAuditEvent)
        .order_by(CommercialConfidentialRuntimeAuditEvent.created_at.desc())
        .limit(100)
    )
