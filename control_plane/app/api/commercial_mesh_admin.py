...
@router.post("/nodes")
def register_node(
    node: NodeCreate, db: Session = Depends(get_db), admin=Depends(require_admin_user)
):
...
def get_replication_logs(
    since_index: int = 0, db: Session = Depends(get_db), admin=Depends(require_admin_user)
):
...
def orchestrate_failover(
    failed_node_id: str, term: int, db: Session = Depends(get_db), admin=Depends(require_admin_user)
):
...
def get_mesh_status(db: Session = Depends(get_db)):
    ...
    return {
        ...
        "active_leaders": sum(1 for n in nodes if n.is_leader),
    }
