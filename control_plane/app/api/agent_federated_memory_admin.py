...
@router.post("/peers")
async def register_peer(config: dict[str, Any], db: AsyncSession = Depends(get_db)):
...
async def sync_memory_summary(
    peer_id: uuid.UUID, tenant_id: str, summary: dict[str, Any], db: AsyncSession = Depends(get_db)
):
...
@router.post("/revoke")
async def propagate_revocation(cluster_id: str, memory_id: str, db: AsyncSession = Depends(get_db)):
