...
async def create_flow(payload: FlowCreate, db: AsyncSession = Depends(get_db_session)):
    flow = AgentFlowDefinition(
        tenant_id="default", name=payload.name, description=payload.description
    )
    db.add(flow)
    await db.flush()
...
async def get_flow(id: uuid.UUID, db: AsyncSession = Depends(get_db_session)):
    ...
    # Get active version
    stmt_v = select(AgentFlowVersion).where(
        AgentFlowVersion.flow_id == id, AgentFlowVersion.is_active == True
    )
...
async def explain_flow(id: uuid.UUID, db: AsyncSession = Depends(get_db_session)):
    ...
    for node in nodes:
        ...
        nodes_explained.append({"node_id": str(node.get("id")), "name": nname, "description": desc})
...
async def dry_run_flow(
    id: uuid.UUID, input_data: dict[str, Any] = None, db: AsyncSession = Depends(get_db_session)
):
    stmt = select(AgentFlowVersion).where(
        AgentFlowVersion.flow_id == id, AgentFlowVersion.is_active == True
    )
    ...
    for node in nodes:
        if node.get("node_type") == "memory_write" and "memory:write" not in version.permissions:
            raise HTTPException(
                status_code=403,
                detail=f"Node {node.get('id')} blocked: missing memory:write permission.",
            )
...
async def get_debug_session(run_id: uuid.UUID, db: AsyncSession = Depends(get_db_session)):
    ...
    stmt_e = (
        select(AgentFlowDebugEvent)
        .where(AgentFlowDebugEvent.session_id == session.id)
        .order_by(AgentFlowDebugEvent.created_at.asc())
    )
