...
async def get_fairness_overview(
    hours: int = 24, db: AsyncSession = Depends(get_db_session), _admin=Depends(get_admin_user)
):
    """
...
async def get_starvation_report(
    db: AsyncSession = Depends(get_db_session), _admin=Depends(get_admin_user)
):
    """
...
async def get_priority_inversions(
    limit: int = 50, db: AsyncSession = Depends(get_db_session), _admin=Depends(get_admin_user)
):
    """
...
async def create_qos_tier(
    ...
    if existing:
        raise HTTPException(
            status_code=409, detail=f"QoS Tier with name '{req.name}' already exists"
        )
...
async def simulate_qos_routing(
    ...
    if not selected and rejected:
        selected, degradation_path = CommercialQoSService.choose_degradation_path(
            tier, ranked, rejected
        )
        sla_pass = False
