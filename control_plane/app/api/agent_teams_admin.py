...
@router.post("/{id}/runs")
async def run_team(
    id: uuid.UUID, payload: TeamRunRequest, db: AsyncSession = Depends(get_db_session)
):
    registry = TeamRegistry(db)
...
async def get_team_run_trace(run_id: uuid.UUID, db: AsyncSession = Depends(get_db_session)):
    ...
    stmt = (
        select(AgentTeamTrace)
        .where(AgentTeamTrace.run_id == run_id)
        .order_by(AgentTeamTrace.created_at.asc())
    )
