...
async def list_peers(db: AsyncSession = Depends(get_db), admin: Any = Depends(get_admin_user)):
...
async def list_checkpoints(
    ...
    result = await db.execute(
        select(CommercialConsistencyCheckpoint).order_by(
            CommercialConsistencyCheckpoint.created_at.desc()
        )
    )
...
async def list_alerts(db: AsyncSession = Depends(get_db), admin: Any = Depends(get_admin_user)):
    result = await db.execute(
        select(CommercialTransparencySplitViewAlert).order_by(
            CommercialTransparencySplitViewAlert.created_at.desc()
        )
    )
...
async def resolve_alert(
    alert_id: uuid.UUID, db: AsyncSession = Depends(get_db), admin: Any = Depends(get_admin_user)
):
    result = await db.execute(
        select(CommercialTransparencySplitViewAlert).where(
            CommercialTransparencySplitViewAlert.id == alert_id
        )
    )
