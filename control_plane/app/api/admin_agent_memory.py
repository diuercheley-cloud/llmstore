...
from app.models.agents.advanced_memory import MemoryScope
...
@router.get("/events")
async def list_memory_events(agent_id: uuid.UUID, session: AsyncSession = Depends(get_db_session)):
    service = AdvancedMemoryService(session)
...
async def forget_agent_memory(
    agent_id: uuid.UUID, event_id: uuid.UUID, session: AsyncSession = Depends(get_db_session)
):
    service = AdvancedMemoryService(session)
...
async def compaction_dry_run(
    ...
    return {
        ...
        "items": [
            {"id": str(m.id), "scope": m.scope, "decay_score": m.decay_score} for m in to_compact
        ],
    }
