...
async def list_profiles(db: AsyncSession = Depends(get_db), admin: Any = Depends(get_admin_user)):
    result = await db.execute(
        select(CommercialAgentProfile).order_by(CommercialAgentProfile.created_at.desc())
    )
...
async def list_executions(db: AsyncSession = Depends(get_db), admin: Any = Depends(get_admin_user)):
    result = await db.execute(
        ...
        .order_by(CommercialAgentExecution.started_at.desc())
        .limit(100)
    )
...
async def list_tool_executions(
    ...
    result = await db.execute(
        select(CommercialAgentToolExecution)
        .order_by(CommercialAgentToolExecution.executed_at.desc())
        .limit(100)
    )
...
async def approve_tool(
    id: uuid.UUID, db: AsyncSession = Depends(get_db), admin: Any = Depends(get_admin_user)
):
    result = await db.execute(
        select(CommercialAgentToolExecution).where(CommercialAgentToolExecution.id == id)
    )
