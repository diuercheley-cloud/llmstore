...
async def handle_agent_webhook(
    trigger_id: uuid.UUID, request: Request, db: AsyncSession = Depends(get_db)
):
    settings = get_settings()
    if (
        not settings.agent_event_driven_enabled
        or not settings.agent_external_webhook_triggers_enabled
    ):
        raise HTTPException(
            status_code=403, detail="Webhook triggers are disabled by feature flag."
        )
