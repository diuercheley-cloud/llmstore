...
async def promote_agent(
    ...
    service = AgentPromotionService(db)
    try:
        return await service.promote_agent(
            agent_id, target_status, approved_by=str(admin.get("id", "admin"))
        )
...
async def get_promotion_history(
    ...
    stmt = (
        select(AgentPromotionGate)
        .where(AgentPromotionGate.agent_id == agent_id)
        .order_by(AgentPromotionGate.created_at.desc())
    )
