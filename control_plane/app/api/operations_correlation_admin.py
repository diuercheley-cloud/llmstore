...
async def run_deterministic_correlation(
    ...
    existing_stmt = select(OperationalCorrelation).where(
        OperationalCorrelation.immutable_hash == corr_hash
    )
    ...
    if created_now:
        await log_correlation_created(
            db, payload.client_id, correlation.id, correlation.correlation_type
        )
        await db.commit()
...
async def list_correlations(
    ...
    stmt = (
        ...
        .where(OperationalCorrelation.client_id == client_id)
