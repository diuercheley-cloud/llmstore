...
class RemediationPlanProposeRequest(BaseModel):
    ...
    source_type: str = Field(
        ..., description="Source of the remediation request (e.g., forecast, correlation)"
    )
    source_ref: str = Field(..., description="Reference ID of the source")
    risk_level: str = Field("low", description="Initial risk level")
    involved_domains: list[str] = Field(
        default_factory=list, description="List of affected domains"
    )
    dry_run: bool = True
...
async def propose_remediation_plan(
    ...
    stmt = select(RemediationPlan).where(
        RemediationPlan.client_id == payload.client_id, RemediationPlan.input_hash == input_hash
    )
    ...
    if existing_plan:
        return {
            ...
            "status": "existing",
            "immutable_hash": existing_plan.immutable_hash,
        }
    ...
    plan_immutable_hash = compute_deterministic_hash(
        fields={
    ...
    for step_data in steps_data:
        step_immutable_hash = compute_deterministic_hash(
            fields={
                "plan_id": str(plan.id),
    ...
    for req_data in approval_reqs_data:
        req_immutable_hash = compute_deterministic_hash(
            fields={
                "plan_id": str(plan.id),
...
async def list_remediation_plans(
    ...
    stmt = (
        ...
        .where(RemediationPlan.client_id == client_id)
...
async def get_remediation_plan(
    plan_id: uuid.UUID, db: AsyncSession = Depends(get_db), _admin: Any = Depends(get_current_admin)
):
...
async def get_remediation_approvals(
    ...
    stmt = select(RemediationApprovalRequirement).where(
