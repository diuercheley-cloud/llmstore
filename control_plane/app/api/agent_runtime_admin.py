...
class AgentDefinitionCreate(BaseModel):
    ...
    policy_id: str | None = Field(None, max_length=128)
    memory_policy_id: str | None = Field(None, max_length=128)
    max_steps: int | None = 10
    max_runtime_seconds: int | None = 300
    max_tokens: int | None = None
    max_cost_brl: float | None = None
...
@router.get(
    "", response_model=list[AgentDefinitionResponse], dependencies=[Depends(verify_runtime_active)]
)
async def get_agents(tenant_id: str | None = None, db: AsyncSession = Depends(get_db_session)):
...
async def create_agent(payload: AgentDefinitionCreate, db: AsyncSession = Depends(get_db_session)):
    agent_def = await agent_state.create_agent_definition(db, payload.model_dump())
    return to_response(agent_def)


@router.patch(
    "/{id}", response_model=AgentDefinitionResponse, dependencies=[Depends(verify_runtime_active)]
)
async def patch_agent(
    id: uuid.UUID, payload: AgentDefinitionUpdate, db: AsyncSession = Depends(get_db_session)
):
    agent_def = await agent_state.get_agent_definition(db, id)
    if not agent_def:
        raise HTTPException(status_code=404, detail="Agent definition not found")

    updated = await agent_state.update_agent_definition(
        db, id, payload.model_dump(exclude_unset=True)
    )
...
async def activate_agent(id: uuid.UUID, db: AsyncSession = Depends(get_db_session)):
    agent_def = await agent_state.get_agent_definition(db, id)
    if not agent_def:
        raise HTTPException(status_code=404, detail="Agent definition not found")
...
async def deprecate_agent(id: uuid.UUID, db: AsyncSession = Depends(get_db_session)):
    agent_def = await agent_state.get_agent_definition(db, id)
    if not agent_def:
        raise HTTPException(status_code=404, detail="Agent definition not found")
...
async def get_slo_report(
    agent_id: uuid.UUID | None = None, db: AsyncSession = Depends(get_db_session)
):
...
async def validate_budget(
    agent_id: uuid.UUID, run_id: uuid.UUID, db: AsyncSession = Depends(get_db_session)
):
    from app.services.agents.agent_budget import AgentBudgetService
...
async def run_playbook(
    id: uuid.UUID, payload: PlaybookRunPayload, db: AsyncSession = Depends(get_db_session)
):
    from app.services.agents.agent_incident_playbooks import AgentIncidentPlaybookService
    ...
    try:
        report = await svc.execute_playbook(
            id, payload.playbook_id, payload.performed_by, payload.confirmation, payload.dry_run
        )
        return report
...
async def list_catalog(item_type: str | None = None, db: AsyncSession = Depends(get_db_session)):
...
async def get_catalog_versions(id: uuid.UUID, db: AsyncSession = Depends(get_db_session)):
    from app.services.agents.agent_catalog import AgentCatalogService
...
async def promote_catalog_item(
    id: uuid.UUID, payload: CatalogPromotePayload, db: AsyncSession = Depends(get_db_session)
):
    from app.services.agents.agent_catalog import AgentCatalogService
...
async def rollback_catalog_item(
    id: uuid.UUID, payload: CatalogRollbackPayload, db: AsyncSession = Depends(get_db_session)
):
    from app.services.agents.agent_catalog import AgentCatalogService

    svc = AgentCatalogService(db)
    try:
        return await svc.rollback(
            id, payload.target_version_id, payload.performed_by, payload.reason
        )
    except ValueError as e:
        raise HTTPException(status_code=400, detail=str(e))
...
async def create_delegation_policy(
    payload: DelegationPolicyCreate, tenant_id: str, db: AsyncSession = Depends(get_db_session)
):
...
async def list_collaboration_sessions(tenant_id: str, db: AsyncSession = Depends(get_db_session)):
    from app.models.agents.agents import AgentCollaborationSession

    res = await db.execute(
        select(AgentCollaborationSession).where(AgentCollaborationSession.tenant_id == tenant_id)
    )
    return list(res.scalars().all())


@router.get("/collaboration-sessions/{id}/trace", dependencies=[Depends(verify_runtime_active)])
async def get_collaboration_trace(id: uuid.UUID, db: AsyncSession = Depends(get_db_session)):
