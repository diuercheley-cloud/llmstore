...
def verify_event_driven_enabled():
    ...
    if not settings.agent_event_driven_enabled:
        raise HTTPException(
            status_code=403, detail="Event-driven agent execution is disabled by feature flag."
        )
...
async def create_event_source(
    source_in: EventSourceCreate,
    db: AsyncSession = Depends(get_db),
    _admin=Depends(get_admin_token),
):
    source = AgentEventSource(
        type=source_in.type, config=source_in.config, tenant_id=source_in.tenant_id
    )
    db.add(source)
    await db.commit()
...
async def create_event_trigger(
    ...
    if trigger.trigger_type == "on_schedule":
        ...
        scheduled = AgentScheduledTrigger(
            trigger_id=trigger.id, cron_expression=cron_expr, timezone=tz_str, next_run_at=next_run
        )
        db.add(scheduled)

        # Subscribe agent to its own schedule event
        sub = AgentEventSubscription(
            event_type="on_schedule", agent_id=trigger.agent_id, tenant_id=trigger.tenant_id
        )
        db.add(sub)
        ...
        webhook = AgentWebhookTrigger(
            trigger_id=trigger.id, secret_hash=secret, signature_header=sig_header
        )
        db.add(webhook)

        # Subscribe agent to its own webhook event
        sub = AgentEventSubscription(
            event_type="on_webhook", agent_id=trigger.agent_id, tenant_id=trigger.tenant_id
        )
        db.add(sub)
        ...
        sub = AgentEventSubscription(
            event_type=trigger.trigger_type, agent_id=trigger.agent_id, tenant_id=trigger.tenant_id
        )
        db.add(sub)
...
async def pause_trigger(
    trigger_id: uuid.UUID, db: AsyncSession = Depends(get_db), _admin=Depends(get_admin_token)
):
...
async def create_event_subscription(
    sub_in: EventSubscriptionCreate,
    db: AsyncSession = Depends(get_db),
    _admin=Depends(get_admin_token),
):
    sub = AgentEventSubscription(
        event_type=sub_in.event_type, agent_id=sub_in.agent_id, tenant_id=sub_in.tenant_id
    )
    db.add(sub)
    await db.commit()
