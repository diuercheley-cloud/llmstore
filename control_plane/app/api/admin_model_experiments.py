...
class ExperimentCreate(BaseModel):
    ...
    target_route_id: uuid.UUID | None = None
...
async def create_experiment(
    ...
    experiment = await registry.create_experiment(
        payload.name, payload.experiment_type, payload.target_route_id, payload.target_tenant_id
    )

    for v in payload.variants:
        await registry.add_variant(
            experiment.id, v.name, v.traffic_weight, v.model_id, v.backend_id, v.is_control
        )
...
async def list_experiments(
    session: AsyncSession = Depends(get_db_session), _=Depends(require_admin_role(AdminRole.READ))
):
    _check_enabled()
