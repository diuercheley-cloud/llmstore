...
from app.models.rag import RAGDocument, RagUsageEvent
...
async def get_usage(session: AsyncSession = Depends(get_db_session)):
    result = await session.execute(
        select(QuotaCounter).order_by(desc(QuotaCounter.updated_at)).limit(200)
    )
...
async def get_latest_security_report_endpoint():
    ...
    checks = report.get("checks", [])
    critical_failures = [
        c for c in checks if c.get("severity") == "critical" and c.get("status") == "fail"
    ]
    ...
    return {
        ...
        "warnings": [{"title": c.get("title"), "details": c.get("details")} for c in warnings],
        "report_path": report.get("_report_internal_path"),
    }
...
async def get_runtime_summary(
    ...
    return {
        ...
        "deep_health": {
            ...
            "data_plane": deep.get("inference_backends", [{}])[0].get("status")
            if deep.get("inference_backends")
            else "offline",
        },
        ...
        "reproducibility": {
            ...
            "coverage": round(
                (reproducibility_replayable / reproducibility_total)
...
async def get_usage_summary(
    ...
    if backend_rows:
        health_results = await asyncio.gather(
            *[proxy.health_url(b.backend_url, b.healthcheck_path) for b in backend_rows]
        )
    ...
    rag_summary = {
        "total_docs": sum(
            item.get("doc_count", item.get("documents_count", 0)) for item in rag_usage_all
        ),
    ...
    for client in clients:
        ...
        if int(client["requests_month"]) > 0:
            bucket["avg_latency_ms_month_weighted"] += float(client["avg_latency_ms_month"]) * int(
                client["requests_month"]
            )
        ...
        plans.append(
            {
                ...
                "cache_hit_rate_month": round(
                    bucket["cache_hits_month"]
                    / (bucket["cache_hits_month"] + bucket["cache_misses_month"]),
                    4,
                )
                if (bucket["cache_hits_month"] + bucket["cache_misses_month"])
                else 0.0,
                "avg_latency_ms_month": round(
                    bucket["avg_latency_ms_month_weighted"] / requests_month, 2
                )
                if requests_month
                else 0.0,
...
async def get_admin_rag_usage(session: AsyncSession = Depends(get_db_session)):
    ...
    start_of_month = datetime.combine(month_start(date.today()), datetime.min.time(), tzinfo=UTC)
    ...
    return [
        ...
        if hasattr(r, "id")
        and hasattr(r, "name")
        and hasattr(r, "doc_count")
        and hasattr(r, "storage_bytes")
    ]
...
async def get_client_usage_summary(
    ...
    today_record = next(
        (r for r in records if r.period_type == "daily" and r.period_start == today_start), None
    )
    month_record = next(
        (r for r in records if r.period_type == "monthly" and r.period_start == month_start), None
    )

    today_tokens = (
        today_record.prompt_tokens + today_record.completion_tokens if today_record else 0
    )
    today_requests = today_record.request_count if today_record else 0

    month_tokens = (
        month_record.prompt_tokens + month_record.completion_tokens if month_record else 0
    )
    ...
    return {
        ...
        "month": {
            ...
            "limit": monthly_quota,
            "percent_used": round(
                (month_tokens / monthly_quota * 100) if monthly_quota > 0 else 0, 2
            ),
