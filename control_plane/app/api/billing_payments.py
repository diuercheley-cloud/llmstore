...
async def stripe_payment_webhook(
    ...
    return await PaymentWebhookService.process_stripe_webhook(
        db=db, payload_bytes=payload, signature_header=stripe_signature
    )
...
async def create_card_payment(
    ...
    return await CardService.create_payment(
        db, client.id, req.amount_cents, req.payment_method_id, req.idempotency_key
    )
...
async def payment_webhook(
    ...
    request: Request,
    db: AsyncSession = Depends(get_db_session),
    x_signature: str | None = Header(None, alias="X-Payment-Signature"),
):
    from app.services.billing.payments.payment_reconciliation import PaymentReconciliationService

    payload = await request.body()
    success = await PaymentReconciliationService.process_webhook(
        db, provider, payload, x_signature or ""
    )
    if not success:
