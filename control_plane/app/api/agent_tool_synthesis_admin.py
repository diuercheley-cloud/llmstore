...
def generate_tool(
    ...
    schema = GeneratedToolSchema(
        name=req.name, description=req.description, parameters=req.parameters
    )
    code = synthesizer.generate_tool_code(schema, req.prompt)

    registry = GeneratedToolRegistry(db)
    tool = registry.register_tool(
        name=req.name, description=req.description, author_id=req.agent_id
    )
    version = registry.add_tool_version(
        tool_id=tool.id, version_tag="v1", code=code, schema_json=req.parameters
    )
...
def validate_tool(
    ...
    try:
        validate_generated_code(
            req.code,
            allow_network=settings.agent_code_sandbox_network_enabled,
            allow_write=settings.agent_code_sandbox_write_enabled,
        )
        return {"status": "valid"}
...
def test_tool(
    ...
    run = interpreter.run_code(
        session_id=session.id, code=req.code, timeout_seconds=req.timeout_seconds
    )
...
def get_sandbox_artifact(
    artifact_id: uuid.UUID, db: Session = Depends(get_db), _: dict = Depends(require_admin)
):
...
def execute_generated_tool(
    ...
    run = interpreter.run_code(
        session_id=session.id, code=code, timeout_seconds=req.timeout_seconds
    )
