...
async def get_circuit_breakers(admin=Depends(get_admin_user)):
    return {"circuit_breakers": _circuit_breakers}
...
async def reset_circuit_breaker(cluster_id: str, admin=Depends(get_admin_user)):
    if cluster_id in _circuit_breakers:
        ...
        return {
            "status": "success",
            "message": f"Circuit breaker for {cluster_id} reset to closed.",
        }
...
async def test_forwarding(
    ...
    res = await session.execute(
        select(CommercialClusterRegistry).where(CommercialClusterRegistry.cluster_id == cluster_id)
    )
    ...
    if not forwarder.should_forward_request(cluster):
        return {
            "status": "blocked",
            "reason": "should_forward_request returned False (check settings or cluster status)",
        }
