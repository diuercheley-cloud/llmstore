...
@router.post("")
async def register_twin(config: dict[str, Any], tenant_id: str, db: AsyncSession = Depends(get_db)):
    service = TwinRegistry(db)
    return await service.register(tenant_id, config)


@router.get("/{twin_id}/state")
async def get_twin_state(twin_id: uuid.UUID, db: AsyncSession = Depends(get_db)):
...
async def send_twin_command(
    twin_id: uuid.UUID, command_data: dict[str, Any], db: AsyncSession = Depends(get_db)
):
    service = DigitalTwinService(db)
    try:
        return await service.send_command(
            twin_id, command_data["command"], command_data.get("parameters", {})
        )
    except PermissionError as e:
        raise HTTPException(status_code=403, detail=str(e))
