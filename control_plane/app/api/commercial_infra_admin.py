...
async def list_executions(limit: int = 50, db: AsyncSession = Depends(get_admin_db)):
    from app.models.commercial.commercial_infra_simulation import CommercialExecutionRecord

    stmt = (
        select(CommercialExecutionRecord)
        .order_by(CommercialExecutionRecord.created_at.desc())
        .limit(limit)
    )
...
async def execute_simulation(
    ...
    approval_id: uuid.UUID | None = Body(None),
    ...
    try:
        if not dry_run:
            ...
            decision = await evaluate_control_policy(
                ...
                before_state={
                    "safety_gate_status": sim.safety_gate_status,
                    "target_scope": sim.target_scope,
                    "target_identifier": sim.target_identifier,
                },
...
async def rollback_execution(
    execution_id: uuid.UUID, db: AsyncSession = Depends(get_super_admin_db)
):
    svc = get_infra_execution_service()
    try:
...
async def get_execution_status(execution_id: uuid.UUID, db: AsyncSession = Depends(get_admin_db)):
    from app.models.commercial.commercial_infra_simulation import CommercialExecutionRecord
...
async def list_simulations(limit: int = 50, db: AsyncSession = Depends(get_admin_db)):
    stmt = (
...
async def create_simulation(
    ...
    if simulation_type == "scale_up":
        ...
        raise HTTPException(
            status_code=400, detail=f"Unsupported simulation type: {simulation_type}"
        )
...
async def update_safety_policy(
    ...
    stmt = (
        update(CommercialSafetyPolicy)
        .where(CommercialSafetyPolicy.id == policy_id)
        .values(**updates)
    )
...
async def validate_action(
    simulation_id: uuid.UUID = Body(...), db: AsyncSession = Depends(get_admin_db)
):
    stmt = select(CommercialInfrastructureSimulation).where(
        CommercialInfrastructureSimulation.id == simulation_id
    )
    result = await db.execute(stmt)
