...
async def restore_dry_run(
    ...
    service = RestoreStagingService(db)
    try:
        return await service.restore_with_staging(
            backup_id, BackupRestoreRequest(dry_run=True), actor=_get_actor(admin)
        )
    except FileNotFoundError as exc:
        raise HTTPException(status_code=404, detail=str(exc)) from exc
...
async def verify_backup_audit(
    ...
    return {"is_valid": is_valid, "failed_block_id": failed_block_id, "reason": reason}
...
async def approve_restore_request(
    ...
    if request_record.status != "pending":
        raise HTTPException(
            status_code=400,
            detail=f"Restore request is in state {request_record.status}, cannot approve.",
        )
...
async def execute_restore_request(
    ...
    try:
        return await service.restore_with_staging(
            request_record.backup_id, restore_req, actor=request_record.requester
        )
    except FileNotFoundError as exc:
        raise HTTPException(status_code=404, detail=str(exc)) from exc
