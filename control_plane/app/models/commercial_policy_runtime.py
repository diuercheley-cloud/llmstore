...
class CommercialPolicyRuntimeBundle(Base):
    ...
    tenant_id: Mapped[uuid.UUID | None] = mapped_column(UUID(as_uuid=True), ForeignKey("clients.id", ondelete="CASCADE"), nullable=True, index=True)
...
class CommercialPolicyEvaluation(Base):
    ...
    id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    bundle_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), ForeignKey("commercial_policy_runtime_bundles.id", ondelete="CASCADE"), index=True)
    
    evaluation_mode: Mapped[str] = mapped_column(String(32), nullable=False) # advisory, dry_run, enforce, sovereign_strict
...
class CommercialPolicyDecisionLog(Base):
    ...
    id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    evaluation_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), ForeignKey("commercial_policy_evaluations.id", ondelete="CASCADE"), index=True)
