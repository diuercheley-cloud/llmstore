...
class CommercialBillingDispute(Base):
    ...
    qos_billing_record_id: Mapped[uuid.UUID | None] = mapped_column(UUID(as_uuid=True), ForeignKey("commercial_qos_billing_records.id"), nullable=True)
    ...
    claimed_amount_brl: Mapped[Decimal] = mapped_column(Numeric(18, 6), default=Decimal("0.000000"), nullable=False)
    disputed_reason: Mapped[str] = mapped_column(Text(), nullable=False)
    
    admin_notes: Mapped[str | None] = mapped_column(Text(), nullable=True)
    resolution_notes: Mapped[str | None] = mapped_column(Text(), nullable=True)
    
    credit_transaction_id: Mapped[uuid.UUID | None] = mapped_column(UUID(as_uuid=True), ForeignKey("ai_wallet_transactions.id"), nullable=True)
