...
class AgentPipeline(Base):
    ...
    config: Mapped[dict] = mapped_column(JSON, nullable=False, default=dict) # steps: validate, test, eval, etc.
...
class AgentDeployment(Base):
    ...
    id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    agent_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), ForeignKey("agent_definitions.id", ondelete="CASCADE"), nullable=False, index=True)
    pipeline_id: Mapped[uuid.UUID | None] = mapped_column(UUID(as_uuid=True), ForeignKey("agent_cicd_pipelines.id"), nullable=True)
    environment: Mapped[str] = mapped_column(String(32), nullable=False) # staging, production
    strategy: Mapped[str] = mapped_column(String(32), default="recreate") # blue-green, canary, recreate
    version_tag: Mapped[str] = mapped_column(String(64), nullable=False)
    status: Mapped[str] = mapped_column(String(32), default="in_progress") # in_progress, completed, failed, rolled_back
...
class AgentDeploymentEvent(Base):
    ...
    id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    deployment_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), ForeignKey("agent_deployments.id", ondelete="CASCADE"), nullable=False, index=True)
    event_type: Mapped[str] = mapped_column(String(64), nullable=False) # switch_traffic, health_check_fail, etc.
