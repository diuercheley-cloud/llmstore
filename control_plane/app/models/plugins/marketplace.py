...
class PluginMarketplaceEntry(Base):
    ...
    )  # provider_adapter, billing_adapter, etc.
...
class PluginVersion(Base):
    ...
    id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    plugin_entry_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True), ForeignKey("plugin_marketplace_entries.id"), nullable=False, index=True
    )
...
class PluginInstall(Base):
    ...
    current_version_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True), ForeignKey("plugin_versions.id"), nullable=False
    )
...
class PluginPermission(Base):
    ...
    id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    plugin_install_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True), ForeignKey("plugin_installs.id"), nullable=False, index=True
    )
...
class PluginTrustReport(Base):
    ...
    plugin_version_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True), ForeignKey("plugin_versions.id"), nullable=False, index=True
    )
...
class PluginExecutionRecord(Base):
    ...
    output: Mapped[dict | None] = mapped_column(JSON_DOCUMENT, nullable=True)
...
class PluginExecutionReceipt(Base):
    ...
    execution_id: Mapped[str] = mapped_column(
        String(64), ForeignKey("plugin_runtime_executions.id"), nullable=False, index=True
    )
