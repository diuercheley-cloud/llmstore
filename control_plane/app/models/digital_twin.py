...
class DigitalTwin(Base):
    ...
    twin_type: Mapped[str] = mapped_column(String(64), nullable=False) # sensor|actuator|controller|industrial_machine
    connector_type: Mapped[str] = mapped_column(String(64), default="mock") # mock|mqtt|opcua|modbus
    
    config: Mapped[dict] = mapped_column(JSON, nullable=False, default=dict) # connection strings, auth
...
class DigitalTwinCommand(Base):
    ...
    status: Mapped[str] = mapped_column(String(32), default="pending") # pending|authorizing|approved|rejected|executing|completed|failed
...
class DigitalTwinSafetyEvent(Base):
    ...
    id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    twin_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), ForeignKey("digital_twins.id", ondelete="CASCADE"), nullable=False, index=True)
    command_id: Mapped[uuid.UUID | None] = mapped_column(UUID(as_uuid=True), ForeignKey("digital_twin_commands.id", ondelete="SET NULL"), nullable=True)
    
    event_type: Mapped[str] = mapped_column(String(64), nullable=False) # interlock_trip|boundary_violation|unsafe_actuation
