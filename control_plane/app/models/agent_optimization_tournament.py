...
class AgentOptimizationTournament(Base):
    ...
    experiment_id: Mapped[uuid.UUID | None] = mapped_column(UUID(as_uuid=True), ForeignKey("agent_optimization_experiments.id", ondelete="SET NULL"), nullable=True, index=True)
...
class AgentOptimizationTournamentCandidate(Base):
    ...
    id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    tournament_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), ForeignKey("agent_optimization_tournaments.id", ondelete="CASCADE"), index=True, nullable=False)
...
class AgentOptimizationTournamentResult(Base):
    ...
    id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    tournament_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), ForeignKey("agent_optimization_tournaments.id", ondelete="CASCADE"), index=True, nullable=False)
    tournament_candidate_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), ForeignKey("agent_optimization_tournament_candidates.id", ondelete="CASCADE"), index=True, nullable=False)
...
class AgentOptimizationPairwiseResult(Base):
    ...
    id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    tournament_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), ForeignKey("agent_optimization_tournaments.id", ondelete="CASCADE"), index=True, nullable=False)
    candidate_a_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), ForeignKey("agent_optimization_tournament_candidates.id", ondelete="CASCADE"), index=True, nullable=False)
    candidate_b_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), ForeignKey("agent_optimization_tournament_candidates.id", ondelete="CASCADE"), index=True, nullable=False)
    winner_id: Mapped[uuid.UUID | None] = mapped_column(UUID(as_uuid=True), nullable=True)
