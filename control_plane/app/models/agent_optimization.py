...
class AgentOptimizationCandidate(Base):
    ...
    id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    experiment_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), ForeignKey("agent_optimization_experiments.id", ondelete="CASCADE"), index=True, nullable=False)
    tenant_id: Mapped[str] = mapped_column(String(128), nullable=False, index=True)
    agent_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), ForeignKey("agent_definitions.id", ondelete="CASCADE"), nullable=False, index=True)
    candidate_type: Mapped[str] = mapped_column(String(32), nullable=False)  # prompt|policy|tool_selection
    status: Mapped[str] = mapped_column(String(32), default="pending")  # pending|evaluating|completed|approved|applied|rejected
...
class AgentOptimizationResult(Base):
    ...
    id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    candidate_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), ForeignKey("agent_optimization_candidates.id", ondelete="CASCADE"), index=True, nullable=False)
    eval_run_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), ForeignKey("agent_eval_runs.id", ondelete="CASCADE"), index=True, nullable=False)
    metrics_delta: Mapped[dict] = mapped_column(JSON, default=dict)  # {eval_pass_rate_delta, cost_delta, latency_delta, tool_error_delta, policy_denial_delta, safety_failure_delta}
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), default=utc_now, nullable=False)
...
class AgentPromptCandidate(Base):
    ...
    id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    candidate_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), ForeignKey("agent_optimization_candidates.id", ondelete="CASCADE"), index=True, unique=True, nullable=False)
