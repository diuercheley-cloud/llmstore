...
class ModelRegistry(Base):
    ...
    inference_backend_id: Mapped[uuid.UUID | None] = mapped_column(
        UUID(as_uuid=True), ForeignKey("inference_backends.id"), nullable=True, index=True
    )
    ...
    backend_routes = relationship(
        "ModelBackendRoute", back_populates="model", cascade="all, delete-orphan"
    )
