...
class VoiceSession(Base):
    ...
    status: Mapped[str] = mapped_column(
        String(32), default="starting", index=True
    ...
    )  # mock|local|deepgram|whisper
    tts_provider: Mapped[str] = mapped_column(
        String(32), default="mock"
    )  # mock|pocket_tts|elevenlabs
    ...
    events = relationship(
        "VoiceStreamEvent", back_populates="session", cascade="all, delete-orphan"
    )
...
class VoiceTurn(Base):
    ...
    status: Mapped[str] = mapped_column(
        String(32), default="listening"
    )  # listening|processing|speaking|completed|error
    ...
    raw_audio_ref: Mapped[str | None] = mapped_column(
        String(512), nullable=True
    )  # storage path, NOT raw bytes
...
class VoiceStreamEvent(Base):
    ...
    )  # audio_chunk|stt_result|tts_start|voice_activity|turn_start|turn_end
...
class VoiceTranscript(Base):
    ...
    turn_id: Mapped[uuid.UUID | None] = mapped_column(
        UUID(as_uuid=True), ForeignKey("voice_turns.id", ondelete="SET NULL"), nullable=True
    )
