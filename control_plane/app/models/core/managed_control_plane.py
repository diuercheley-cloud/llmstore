...
class ManagedWorkspace(Base):
    ...
    organization_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True), ForeignKey("managed_organizations.id"), nullable=False
    )
...
class ManagedAppliance(Base):
    ...
    workspace_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True), ForeignKey("managed_workspaces.id"), nullable=False
    )

    appliance_external_id: Mapped[str] = mapped_column(
        String(128), unique=True, index=True
    )  # ID reported by the appliance
    name: Mapped[str] = mapped_column(String(255), nullable=False)

    status: Mapped[str] = mapped_column(
        String(32), default="enrolled"
    )  # enrolled, online, offline, revoked

    # Sanitized status from heartbeats
    version: Mapped[str | None] = mapped_column(String(64))
    health_status: Mapped[str | None] = mapped_column(String(32))  # healthy, degraded, critical
    readiness: Mapped[bool | None] = mapped_column(Boolean, default=False)

    last_heartbeat_at: Mapped[datetime | None] = mapped_column(DateTime)

    capacity_summary: Mapped[dict | None] = mapped_column(JSON)
    enabled_providers: Mapped[list | None] = mapped_column(JSON)
    available_models: Mapped[list | None] = mapped_column(JSON)
...
class ApplianceEnrollment(Base):
    ...
    used_by_appliance_id: Mapped[uuid.UUID | None] = mapped_column(
        UUID(as_uuid=True), ForeignKey("managed_appliances.id")
    )
...
class ApplianceHeartbeat(Base):
    ...
    appliance_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True), ForeignKey("managed_appliances.id"), nullable=False
    )
...
class ManagedBillingAccount(Base):
    ...
    payment_method: Mapped[str | None] = mapped_column(String(64))  # stripe, bank_transfer, etc.
...
class ManagedSupportCase(Base):
    ...
    workspace_id: Mapped[uuid.UUID | None] = mapped_column(
        UUID(as_uuid=True), ForeignKey("managed_workspaces.id")
    )
...
class ManagedControlPlaneLink(Base):
    __tablename__ = "managed_control_plane_links"
    ...
    sync_policies: Mapped[bool] = mapped_column(Boolean, default=False)
...
class ManagedPolicySyncEvent(Base):
    __tablename__ = "managed_policy_sync_events"

    id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    link_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True), ForeignKey("managed_control_plane_links.id"), nullable=False
    )
