...
class CommercialApplianceProfile(Base):
    ...
    deployment_tier = Column(String(64), default="regulated") # airgap, government, defense, regulated
...
class CommercialOfflineSyncManifest(Base):
    ...
    id = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    appliance_id = Column(String(128), ForeignKey("commercial_appliance_profiles.appliance_id"), nullable=False)
