...
class AgentMCPDelegatedGrant(Base):
    ...
    access_token: Mapped[str] = mapped_column(String(2048), nullable=False)  # Raw token, never logged
...
class AgentMCPTokenExchange(Base):
    ...
    agent_id: Mapped[uuid.UUID | None] = mapped_column(UUID(as_uuid=True), ForeignKey("agent_definitions.id", ondelete="CASCADE"), nullable=True, index=True)
    grant_id: Mapped[uuid.UUID | None] = mapped_column(UUID(as_uuid=True), ForeignKey("agent_mcp_delegated_grants.id", ondelete="SET NULL"), nullable=True, index=True)
    mcp_server: Mapped[str] = mapped_column(String(256), nullable=False, index=True)
