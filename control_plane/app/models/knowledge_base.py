...
class KnowledgeBase(Base):
    ...
    provider_config: Mapped[dict] = mapped_column(JSON, nullable=False, default=dict) # e.g. vector store type, index name
...
class KBDocument(Base):
    ...
    versions = relationship("KBDocumentVersion", back_populates="document", cascade="all, delete-orphan")
...
class KBDocumentVersion(Base):
    ...
    storage_path: Mapped[str | None] = mapped_column(Text, nullable=True) # Path to raw file in object store
    status: Mapped[str] = mapped_column(String(32), default="processed") # pending, processed, failed
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), default=utc_now, nullable=False)
...
class KBChunk(Base):
    ...
    id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    version_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), ForeignKey("kb_document_versions.id", ondelete="CASCADE"), nullable=False, index=True)
    chunk_index: Mapped[int] = mapped_column(Integer, nullable=False)
    content: Mapped[str] = mapped_column(Text, nullable=False)
    vector_id: Mapped[str | None] = mapped_column(String(255), nullable=True) # ID in external vector store
    metadata_json: Mapped[dict] = mapped_column(JSON, nullable=False, default=dict)
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), default=utc_now, nullable=False)
