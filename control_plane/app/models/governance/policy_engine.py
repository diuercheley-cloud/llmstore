...
class DeterministicPolicy(Base):
    ...
    policy_status: Mapped[str] = mapped_column(String(32), nullable=False, default="draft", index=True)
...
class PolicyEvaluationResult(Base):
    ...
    id: Mapped[str] = mapped_column(String(64), primary_key=True)
    client_id: Mapped[UUID] = mapped_column(PGUUID(as_uuid=True), ForeignKey("clients.id", ondelete="CASCADE"), nullable=False, index=True)
    policy_id: Mapped[str] = mapped_column(String(64), ForeignKey("deterministic_policies.id", ondelete="CASCADE"), nullable=False, index=True)
