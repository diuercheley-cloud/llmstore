...
class AgentExecutionJob(Base):
    ...
    queue_status: Mapped[str] = mapped_column(String(32), default="queued", index=True)  # queued|leased|running|waiting_approval|completed|failed|cancelled|dead_letter
    
    # Durable Queue Extensions
    priority: Mapped[int] = mapped_column(Integer, default=0, index=True)
    idempotency_key: Mapped[str | None] = mapped_column(String(256), unique=True, nullable=True, index=True)
    ...
    lease = relationship("AgentExecutionLease", back_populates="job", uselist=False, cascade="all, delete-orphan")
    retries = relationship("AgentExecutionRetry", back_populates="job", cascade="all, delete-orphan")
...
class AgentExecutionLease(Base):
    ...
    job_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True), ForeignKey("agent_execution_jobs.id", ondelete="CASCADE"), unique=True, index=True, nullable=False
    )
    worker_id: Mapped[str] = mapped_column(String(128), index=True, nullable=False)
    leased_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), default=utc_now, nullable=False)
...
class AgentExecutionRetry(Base):
    ...
    id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    job_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True), ForeignKey("agent_execution_jobs.id", ondelete="CASCADE"), index=True, nullable=False
    )
...
class AgentExecutionDeadLetter(Base):
    ...
    failed_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), default=utc_now, nullable=False)
