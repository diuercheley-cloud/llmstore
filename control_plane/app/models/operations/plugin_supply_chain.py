...
class PluginProvenanceRecord(Base):
    ...
    plugin_contract_id: Mapped[str] = mapped_column(
        String(64),
...
class PluginSBOMPlaceholder(Base):
    ...
    provenance_record_id: Mapped[str] = mapped_column(
        String(64),
        ForeignKey("plugin_provenance_records.id", ondelete="CASCADE"),
        nullable=False,
        index=True,
    )
    sbom_format: Mapped[str] = mapped_column(
        String(64), nullable=False, default="placeholder_v1", index=True
    )
...
class DependencyGovernancePolicy(Base):
    ...
    require_reproducible_builds: Mapped[bool] = mapped_column(
        Boolean(), nullable=False, default=True
    )
    require_offline_verification: Mapped[bool] = mapped_column(
        Boolean(), nullable=False, default=True
    )
...
class PluginSignedArtifact(Base):
    ...
    signature_scope: Mapped[str] = mapped_column(
        String(64), nullable=False, default="provenance_record", index=True
    )
    signature_status: Mapped[str] = mapped_column(
        String(32), nullable=False, default="signature_only", index=True
    )
    immutable_hash: Mapped[str] = mapped_column(String(64), nullable=False, index=True, unique=True)
    created_at: Mapped[datetime] = mapped_column(
        DateTime(timezone=True), nullable=False, default=utc_now
