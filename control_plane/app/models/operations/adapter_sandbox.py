...
class AdapterManifest(Base):
    ...
    external_system_access_allowed: Mapped[bool] = mapped_column(Boolean(), default=False, nullable=False)
...
class AdapterSandboxRun(Base):
    ...
    plan_id: Mapped[Optional[uuid.UUID]] = mapped_column(UUID(as_uuid=True), ForeignKey("remediation_plans.id", ondelete="CASCADE"), nullable=True, index=True)
    sandbox_mode: Mapped[str] = mapped_column(String(50), nullable=False)  # simulation, contract_validation
...
class AdapterSandboxStepResult(Base):
    ...
    id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    client_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), ForeignKey("clients.id", ondelete="CASCADE"), nullable=False, index=True)
    sandbox_run_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), ForeignKey("adapter_sandbox_runs.id", ondelete="CASCADE"), nullable=False, index=True)
...
class AdapterSandboxPolicyViolation(Base):
    ...
    id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    client_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), ForeignKey("clients.id", ondelete="CASCADE"), nullable=False, index=True)
    manifest_id: Mapped[Optional[uuid.UUID]] = mapped_column(UUID(as_uuid=True), ForeignKey("adapter_manifests.id", ondelete="CASCADE"), nullable=True, index=True)
    sandbox_run_id: Mapped[Optional[uuid.UUID]] = mapped_column(UUID(as_uuid=True), ForeignKey("adapter_sandbox_runs.id", ondelete="CASCADE"), nullable=True, index=True)
