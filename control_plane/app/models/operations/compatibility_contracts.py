...
class VersionNegotiationSession(Base):
    ...
    source_version: Mapped[str] = mapped_column(String(32), nullable=False, index=True)
    target_version: Mapped[str] = mapped_column(String(32), nullable=False, index=True)
    negotiated_version: Mapped[str | None] = mapped_column(String(32), nullable=True, index=True)
...
class CapabilityNegotiation(Base):
    ...
    negotiation_session_id: Mapped[str] = mapped_column(
        String(64),
        ForeignKey("version_negotiation_sessions.id", ondelete="CASCADE"),
        nullable=False,
        index=True,
    )
...
class FeatureCompatibilityFlag(Base):
    ...
    deprecated_after_version: Mapped[str | None] = mapped_column(String(32), nullable=True)
    replay_safe: Mapped[bool] = mapped_column(Boolean(), nullable=False, default=True)
    immutable_hash: Mapped[str] = mapped_column(String(64), nullable=False, index=True, unique=True)
    created_at: Mapped[datetime] = mapped_column(
        DateTime(timezone=True), nullable=False, default=utc_now
...
class DeprecationLifecycle(Base):
    ...
    contract_id: Mapped[str] = mapped_column(
        String(64),
        ForeignKey("compatibility_contracts.id", ondelete="CASCADE"),
        nullable=False,
        index=True,
    )
    deprecation_reason: Mapped[str] = mapped_column(Text(), nullable=False)
    deprecation_status: Mapped[str] = mapped_column(
        String(32), nullable=False, default="proposed", index=True
    )
    replacement_contract: Mapped[str | None] = mapped_column(String(64), nullable=True)
