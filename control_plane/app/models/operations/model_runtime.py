...
class ModelRuntimeInstance(Base):
    ...
    backend_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True), ForeignKey("inference_backends.id"), nullable=False, index=True
    )
    model_path: Mapped[str] = mapped_column(String(1024), nullable=False)
    port: Mapped[int] = mapped_column(Integer, nullable=False)
    status: Mapped[str] = mapped_column(
        String(32), default="loading", nullable=False
    )  # loading|ready|failed|unloading|stopped
