...
class DeterministicEventContract(Base):
    ...
    compatibility_status: Mapped[str] = mapped_column(String(32), nullable=False, default="compatible", index=True)
...
class EventSchemaCompatibility(Base):
    ...
    id: Mapped[str] = mapped_column(String(64), primary_key=True)
    client_id: Mapped[UUID] = mapped_column(PGUUID(as_uuid=True), ForeignKey("clients.id", ondelete="CASCADE"), nullable=False, index=True)
    event_contract_id: Mapped[str] = mapped_column(String(64), ForeignKey("deterministic_event_contracts.id", ondelete="CASCADE"), nullable=False, index=True)
