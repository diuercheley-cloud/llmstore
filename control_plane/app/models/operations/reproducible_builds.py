...
class ReproducibleBuildManifest(Base):
    ...
    build_manifest_hash: Mapped[str] = mapped_column(String(64), nullable=False, index=True, unique=True)
    reproducibility_status: Mapped[str] = mapped_column(String(32), nullable=False, default="proposed", index=True)
...
class ArtifactVerificationRecord(Base):
    ...
    id: Mapped[str] = mapped_column(String(64), primary_key=True)
    client_id: Mapped[UUID] = mapped_column(PGUUID(as_uuid=True), ForeignKey("clients.id", ondelete="CASCADE"), nullable=False, index=True)
    build_manifest_id: Mapped[str] = mapped_column(String(64), ForeignKey("reproducible_build_manifests.id", ondelete="CASCADE"), nullable=False, index=True)
...
class BuildEnvironmentConstraint(Base):
    ...
    external_dependency_resolution_allowed: Mapped[bool] = mapped_column(Boolean(), nullable=False, default=False)
    immutable_hash: Mapped[str] = mapped_column(String(64), nullable=False, index=True, unique=True)
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), nullable=False, default=utc_now)
...
class ArtifactReplayVerification(Base):
    ...
    id: Mapped[str] = mapped_column(String(64), primary_key=True)
    client_id: Mapped[UUID] = mapped_column(PGUUID(as_uuid=True), ForeignKey("clients.id", ondelete="CASCADE"), nullable=False, index=True)
    artifact_verification_id: Mapped[str] = mapped_column(String(64), ForeignKey("artifact_verification_records.id", ondelete="CASCADE"), nullable=False, index=True)
