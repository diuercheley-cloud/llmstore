...
class AdapterPromotionWorkflow(Base):
    ...
    registry_entry_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True),
        ForeignKey("signed_adapter_registry_entries.id", ondelete="CASCADE"),
        nullable=False,
        index=True,
    )
    ...
    )  # draft, sandboxed, registry_approved, staging_simulated, production_eligible
    target_stage: Mapped[str] = mapped_column(String(50), nullable=False)
    promotion_status: Mapped[str] = mapped_column(
        String(50), default="pending", nullable=False
    )  # pending, blocked, approved, simulated, promoted, rolled_back, failed
...
class AdapterPromotionGateResult(Base):
    ...
    workflow_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True),
        ForeignKey("adapter_promotion_workflows.id", ondelete="CASCADE"),
        nullable=False,
        index=True,
    )
...
class AdapterPromotionStageTransition(Base):
    ...
    )  # proposed, allowed, blocked, completed, rolled_back
    reason: Mapped[str | None] = mapped_column(String(500), nullable=True)
    immutable_hash: Mapped[str] = mapped_column(String(64), nullable=False, index=True, unique=True)
    created_at: Mapped[datetime] = mapped_column(
        DateTime(timezone=True), default=utc_now, nullable=False
