...
class RemediationExecution(Base):
    ...
    )  # pending, approved, running, succeeded, failed, cancelled, rolled_back
...
class RemediationExecutionStep(Base):
    ...
    plan_step_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True), ForeignKey("remediation_steps.id", ondelete="CASCADE"), nullable=False
    )
