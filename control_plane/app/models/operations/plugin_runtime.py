...
class PluginCapabilityBoundary(Base):
    ...
    abi_contract_id: Mapped[str] = mapped_column(
        String(64),
        ForeignKey("plugin_abi_contracts.id", ondelete="CASCADE"),
        nullable=False,
        index=True,
    )
    ...
    external_secret_access_allowed: Mapped[bool] = mapped_column(
...
class PluginIsolationPolicy(Base):
    ...
    isolation_level: Mapped[str] = mapped_column(
        String(32), nullable=False, default="strict", index=True
    )
    ...
    deny_external_filesystem_write: Mapped[bool] = mapped_column(
        Boolean(), nullable=False, default=True
    )
    deny_plaintext_secret_access: Mapped[bool] = mapped_column(
        Boolean(), nullable=False, default=True
    )
    immutable_hash: Mapped[str] = mapped_column(String(64), nullable=False, index=True, unique=True)
    created_at: Mapped[datetime] = mapped_column(
...
class PluginFederationCompatibility(Base):
    ...
    compatibility_hash: Mapped[str] = mapped_column(
...
class PluginRuntimeActivation(Base):
    ...
    load_plan_id: Mapped[str | None] = mapped_column(
        String(64),
        ForeignKey("deterministic_extension_load_plans.id", ondelete="SET NULL"),
        nullable=True,
        index=True,
    )
...
class PluginRuntimeExecution(Base):
    ...
    activation_id: Mapped[str | None] = mapped_column(
        String(64),
        ForeignKey("plugin_runtime_activations.id", ondelete="SET NULL"),
        nullable=True,
        index=True,
    )
