...
class AgentApiDeployment(Base):
    ...
    status: Mapped[str] = mapped_column(String(32), default="active", index=True)  # active|paused|archived
    ...
    # Billing
    billing_tier: Mapped[str] = mapped_column(String(32), default="free")  # free|basic|pro|enterprise
...
class AgentApiEndpointKey(Base):
    ...
    id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    deployment_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), ForeignKey("agent_api_deployments.id", ondelete="CASCADE"), nullable=False, index=True)
    tenant_id: Mapped[str] = mapped_column(String(128), nullable=False, index=True)
...
class AgentApiUsageEvent(Base):
    ...
    endpoint_key_id: Mapped[uuid.UUID | None] = mapped_column(UUID(as_uuid=True), ForeignKey("agent_api_endpoint_keys.id", ondelete="SET NULL"), nullable=True)
    mode: Mapped[str] = mapped_column(String(16), nullable=False)  # sync|async
    status: Mapped[str] = mapped_column(String(32), nullable=False)  # started|completed|failed|timeout|rate_limited
...
class AgentApiSlaEvent(Base):
    ...
    deployment_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), ForeignKey("agent_api_deployments.id", ondelete="CASCADE"), nullable=False, index=True)
    tenant_id: Mapped[str] = mapped_column(String(128), nullable=False, index=True)
    event_type: Mapped[str] = mapped_column(String(64), nullable=False)  # timeout|rate_limit_exceeded|concurrency_exceeded|error_spike|recovery
