...
class FederatedMemoryPeer(Base):
    ...
    trust_level: Mapped[str] = mapped_column(String(32), default="standard") # low|standard|high|sovereign
...
class RemoteMemoryReference(Base):
    ...
    reference_type: Mapped[str] = mapped_column(String(32), default="pointer") # proxy|summary_only|full_link
...
class FederatedSyncEvent(Base):
    ...
    id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    peer_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), ForeignKey("federated_memory_peers.id"))
    
    event_type: Mapped[str] = mapped_column(String(64)) # sync_start|sync_complete|sync_failed|revocation_propagation
