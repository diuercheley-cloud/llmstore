...
class DistributedAgentJob(Base):
    ...
    target_cluster_id: Mapped[Optional[uuid.UUID]] = mapped_column(UUID(as_uuid=True), ForeignKey("runtime_clusters.id"), nullable=True)
    target_node_id: Mapped[Optional[uuid.UUID]] = mapped_column(UUID(as_uuid=True), ForeignKey("runtime_nodes.id"), nullable=True)
    tenant_id: Mapped[str] = mapped_column(String(255), nullable=False)
    requires_gpu: Mapped[bool] = mapped_column(Boolean, default=False)
    data_residency: Mapped[Optional[str]] = mapped_column(String(64), nullable=True)
...
class DistributedJobLease(Base):
    ...
    id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    job_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), ForeignKey("distributed_agent_jobs.id"), nullable=False, unique=True)
    node_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), ForeignKey("runtime_nodes.id"), nullable=False)
...
class DistributedFailoverEvent(Base):
    ...
    id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    job_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), ForeignKey("distributed_agent_jobs.id"), nullable=False)
    from_node_id: Mapped[Optional[uuid.UUID]] = mapped_column(UUID(as_uuid=True), ForeignKey("runtime_nodes.id"), nullable=True)
    to_node_id: Mapped[Optional[uuid.UUID]] = mapped_column(UUID(as_uuid=True), ForeignKey("runtime_nodes.id"), nullable=True)
