...
class GpuDevice(Base):
    ...
    runtime_node_id: Mapped[uuid.UUID] = mapped_column(
...
class GpuAllocation(Base):
    ...
    gpu_device_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True), ForeignKey("gpu_devices.id"), nullable=False, index=True
    )
    ...
    status: Mapped[str] = mapped_column(
        String(32), default="allocated"
    )  # allocated, releasing, released
...
class AutoscalingPolicy(Base):
    ...
    strategy: Mapped[str] = mapped_column(
        String(64), default="queue_depth"
    )  # queue_depth, latency_p95, gpu_pressure
    ...
    )  # e.g. target queue depth or latency
    cooldown_seconds: Mapped[int] = mapped_column(Integer, default=300)
    is_active: Mapped[bool] = mapped_column(Boolean, default=True)
    mode: Mapped[str] = mapped_column(
        String(32), default="recommendation"
...
class AutoscalingEvent(Base):
    ...
    policy_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True), ForeignKey("autoscaling_policies.id"), nullable=False, index=True
    )
