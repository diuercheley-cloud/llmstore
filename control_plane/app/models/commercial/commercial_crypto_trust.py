...
class CommercialKeyMaterial(Base):
    ...
    provider_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True),
        ForeignKey("commercial_kms_providers.id", ondelete="CASCADE"),
        nullable=False,
    )
    tenant_id: Mapped[uuid.UUID | None] = mapped_column(
        UUID(as_uuid=True), nullable=True
    )  # Null for system keys
    ...
    status: Mapped[KeyUsageStatus] = mapped_column(
        String(50), default=KeyUsageStatus.ACTIVE, nullable=False
    )
...
class CommercialSigningProfile(Base):
    ...
    key_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True),
        ForeignKey("commercial_key_materials.id", ondelete="RESTRICT"),
        nullable=False,
    )
...
class CommercialKeyRotationSchedule(Base):
    ...
    key_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True),
        ForeignKey("commercial_key_materials.id", ondelete="CASCADE"),
        nullable=False,
    )
