...
class CommercialRAGDocument(Base):
    ...
    classification_level = Column(
        String(64), default="confidential"
...
class CommercialRAGChunk(Base):
    ...
    id = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    document_id = Column(
        UUID(as_uuid=True), ForeignKey("commercial_rag_documents.id"), nullable=False
    )
...
class CommercialRetrievalPolicyViolation(Base):
    ...
    )  # cross_tenant|classification_mismatch|expired_document
...
class CommercialRAGLegalHold(Base):
    ...
    vault_id = Column(
        UUID(as_uuid=True), ForeignKey("commercial_rag_vaults.id"), nullable=True, index=True
    )
    document_id = Column(
        UUID(as_uuid=True), ForeignKey("commercial_rag_documents.id"), nullable=True, index=True
    )
...
class CommercialRAGPoisoningAlert(Base):
    ...
    document_id = Column(
        UUID(as_uuid=True), ForeignKey("commercial_rag_documents.id"), nullable=True
    )
