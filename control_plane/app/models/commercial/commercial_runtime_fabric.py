...
class CommercialRuntimeFabricEvent(Base):
    ...
    event_type = Column(
        String, index=True
    )  # failure_detected, recovery_initiated, healing_completed, drift_detected
...
class CommercialRuntimeHealingAction(Base):
    ...
    action_type = Column(
        String
    )  # restart_service, rollback_state, replay_workflow, isolate_node, resync_mesh
...
class CommercialRuntimeDeterminismDrift(Base):
    ...
    repair_plan_id = Column(
        String, ForeignKey("commercial_runtime_recovery_plans.id"), nullable=True
    )
