...
class CommercialPublicAttestationRequest(Base):
    ...
    )  # received|verified|invalid|rejected
...
class CommercialPublicAttestationResult(Base):
    ...
    id = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    request_id = Column(
        UUID(as_uuid=True), ForeignKey("commercial_public_attestation_requests.id"), nullable=False
    )
    verification_type = Column(
        String(50), nullable=False
    )  # receipt|proof|timeline|witness_quorum|checkpoint
