...
class CommercialQoSBillingRecord(Base):
    ...
    __table_args__ = (UniqueConstraint("idempotency_key", name="uq_qos_billing_idempotency"),)
    ...
    chargeback_id: Mapped[uuid.UUID | None] = mapped_column(
        UUID(as_uuid=True), ForeignKey("commercial_queue_chargebacks.id"), nullable=True
    )

    compute_seconds: Mapped[float] = mapped_column(Numeric(18, 6), default=0.0, nullable=False)
    priority_slots_consumed: Mapped[float] = mapped_column(
        Numeric(18, 6), default=0.0, nullable=False
    )

    estimated_internal_cost_brl: Mapped[Decimal] = mapped_column(
        Numeric(18, 6), default=Decimal("0.000000"), nullable=False
    )
    opportunity_cost_brl: Mapped[Decimal] = mapped_column(
        Numeric(18, 6), default=Decimal("0.000000"), nullable=False
    )
    billable_amount_brl: Mapped[Decimal] = mapped_column(
        Numeric(18, 6), default=Decimal("0.000000"), nullable=False
    )
    ...
    status: Mapped[str] = mapped_column(
        String(24), default="calculated", nullable=False, index=True
    )
