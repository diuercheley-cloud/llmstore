...
class PromptTemplate(Base):
    ...
    declared_variables = relationship(
        "PromptTemplateVariable", back_populates="template", cascade="all, delete-orphan"
    )
    active_version_id: Mapped[uuid.UUID | None] = mapped_column(
        UUID(as_uuid=True), ForeignKey("prompt_template_versions.id", use_alter=True), nullable=True
    )
...
class PromptTemplateVersion(Base):
    ...
    content: Mapped[str] = mapped_column(
        Text, nullable=False
    )  # The actual prompt text with {{vars}}
...
class PromptExperiment(Base):
    ...
    winner_version_id: Mapped[uuid.UUID | None] = mapped_column(
