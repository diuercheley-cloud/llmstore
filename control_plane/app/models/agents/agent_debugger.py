...
class AgentRunSnapshot(Base):
    ...
    memory_references: Mapped[list] = mapped_column(
        JSON, nullable=False
    )  # IDs of memory items accessed
...
class AgentDebugReplay(Base):
    ...
    source_snapshot_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True), ForeignKey("agent_run_snapshots.id")
    )
...
class AgentDebugStateEdit(Base):
    ...
    replay_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True),
        ForeignKey("agent_debug_replays.id", ondelete="CASCADE"),
        nullable=False,
        index=True,
    )
...
class AgentBreakpoint(Base):
    ...
    )  # step_type, tool_name, policy_denial, approval_request, error
...
class AgentDebugStepEvent(Base):
    ...
    session_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True),
        ForeignKey("agent_debug_sessions.id", ondelete="CASCADE"),
        nullable=False,
        index=True,
    )
    step_number: Mapped[int] = mapped_column(Integer, nullable=False)
    event_type: Mapped[str] = mapped_column(
        String(64), nullable=False
    )  # step_start, step_end, breakpoint_hit
