...
class AgentEventTrigger(Base):
    ...
    source_id: Mapped[uuid.UUID | None] = mapped_column(
        UUID(as_uuid=True), ForeignKey("agent_event_sources.id", ondelete="SET NULL"), nullable=True
    )
    trigger_type: Mapped[str] = mapped_column(
        String(64), nullable=False
    )  # on_schedule, on_webhook, on_message_queue, etc
    config: Mapped[dict] = mapped_column(JSON, default=dict)
    filter_expression: Mapped[str | None] = mapped_column(
        Text, nullable=True
    )  # JSONPath or simple expression
...
class AgentEventDelivery(Base):
    ...
    trigger_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True),
        ForeignKey("agent_event_triggers.id", ondelete="CASCADE"),
        nullable=False,
        index=True,
    )
