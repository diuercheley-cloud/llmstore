...
class AgentTeam(Base):
    ...
    )  # hierarchical|debate|peer_to_peer
...
class AgentTeamMember(Base):
    ...
    role: Mapped[str] = mapped_column(
        String(64), nullable=False
    )  # manager|specialist|critic|proposer|synthesizer
...
class AgentTeamRun(Base):
    ...
    delegations = relationship(
        "AgentTeamDelegation", back_populates="run", cascade="all, delete-orphan"
    )
...
class AgentTeamMessage(Base):
    ...
    run_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True),
        ForeignKey("agent_team_runs.id", ondelete="CASCADE"),
        nullable=False,
        index=True,
    )
    ...
    message_type: Mapped[str] = mapped_column(
        String(64), nullable=False
    )  # proposal|critique|instruction|result
    created_at: Mapped[datetime] = mapped_column(
        DateTime(timezone=True), default=utc_now, nullable=False
    )
