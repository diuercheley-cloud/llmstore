...
class AgentToolCredential(Base):
    ...
    grants = relationship(
        "AgentToolCredentialGrant", back_populates="credential", cascade="all, delete-orphan"
    )
...
class AgentToolCredentialGrant(Base):
    ...
    credential_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True),
        ForeignKey("agent_tool_credentials.id", ondelete="CASCADE"),
        index=True,
        nullable=False,
    )
...
class AgentToolSideEffect(Base):
    ...
    rollback_actions = relationship(
        "AgentToolRollbackAction", back_populates="side_effect", cascade="all, delete-orphan"
    )
...
class AgentToolRollbackAction(Base):
    ...
    side_effect_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True),
        ForeignKey("agent_tool_side_effects.id", ondelete="CASCADE"),
        index=True,
        nullable=False,
    )
    tenant_id: Mapped[str] = mapped_column(String(128), index=True, nullable=False)
