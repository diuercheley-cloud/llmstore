...
class AgentFlowDefinition(Base):
    ...
    versions = relationship(
        "AgentFlowVersion", back_populates="flow_definition", cascade="all, delete-orphan"
    )
...
class AgentFlowVersion(Base):
    ...
    flow_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True),
        ForeignKey("agent_flow_definitions.id", ondelete="CASCADE"),
        nullable=False,
        index=True,
    )
    ...
    nodes = relationship(
        "AgentFlowNode", back_populates="flow_version", cascade="all, delete-orphan"
    )
    edges = relationship(
        "AgentFlowEdge", back_populates="flow_version", cascade="all, delete-orphan"
    )
...
class AgentFlowNode(Base):
    ...
    version_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True),
        ForeignKey("agent_flow_versions.id", ondelete="CASCADE"),
        primary_key=True,
    )
...
class AgentFlowDebugSession(Base):
    ...
    flow_version_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True), ForeignKey("agent_flow_versions.id")
    )
...
class AgentFlowDebugEvent(Base):
    ...
    session_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True),
        ForeignKey("agent_flow_debug_sessions.id", ondelete="CASCADE"),
        index=True,
    )
