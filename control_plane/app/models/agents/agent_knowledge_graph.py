...
class AgentKGEntity(Base):
    ...
    # Relationships
    sources: Mapped[list["AgentKGSource"]] = relationship(
        "AgentKGSource", secondary="agent_kg_entity_sources", back_populates="entities"
    )

    __table_args__ = (Index("ix_agent_kg_entities_tenant_type_name", "tenant_id", "type", "name"),)
...
class AgentKGRelation(Base):
    ...
    source_entity_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True), ForeignKey("agent_kg_entities.id", ondelete="CASCADE"), nullable=False
    )
    ...
    source_entity: Mapped["AgentKGEntity"] = relationship(
        "AgentKGEntity", foreign_keys=[source_entity_id]
    )
    target_entity: Mapped["AgentKGEntity"] = relationship(
        "AgentKGEntity", foreign_keys=[target_entity_id]
    )
...
class AgentKGSource(Base):
    ...
    entities: Mapped[list["AgentKGEntity"]] = relationship(
        "AgentKGEntity", secondary="agent_kg_entity_sources", back_populates="sources"
    )
...
class AgentKGEntitySource(Base):
    ...
    entity_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True), ForeignKey("agent_kg_entities.id", ondelete="CASCADE"), primary_key=True
    )
    source_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True), ForeignKey("agent_kg_sources.id", ondelete="CASCADE"), primary_key=True
    )
