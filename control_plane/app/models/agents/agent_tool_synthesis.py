...
class AgentGeneratedTool(Base):
    ...
    versions: Mapped[list["AgentGeneratedToolVersion"]] = relationship(
        "AgentGeneratedToolVersion", back_populates="tool", cascade="all, delete-orphan"
    )
...
class AgentGeneratedToolVersion(Base):
    ...
    tool_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True),
        ForeignKey("agent_generated_tools.id", ondelete="CASCADE"),
        nullable=False,
    )
...
class AgentCodeInterpreterRun(Base):
    ...
    session_id: Mapped[uuid.UUID | None] = mapped_column(
        UUID(as_uuid=True),
        ForeignKey("agent_sandbox_sessions.id", ondelete="SET NULL"),
        nullable=True,
    )
...
class AgentSandboxSession(Base):
    ...
    artifacts: Mapped[list["AgentSandboxArtifact"]] = relationship(
        "AgentSandboxArtifact", back_populates="session", cascade="all, delete-orphan"
    )
    runs: Mapped[list["AgentCodeInterpreterRun"]] = relationship(
        "AgentCodeInterpreterRun", cascade="all, delete-orphan"
    )
...
class AgentSandboxArtifact(Base):
    ...
    session_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True),
        ForeignKey("agent_sandbox_sessions.id", ondelete="CASCADE"),
        nullable=False,
    )
    ...
    session: Mapped[AgentSandboxSession] = relationship(
        "AgentSandboxSession", back_populates="artifacts"
    )
...
class AgentSandboxPolicyEvent(Base):
    ...
    )  # e.g., "network_blocked", "file_write_blocked", "timeout"
