...
class AgentDelegatedToken(Base):
    ...
    token_type: Mapped[str] = mapped_column(
        String(64), default="connector"
    )  # connector|manual|test
    scopes: Mapped[list] = mapped_column(
        JSON, nullable=False, default=list
    )  # e.g., [{"connector": "slack", "action": "write"}]
...
class AgentTokenGrant(Base):
    ...
    )  # e.g. "slack" or None for all
    scopes: Mapped[list] = mapped_column(
        JSON, nullable=False, default=list
    )  # e.g. ["slack:write", "github:read"]
...
class AgentScopePolicy(Base):
    ...
    )  # rules mapping connector/action to allow/deny/limits
    created_at: Mapped[datetime] = mapped_column(
        DateTime(timezone=True), default=utc_now, nullable=False
    )
...
class AgentCredentialAuditEvent(Base):
    ...
    )  # e.g. service_principal_created, token_exchanged, token_revoked
    actor_id: Mapped[str | None] = mapped_column(String(128), nullable=True)
    actor_type: Mapped[str | None] = mapped_column(
        String(64), nullable=True
    )  # user, agent_service_principal, system
