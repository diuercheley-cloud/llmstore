...
class AgentWorkflowRun(Base):
    ...
    workflow_definition_id: Mapped[uuid.UUID | None] = mapped_column(
        UUID(as_uuid=True),
        ForeignKey("agent_workflow_definitions.id", ondelete="CASCADE"),
        nullable=True,
        index=True,
    )
    tenant_id: Mapped[str] = mapped_column(String(128), nullable=False, index=True)
    ...
    signals = relationship(
        "AgentWorkflowSignal", back_populates="run", cascade="all, delete-orphan"
    )
    webhook_waits = relationship(
        "AgentWorkflowWebhookWait", back_populates="run", cascade="all, delete-orphan"
    )
...
class AgentWorkflowDefinition(Base):
    ...
    nodes = relationship(
        "AgentWorkflowNode", back_populates="workflow_definition", cascade="all, delete-orphan"
    )
    edges = relationship(
        "AgentWorkflowEdge", back_populates="workflow_definition", cascade="all, delete-orphan"
    )
...
class AgentWorkflowNode(Base):
    ...
    )  # task, condition, parallel_fanout, fanin_join, subworkflow, approval, timer, webhook_wait
...
class AgentWorkflowParallelGroup(Base):
    ...
    failure_policy: Mapped[str] = mapped_column(
        String(32), default="fail_fast"
    )  # fail_fast, continue, compensate
...
class AgentSubworkflowRun(Base):
    ...
    subworkflow_definition_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True), ForeignKey("agent_workflow_definitions.id"), nullable=False
    )
    subworkflow_run_id: Mapped[uuid.UUID | None] = mapped_column(
        UUID(as_uuid=True), nullable=True
    )  # ID of the run in agent_workflow_runs
