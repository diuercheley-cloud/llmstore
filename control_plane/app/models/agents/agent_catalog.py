...
class ConnectorCatalogEntry(Base):
    ...
    catalog_entry_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True), ForeignKey("agent_capability_catalog_entries.id"), nullable=False
    )
    connector_type: Mapped[str] = mapped_column(String(64), nullable=False)
    credential_references: Mapped[dict] = mapped_column(
...
class PluginSignature(Base):
    ...
    plugin_entry_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True), ForeignKey("plugin_catalog_entries.id"), nullable=False
    )
