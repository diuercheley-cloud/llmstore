...
class AgentSession(Base):
    ...
    threads = relationship(
        "AgentConversationThread", back_populates="session", cascade="all, delete-orphan"
    )
    runs = relationship("AgentSessionRun", back_populates="session", cascade="all, delete-orphan")
    summaries = relationship(
        "AgentSessionSummary", back_populates="session", cascade="all, delete-orphan"
    )
    messages = relationship(
        "AgentThreadMessage", back_populates="session", cascade="all, delete-orphan"
    )
...
class AgentConversationThread(Base):
    ...
    messages = relationship(
        "AgentThreadMessage", back_populates="thread", cascade="all, delete-orphan"
    )
...
class AgentThreadMessage(Base):
    ...
    thread_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True),
        ForeignKey("agent_conversation_threads.id", ondelete="CASCADE"),
        nullable=False,
        index=True,
    )
    session_id: Mapped[uuid.UUID] = mapped_column(
