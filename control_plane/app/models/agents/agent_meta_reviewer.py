...
class AgentMetaReview(Base):
    ...
    overall_decision: Mapped[str] = mapped_column(
        String(32), default="allow"
    )  # allow|require_revision|require_human_approval|block
    ...
    findings = relationship(
        "AgentMetaReviewFinding", back_populates="review", cascade="all, delete-orphan"
    )
...
class AgentMetaReviewFinding(Base):
    ...
    review_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True),
        ForeignKey("agent_meta_reviews.id", ondelete="CASCADE"),
        nullable=False,
        index=True,
    )
    ...
    )  # hallucination|policy_drift|ethical_risk
