...
class AgentRun(Base):
    ...
    )  # queued|running|waiting_approval|completed|failed|cancelled|paused
    ...
    multimodal_asset_id: Mapped[uuid.UUID | None] = mapped_column(
        UUID(as_uuid=True), ForeignKey("multimodal_assets.id", ondelete="SET NULL"), nullable=True
    )
    ...
    checkpoints = relationship(
        "AgentRunCheckpoint", back_populates="run", cascade="all, delete-orphan"
    )
...
class AgentRunStep(Base):
    ...
    )  # model_call|tool_call|memory_read|memory_write|approval|handoff|final
...
class AgentRegistryEntry(Base):
    ...
    supported_surface_status: Mapped[str] = mapped_column(
        String(32), default="internal"
    )  # supported|beta|experimental|internal
    risk_level: Mapped[str] = mapped_column(String(32), default="low")  # low|medium|high|critical
    ...
    status: Mapped[str] = mapped_column(
        String(32), default="draft"
    )  # draft|review|approved|active|paused|deprecated|archived
    ...
    versions = relationship(
        "AgentVersion", back_populates="agent_entry", cascade="all, delete-orphan"
    )
    promotions = relationship(
        "AgentPromotion", back_populates="agent_entry", cascade="all, delete-orphan"
    )
    deprecations = relationship(
        "AgentDeprecation", back_populates="agent_entry", cascade="all, delete-orphan"
    )
    lifecycle_events = relationship(
        "AgentLifecycleEvent", back_populates="agent_entry", cascade="all, delete-orphan"
    )
...
class AgentVersion(Base):
    ...
    agent_registry_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True),
        ForeignKey("agent_registry_entries.id", ondelete="CASCADE"),
        nullable=False,
        index=True,
    )
...
class AgentTool(Base):
    ...
    category: Mapped[str] = mapped_column(
        String(64), nullable=False
    )  # retrieval|filesystem_safe|database_read|...
    ...
    )  # none|read|write|destructive|external
    ...
    scope: Mapped[str] = mapped_column(
        String(64), default="tenant", nullable=False
    ...
    versions = relationship(
        "AgentToolVersion", back_populates="agent_tool", cascade="all, delete-orphan"
    )
    permissions = relationship(
        "AgentToolPermission", back_populates="agent_tool", cascade="all, delete-orphan"
    )
    invocations = relationship(
        "AgentToolInvocation", back_populates="agent_tool", cascade="all, delete-orphan"
    )
    safety_reviews = relationship(
        "AgentToolSafetyReview", back_populates="agent_tool", cascade="all, delete-orphan"
    )
...
class AgentToolVersion(Base):
    ...
    id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    agent_tool_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True),
...
class AgentToolInvocation(Base):
    ...
    )  # success|failed|dry_run|rolled_back
    latency_ms: Mapped[int | None] = mapped_column(Integer, nullable=True)
...
class AgentApprovalRequest(Base):
    ...
    decisions = relationship(
        "AgentApprovalDecision", back_populates="approval_request", cascade="all, delete-orphan"
    )
...
class AgentEvalResult(Base):
    ...
    case_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True),
        ForeignKey("agent_eval_cases.id", ondelete="CASCADE"),
        nullable=False,
        index=True,
    )
    ...
    run_id_ref: Mapped[uuid.UUID | None] = mapped_column(
        UUID(as_uuid=True), nullable=True
    )  # Reference to actual AgentRun
    created_at: Mapped[datetime] = mapped_column(
        DateTime(timezone=True), default=utc_now, nullable=False
    )
...
class AgentEvalBaseline(Base):
    ...
    run_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True), ForeignKey("agent_eval_runs.id", ondelete="CASCADE"), nullable=False
    )
...
class AgentLLMJudgeRun(Base):
    ...
    eval_result_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True),
        ForeignKey("agent_eval_results.id", ondelete="CASCADE"),
        nullable=False,
        index=True,
    )
...
class AgentRedTeamCase(Base):
    ...
    attack_type: Mapped[str] = mapped_column(
        String(64), nullable=False
    )  # jailbreak, injection, exfiltration, unsafe_tool
...
class AgentEvalPairwiseResult(Base):
    ...
    id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    ab_run_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True),
        ForeignKey("agent_ab_eval_runs.id", ondelete="CASCADE"),
        nullable=False,
        index=True,
    )
    input_text: Mapped[str] = mapped_column(Text, nullable=False)
...
class AgentEvalDataset(Base):
    ...
    versions = relationship(
        "AgentEvalDatasetVersion", back_populates="dataset", cascade="all, delete-orphan"
    )
...
class AgentEvalDatasetVersion(Base):
    ...
    dataset_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True),
        ForeignKey("agent_eval_datasets.id", ondelete="CASCADE"),
        nullable=False,
        index=True,
    )
    version: Mapped[str] = mapped_column(String(64), nullable=False)
...
class AgentEvalRegressionResult(Base):
    ...
    baseline_run_id: Mapped[uuid.UUID | None] = mapped_column(
        UUID(as_uuid=True),
        ForeignKey("agent_eval_runs.id", ondelete="SET NULL"),
        nullable=True,
        index=True,
    )
    passed: Mapped[bool] = mapped_column(Boolean, nullable=False)
...
class AgentPromotionGateResult(Base):
    ...
    gate_result_id: Mapped[uuid.UUID | None] = mapped_column(
        UUID(as_uuid=True),
        ForeignKey("agent_eval_gate_results.id", ondelete="SET NULL"),
        nullable=True,
        index=True,
    )
    regression_result_id: Mapped[uuid.UUID | None] = mapped_column(
        UUID(as_uuid=True),
        ForeignKey("agent_eval_regression_results.id", ondelete="SET NULL"),
        nullable=True,
        index=True,
    )
...
class AgentMemoryPolicy(Base):
    ...
    )  # short_term|long_term|episodic|semantic|preference|operational
...
class AgentMemoryItem(Base):
    ...
    collection_id: Mapped[uuid.UUID | None] = mapped_column(
        UUID(as_uuid=True),
        ForeignKey("agent_memory_collections.id", ondelete="SET NULL"),
        nullable=True,
    )
...
class AgentMemoryAccessEvent(Base):
    ...
    memory_item_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True), ForeignKey("agent_memory_items.id", ondelete="CASCADE"), nullable=False
    )
...
class AgentPlan(Base):
    ...
    cost_estimates = relationship(
        "AgentPlanCostEstimate", back_populates="plan", cascade="all, delete-orphan"
    )
...
class AgentTask(Base):
    ...
    task_type: Mapped[str] = mapped_column(
        String(64), nullable=False
    )  # tool_call|model_call|memory_op|sub_task
    status: Mapped[str] = mapped_column(
        String(32), default="pending"
    )  # pending|running|blocked|waiting_approval|completed|failed|skipped
    ...
    dependencies = relationship(
        "AgentTaskDependency",
        foreign_keys="[AgentTaskDependency.task_id]",
        back_populates="task",
...
class AgentHandoffPolicy(Base):
    ...
    requires_approval: Mapped[bool] = mapped_column(Boolean, default=False)
    tenant_boundary_mode: Mapped[str] = mapped_column(
        String(32), default="strict"
    )  # strict|relaxed
    created_at: Mapped[datetime] = mapped_column(
        DateTime(timezone=True), default=utc_now, nullable=False
    )
...
class AgentHandoffEvent(Base):
    ...
    session_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True),
        ForeignKey("agent_collaboration_sessions.id", ondelete="CASCADE"),
        nullable=False,
        index=True,
    )
...
class AgentMarketplaceEntry(Base):
    ...
    category: Mapped[str] = mapped_column(
        String(64), nullable=False
    )  # support, operations, billing, etc.
    ...
    versions = relationship(
        "AgentBundleVersion", back_populates="entry", cascade="all, delete-orphan"
    )
...
class AgentBundleVersion(Base):
    ...
    entry_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True),
        ForeignKey("agent_marketplace_entries.id", ondelete="CASCADE"),
        nullable=False,
        index=True,
    )
    version: Mapped[str] = mapped_column(String(64), nullable=False)
...
class AgentBundleInstall(Base):
    ...
    version_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True), ForeignKey("agent_bundle_versions.id"), nullable=False
    )
    agent_id: Mapped[uuid.UUID | None] = mapped_column(
        UUID(as_uuid=True), ForeignKey("agent_definitions.id", ondelete="SET NULL"), nullable=True
    )
    status: Mapped[str] = mapped_column(
...
class AgentBundleTrustReport(Base):
    ...
    version_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True),
        ForeignKey("agent_bundle_versions.id", ondelete="CASCADE"),
        nullable=False,
        index=True,
    )
...
class AgentApprovalDecision(Base):
    ...
    approval_request_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True),
        ForeignKey("agent_approval_requests.id", ondelete="CASCADE"),
        nullable=False,
        index=True,
    )
...
class AgentApprovalPolicy(Base):
    ...
    trigger_type: Mapped[str] = mapped_column(
        String(64), nullable=False
    )  # risk_level | tool_call | agent_run | always
...
class AgentMemoryRedactionEvent(Base):
    ...
    memory_item_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True),
        ForeignKey("agent_memory_items.id", ondelete="CASCADE"),
        nullable=False,
        index=True,
    )
    redacted_types: Mapped[str] = mapped_column(
        String(256), nullable=False
    )  # csv of types like email, secret, key
    created_at: Mapped[datetime] = mapped_column(
        DateTime(timezone=True), default=utc_now, nullable=False
    )
...
class AgentIncident(Base):
    ...
    incident_type: Mapped[str] = mapped_column(
        String(64), nullable=False
    )  # e.g., repeated_tool_failure, run_stuck
    title: Mapped[str] = mapped_column(String(256), nullable=False)
...
class AgentIncidentEvent(Base):
    ...
    incident_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True),
        ForeignKey("agent_incidents.id", ondelete="CASCADE"),
        nullable=False,
        index=True,
    )
    event_type: Mapped[str] = mapped_column(String(64), nullable=False)
...
class AgentTrace(Base):
    ...
    trace_type: Mapped[str] = mapped_column(
        String(64), nullable=False
    )  # reasoning_step, tool_call, memory_read, memory_write, retry, planning, review
...
class AgentPolicyDecision(Base):
    ...
    action_type: Mapped[str] = mapped_column(
        String(64), nullable=False
    )  # tool_call|memory_read|memory_write|handoff|planner_exec
    subject: Mapped[str] = mapped_column(String(256), nullable=False)  # e.g. tool name, memory type
    tenant_id: Mapped[str] = mapped_column(String(128), nullable=False, index=True)
    agent_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True), ForeignKey("agent_registry_entries.id"), nullable=False, index=True
    )
    risk_level: Mapped[str] = mapped_column(String(32), default="low")
...
class AgentGuardrailEvent(Base):
    ...
    guardrail_type: Mapped[str] = mapped_column(
        String(64), nullable=False
    )  # jailbreak, pii, secret, etc.
    detection_point: Mapped[str] = mapped_column(
        String(64), nullable=False
    )  # input, model_output, tool_output
    raw_content: Mapped[str | None] = mapped_column(Text, nullable=True)
...
class AgentGuardrailDecision(Base):
    ...
    decision: Mapped[str] = mapped_column(
        String(32), nullable=False
    )  # allow, redact, require_human_review, block
    reason: Mapped[str | None] = mapped_column(Text, nullable=True)
...
class AgentEvalFailure(Base):
    ...
    failure_type: Mapped[str] = mapped_column(
        String(64), nullable=False
    )  # regression, secret_leak, tool_error, etc.
    details: Mapped[dict] = mapped_column(JSON, nullable=False, default=dict)
    created_at: Mapped[datetime] = mapped_column(
        DateTime(timezone=True), default=utc_now, nullable=False
    )
...
class AgentIncidentLink(Base):
    ...
    linked_incident_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True),
        ForeignKey("agent_incidents.id", ondelete="CASCADE"),
        nullable=False,
        index=True,
    ...
    link_type: Mapped[str] = mapped_column(
        String(32), default="related"
    )  # related, causal, duplicate
    created_at: Mapped[datetime] = mapped_column(
        DateTime(timezone=True), default=utc_now, nullable=False
    )
...
class AgentSLOWindow(Base):
    ...
    metrics_json: Mapped[dict] = mapped_column(
        JSON, default=dict
    )  # detailed SLO metrics like p95 latency
    created_at: Mapped[datetime] = mapped_column(
        DateTime(timezone=True), default=utc_now, nullable=False
    )
...
class AgentEnvironmentPolicy(Base):
    ...
    )  # dev, test, staging, production, sovereign, managed
...
class AgentRBACEvent(Base):
    ...
    event_type: Mapped[str] = mapped_column(
        String(64), nullable=False
    )  # permission.denied, role.assigned
...
class AgentPolicyException(Base):
    ...
    policy_type: Mapped[str] = mapped_column(
        String(64), nullable=False
    )  # sovereign_tool_block, destructive_tool_requirement
...
class AgentPromptBaseline(Base):
    ...
    eval_run_id: Mapped[uuid.UUID | None] = mapped_column(
        UUID(as_uuid=True), ForeignKey("agent_eval_runs.id"), nullable=True
    )
...
class AgentCatalogItem(Base):
    ...
    item_type: Mapped[str] = mapped_column(
        String(32), nullable=False, index=True
    )  # agent|tool|prompt|eval_dataset|policy|planner
    name: Mapped[str] = mapped_column(String(128), nullable=False, index=True)
    owner: Mapped[str] = mapped_column(String(128), nullable=False)
    status: Mapped[str] = mapped_column(
        String(32), default="draft"
    )  # draft|active|deprecated|production
    created_at: Mapped[datetime] = mapped_column(
        DateTime(timezone=True), default=utc_now, nullable=False
    )
    updated_at: Mapped[datetime] = mapped_column(
    ...
    versions = relationship(
        "AgentCatalogVersion", back_populates="catalog_item", cascade="all, delete-orphan"
    )
    rollbacks = relationship(
        "AgentCatalogRollback", back_populates="catalog_item", cascade="all, delete-orphan"
    )
...
class AgentCatalogVersion(Base):
    ...
    catalog_item_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True),
        ForeignKey("agent_catalog_items.id", ondelete="CASCADE"),
        nullable=False,
        index=True,
    )
    version: Mapped[str] = mapped_column(String(64), nullable=False)
...
class AgentCatalogRollback(Base):
    ...
    from_version_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True), ForeignKey("agent_catalog_versions.id"), nullable=False
    )
...
class AgentSharedMemoryPolicy(Base):
    ...
    id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    tenant_id: Mapped[str] = mapped_column(String(128), nullable=False, index=True)
    agent_group_id: Mapped[str] = mapped_column(
        String(128), nullable=False
    )  # logical group for memory sharing
    agent_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True), ForeignKey("agent_definitions.id", ondelete="CASCADE"), nullable=False
    )
...
class AgentA2ARegistration(Base):
    ...
    auth_token: Mapped[str] = mapped_column(
        String(256), nullable=False
    )  # Authorization token for incoming/outgoing A2A calls
    capabilities: Mapped[dict] = mapped_column(
        JSON, default=dict
    )  # discovery info (e.g. tools, description)
