...
def upgrade():
    ...
    op.create_index(op.f('ix_commercial_qos_tiers_name'), 'commercial_qos_tiers', ['name'], unique=True)

    # Add fields to commercial_routing_events for SLA tracking
    op.add_column('commercial_routing_events', sa.Column('qos_tier', sa.String(length=64), nullable=True))
    op.add_column('commercial_routing_events', sa.Column('sla_pass', sa.Boolean(), nullable=True))
    op.add_column('commercial_routing_events', sa.Column('degradation_applied', sa.String(length=32), nullable=True))
    op.add_column('commercial_routing_events', sa.Column('qos_priority', sa.Integer(), nullable=True))

    # Add priority to generation_jobs
    op.add_column('generation_jobs', sa.Column('priority', sa.Integer(), server_default='100', nullable=False))
