...
def upgrade() -> None:
    ...
    op.create_index(
        op.f("ix_managed_organizations_slug"), "managed_organizations", ["slug"], unique=True
    )
    ...
    op.create_table(
        ...
        sa.ForeignKeyConstraint(
            ["organization_id"],
            ["managed_organizations.id"],
        ),
        sa.PrimaryKeyConstraint("id"),
    )
    op.create_index(
        op.f("ix_managed_workspaces_slug"), "managed_workspaces", ["slug"], unique=False
    )
    ...
    op.create_index(
        op.f("ix_managed_appliances_appliance_external_id"),
        "managed_appliances",
    ...
    op.create_table(
        ...
        sa.Column("created_at", sa.DateTime(), nullable=False),
        sa.ForeignKeyConstraint(
            ["used_by_appliance_id"],
            ["managed_appliances.id"],
        ),
        sa.ForeignKeyConstraint(
            ["workspace_id"],
            ["managed_workspaces.id"],
    ...
    op.create_index(
        op.f("ix_appliance_enrollments_enrollment_token"),
        "appliance_enrollments",
        ["enrollment_token"],
        unique=True,
    )
    ...
    op.add_column(
        "clients", sa.Column("organization_id", postgresql.UUID(as_uuid=True), nullable=True)
    )
