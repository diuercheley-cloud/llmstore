...
def upgrade():
    ...
    op.add_column(
        "generation_jobs",
        sa.Column("effective_priority", sa.Numeric(precision=20, scale=6), nullable=True),
    )
    op.add_column(
        "generation_jobs", sa.Column("dequeued_at", sa.DateTime(timezone=True), nullable=True)
    )
    op.add_column("generation_jobs", sa.Column("queue_wait_ms", sa.Integer(), nullable=True))
    op.add_column(
        "generation_jobs", sa.Column("rate_limit_status", sa.String(length=32), nullable=True)
    )
    op.add_column(
        "generation_jobs", sa.Column("rate_limit_reason", sa.String(length=255), nullable=True)
    )
