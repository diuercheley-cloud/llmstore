...
def upgrade() -> None:
    op.add_column(
        "clients",
        sa.Column("billing_status", sa.String(length=16), nullable=False, server_default="active"),
    )
    ...
    op.create_table(
        ...
        sa.Column(
            "pricing_rule_id",
            postgresql.UUID(as_uuid=True),
            sa.ForeignKey("pricing_rules.id"),
            nullable=True,
        ),
        ...
        sa.Column(
            "payment_method", sa.String(length=32), nullable=False, server_default="manual_pix"
        ),
