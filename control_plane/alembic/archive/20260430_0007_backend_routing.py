...
def upgrade() -> None:
    op.create_table(
        ...
        sa.Column(
            ...
            postgresql.UUID(as_uuid=True),
            sa.ForeignKey("inference_backends.id", ondelete="CASCADE"),
            nullable=False,
        ),
    ...
    op.create_index(
        "ix_model_backend_routes_model_registry_id", "model_backend_routes", ["model_registry_id"]
    )
    op.create_index(
        "ix_model_backend_routes_inference_backend_id",
        "model_backend_routes",
        ["inference_backend_id"],
    )
    ...
    op.add_column(
        "request_logs", sa.Column("attempts", sa.Integer(), nullable=False, server_default="1")
    )
    op.add_column(
        "request_logs",
