...
def upgrade() -> None:
    ...
    op.create_index(
        op.f("ix_plugin_marketplace_entries_name"),
        "plugin_marketplace_entries",
        ["name"],
        unique=True,
    )
    ...
    op.create_table(
        ...
        sa.ForeignKeyConstraint(
            ["plugin_entry_id"],
            ["plugin_marketplace_entries.id"],
        ),
        sa.PrimaryKeyConstraint("id"),
    )
    op.create_index(
        op.f("ix_plugin_versions_plugin_entry_id"),
        "plugin_versions",
        ["plugin_entry_id"],
        unique=False,
    )
    ...
    op.create_table(
        ...
        sa.Column(
            "status", sa.String(length=32), nullable=False, server_default=sa.text("'installed'")
        ),
    ...
    op.create_index(
        op.f("ix_plugin_installs_plugin_entry_id"),
        "plugin_installs",
        ["plugin_entry_id"],
        unique=False,
    )
    ...
    op.create_index(
        op.f("ix_plugin_permissions_plugin_install_id"),
        "plugin_permissions",
        ["plugin_install_id"],
        unique=False,
    )
    ...
    op.create_table(
        ...
        sa.Column(
            "vulnerabilities_found", sa.Integer(), nullable=False, server_default=sa.text("0")
        ),
    ...
    op.create_index(
        op.f("ix_plugin_trust_reports_plugin_version_id"),
        "plugin_trust_reports",
        ["plugin_version_id"],
        unique=False,
    )
    ...
    op.create_index(
        op.f("ix_plugin_reviews_plugin_entry_id"),
        "plugin_reviews",
        ["plugin_entry_id"],
        unique=False,
    )
    op.create_index(
        op.f("ix_plugin_reviews_admin_user_id"), "plugin_reviews", ["admin_user_id"], unique=False
    )
