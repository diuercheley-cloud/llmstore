...
def upgrade() -> None:
    ...
    op.create_index(
        op.f("ix_rag_documents_client_id"), "rag_documents", ["client_id"], unique=False
    )
    ...
    if res:
        ...
        op.add_column(
            "rag_document_chunks", sa.Column("embedding", postgresql.JSONB(), nullable=True)
        )

    op.create_index(
        op.f("ix_rag_document_chunks_client_id"), "rag_document_chunks", ["client_id"], unique=False
    )
    op.create_index(
