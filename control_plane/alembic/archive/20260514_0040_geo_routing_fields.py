...
def upgrade():
    ...
    op.add_column(
        "commercial_cluster_registry", sa.Column("datacenter", sa.String(length=64), nullable=True)
    )
    op.add_column(
        "commercial_cluster_registry", sa.Column("continent", sa.String(length=32), nullable=True)
    )
    op.add_column(
        "commercial_cluster_registry", sa.Column("country", sa.String(length=32), nullable=True)
    )
    op.add_column(
    ...
    op.add_column(
        "commercial_cluster_registry",
        sa.Column("avg_public_latency_ms", sa.Integer(), nullable=True),
    )
    op.add_column(
        "commercial_cluster_registry", sa.Column("geo_metadata_json", sa.JSON(), nullable=True)
    )

    op.create_index(
        op.f("ix_commercial_cluster_registry_region_group"),
        "commercial_cluster_registry",
        ["region_group"],
        unique=False,
    )
...
def downgrade():
    op.drop_index(
        op.f("ix_commercial_cluster_registry_region_group"),
        table_name="commercial_cluster_registry",
    )
