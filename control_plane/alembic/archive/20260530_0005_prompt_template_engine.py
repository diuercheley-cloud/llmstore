...
def upgrade() -> None:
    ...
    prompt_indexes = (
        {index["name"] for index in inspector.get_indexes("prompt_templates")}
    ...
    version_indexes = (
        {index["name"] for index in inspector.get_indexes("prompt_template_versions")}
        if inspector.has_table("prompt_template_versions")
        else set()
    )
    if "ix_prompt_template_versions_template_id" not in version_indexes:
        op.create_index(
            "ix_prompt_template_versions_template_id", "prompt_template_versions", ["template_id"]
        )

    existing_fks = (
        {fk["name"] for fk in inspector.get_foreign_keys("prompt_templates")}
        if inspector.has_table("prompt_templates")
        else set()
    )
    ...
    if not inspector.has_table("prompt_experiments"):
        op.create_table(
            ...
            sa.Column(
                "version_a_id",
                postgresql.UUID(as_uuid=True),
                sa.ForeignKey("prompt_template_versions.id"),
                nullable=False,
            ),
            sa.Column(
                "version_b_id",
    ...
    experiment_indexes = (
        {index["name"] for index in inspector.get_indexes("prompt_experiments")}
        if inspector.has_table("prompt_experiments")
        else set()
    )
    ...
    if not inspector.has_table("prompt_playground_runs"):
        op.create_table(
            ...
            sa.Column(
                "version_id",
                postgresql.UUID(as_uuid=True),
                sa.ForeignKey("prompt_template_versions.id", ondelete="CASCADE"),
                nullable=False,
            ),
    ...
    variable_indexes = (
        {index["name"] for index in inspector.get_indexes("prompt_template_variables")}
        if inspector.has_table("prompt_template_variables")
        else set()
    )
    if "ix_prompt_template_variables_template_id" not in variable_indexes:
        op.create_index(
            "ix_prompt_template_variables_template_id", "prompt_template_variables", ["template_id"]
        )
    ...
    if not inspector.has_table("prompt_template_render_events"):
        op.create_table(
            ...
            sa.Column(
                "version_id",
                postgresql.UUID(as_uuid=True),
                sa.ForeignKey("prompt_template_versions.id", ondelete="SET NULL"),
                nullable=True,
            ),
            sa.Column("tenant_id", sa.String(length=128), nullable=False),
    ...
    render_indexes = (
        {index["name"] for index in inspector.get_indexes("prompt_template_render_events")}
        if inspector.has_table("prompt_template_render_events")
        else set()
    )
    if "ix_prompt_template_render_events_template_id" not in render_indexes:
        op.create_index(
            "ix_prompt_template_render_events_template_id",
            "prompt_template_render_events",
            ["template_id"],
        )
    if "ix_prompt_template_render_events_version_id" not in render_indexes:
        op.create_index(
            "ix_prompt_template_render_events_version_id",
            "prompt_template_render_events",
            ["version_id"],
        )
    if "ix_prompt_template_render_events_tenant_id" not in render_indexes:
        op.create_index(
            "ix_prompt_template_render_events_tenant_id",
            "prompt_template_render_events",
            ["tenant_id"],
        )
    if "ix_prompt_template_render_events_agent_run_id" not in render_indexes:
        op.create_index(
            "ix_prompt_template_render_events_agent_run_id",
            "prompt_template_render_events",
            ["agent_run_id"],
        )
    if "ix_prompt_template_render_events_agent_id" not in render_indexes:
        op.create_index(
            "ix_prompt_template_render_events_agent_id",
            "prompt_template_render_events",
            ["agent_id"],
        )
    ...
    if "ix_agent_definitions_prompt_template_id" not in agent_definition_indexes:
        op.create_index(
            "ix_agent_definitions_prompt_template_id", "agent_definitions", ["prompt_template_id"]
        )
    if "ix_agent_definitions_prompt_template_version_id" not in agent_definition_indexes:
        op.create_index(
            "ix_agent_definitions_prompt_template_version_id",
            "agent_definitions",
            ["prompt_template_version_id"],
        )
...
def downgrade() -> None:
    ...
    if inspector.has_table("agent_definitions"):
        ...
        if "ix_agent_definitions_prompt_template_version_id" in indexes:
            op.drop_index(
                "ix_agent_definitions_prompt_template_version_id", table_name="agent_definitions"
            )
