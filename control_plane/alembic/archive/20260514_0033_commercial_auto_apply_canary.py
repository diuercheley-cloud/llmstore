...
def upgrade() -> None:
    ...
    op.add_column(
        "commercial_routing_config",
        sa.Column("can_auto_apply", sa.Boolean(), nullable=False, server_default="false"),
    )
    op.add_column(
        "commercial_routing_config",
    ...
    op.add_column(
        "commercial_routing_config",
        sa.Column("auto_apply_source_event_count", sa.Integer(), nullable=True),
    )
    op.add_column(
        "commercial_routing_config",
        sa.Column("auto_apply_confidence", sa.String(length=50), nullable=True),
    )
    op.add_column(
        "commercial_routing_config", sa.Column("promoted_from_config_id", sa.UUID(), nullable=True)
    )
    op.add_column(
        "commercial_routing_config",
    ...
    op.create_index(
        "ix_commercial_routing_config_canary_lookup",
        "commercial_routing_config",
        ["provider", "model", "canary_enabled"],
    )
    op.create_index(
        "ix_commercial_routing_config_auto_applied", "commercial_routing_config", ["auto_applied"]
    )
    op.create_index(
        "ix_commercial_routing_config_expires_at", "commercial_routing_config", ["expires_at"]
    )
...
def downgrade() -> None:
    ...
    op.drop_index(
        "ix_commercial_routing_config_auto_applied", table_name="commercial_routing_config"
    )
    op.drop_index(
        "ix_commercial_routing_config_canary_lookup", table_name="commercial_routing_config"
    )
