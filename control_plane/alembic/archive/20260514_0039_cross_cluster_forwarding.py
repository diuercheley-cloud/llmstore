...
def upgrade() -> None:
    ...
    op.add_column(
        ...
        sa.Column("forwarding_enabled", sa.Boolean(), server_default="false", nullable=False),
    )
    op.add_column(
        "commercial_cluster_registry",
        sa.Column("forwarding_base_url", sa.String(length=500), nullable=True),
    )
    op.add_column(
        "commercial_cluster_registry",
        sa.Column("forwarding_public_key", sa.String(length=2048), nullable=True),
    )
    op.add_column(
        "commercial_cluster_registry",
        sa.Column("forwarding_jwt_audience", sa.String(length=255), nullable=True),
    )
    op.add_column(
        "commercial_cluster_registry",
        sa.Column("forwarding_require_mtls", sa.Boolean(), server_default="false", nullable=False),
    )
    op.add_column(
        "commercial_cluster_registry",
        sa.Column(
            "forwarding_status", sa.String(length=32), server_default="healthy", nullable=False
        ),
    )
    op.add_column(
        "commercial_cluster_registry",
        sa.Column("forwarding_metadata_json", sa.JSON(), nullable=True),
    )

    # Create commercial_cross_cluster_forwarding_event table
    op.create_table(
        "commercial_cross_cluster_forwarding_event",
    ...
    op.create_index(
        op.f("ix_commercial_cross_cluster_forwarding_event_source_cluster_id"),
        "commercial_cross_cluster_forwarding_event",
        ["source_cluster_id"],
        unique=False,
    )
    op.create_index(
        op.f("ix_commercial_cross_cluster_forwarding_event_target_cluster_id"),
        "commercial_cross_cluster_forwarding_event",
        ["target_cluster_id"],
        unique=False,
    )
    op.create_index(
        op.f("ix_commercial_cross_cluster_forwarding_event_correlation_id"),
        "commercial_cross_cluster_forwarding_event",
        ["correlation_id"],
        unique=False,
    )
    op.create_index(
        op.f("ix_commercial_cross_cluster_forwarding_event_created_at"),
        "commercial_cross_cluster_forwarding_event",
        ["created_at"],
        unique=False,
    )
...
def downgrade() -> None:
    ...
    op.drop_index(
        op.f("ix_commercial_cross_cluster_forwarding_event_created_at"),
        table_name="commercial_cross_cluster_forwarding_event",
    )
    op.drop_index(
        op.f("ix_commercial_cross_cluster_forwarding_event_correlation_id"),
        table_name="commercial_cross_cluster_forwarding_event",
