...
def upgrade() -> None:
    ...
    thread_indexes = (
        {index["name"] for index in inspector.get_indexes("agent_conversation_threads")}
        if inspector.has_table("agent_conversation_threads")
        else set()
    )
    if "ix_agent_conversation_threads_session_id" not in thread_indexes:
        op.create_index(
            "ix_agent_conversation_threads_session_id", "agent_conversation_threads", ["session_id"]
        )
    ...
    if not inspector.has_table("agent_thread_messages"):
        op.create_table(
            ...
            sa.Column(
                "thread_id",
                postgresql.UUID(as_uuid=True),
                sa.ForeignKey("agent_conversation_threads.id", ondelete="CASCADE"),
                nullable=False,
            ),
            sa.Column(
                "session_id",
    ...
    message_indexes = (
        {index["name"] for index in inspector.get_indexes("agent_thread_messages")}
        if inspector.has_table("agent_thread_messages")
        else set()
    )
    if "ix_agent_thread_messages_thread_id" not in message_indexes:
        op.create_index(
            "ix_agent_thread_messages_thread_id", "agent_thread_messages", ["thread_id"]
        )
    if "ix_agent_thread_messages_session_id" not in message_indexes:
        op.create_index(
            "ix_agent_thread_messages_session_id", "agent_thread_messages", ["session_id"]
        )
    ...
    if "ix_agent_thread_messages_created_at" not in message_indexes:
        op.create_index(
            "ix_agent_thread_messages_created_at", "agent_thread_messages", ["created_at"]
        )
    ...
    session_run_indexes = (
        {index["name"] for index in inspector.get_indexes("agent_session_runs")}
