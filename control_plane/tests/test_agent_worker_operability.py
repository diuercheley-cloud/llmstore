...
async def test_job_reaches_dlq_after_max_attempts(test_db):
    ...
    async with test_db() as db:
        ...
        agent = AgentDefinition(id=agent_id, name="A", tenant_id="t1", status="active", instructions="I", model_id="m", owner="o", version="1")
        run = AgentRun(id=run_id, agent_id=agent_id, tenant_id="t1", status="running", total_steps=0, total_tokens=0, estimated_cost_brl=0.0)
        
        job = AgentExecutionJob(
            id=job_id, agent_run_id=run_id, agent_id=agent_id,
            tenant_id="t1", status="running", attempts=2, max_attempts=3,
            initial_delay_seconds=1, backoff_factor=2
        )
        db.add(agent)
        db.add(run)
        db.add(job)
