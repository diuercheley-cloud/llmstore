...
def test_metrics_emission():
    ...
    val = REGISTRY.get_sample_value("llm_requests_total", {
        "model": "gpt-4",
        "backend": "openai",
        "plan": "premium",
        "endpoint": "/v1/chat/completions",
...
def test_provider_failure_increment():
    initial = REGISTRY.get_sample_value("llm_provider_failures_total", {
    ...
    final = REGISTRY.get_sample_value("llm_provider_failures_total", {
        "provider_name": "test-provider",
        "error_code": "500"
    })
...
def test_routing_fallback_increment():
    initial = REGISTRY.get_sample_value("llm_routing_fallbacks_total", {
        "reason": "timeout",
        "model": "gpt-4"
    }) or 0.0
    ...
    final = REGISTRY.get_sample_value("llm_routing_fallbacks_total", {
        "reason": "timeout",
        "model": "gpt-4"
    })
    assert final == initial + 1.0
...
def test_rbac_denial_increment():
    initial = REGISTRY.get_sample_value("llm_rbac_denials_total", {
        "client_id": "c1",
        "resource": "r1",
        "action": "a1"
    }) or 0.0
    ...
    final = REGISTRY.get_sample_value("llm_rbac_denials_total", {
        "client_id": "c1",
        "resource": "r1",
        "action": "a1"
    })
...
def test_hot_swap_failure_increment():
    initial = REGISTRY.get_sample_value("llm_model_hot_swap_failures_total", {
        "model_id": "m1",
        "reason": "oom"
    }) or 0.0
    ...
    final = REGISTRY.get_sample_value("llm_model_hot_swap_failures_total", {
        "model_id": "m1",
        "reason": "oom"
    })
    assert final == initial + 1.0
