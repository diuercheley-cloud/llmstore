...
async def test_sign_payload_success(signing_service, mock_db_session):
    ...
    mock_provider = CommercialKMSProvider(id=provider_id, provider_type=CryptoProviderType.VAULT, is_active=True)

    # signing_service.get_signing_profile -> profile
    # kms_runtime.get_key_material -> key
    # kms_runtime.get_provider_config -> provider
    mock_db_session.query.return_value.filter.return_value.first.side_effect = [mock_profile, mock_key, mock_provider]
...
async def test_verify_signature_success(signing_service, mock_db_session):
    ...
    mock_profile = CommercialSigningProfile(id=profile_id, key_id=key_id, algorithm="RSA-SHA256")
    mock_key = CommercialKeyMaterial(id=key_id, provider_id=provider_id, status=KeyUsageStatus.ACTIVE, encrypted_key_blob="mock_blob")
    mock_provider = CommercialKMSProvider(id=provider_id, provider_type=CryptoProviderType.HSM, is_active=True)
