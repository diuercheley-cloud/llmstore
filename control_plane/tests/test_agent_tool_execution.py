...
async def create_mock_agent(db: AsyncSession, surface_status: str = "internal") -> AgentRegistryEntry:
...
async def test_sandbox_mock_execution(run_settings):
    async with SessionLocal() as db:
        ...
        stmt = select(AgentToolExecutionSandbox).where(AgentToolExecutionSandbox.tenant_id == "tenant-1")
...
async def test_sandbox_output_truncation(run_settings):
    async with SessionLocal() as db:
        ...
        stmt = select(AgentToolExecutionSandbox).where(AgentToolExecutionSandbox.invocation_id == invocation_id)
        res = await db.execute(stmt)
        sandbox = res.scalar_one()
...
async def test_credential_delegation_resolution(run_settings):
    async with SessionLocal() as db:
        ...
        stmt = select(AgentToolExecutionAudit).where(AgentToolExecutionAudit.tenant_id == "tenant-1")
        res = await db.execute(stmt)
...
async def test_api_endpoints(async_client, admin_token_headers, run_settings):
    ...
    response = await async_client.get(
        "/admin/agent-tools/side-effects",
        headers=admin_token_headers
    )
    assert response.status_code == status.HTTP_200_OK
    ...
    response = await async_client.post(
        f"/admin/agent-tools/credentials/{cred_id}/revoke",
        headers=admin_token_headers
    )
    assert response.status_code == status.HTTP_200_OK
