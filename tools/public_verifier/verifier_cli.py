...
def verify_file(file_path: str, export_path: str = None, gateway_url: str = None):
    ...
    if gateway_url:
        ...
        try:
            ...
            if "receipt" in data:
                res = requests.post(
                    f"{gateway_url}/verify/receipt",
                    json={"receipt_hash": data["receipt"]["receipt_hash"]},
                )
            ...
            if "merkle_root" in data:
                res = requests.post(
                    f"{gateway_url}/verify/witness-quorum",
                    json={"merkle_root": data["merkle_root"]},
                )
                if res.status_code == 200:
            ...
            if data.get("proof_type") == "retrieval" and data.get("proof_hash"):
                res = requests.post(
                    f"{gateway_url}/verify/retrieval-proof", json={"proof_hash": data["proof_hash"]}
                )
                if res.status_code == 200:
                    print(
                        f"  [Online] Retrieval Proof: {res.json().get('status', 'unknown').upper()}"
                    )
                res = requests.post(
                    f"{gateway_url}/verify/lineage-consistency",
                    json={"proof_hash": data["proof_hash"]},
                )
                if res.status_code == 200:
                    print(
                        f"  [Online] Lineage Consistency: {res.json().get('status', 'unknown').upper()}"
                    )
                res = requests.post(
                    f"{gateway_url}/verify/retrieval-replay",
                    json={"proof_hash": data["proof_hash"]},
                )
                if res.status_code == 200:
                    print(
                        f"  [Online] Retrieval Replay: {res.json().get('status', 'unknown').upper()}"
                    )
...
def main():
    parser = argparse.ArgumentParser(
        description="Public Verifier CLI for Verifiable AI Execution Proofs"
    )
    ...
    online_parser = subparsers.add_parser(
        "verify-online", help="Verify a proof against a public gateway"
    )
    online_parser.add_argument("file", help="Path to the proof JSON file")
    online_parser.add_argument(
        "--gateway-url", required=True, help="Gateway URL (e.g., http://localhost:8080/attestation)"
    )

    # Batch command
    batch_parser = subparsers.add_parser(
        "verify-batch", help="Verify all JSON proofs in a directory"
    )
