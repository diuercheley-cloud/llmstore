...
principles = [
    (
        "Determinístico",
        "Toda operação produz saídas reproduzíveis e verificáveis. Cada execução gera um receipt criptográfico.",
    ),
    (
        "Replay-Safe",
        "Todas as transições de estado podem ser reproduzidas a partir de logs de eventos.",
    ),
    ...
    (
        "Feature-Flag Driven",
        "~200+ feature flags controlam todas as capacidades do sistema, todas default safe/off.",
    ),
]
...
doc.add_paragraph("O sistema é dividido em três grandes planos:")
...
cp_components = [
    (
        "app/api/",
        "215 arquivos de rota cobrindo toda a superfície da API: endpoints compatíveis com OpenAI, "
        "CRUD administrativo, gestão de agentes, billing, compliance, federação, observabilidade.",
    ),
    (
        "app/core/",
        "Infraestrutura central: configuração, segurança, CORS, métricas, logging, feature flags.",
    ),
    (
        "app/services/",
        "107 módulos de serviço implementando toda a lógica de negócio: proxy de inferência, "
        "roteamento, autenticação, billing, RAG, execução de agentes, cache, governança, compliance.",
    ),
    (
        "app/models/",
        "Modelos SQLAlchemy ORM organizados em subdiretórios: agents, billing, commercial, "
        "core, governance, operations, plugins, rag, runtime.",
    ),
    (
        "app/domains/",
        "16 bounded contexts representando os domínios do DDD (detalhados na seção 6).",
    ),
...
doc.add_paragraph("Dashboard administrativo completo em React 19 + TypeScript + Vite 8:")
admin_features = [
...
doc.add_paragraph("Plataforma completa de agentes de IA com execução governada. Inclui:")
agent_features = [
...
modes = [
    (
        "Docker Compose",
        "Modo principal e mais simples. Arquivos docker-compose.yml, docker-compose.dev.yml, docker-compose.prod.yml.",
    ),
    (
        "Kubernetes",
        "Suporte completo com manifests em deploy/kubernetes/ e Helm chart em deploy/helm/llm-inference-stack/.",
    ),
...
sdks = [
    (
        "Python SDK (kleberai)",
        "SDK principal com CLI agentctl. Localizado em sdk/python/. "
...
integrations = [
    (
        "Crossplane Provider",
        "Provider Kubernetes-native para composição de control plane. "
        "Localizado em integrations/crossplane-provider-llmstack/",
    ),
    (
        "Terraform Provider",
        "Provider Terraform para Infrastructure as Code. "
        "Localizado em integrations/terraform-provider-llmstack/",
    ),
    (
        "VS Code Extension",
        "Extensão para IDE VS Code. Localizado em integrations/vscode-extension/",
    ),
...
github_workflows = [
    (
        "ci.yml",
        "CI principal: lint, test, typecheck, security scans, E2E, docs validation. "
...
doc.add_paragraph(
    "Estágios do CI: preflight -> lint -> test -> typecheck -> security -> build -> validate -> chaos -> benchmark -> package -> release."
)
...
doc.add_paragraph("O sistema possui uma camada comercial definida em commercial/ com:")
commercial_items = [
...
strengths = [
    (
        "Soberania Total",
        "Zero dependência de nuvem. Todo o sistema funciona offline, "
        "garantindo que dados nunca saiam do ambiente controlado.",
    ),
    (
        "Compatibilidade OpenAI",
        "API compatível com OpenAI permite migração transparente "
        "de aplicações existentes sem alteração de código.",
    ),
    (
        "Feature Flags Granular",
        "~200+ feature flags permitem controle preciso sobre cada "
        "capacidade do sistema, com defaults seguros.",
    ),
    (
        "Governança Enterprise",
        "Framework de compliance com SOC 2/ISO 27001, audit trail "
        "criptográfico, políticas de governança, e RBAC completo.",
    ),
    (
        "Multi-tenant Nativo",
        "Suporte completo a múltiplos tenants com isolamento, quotas, "
        "rate limiting, e billing individualizado.",
    ),
    (
        "Multi-modalidade",
        "Suporte a texto, RAG (busca semântica), TTS (voz), e futuramente visão/áudio.",
    ),
    (
        "Plataforma de Agentes",
        "Runtime completo de agentes com memória federada, grafo "
        "de conhecimento, IAM, e execução governada.",
    ),
    (
        "Maturidade DevOps",
        "CI/CD em GitHub Actions e GitLab CI, Docker Compose e "
        "Kubernetes, Helm charts, Crossplane e Terraform providers.",
    ),
    (
        "Observabilidade Completa",
        "Stack Grafana/Prometheus/Loki/Tempo/OpenTelemetry "
        "com dashboards e alertas pré-configurados.",
    ),
    (
        "Documentação Extensa",
        "214+ arquivos de documentação cobrindo todos os aspectos "
        "do sistema, desde arquitetura até operações.",
    ),
    (
        "SDKs Multi-linguagem",
        "Python, Go, e Node.js SDKs para integração com diferentes "
        "ecossistemas de desenvolvimento.",
    ),
    (
        "Testes e Qualidade",
        "Pirâmide de testes completa com unitários, integração, E2E, caos, e 280+ validadores.",
    ),
]
...
risks = [
    (
        ...
        "Com 215+ endpoints, 107 serviços, 16 bounded contexts, "
...
run = footer2.add_run(
    f"Gerado em {datetime.now().strftime('%d/%m/%Y %H:%M')} | LLM Inference Stack v2.0.9"
)
run.font.size = Pt(10)
