...
async def test_cross_tenant_blocked(session: AsyncSession):
    ...
    doc_a = await confidential_rag_vault.add_document_to_vault(session, vault_a.id, "A Content", "internal")
...
async def test_classification_mismatch_blocked(session: AsyncSession):
    vault_a = await confidential_rag_vault.create_vault(session, "tenant-A", "Vault A")
    doc_a = await confidential_rag_vault.add_document_to_vault(session, vault_a.id, "Restricted Content", "restricted")
