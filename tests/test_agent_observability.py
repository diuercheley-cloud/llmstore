...
async def test_agent_observability_metrics_increment(session):
    ...
    before = REGISTRY.get_sample_value('llm_agent_runs_total', {'agent_id': str(agent_def.id), 'status': 'started'}) or 0
    
    await executor.execute_step()
    
    # Check if metrics were incremented
    after_started = REGISTRY.get_sample_value('llm_agent_runs_total', {'agent_id': str(agent_def.id), 'status': 'started'})
    after_completed = REGISTRY.get_sample_value('llm_agent_runs_total', {'agent_id': str(agent_def.id), 'status': 'completed'})
