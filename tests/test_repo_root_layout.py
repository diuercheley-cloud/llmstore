...
def test_moved_files_exist_in_new_locations():
    ...
    expected_locations = {
        "scripts/benchmark_model_local_runner.py": True,
        "scripts/benchmark_runner.py": True,
        "scripts/local_dr_backup.py": True,
        "scripts/redact_json.py": True,
        "scripts/llm_stack_client.py": True,
        "scripts/test-max-concurrency.py": True,
        "tests/test_simulate.py": True,
    }
    
    for path_str, should_exist in expected_locations.items():
        path = root_dir / path_str
        assert path.exists() == should_exist, f"File {path_str} existence check failed (expected {should_exist})"
...
def test_scripts_still_executable():
    ...
    scripts_to_test = [
        "scripts/benchmark_model_local_runner.py",
        "scripts/redact_json.py",
        "scripts/llm_stack_client.py", # This is a library but should be importable
    ]
    ...
    for script_path in scripts_to_test:
        ...
        if "client" in script_path:
            cmd = [sys.executable, "-c", f"import sys; sys.path.append('scripts'); import {Path(script_path).stem}"]
        else:
            cmd = [sys.executable, str(full_path), "--help"]
            
        result = subprocess.run(cmd, capture_output=True, text=True)
        assert result.returncode == 0, f"Script {script_path} failed to run or be imported: {result.stderr}"

def test_common_sh_sets_pythonpath():
    """Verify that scripts/common.sh exports PYTHONPATH including scripts/."""
    root_dir = Path(__file__).resolve().parent.parent
    ...
    assert "scripts" in result.stdout, "PYTHONPATH does not include scripts/ directory after sourcing common.sh"
