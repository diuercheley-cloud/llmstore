...
async def test_generate_calibration_report(session):
    ...
    session.add(CommercialRoutingEvent(
        selected_provider="openai", selected_model="gpt-4",
        estimated_cost_brl=1.0, actual_cost_brl=1.5, # 50% under
        created_at=datetime.now(timezone.utc)
    ))
    session.add(CommercialRoutingEvent(
        selected_provider="anthropic", selected_model="claude-3",
        estimated_cost_brl=1.5, actual_cost_brl=1.0, # 50% over
        created_at=datetime.now(timezone.utc)
    ))
    await session.commit()
    ...
    openai_mult = next(m for m in report["recommended_cost_multipliers"] if m["provider"] == "openai")
    assert openai_mult["recommended_cost_multiplier"] > 1.0
    
    # Check if anthropic has a multiplier < 1
    anthropic_mult = next(m for m in report["recommended_cost_multipliers"] if m["provider"] == "anthropic")
...
async def test_calculate_estimation_error_with_outlier(session, monkeypatch):
    ...
    for _ in range(20):
        event = CommercialRoutingEvent(
            selected_provider="openai", selected_model="gpt-4",
            estimated_cost_brl=1.0, actual_cost_brl=1.0,
            created_at=datetime.now(timezone.utc)
        )
        session.add(event)
    ...
    for _ in range(2):
        event = CommercialRoutingEvent(
            selected_provider="openai", selected_model="gpt-4",
            estimated_cost_brl=1.0, actual_cost_brl=100.0, # 100x cost!
            created_at=datetime.now(timezone.utc)
        )
        session.add(event)
...
async def test_confidence_levels(session, monkeypatch):
    ...
    await session.commit()
    result = await commercial_calibration.calculate_estimation_error(session, provider="openai", min_samples=10)
    assert result["confidence"] == "low"
...
async def test_admin_endpoints_auth(admin_client, admin_token_headers):
    ...
    # Valid token
    response = await admin_client.get("/admin/routing/calibration/report", headers=admin_token_headers)
