...
async def test_trace_correlation_links():
    async with SessionLocal() as db:
        agent = await _create_test_agent(db)
        run = AgentRun(
            id=uuid.uuid4(), agent_id=agent.id, tenant_id="t1", status="running", input_data={}
        )
        db.add(run)
        await db.commit()
...
async def test_prompt_redaction_in_observability():
    async with SessionLocal() as db:
        ...
        await svc._record_timeline_event(
            run_id, "run.started", {"prompt": "my secret prompt", "other": "val"}
        )
        await db.commit()
        ...
        res = await db.execute(
            select(AgentTimelineEvent).where(AgentTimelineEvent.run_id == run_id)
        )
