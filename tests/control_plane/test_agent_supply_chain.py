...
from app.models.agents.agents import AgentBundleSignature, AgentBundleVersion, AgentMarketplaceEntry
...
async def _create_test_bundle(db: AsyncSession, content: bytes, signature_needed: bool = False):
    entry = AgentMarketplaceEntry(
        name="Test Agent", description="Test", author="test-author", category="test"
    )
    db.add(entry)
    await db.flush()
    ...
    version = AgentBundleVersion(
        entry_id=entry.id, version="1.0.0", checksum_sha256=checksum, manifest_json={}
    )
    db.add(version)
    await db.flush()
