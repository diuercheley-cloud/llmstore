...
async def setup_db():
    async with engine.begin() as conn:
        ...
        import app.models.core.admin_rbac  # noqa
...
def _mock_admin(permissions: list[str]) -> AuthenticatedAdmin:
    ...
    return AuthenticatedAdmin(
        user=user, role_names=["test-role"], permission_codes=set(permissions), token_prefix="test"
    )
...
async def test_denied_permission_generates_audit_event():
    async with SessionLocal() as db:
        ...
        res = await db.execute(
            select(AgentRBACEvent).where(AgentRBACEvent.admin_user_id == admin.user.id)
        )
