...
async def setup_agent(db_session: AsyncSession):
    ...
    run = AgentRun(id=uuid.uuid4(), agent_id=agent.id, tenant_id="test-tenant", status="running")
    db_session.add(run)
    await db_session.commit()
    return agent, run
...
@pytest.mark.asyncio
async def test_routing_formatting_uses_cheap_model(
    db_session: AsyncSession, setup_models, setup_agent
):
    ...
    result = await db_session.execute(
        select(AgentStepRoutingDecision).filter(AgentStepRoutingDecision.run_id == run.id)
    )
...
@pytest.mark.asyncio
async def test_routing_complex_reasoning_uses_high_quality_model(
    db_session: AsyncSession, setup_models, setup_agent
):
    agent, run = setup_agent
    router = AgenticRouterV2(db_session)
...
@pytest.mark.asyncio
async def test_sovereign_only_blocks_cloud_provider(
    db_session: AsyncSession, setup_models, setup_agent
):
    agent, run = setup_agent
    router = AgenticRouterV2(db_session)
...
async def test_budget_limits_choice(db_session: AsyncSession, setup_models, setup_agent):
    ...
    profile = AgentCostQualityProfile(name="strict-budget", max_cost_input=0.1)
