...
async def test_critic_review_via_mock_llm_provider():
    ...
    mock_resp = ProviderResponse(type="final", output=json.dumps(mock_response_json))
    ...
    with patch(
        "app.services.agents.multi_agent.arbitration_engine.get_agent_llm_provider",
        ...
        assert (
            res["case"]["critic_reviews"][0]["reasoning"]
            == "Strong evidence and correct implementation logic."
        )
...
async def test_heuristic_only_disabled_in_production():
    ...
    engine = ArbitrationEngine()
    candidates = [CandidateResponse(agent_id="a1", content="res")]
...
async def test_real_arbitration_requires_critic_review_flag():
    ...
    engine = ArbitrationEngine()
    with pytest.raises(
        ArbitrationExecutionError, match="requires AGENT_MULTI_AGENT_CRITIC_REVIEW_ENABLED=true"
    ):
...
async def test_safety_failure_loses():
    ...
    assert (
        res["decision"]["scores"]["unsafe-agent"] == 0.0
    )  # Safety failure sets final score to 0.0
...
async def test_output_without_evidence_loses():
    ...
    assert (
        res["decision"]["scores"]["spec-without-evidence"] == 0.0
    )  # Lack of evidence sets final score to 0.0 when competing with evidence
...
async def test_final_decision_has_receipt():
    ...
    engine = ArbitrationEngine()
    candidates = [
        CandidateResponse(agent_id="agent-1", content="Simple text response", confidence=0.8)
    ]
