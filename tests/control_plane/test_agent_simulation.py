...
def test_simulated_outputs():
    # Email
    out_email = SimulationRuntime.simulate_tool_execution(
        "send_email", "email", {"to": "user@test.com", "subject": "Hello"}
    )
    ...
    # Filesystem
    out_fs = SimulationRuntime.simulate_tool_execution(
        "write_file", "filesystem", {"path": "/tmp/test.txt", "content": "data"}
    )
    ...
    # APIs
    out_api = SimulationRuntime.simulate_tool_execution(
        "fetch_api", "APIs", {"url": "https://api.com"}
    )
    ...
    # Database
    out_db = SimulationRuntime.simulate_tool_execution(
        "db_write", "database writes", {"query": "UPDATE"}
    )
...
async def test_agent_run_simulation_interception(test_db):
    ...
    async with session_factory() as db:
            ...
            [
                {
                    ...
                    "tool_input": {"path": "/var/log/app.log", "content": "Log entry"},
                }
            ]
        )
        ...
        stmt = select(AgentRunStep).where(
            AgentRunStep.run_id == run.id, AgentRunStep.step_type == "tool_call"
        )
