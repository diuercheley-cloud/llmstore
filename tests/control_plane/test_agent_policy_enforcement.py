...
async def test_tool_call_enforces_policy(test_db):
    ...
    async with session_factory() as db:
        ...
        stmt = select(AgentPolicyDecision).where(
            AgentPolicyDecision.run_id == run.id, AgentPolicyDecision.action_type == "tool_call"
        )
...
async def test_planner_exec_enforces_policy(test_db):
    ...
    async with session_factory() as db:
        ...
        stmt = select(AgentPolicyDecision).where(
            AgentPolicyDecision.run_id == run.id, AgentPolicyDecision.action_type == "planner_exec"
        )
