...
def test_security_blocks_subprocess():
    ...
    with pytest.raises(
        SecurityException,
        match="Importing 'subprocess' is not allowed|Module 'subprocess' is restricted",
    ):
...
def test_security_blocks_open_passwd():
    ...
    with pytest.raises(
        SecurityException, match="Function 'open' is blocked|Function 'open' is not allowed"
    ):
        validate_generated_code(code)
...
def test_security_blocks_network_by_default():
    ...
    with pytest.raises(
        SecurityException,
        match="Importing 'requests' is not allowed|Module 'requests' is restricted",
    ):
...
def test_high_risk_requires_approval(db_session):
    ...
    policy.check_approval = MagicMock(
        side_effect=lambda version_is_approved, risk_level: (
            version_is_approved if risk_level == "high" else True
        )
    )
