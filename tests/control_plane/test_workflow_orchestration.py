...
async def setup_db():
    async with engine.begin() as conn:
        import app.models.agents.agent_workflows  # noqa
...
async def workflow(db_session):
    workflow = AgentWorkflow(name="test-workflow", version="1.0.0", tenant_id="default")
        ...
        timers = (
            (
                await db_session.execute(
                    select(AgentWorkflowTimer).where(AgentWorkflowTimer.run_id == run.id)
                )
            )
            .scalars()
            .all()
...
async def test_workflow_handoff_and_signal_resume_end_to_end(db_session, workflow):
    with patch.dict(os.environ, {"AGENT_STATEFUL_WORKFLOWS_ENABLED": "true"}):
        ...
        events = (
            (
                await db_session.execute(
                    select(AgentWorkflowEvent).where(AgentWorkflowEvent.run_id == run.id)
                )
            )
            .scalars()
            .all()
