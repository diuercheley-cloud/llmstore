...
from app.models.agents.agents import AgentDefinition, AgentRun, AgentTask
...
async def test_task_engine_real_execution_tool_call(db_session: AsyncSession):
    ...
    run = AgentRun(
        id=uuid.uuid4(), agent_id=agent.id, tenant_id="t1", status="running", input_text="test"
    )
    db_session.add(run)
...
async def test_task_dependency_blocking(db_session: AsyncSession):
    ...
    planner = AgentPlanner(db_session)
    tasks_data = [{"title": "task1", "ref": "t1"}, {"title": "task2", "dependencies": ["t1"]}]
