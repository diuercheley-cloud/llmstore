...
async def test_incident_created_policy_denial_spike():
    async with SessionLocal() as db:
        ...
        inc = await svc.detect_and_create_incident(
            ...
            run.id,
            "policy_denial_spike",
            "Agent repeatedly denied by policy",
        )
        assert inc is not None
...
def test_api_incident_endpoints(client):
    ...
    async def create_incident():
        async with SessionLocal() as db:
            ...
            svc = AgentIncidentService(db)
            inc = await svc.detect_and_create_incident(
                "t1", agent.id, run.id, "test", "Test Incident"
            )
            return str(inc.id)
    ...
    # 3. Resolve
    r = client.post(
        f"/admin/agents/incidents/{incident_id}/resolve", json={"resolution_notes": "Fixed it"}
    )
    assert r.status_code == 200
...
async def test_exports_sanitized():
    async with SessionLocal() as db:
        ...
        event = await svc.record_event(run.id, "tool.called", details={"input": "my secret is 123"})
