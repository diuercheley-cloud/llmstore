...
async def create_mock_agent(
    db: AsyncSession, tenant_id: str, risk_level: str = "low"
) -> AgentDefinition:
...
async def test_disabled_flag_blocks_execution(run_settings):
    ...
    async with SessionLocal() as db:
        ...
        with pytest.raises(
            agent_runtime.RuntimeDisabledError, match="Agent Execution Plane is disabled"
        ):
...
async def test_lease_expiration_and_recovery(run_settings):
    ...
    async with SessionLocal() as db:
        ...
        job = await queue_mgr.dequeue_job(
            "worker-dead", lease_timeout_seconds=-10
        )  # expires in past
...
async def test_retry_limits_to_dlq(run_settings):
    ...
    async with SessionLocal() as db:
        ...
        stmt_dlq = select(AgentExecutionDeadLetter).where(
            AgentExecutionDeadLetter.job_id == job_final.id
        )
...
async def test_admin_api_endpoints(async_client, admin_token_headers, run_settings):
    ...
    resp_workers = await async_client.get(
        "/admin/agents/execution/workers", headers=admin_token_headers
    )
    ...
    resp_cancel = await async_client.post(
        f"/admin/agents/execution/jobs/{job.id}/cancel", headers=admin_token_headers
    )
