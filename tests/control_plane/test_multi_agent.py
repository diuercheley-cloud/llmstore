...
async def test_hierarchical_team_execution(db_session: AsyncSession):
    ...
    await registry.add_member(
        team.id, spec1.id, "specialist", {"task_description": "Review auth flow"}
    )
    await db_session.commit()
    ...
    workspace_rows = (
        (
            await db_session.execute(
                select(AgentSharedWorkspace).where(AgentSharedWorkspace.team_run_id == run.id)
            )
        )
        .scalars()
        .all()
    ...
    delegations = (
        (
            await db_session.execute(
                select(AgentTeamDelegation).where(AgentTeamDelegation.run_id == run.id)
            )
        )
        .scalars()
        .all()
    ...
    messages = (
        (
            await db_session.execute(
                select(AgentTeamMessage).where(AgentTeamMessage.run_id == run.id)
            )
        )
        .scalars()
        .all()
...
async def test_dynamic_team_routing_with_recovery(db_session: AsyncSession):
    ...
    team = await registry.create_team(
        ...
        config={
            "work_items": [
                {"task_id": "auth", "description": "Review auth", "required_capability": "security"}
            ]
        },
    )
    await registry.add_member(
        ...
        failing.id,
        "specialist",
        {"capabilities": ["security"], "priority": 10, "simulate_failure": True},
    )
    await registry.add_member(
        team.id, backup.id, "specialist", {"capabilities": ["security"], "priority": 5}
    )
    await registry.add_member(
        team.id, general.id, "worker", {"capabilities": ["general"], "priority": 1}
    )
    await db_session.commit()
...
async def test_loop_detection(db_session: AsyncSession):
    ...
    del1 = AgentTeamDelegation(
        run_id=run_id, parent_agent_id=a1, child_agent_id=a2, task_description="T1", status="active"
    )
    db_session.add(del1)
    await db_session.commit()
