...
@pytest.fixture
def settings():
    return Settings(jaeger_export_enabled=True, zipkin_export_enabled=True)


def test_jaeger_exporter_setup_no_package():
    with (
        patch("app.services.observability.jaeger_exporter.JaegerExporter", None),
...
def test_zipkin_exporter_setup_no_package():
    with (
        patch("app.services.observability.zipkin_exporter.ZipkinExporter", None),
...
def test_exporters_disabled():
    with (
        patch("app.services.observability.jaeger_exporter.get_settings") as mock_settings_j,
