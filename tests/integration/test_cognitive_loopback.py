...
async def test_feedback_positivo_gera_candidate(session, setup_agent, agent_id, run_id):
    ...
    # Mock run
    run = AgentRun(
        id=run_id, agent_id=agent_id, tenant_id="tenant_a", status="completed", input_text="hello"
    )
    session.add(run)
    await session.commit()
...
@pytest.mark.asyncio
async def test_feedback_negativo_nao_vira_fewshot_automaticamente(
    session, setup_agent, agent_id, run_id
):
    settings = get_settings()
    settings.agent_cognitive_loopback_enabled = True
    settings.agent_feedback_learning_enabled = True
