...
async def test_portal_me_plan_fields(admin_client: httpx.AsyncClient, admin_token_headers):
    ...
    resp = await admin_client.post(
        "/admin/billing/plans", json=plan_payload, headers=admin_token_headers
    )
    ...
    resp = await admin_client.post(
        "/admin/clients",
        json={"name": "portal-client", "billing_plan_id": plan_id},
...
async def test_portal_account_plan_fields(admin_client: httpx.AsyncClient, admin_token_headers):
    ...
    resp = await admin_client.post(
        "/admin/clients",
        json={"name": "account-client", "billing_plan_id": plan_id},
        headers=admin_token_headers,
    )
    client_id = resp.json()["id"]
    resp = await admin_client.post(
