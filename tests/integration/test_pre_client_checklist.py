...
def test_script_help():
    result = subprocess.run(["./scripts/validators/pre-client-checklist-local.sh", "--help"], capture_output=True, text=True)
    assert result.returncode == 0
    assert "Usage:" in result.stdout

def test_script_requires_mode():
    result = subprocess.run(["./scripts/validators/pre-client-checklist-local.sh"], capture_output=True, text=True)
...
def test_script_demo_mode_executes():
    ...
    result = subprocess.run(["./scripts/validators/pre-client-checklist-local.sh", "--demo", "--skip-lmstudio", "--skip-rag", "--skip-tts"], capture_output=True, text=True)
