...
def test_release_script_detects_appliance_mode():
    ...
    with open("scripts/release/release-local-production.sh", "r") as f:
        content = f.read()
...
def test_bundle_script_exclusions():
    ...
    with open("scripts/release/create-release-bundle.sh", "r") as f:
        content = f.read()
...
def test_wizard_sets_appliance_mode():
    with open("scripts/dev/configure-local-wizard.sh", "r") as f:
        content = f.read()
