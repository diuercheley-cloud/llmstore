...
@pytest.mark.asyncio
async def test_create_and_list_model(
...
@pytest.mark.asyncio
async def test_model_enable_disable(
    admin_client: AsyncClient, admin_token_headers, models_dir: Path
):
    (models_dir / "test.gguf").touch()
    ...
    resp = await admin_client.post(
        f"/admin/models/{model_uuid}/enable", headers=admin_token_headers
    )
    ...
    # Disable
    resp = await admin_client.post(
        f"/admin/models/{model_uuid}/disable", headers=admin_token_headers
    )
    assert resp.status_code == 200
...
async def test_remove_model(admin_client: AsyncClient, admin_token_headers, models_dir: Path):
    ...
    resp = await admin_client.request(
        "DELETE", f"/admin/models/{model_uuid}", json=remove_payload, headers=admin_token_headers
    )
    assert resp.status_code == 200
