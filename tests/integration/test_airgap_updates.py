...
def test_airgap_bundle_verification():
    ...
    subprocess.run(
        [
            "python3",
            "scripts/dev/airgap/create_bundle.py",
            "--version",
            "1.2.0",
            "--out",
            bundle_path,
    ...
    # 2. Verify it
    res = subprocess.run(
        ["python3", "scripts/dev/airgap/verify_bundle.py", "--bundle", bundle_path],
        capture_output=True,
        text=True,
    )
...
def test_airgap_dry_run():
    ...
    # Run with dry-run
    res = subprocess.run(
        ["python3", "scripts/dev/airgap/apply_bundle.py", "--bundle", bundle_path, "--dry-run"],
        capture_output=True,
        text=True,
    )
    assert res.returncode == 0
...
def test_airgap_downgrade_protection():
    ...
    res = subprocess.run(
        ["python3", "scripts/dev/airgap/apply_bundle.py", "--bundle", bundle_path],
        capture_output=True,
        text=True,
    )
    assert res.returncode != 0
    ...
    res_force = subprocess.run(
        [
            "python3",
            "scripts/dev/airgap/apply_bundle.py",
            "--bundle",
            bundle_path,
            "--force-downgrade",
        ],
