...
async def test_stage_offline_model_bundle(session: AsyncSession):
    manifest = await sovereign_appliance.create_offline_sync_manifest(session, "import", "models")
...
async def test_generate_offline_audit_package(session: AsyncSession):
    ...
    package = await sovereign_appliance.generate_offline_audit_package(session, start, end)
