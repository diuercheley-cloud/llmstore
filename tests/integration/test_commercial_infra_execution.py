...
async def test_execution_disabled_blocks(base_settings):
    ...
    with patch(
        "app.services.routing.commercial_infra_execution.get_settings", return_value=base_settings
    ):
...
async def test_real_execution_requires_approval_and_leader(base_settings):
    ...
    with patch(
        ...
        approval = CommercialApprovalRecord(
            id=uuid.uuid4(), simulation_id=sim.id, status="approved"
        )
        db.execute.return_value.scalars.return_value.first.return_value = approval

        # 3. Test without leader
        with patch(
            "app.services.routing.commercial_infra_execution.is_current_leader",
            new_callable=AsyncMock,
        ) as mock_is_leader:
            mock_is_leader.return_value = False

            record = await svc.execute_simulation(
                db, sim.id, "mock", dry_run=False, confirm=True, approval_id=approval.id
            )
            assert record.status == "blocked"
        ...
        with patch(
            ...
            with patch(
                "app.services.routing.commercial_infra_execution._active_lease_query",
                new_callable=AsyncMock,
            ) as mock_lease:
...
async def test_kubernetes_adapter_unavailable_without_lib(base_settings):
    ...
    with patch(
        "app.services.routing.infra_adapters.kubernetes_adapter.get_settings",
