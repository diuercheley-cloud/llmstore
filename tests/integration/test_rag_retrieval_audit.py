...
async def test_immutable_retrieval_audit_created(session: AsyncSession):
    ...
    session.add(client)
    await session.flush()
    vault = CommercialRAGVault(client_id=client.id, vault_name="regulated", vault_mode="confidential", encryption_required=True, retrieval_mode="hybrid", immutable_audit_enabled=True)
    session.add(vault)
    await session.flush()
...
async def test_admin_retrieval_audit_auth(session: AsyncSession):
    ...
    async with httpx.AsyncClient(transport=httpx.ASGITransport(app=app), base_url="http://testserver") as client:
        ...
        authorized = await client.get("/admin/rag/retrieval-audit", headers={"X-Admin-Token": "test-admin-token"})
