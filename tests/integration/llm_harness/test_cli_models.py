...
async def test_run_models_command_list(capsys):
    ...
    args.config = None
    with (
        patch("scripts.llm_harness.cli_commands.build_config_overrides", return_value={}),
        patch("scripts.llm_harness.config.get_config") as mock_get_config,
    ):
        mock_cfg = MagicMock()
        mock_cfg.models = {
            "default": "local-qwen",
            "profiles": {"local-qwen": {"provider": "stub", "model": "qwen"}},
            "routing": {"bugfix": "local-qwen"},
        }
...
async def test_run_models_command_test_success(capsys):
    ...
    with (
        ...
        mock_cfg = MagicMock()
        mock_cfg.models = {"profiles": {"local-qwen": {"provider": "stub", "model": "qwen"}}}
