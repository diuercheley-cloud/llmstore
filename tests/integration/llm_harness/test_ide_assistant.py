...
async def test_inline_edit_generates_patch(temp_repo):
    ...
    async with ws as workspace:
        ...
        subprocess.run(
            ["git", "add", "calc.py"], cwd=workspace.path, check=True, capture_output=True
        )
...
async def test_rules_in_harness_rules_enter_prompt(temp_repo):
    ...
    async with ws as workspace:
        ...
        prompt_builder = PromptBuilder(
            policy_summary=policy_engine.describe_for_agent(), rules_context=rules
        )
...
async def test_rules_cannot_free_blocked_command(temp_repo):
    policy_engine = PolicyEngine(config={"shell_policy": {"global_deny": {"commands": ["rm"]}}})
