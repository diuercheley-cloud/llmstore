...
def test_tracer_otel_secret_redaction(monkeypatch):
    ...
    with patch("opentelemetry.trace.get_tracer", return_value=mock_otel_tracer):
        ...
        with tracer.trace_span(
            "test-ctx",
            attributes={
                "bearer_token": "bearer eyJ1c2VyIjoiYW50aWdyYXZpdHkifQ.eyJpYXQiOjE2MjIzNTQ4MDB9.xyz"
            },
        ):
            pass
        args, kwargs = mock_otel_tracer.start_as_current_span.call_args
        assert (
            "[REDACTED]" in kwargs["attributes"]["bearer_token"]
            or "Bearer [REDACTED]" in kwargs["attributes"]["bearer_token"]
        )
...
async def test_coding_loop_span_calls(monkeypatch, temp_repo, fake_provider):
    ...
    with patch("opentelemetry.trace.get_tracer", return_value=mock_otel_tracer):
        ...
        loop = CodingLoop(
            agent_client=client, workspace=temp_repo, use_docker=False, checkpoint_every_step=False
        )
