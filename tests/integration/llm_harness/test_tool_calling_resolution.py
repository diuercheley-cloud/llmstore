...
async def test_resolve_mode_json_forced():
    agent = create_code_agent(
        "local-openai-compatible", {"agent_id": "test", "tool_calling": "json"}
    )
...
async def test_resolve_mode_native_forced_unsupported_fails():
    agent = create_code_agent(
        ...
        {"agent_id": "test", "tool_calling": "native", "base_url": "http://fake/v1"},
    )
...
async def test_resolve_mode_auto_local_defaults_to_json():
    agent = create_code_agent(
        "local-openai-compatible",
        {"agent_id": "test", "tool_calling": "auto", "base_url": "http://fake/v1"},
    )
