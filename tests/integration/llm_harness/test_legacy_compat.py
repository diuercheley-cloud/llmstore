...
def test_legacy_wrapper_delegates_to_main():
    with patch("scripts.llm_harness.legacy_runner.main") as mock_main:
        runpy.run_path("scripts/llm_harness/agent_harness.py", run_name="__main__")
        mock_main.assert_called_once()


def test_legacy_wrapper_help():
    # Test that running python3 scripts/llm_harness/agent_harness.py --help outputs help message
    result = subprocess.run(
        [sys.executable, "scripts/llm_harness/agent_harness.py", "--help"],
        capture_output=True,
        text=True,
        check=True,
    )
...
def test_agent_test_sh_help():
    # Test that running ./scripts/dev/agent-test.sh --help outputs usage
    result = subprocess.run(
        ["./scripts/dev/agent-test.sh", "--help"],
        capture_output=True,
        text=True,
        check=True,
    )
    assert "Usage: ./scripts/dev/agent-test.sh" in result.stdout
