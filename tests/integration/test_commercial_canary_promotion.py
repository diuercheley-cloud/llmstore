...
async def test_promote_canary_step_dry_run(service, mock_db):
    ...
    config = CommercialRoutingConfig(
        ...
        canary_enabled=True,
        canary_percent=5,
        canary_started_at=datetime.now(timezone.utc) - timedelta(hours=2)
    )
    mock_db.get.return_value = config
    service.settings.commercial_canary_auto_promotion_mode = "dry_run"
    
    # Mock SLO pass
    with patch.object(CommercialCanaryPromotionService, "evaluate_canary_slo", return_value={"pass": True, "metrics": {"canary": {"request_count": 150}}}):
