...
@pytest_asyncio.fixture
async def demo_client_id(client, admin_token_headers):
    name = f"wallet-test-{uuid4().hex[:8]}"
    resp = await client.post("/admin/clients", json={"name": name}, headers=admin_token_headers)
    assert resp.status_code == 201, resp.text
    return resp.json()["id"]


@pytest.mark.asyncio
async def test_admin_list_wallets(client, admin_token_headers):
    resp = await client.get("/admin/billing/wallets", headers=admin_token_headers)
...
@pytest.mark.asyncio
async def test_admin_get_wallet_creates_if_not_exists(client, admin_token_headers, demo_client_id):
    resp = await client.get(f"/admin/billing/wallets/{demo_client_id}", headers=admin_token_headers)
    assert resp.status_code == 200
    data = resp.json()
...
@pytest.mark.asyncio
async def test_admin_manual_credit_increases_balance(client, admin_token_headers, demo_client_id):
...
@pytest.mark.asyncio
async def test_admin_adjustment(client, admin_token_headers, demo_client_id):
    await client.post(
        f"/admin/billing/wallets/{demo_client_id}/manual-credit",
...
@pytest.mark.asyncio
async def test_admin_transactions_list(client, admin_token_headers, demo_client_id):
    await client.post(
        f"/admin/billing/wallets/{demo_client_id}/manual-credit",
