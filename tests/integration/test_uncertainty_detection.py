...
async def test_contradicao_reduz_confidence(session, setup_compliance_agent, agent_id, run_id):
    ...
    # High contradiction
    context = {"evidence_score": 0.9, "tool_result_consistency": 0.2, "contradiction_score": 0.8}
    estimation = await estimator.estimate(run_id, context)
...
@pytest.mark.asyncio
async def test_auto_research_disabled_nao_chama_tool(
    session, setup_compliance_agent, agent_id, run_id
):
    # Mock run
    run = AgentRun(id=run_id, agent_id=agent_id, tenant_id="tenant_a", status="running")
    session.add(run)
