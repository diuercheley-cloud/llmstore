...
def test_demo_full_script_help():
    result = subprocess.run(
        ["./scripts/dev/demo-full-local.sh", "--help"], capture_output=True, text=True
    )
...
def test_demo_full_script_no_secrets():
    with open("scripts/dev/demo-full-local.sh") as f:
        content = f.read()
...
def test_validate_demo_client_portal_does_not_reactivate_stack():
    with open("scripts/legacy/validate-demo-client-portal.sh") as f:
        content = f.read()
