...
def test_demo_full_script_exists():
    assert os.path.exists("scripts/dev/demo-full-local.sh")

def test_demo_full_script_is_executable():
    assert os.access("scripts/dev/demo-full-local.sh", os.X_OK)

def test_demo_full_script_help():
    result = subprocess.run(["./scripts/dev/demo-full-local.sh", "--help"], capture_output=True, text=True)
    assert result.returncode == 0
    assert "Usage:" in result.stdout
    assert "--no-build" in result.stdout

def test_demo_full_script_no_secrets():
    with open("scripts/dev/demo-full-local.sh", "r") as f:
        content = f.read()
...
def test_validate_demo_client_portal_does_not_reactivate_stack():
    with open("scripts/legacy/validate-demo-client-portal.sh", "r") as f:
        content = f.read()
