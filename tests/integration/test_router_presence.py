...
def test_required_routers_are_present():
    ...
    for path, method in required_endpoints:
        assert path in routes_map, (
            f"Rota crítica ausente: {path}. Verifique o main.py e a importação dos routers."
        )
        assert method in routes_map[path], (
            f"Método {method} ausente para a rota {path}. Métodos encontrados: {routes_map[path]}"
        )

    # Verifica também se existem endpoints baseados no admin base de forma genérica
    # Pois /admin engloba várias rotas
    admin_routes_present = any(p.startswith("/admin/") and "tests" not in p for p in routes_map)
    assert admin_routes_present, (
        "Nenhuma rota administrativa (/admin/*) encontrada! admin_router não foi incluído?"
    )
