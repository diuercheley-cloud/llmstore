...
async def test_rag_document_count_limit(mock_verify, client_and_session):
    ...
    async with sessionmaker() as session:
        ...
        response = await async_client.post(
            "/v1/rag/files", headers=headers, files={"file": ("second.pdf", b"%PDF-1.4 content")}
        )
...
async def test_rag_feature_block(mock_verify, client_and_session):
    ...
    async with sessionmaker() as session:
        ...
        # Try to upload
        response = await async_client.post(
            "/v1/rag/files", headers=headers, files={"file": ("test.pdf", b"%PDF-1.4 content")}
        )
...
async def test_admin_block_unblock(mock_get_role, client_and_session):
    ...
    async with sessionmaker() as session:
        ...
        # Unblock
        response = await async_client.post(
            f"/admin/tests/rag/clients/{client.id}/unblock", headers=headers
        )
        assert response.status_code == 200
