...
SCRIPT = ROOT / "scripts/legacy/generate-proposal-pdf.sh"
SAMPLE_MD = ROOT / "proposals/TECHNICAL_PROPOSAL_TEMPLATE.md"


class TestGenerateProposalPdfScript:
    def test_script_exists(self):
        assert SCRIPT.exists(), "scripts/legacy/generate-proposal-pdf.sh não encontrado"
    ...
    def test_invalid_option_shows_error(self):
        result = subprocess.run(
            [str(SCRIPT), "--invalid-flag"], capture_output=True, text=True, timeout=30
        )
        assert result.returncode != 0
    ...
    def test_script_does_not_send_to_internet(self):
        ...
        assert (
            ...
            or "http" not in content.replace("localhost", "")
    ...
    def test_generates_pdf_if_tool_available(self):
        ...
        # Check for tools
        has_pandoc = subprocess.run(["which", "pandoc"], capture_output=True).returncode == 0
        has_chrome = subprocess.run(["which", "google-chrome"], capture_output=True).returncode == 0
        if not has_chrome:
            has_chrome = (
                subprocess.run(["which", "chromium-browser"], capture_output=True).returncode == 0
            )
        if not has_chrome:
            has_chrome = subprocess.run(["which", "chromium"], capture_output=True).returncode == 0

        if not has_pandoc and not has_chrome:
            # No tools available - just check that script exits 0
            result = subprocess.run(
                [str(SCRIPT), "--input", str(SAMPLE_MD)], capture_output=True, text=True, timeout=60
            )
            assert result.returncode == 0
            return
    ...
    def test_does_not_commit_pdfs(self):
        ...
        assert (
            ...
            or "artifacts/" in gitignore
        ), ".gitignore deve cobrir PDFs gerados"
