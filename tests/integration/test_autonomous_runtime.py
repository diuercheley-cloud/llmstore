...
async def test_autonomous_cycle_success(mock_registry_yaml, tmp_path):
    config = HarnessConfig(
        agent_registry_file=mock_registry_yaml, checkpoint_dir=str(tmp_path / "cp")
    )
    ...
    result = await runtime.run_autonomous(
        agent_id="maintainer", goal="Maintain codebase", max_steps=5, cooldown=0
    )
...
async def test_autonomous_budget_exceeded(mock_registry_yaml, tmp_path):
    ...
    runtime._decide = AsyncMock(
        return_value={"type": "action", "next_agent": "maintainer", "instruction": "Work"}
    )

    # Mock loop.run to return result with cost
    loop.run = AsyncMock(
        return_value=ExecutionResult(success=True, message="Done", metrics={"estimated_cost": 1.0})
    )

    result = await runtime.run_autonomous(
        agent_id="maintainer", goal="Expensive task", budget=0.5, max_steps=5, cooldown=0
    )
...
async def test_autonomous_pause_resume(mock_registry_yaml, tmp_path):
    ...
    runtime._decide = AsyncMock(
        return_value={"type": "action", "next_agent": "maintainer", "instruction": "Step"}
    )
    ...
    result = await runtime.run_autonomous(
        agent_id="maintainer", goal="Long task", max_steps=5, cooldown=0
    )
