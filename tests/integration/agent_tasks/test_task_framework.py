...
async def test_task_execution_order(session):
    ...
    planner = AgentPlanner(session)
    tasks_data = [{"title": "Step 1", "ref": 0}, {"title": "Step 2", "ref": 1, "dependencies": [0]}]
...
async def test_retry_logic(session):
    ...
    with unittest.mock.patch.object(
        engine, "_execute_with_mode", new_callable=AsyncMock
    ) as mock_execute:
        mock_execute.side_effect = TaskExecutionError(
            "transient_error", "Transient error", retryable=True
        )
