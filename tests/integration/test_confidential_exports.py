...
@pytest.mark.asyncio
async def test_export_redaction(
    session: AsyncSession, encryption_service: TenantEncryptionService, settings: Settings
):
...
@pytest.mark.asyncio
async def test_export_encryption_required(
    session: AsyncSession, encryption_service: TenantEncryptionService, settings: Settings
):
    client_id = uuid.uuid4()
