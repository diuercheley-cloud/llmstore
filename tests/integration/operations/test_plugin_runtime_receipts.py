...
def test_plugin_runtime_receipts_have_placeholders():
    ...
    load_plan = SimpleNamespace(
        client_id="client", id="plan", immutable_hash="c" * 64, load_plan_hash="d" * 64
    )
    ...
    replay = SimpleNamespace(
        client_id="client", id="replay", immutable_hash="f" * 64, replay_hash="g" * 64
    )
    federation = SimpleNamespace(
        client_id="client", id="fed", immutable_hash="h" * 64, compatibility_hash="i" * 64
    )
    assert (
        isinstance(build_abi_contract_receipt(contract)["signature"], str)
        and len(build_abi_contract_receipt(contract)["signature"]) > 0
    )
    ...
    assert (
        build_replay_verification_receipt(replay)["receipt_type"] == "replay_verification_receipt"
    )
    assert (
        build_federation_compatibility_receipt(federation)["receipt_type"]
        == "federation_compatibility_receipt"
    )
