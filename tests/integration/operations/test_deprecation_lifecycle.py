...
def test_deprecation_requires_replacement_when_migration_required():
    ...
    with pytest.raises(ValueError):
        service.propose_deprecation({"id": "c1", "migration_required": True, "replacement_contract": None})


def test_deprecation_enforcement_transitions():
    service = DeprecationLifecycleService()
    result = service.enforce_deprecation({"id": "c1", "migration_required": True, "replacement_contract": "c2"})
