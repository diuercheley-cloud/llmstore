...
def test_hash_utils_are_deterministic():
    ...
    assert compute_attestation_hash({"payload": payload}) == compute_attestation_hash({"payload": {"a": 1, "b": 2}})
    assert compute_bundle_hash({"bundle": ["a", "b"]}) == compute_bundle_hash({"bundle": ["a", "b"]})
