...
def test_operational_readiness_script():
    # Run scripts/dev/operational-readiness-pack.sh using bash
    ...
    res = subprocess.run(
        ["bash", str(script_path)], capture_output=True, text=True, cwd=str(project_dir), env=env
    )
    ...
    recs_file = (
        project_dir / "artifacts" / "operational-readiness" / "latest" / "recommendations.md"
    )
