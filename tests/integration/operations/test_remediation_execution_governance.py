...
class TestRemediationGovernance:
    async def _setup_plan(
        self, session: AsyncSession, client_id: uuid.UUID, requires_approval=True
    ):
    ...
    async def test_execution_with_valid_approval_succeeds(self, session: AsyncSession):
        ...
        execution = await executor.prepare_execution(
            session, plan, steps, requested_by="admin1", dry_run=False
        )
        await session.commit()
        ...
        # Verify step record
        stmt = select(RemediationExecutionStep).where(
            RemediationExecutionStep.execution_id == execution.id
        )
    ...
    async def test_step_failure_persists_status(self, session: AsyncSession):
        ...
        stmt = select(AdminActionLog).where(
            AdminActionLog.action == "ops_remediation_exec:remediation_failed"
        )
