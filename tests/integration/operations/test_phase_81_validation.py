...
MIGRATION_PATH = (
    ...
    / "alembic"
    / "versions"
    / "phase81_reproducible_build_artifact_verification.py"
)


def test_phase_81_validation_script_present():
    content = (ROOT / "scripts" / "validate_phase_81_reproducible_builds.py").read_text(
        encoding="utf-8"
    )
    assert "Phase 81 reproducible build validation passed." in content
    assert "offline-first reproducible build framework" in (
        ROOT / "control_plane" / "app" / "static" / "admin" / "index.html"
