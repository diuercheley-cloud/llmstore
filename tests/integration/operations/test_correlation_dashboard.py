...
class TestCorrelationDashboard:
    async def test_portal_list_correlations(
        self, admin_client: AsyncClient, client_token_headers: dict, portal_client_data: dict
    ):
    ...
    async def test_portal_trust_graph_summary(
        self, admin_client: AsyncClient, client_token_headers: dict
    ):
        r = await admin_client.get(
            "/portal/operations/correlations/trust-graph", headers=client_token_headers
        )
        assert r.status_code == 200
        data = r.json()
    ...
    async def test_admin_correlation_dashboard_data(
        self, admin_client: AsyncClient, admin_token_headers: dict
    ):
        # Verify admin can still list correlations with client_id
        client_id = str(uuid.uuid4())
        r = await admin_client.get(
            f"/admin/operations/correlations/?client_id={client_id}", headers=admin_token_headers
        )
        assert r.status_code == 200

    async def test_security_portal_isolation(
        self, admin_client: AsyncClient, client_token_headers: dict
    ):
