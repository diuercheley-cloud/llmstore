...
def mock_backends():
    b1 = InferenceBackend(
        name="vllm-primary", provider="vllm", backend_url="http://vllm:8000", is_active=True
    )
    b2 = InferenceBackend(
        name="tgi-secondary", provider="tgi", backend_url="http://tgi:8080", is_active=True
    )
    return [b1, b2]
...
async def test_admin_api_list_backends(mock_session, mock_backends):
    ...
    with (
        patch.object(VLLMBackend, "health", return_value=True),
