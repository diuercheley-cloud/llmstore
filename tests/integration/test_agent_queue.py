...
async def test_concurrent_pickup(session: AsyncSession):
    ...
    job = await queue_mgr.enqueue_job(
        agent_run_id=uuid.uuid4(), agent_id=uuid.uuid4(), tenant_id="test-tenant"
    )
...
async def test_lease_expiration(session: AsyncSession):
    ...
    leased_job = await queue_mgr.dequeue_job(
        "worker-1", lease_timeout_seconds=-10
    )  # Immediately expired
    await session.commit()
...
async def test_max_attempts_to_dlq(session: AsyncSession):
    ...
    job = await queue_mgr.enqueue_job(
        agent_run_id=uuid.uuid4(), agent_id=uuid.uuid4(), tenant_id="test-tenant", max_attempts=1
    )
...
async def test_idempotency_key(session: AsyncSession):
    ...
    stmt = select(func.count(AgentExecutionJob.id)).where(
        AgentExecutionJob.idempotency_key == idem_key
    )
    res = await session.execute(stmt)
    assert res.scalar() == 1
...
async def test_deduplication_key(session: AsyncSession):
    ...
    job2 = await queue_mgr.enqueue_job(
        uuid.uuid4(), agent_id, "tenant-1", deduplication_key=dedup_key
    )
    await session.commit()
    ...
    job3 = await queue_mgr.enqueue_job(
        uuid.uuid4(), agent_id, "tenant-1", deduplication_key=dedup_key
    )
    await session.commit()
