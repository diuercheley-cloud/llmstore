...
def run_script(script_name, args=None, input_text=None):
    ...
    return subprocess.run(cmd, capture_output=True, text=True, input=input_text, cwd=str(ROOT_DIR))
...
def test_rollback_dry_run():
    result = run_script(
        ...
        [
            ...
            "v1.0.0",
            "--backup-id",
            str(ROOT_DIR / "artifacts" / "backups-local" / "mock-test"),
            "--dry-run",
            "--yes",
        ],
    )
...
def test_rollback_strong_confirmation():
    ...
    result = run_script(
        "rollback-local.sh",
        ["--to-version", "v1.0.0", "--backup-id", str(mock_backup), "--skip-git-check"],
