...
def test_report_md_has_status_heading():
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        assert (
            "MEETING_READY" in content or "READY_WITH_WARNINGS" in content or "NOT_READY" in content
        )
...
def test_report_md_has_emergency_commands():
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        assert (
            "make logs" in content
            or "./scripts/deploy/up.sh" in content
            or "./scripts/deploy/down.sh" in content
        )
...
def test_report_md_has_visual_checklist():
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        assert checkbox_count >= 10, f"Only {checkbox_count} checkboxes found (expected >= 10)"
