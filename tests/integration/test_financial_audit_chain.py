...
async def test_audit_hash_chain(session):
    ...
    event1 = await FinancialAuditTrailService.create_audit_event(
        session, event_type="qos_billing_created", amount_brl=Decimal("10.00")
    )
    await session.flush()

    # Create second event
    event2 = await FinancialAuditTrailService.create_audit_event(
        session, event_type="qos_wallet_debited", amount_brl=Decimal("10.00")
    )
...
async def test_audit_chain_tamper_detection(session):
    ...
    await FinancialAuditTrailService.create_audit_event(
        session, event_type="event_1", amount_brl=Decimal("1.00")
    )
    await session.flush()

    event2 = await FinancialAuditTrailService.create_audit_event(
        session, event_type="event_2", amount_brl=Decimal("2.00")
    )
    await session.commit()
