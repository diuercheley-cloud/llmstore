...
WIZARD_PATH = os.path.abspath("scripts/dev/configure-local-wizard.sh")
...
def test_wizard_help(temp_workspace):
    result = subprocess.run(
        ["bash", "scripts/dev/configure-local-wizard.sh", "--help"], capture_output=True, text=True
    )
...
def test_wizard_non_interactive_defaults(temp_workspace):
    result = subprocess.run(
        ["bash", "scripts/dev/configure-local-wizard.sh", "--non-interactive", "--yes"],
        capture_output=True,
        text=True,
    )
    assert result.returncode == 0
