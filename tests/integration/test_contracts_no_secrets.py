...
def test_template_contains_no_secrets(tpl):
    ...
    for i, pattern in enumerate(SECRET_PATTERNS):
        ...
        if matches:
            pytest.fail(f"{tpl} contains potential secret matching pattern {i}: {matches[0]}")
...
def test_artifacts_contracts_not_tracked():
    result = subprocess.check_output(
        ["git", "ls-files", "artifacts/contracts/"], stderr=subprocess.DEVNULL, text=True
    )
    assert result.strip() == "", (
        "artifacts/contracts/ should NOT be tracked by Git, but found: " + result.strip()
    )
