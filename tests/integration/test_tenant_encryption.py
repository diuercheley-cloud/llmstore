...
@pytest.mark.asyncio
async def test_encrypt_decrypt_cycle(session: AsyncSession, encryption_service: TenantEncryptionService):
...
async def test_classification(encryption_service: TenantEncryptionService):
    ...
    assert await encryption_service.classify_sensitive_payload("my email is test@example.com") == "confidential"
    assert await encryption_service.classify_sensitive_payload("the api_key is 12345") == "restricted"

@pytest.mark.asyncio
async def test_confidential_export_blocked(session: AsyncSession, encryption_service: TenantEncryptionService):
    client_id = uuid.uuid4()
    data = {
        "name": "John Doe",
        "api_key": "sk-12345",
