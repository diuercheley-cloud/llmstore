...
async def test_prompt_template_rendering(session: AsyncSession):
    ...
    rendered, hashes = renderer.resolve_instructions(
        template_str=template, instructions=None, variables=variables
    )
...
async def test_required_variable_missing(session: AsyncSession):
    ...
    try:
        await renderer.render(template_str=template, variables={}, declared_vars=declared_vars)
...
async def test_secret_blocking(session: AsyncSession):
    ...
    try:
        renderer.render(
            template_str="Secret: {{ key }}", variables={"key": "sk-12345678901234567890"}
        )
...
async def test_agent_executor_integration(session: AsyncSession):
    ...
    run = AgentRun(
        agent_id=agent_def.id, tenant_id=tenant_id, input_text="User data", status="running"
    )
    session.add(run)
    await session.flush()
