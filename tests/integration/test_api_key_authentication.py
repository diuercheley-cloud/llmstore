...
async def test_auth_expired_key(admin_client: AsyncClient, admin_token_headers, test_client_id):
    # Expired 1 hour ago
    expired_dt = (datetime.now(UTC) - timedelta(hours=1)).isoformat()
    create_resp = await admin_client.post(
        "/admin/api-keys",
        headers=admin_token_headers,
        json={"client_id": test_client_id, "name": "Expired Key", "expires_at": expired_dt},
    )
    api_key = create_resp.json()["api_key"]
...
@pytest.mark.asyncio
async def test_auth_last_used_at_updates(
