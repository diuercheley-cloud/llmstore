...
async def sample_client(session):
    client = Client(id=uuid.uuid4(), name="Test QoS Client", billing_status="active")
    session.add(client)
...
@pytest.mark.asyncio
async def test_debit_wallet_blocked_without_opt_in(
    session, sample_client, sample_chargeback, settings
):
    settings.commercial_qos_billing_enabled = True
    settings.commercial_qos_billing_mode = "report_only"
