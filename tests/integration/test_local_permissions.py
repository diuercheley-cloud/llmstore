...
def test_fix_permissions_help():
    result = subprocess.run(["bash", "scripts/dev/fix-local-permissions.sh", "--help"], capture_output=True, text=True)
...
def test_fix_permissions_dry_run():
    ...
    result = subprocess.run(["bash", "scripts/dev/fix-local-permissions.sh", "--dry-run"], capture_output=True, text=True)
    assert result.returncode == 0
    assert "[DRY-RUN]" in result.stdout

def test_no_dangerous_chmod_recursive():
    with open("scripts/dev/fix-local-permissions.sh", "r") as f:
        content = f.read()
...
@pytest.mark.parametrize("script", ["scripts/dev/fix-local-permissions.sh", "scripts/validators/validate-local-permissions.sh"])
