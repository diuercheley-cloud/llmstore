...
async def mock_publisher(session):
    pub = MarketplacePublisher(tenant_id="pub-tenant-1", name="Trusted Publisher", is_verified=True)
    session.add(pub)
...
async def test_marketplace_api_flow(admin_client, session, mock_publisher):
    ...
    resp_dry = await admin_client.post(
        "/api/admin/marketplace/install/dry-run", json=payload, headers=headers
    )
    ...
    resp_danger = await admin_client.post(
        "/api/admin/marketplace/install/dry-run", json=payload_danger, headers=headers
    )
