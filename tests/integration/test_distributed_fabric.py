...
async def test_job_placement_local_first(session):
    ...
    # create remote node
    remote_node = RuntimeNode(
        name="remote-node", base_url="http://remote-node", node_type="remote", status="ready"
    )
    session.add(remote_node)

    # create local node
    local_node = RuntimeNode(
        name="local-node", base_url="http://local-node", node_type="local", status="ready"
    )
...
async def test_unhealthy_node_no_job(session):
    ...
    unhealthy_node = RuntimeNode(
        name="unhealthy", base_url="http://unhealthy", node_type="local", status="offline"
    )
