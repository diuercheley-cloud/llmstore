...
async def test_gateway_llm_provider_success(session: AsyncSession, monkeypatch):
    ...
    model = ModelRegistry(
        model_id="gpt-4", model_file="gpt4.bin", provider="openai", status="active"
    )
    session.add(model)
    ...
    run = AgentRun(agent_id=agent_def.id, tenant_id=tenant_id, input_text="Hello", status="running")
    session.add(run)
    ...
    with (
        patch("app.services.agents.agent_llm_provider.ensure_quota", AsyncMock()),
