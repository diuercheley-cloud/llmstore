...
async def test_lmstudio_routing_fallback(admin_client: AsyncClient, admin_token_headers):
    ...
    with patch("app.services.queue_manager.QueueManager.slot") as mock_slot:
        ...
        resp = await admin_client.post(
            "/admin/backends", json=backend_payload, headers=admin_token_headers
        )
        ...
        resp = await admin_client.post(
            "/admin/models", json=model_payload, headers=admin_token_headers
        )
        assert resp.status_code == 201
        ...
        resp = await admin_client.post(
            "/v1/chat/completions", json=chat_payload, headers=client_headers
        )
