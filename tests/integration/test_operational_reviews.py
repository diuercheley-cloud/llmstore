...
async def test_quarterly_review_auto_generation_and_overdue_detection(session):
    ...
    reviews = (
        (
            await session.execute(
                select(CommercialOperationalReview).where(
                    CommercialOperationalReview.control_id == control.id
                )
            )
        )
        .scalars()
    ...
    overdue = await detect_overdue_reviews(
        session, control_id=control.id, as_of=reviews[0].created_at + timedelta(days=120)
    )
    await session.commit()
