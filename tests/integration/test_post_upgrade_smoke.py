...
def test_smoke_run_help():
    result = subprocess.run(["./scripts/validators/post-upgrade-smoke-local.sh", "--help"], capture_output=True, text=True)
...
def test_smoke_json_output():
    ...
    result = subprocess.run(["./scripts/validators/post-upgrade-smoke-local.sh", "--json", "--skip-rag", "--skip-tts"], capture_output=True, text=True)
