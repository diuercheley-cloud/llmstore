...
async def test_commercial_routing_simulate_endpoint(admin_client: AsyncClient, admin_token_headers):
    ...
    response = await admin_client.post(
        "/admin/routing/commercial/simulate", json=payload, headers=admin_token_headers
    )
...
@pytest.mark.asyncio
async def test_commercial_profit_basic_plan_prioritizes_local(
    monkeypatch, admin_client: AsyncClient, admin_token_headers
):
...
@pytest.mark.asyncio
async def test_commercial_profit_high_margin_selection(
    monkeypatch, admin_client: AsyncClient, admin_token_headers
):
...
@pytest.mark.asyncio
async def test_commercial_profit_kill_switch_blocks_cloud(
    monkeypatch, admin_client: AsyncClient, admin_token_headers
):
...
@pytest.mark.asyncio
async def test_commercial_profit_insufficient_balance(
    monkeypatch, admin_client: AsyncClient, admin_token_headers
):
...
@pytest.mark.asyncio
async def test_commercial_profit_negative_margin(
    monkeypatch, admin_client: AsyncClient, admin_token_headers
):
