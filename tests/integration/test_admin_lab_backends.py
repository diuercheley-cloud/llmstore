...
async def test_create_and_patch_backend(admin_client: AsyncClient, admin_token_headers):
    ...
    resp = await admin_client.patch(
        f"/admin/backends/{backend_id}", json=patch_payload, headers=admin_token_headers
    )
...
async def test_backend_health_and_logs_endpoints(admin_client: AsyncClient, admin_token_headers):
    ...
    resp = await admin_client.get(
        f"/admin/backends/{backend_id}/health", headers=admin_token_headers
    )
    assert resp.status_code == 200
    ...
    assert (
        "service_name" not in resp.json()
        or resp.json()["detail"] == "backend is not mapped to a compose service"
    )
