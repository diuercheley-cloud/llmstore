...
async def test_predictive_aiops_cycle(session: AsyncSession):
    ...
    health = CommercialRuntimeFabricHealth(
        id=str(uuid.uuid4()), node_id=node_id, status="degraded", metrics={"cpu": 95, "mem": 80}
    )
    session.add(health)
    ...
    stmt = select(CommercialAIOpsRecommendation).filter(
        CommercialAIOpsRecommendation.client_id == "test-client"
    )
