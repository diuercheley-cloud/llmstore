...
def test_validate_script_exists_and_executable():
    script_path = Path("scripts/validators/validate-release-artifacts-security.sh")
...
def test_redaction_logic_mock(tmp_path: Path):
    ...
    try:
        ...
        result = subprocess.run(
            ["./scripts/validators/validate-release-artifacts-security.sh", "--release-dir", str(dummy_release_dir)],
            capture_output=True,
            text=True
        )
