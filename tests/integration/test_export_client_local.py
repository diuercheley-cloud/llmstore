...
def test_export_script_exists():
    assert os.path.isfile("scripts/dev/export-client-local.sh")
    assert os.access("scripts/dev/export-client-local.sh", os.X_OK)
...
def test_export_dry_run(admin_token):
    ...
    result = subprocess.run(
        ["./scripts/dev/export-client-local.sh", "--client-id", random_id, "--dry-run"],
        capture_output=True,
        text=True,
...
def test_validate_script_runs(admin_token):
    ...
    result = subprocess.run(
        ["./scripts/legacy/validate-export-client-local.sh"],
        capture_output=True,
        text=True,
        env={**os.environ, "ADMIN_TOKEN": admin_token},
    )
