...
def test_local_production_summary_structure():
    ...
    subdirs = sorted(
        [d for d in artifact_dir.iterdir() if d.is_dir()], key=os.path.getmtime, reverse=True
    )
    if not subdirs:
        return
    ...
    for candidate in subdirs:
        ...
        if not candidate_summary.exists():
            continue
        try:
            with open(candidate_summary) as f:
    ...
    for keyword in secrets_keywords:
        assert (
            keyword not in content.upper()
            or "API_KEY" in content.upper()
            or "KEY" in content.upper()
        )
