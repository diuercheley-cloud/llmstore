...
async def test_admin_readiness_latest_api(admin_client: AsyncClient):
    ...
    response = await admin_client.get("/admin/readiness/latest", headers={"X-Admin-Token": token})
...
async def test_admin_security_latest_api(admin_client: AsyncClient):
    token = os.environ.get("ADMIN_TOKEN", "test-admin-token")
    response = await admin_client.get("/admin/security/latest", headers={"X-Admin-Token": token})
    assert response.status_code == 200
    data = response.json()
    if data.get("status") == "not_generated":
...
async def test_admin_runtime_summary_api(admin_client: AsyncClient):
    token = os.environ.get("ADMIN_TOKEN", "test-admin-token")
    response = await admin_client.get("/admin/runtime/summary", headers={"X-Admin-Token": token})
    assert response.status_code == 200
    data = response.json()
