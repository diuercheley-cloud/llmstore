...
from app.models.commercial.commercial_merkle_timelines import CommercialMerkleTimeline
...
async def test_witness_registration(session: AsyncSession):
    witness = await witness_federation.register_witness(
        session, name="Test Witness", witness_type="external", trust_level="high"
    )
...
async def test_witness_signature_and_quorum(session: AsyncSession):
    # 1. Create a witness
    witness = await witness_federation.register_witness(
        session, name="Auditor A", witness_type="external"
    )
