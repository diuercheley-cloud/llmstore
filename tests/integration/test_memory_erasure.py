...
async def test_erasure_remove_memoria_vetorial(session):
    ...
    item = AgentMemoryItem(tenant_id="t1", agent_id=a_id, content_hash="hash1", raw_content="test", memory_type="long_term", retention_until=now)
    session.add(item)
...
async def test_tombstone_preserva_audit_sem_conteudo_bruto(session):
    ...
    from datetime import datetime, timezone
    now = datetime.now(timezone.utc)
    item = AgentMemoryItem(tenant_id="t2", agent_id=a_id, content_hash="hash2", raw_content="test2", memory_type="long_term", retention_until=now)
    session.add(item)
    await session.commit()
...
async def test_tenant_a_nao_apaga_dados_de_tenant_b(session):
    ...
    from datetime import datetime, timezone
    now = datetime.now(timezone.utc)
    item_a = AgentMemoryItem(tenant_id="tA", agent_id=a_id, content_hash="hashA", raw_content="testA", memory_type="long_term", retention_until=now)
    item_b = AgentMemoryItem(tenant_id="tB", agent_id=a_id, content_hash="hashB", raw_content="testB", memory_type="long_term", retention_until=now)
