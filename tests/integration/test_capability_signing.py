...
async def test_pacote_interno_unsigned_bloqueia_em_production(session, monkeypatch):
    ...
    monkeypatch.setattr(
        service.settings,
        "agent_internal_bundle_signature_required_in_production",
        True,
        raising=False,
    )
...
async def test_assinatura_invalida_bloqueia(session):
    ...
    version = AgentBundleVersion(
        id=v_id, version="1.0", checksum_sha256=checksum, entry_id=uuid.uuid4(), manifest_json="{}"
    )
    sig = AgentBundleSignature(
        version_id=v_id,
        signature_type="ed25519",
        signature_value="invalid_sig",
        public_key_id="k1",
        signed_by="test",
    )
...
async def test_trust_report_gerado(session):
    ...
    sig = AgentBundleSignature(
        version_id=v_id,
        signature_type="ed25519",
        signature_value=f"sig:{checksum}:k1",
        public_key_id="k1",
        signed_by="test",
    )
    session.add_all([version, sig])
...
async def test_capability_sem_manifest_falha(session):
    ...
    version = AgentBundleVersion(
        id=v_id, version="1.0", checksum_sha256="", entry_id=uuid.uuid4(), manifest_json="{}"
    )
    session.add(version)
    await session.commit()
