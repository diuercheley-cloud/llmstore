...
async def test_agent_session_lifecycle(session: AsyncSession):
    ...
    session_obj = await svc.create_session(
        tenant_id=tenant_id, agent_id=agent_id, title="Test Session"
    )
    ...
    msg = await thread_svc.add_message(
        session_id=session_obj.id, role="user", content="Hello Agent"
    )
...
async def test_session_redaction(session: AsyncSession):
    ...
    await thread_svc.add_message(
        session_obj.id, "user", "My email is test@example.com and key is sk-12345678901234567890"
    )
