...
async def abuse_setup(abuse_auth_env):
    ...
    async with sessionmaker() as session:
        ...
        active_client = Client(
            name="active-client", billing_plan_id=plan.id, billing_status="active"
        )
        suspended_client = Client(
            name="suspended-client", billing_plan_id=plan.id, billing_status="suspended"
        )
        blocked_client = Client(
            name="blocked-client", billing_plan_id=plan.id, billing_status="active", is_blocked=True
        )
...
async def test_revoked_api_key_returns_401(mock_verify, abuse_setup):
    ac = abuse_setup["ac"]
    resp = await ac.get(
        "/v1/models", headers={"Authorization": f"Bearer {abuse_setup['revoked_key']}"}
    )
    assert resp.status_code == 401
...
async def test_suspended_client_returns_402(mock_verify, abuse_setup):
    ac = abuse_setup["ac"]
    resp = await ac.get(
        "/v1/models", headers={"Authorization": f"Bearer {abuse_setup['suspended_key']}"}
    )
...
async def test_blocked_client_returns_403(mock_verify, abuse_setup):
    ac = abuse_setup["ac"]
    resp = await ac.get(
        "/v1/models", headers={"Authorization": f"Bearer {abuse_setup['blocked_key']}"}
    )
...
async def test_light_flood_triggers_rate_limit(mock_verify, abuse_auth_env):
    ...
    async with sessionmaker() as session:
        ...
        backend = InferenceBackend(
            name="flood-backend", provider="test", backend_url="http://test", is_active=True
        )
        session.add(backend)
        await session.flush()
