...
async def test_multitenant_export_isolation(admin_client: AsyncClient, admin_token_headers):
    ...
    resp_a = await admin_client.post(
        "/admin/clients", json={"name": "Client A"}, headers=admin_token_headers
    )
    client_a_id = resp_a.json()["id"]

    # Create Client B
    resp_b = await admin_client.post(
        "/admin/clients", json={"name": "Client B"}, headers=admin_token_headers
    )
    client_b_id = resp_b.json()["id"]

    # Export A
    export_a = await admin_client.get(
        f"/admin/clients/export-data?client_id={client_a_id}", headers=admin_token_headers
    )
    ...
    await admin_client.post(
        f"/admin/clients/{client_a_id}/purge",
        json={"delete_usage": True},
        headers=admin_token_headers,
    )
    await admin_client.post(
        f"/admin/clients/{client_b_id}/purge",
        json={"delete_usage": True},
        headers=admin_token_headers,
    )
