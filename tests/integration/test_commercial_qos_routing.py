...
def test_evaluate_route_against_qos_free_blocks_cloud():
    tier = CommercialQoSTier(
        name="Free", allow_cloud=False, min_margin_percent=20.0, max_cost_per_request_brl=0.01
    )
...
def test_evaluate_route_against_qos_premium_allows_cloud():
    tier = CommercialQoSTier(
        name="Premium", allow_cloud=True, min_margin_percent=5.0, max_cost_per_request_brl=1.0
    )
...
def test_evaluate_route_against_qos_latency_violation():
    tier = CommercialQoSTier(name="Enterprise", max_p95_latency_ms=400)
