...
async def test_batch_tenant_isolation(batches_client, session):
    ...
    response = await batches_client.post(
        "/v1/batches", json=batch_data, headers={"X-Tenant-ID": "tenant-1"}
    )
    batch_id = response.json()["id"]

    # Try to access from tenant 2
    response = await batches_client.get(
        f"/v1/batches/{batch_id}", headers={"X-Tenant-ID": "tenant-2"}
    )
