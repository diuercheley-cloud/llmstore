...
async def test_max_tokens_is_capped_force_512():
    ...
    _, capped_tokens, metrics = await cm.manage(
        [{"role": "user", "content": "test message"}], requested_max_tokens=32000
    )
...
async def test_max_tokens_default_256():
    ...
    _, capped_tokens, _ = await cm.manage(
        [{"role": "user", "content": "test message"}], requested_max_tokens=None
    )
