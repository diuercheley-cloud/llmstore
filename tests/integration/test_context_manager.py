...
@pytest.mark.asyncio
async def test_context_manager_keeps_last_user_message():
    cm = ContextManager()
    messages = [
    ...
    processed, _, metrics = await cm.manage(messages)
...
async def test_context_manager_limits_history():
    ...
    processed, _, _ = await cm.manage(messages)
...
@pytest.mark.asyncio
async def test_context_manager_simple_input_mode():
    cm = ContextManager()
    messages = [
...
@pytest.mark.asyncio
async def test_max_tokens_is_capped_force_512():
    cm = ContextManager()
    
    _, capped_tokens, metrics = await cm.manage([{"role": "user", "content": "test message"}], requested_max_tokens=32000)
    
    assert capped_tokens == 512
    assert metrics["truncated"] is True

@pytest.mark.asyncio
async def test_max_tokens_default_256():
    cm = ContextManager()
    
    _, capped_tokens, _ = await cm.manage([{"role": "user", "content": "test message"}], requested_max_tokens=None)
    
    assert capped_tokens == 256

@pytest.mark.asyncio
async def test_system_prompt_truncation_stricter():
    cm = ContextManager()
...
@pytest.mark.asyncio
async def test_empty_and_duplicate_short_messages():
    cm = ContextManager()
    messages = [
        {"role": "system", "content": "sys"},
        {"role": "user", "content": ""},
...
@pytest.mark.asyncio
async def test_context_manager_token_limit():
    cm = ContextManager()
