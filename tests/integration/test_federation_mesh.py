import pytest
from app.models.core.federation_mesh import ClusterNode, SyncCommit, ConflictRecord
...
async def test_mesh_conflict_detection(session):
    ...
    local_child = SyncCommit(hash="local-child", parent_hash="parent-123", author_node_id="node-a", payload={"x": 1}, logical_clock=6)
...
async def test_mesh_api_endpoints(admin_client, session):
    ...
    resp_imp = await admin_client.post("/api/admin/federation/sync/import", json={"schema_version": "0.0.1"}, headers=headers)
