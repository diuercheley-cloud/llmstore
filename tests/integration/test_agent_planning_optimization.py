...
async def test_step_cache_hit_avoids_llm_call(session, setup_agent):
    ...
    input_data = {
        "instructions": setup_agent.instructions,
        "input_text": run.input_text,
        "steps": 0,
    }
    output_data = {"type": "final", "content": "Cached Answer"}
    await cache.set_cached_step(
        setup_agent.id, "test-tenant", "model_call", input_data, output_data, model_version="gpt-4o"
    )
    await session.commit()
...
async def test_side_effect_step_not_cached(session, setup_agent):
    ...
    await cache.set_cached_step(
        setup_agent.id, "test-tenant", "model_call", input_data, output_data
    )
    await session.commit()
...
async def test_cache_expiration(session, setup_agent):
    ...
    await cache.set_cached_step(
        setup_agent.id, "test-tenant", "model_call", input_data, output_data, ttl_hours=-1
    )
    await session.commit()
...
async def test_low_budget_chooses_cheap_plan(session, setup_agent):
    ...
    # Create two plans
    plan1 = AgentPlan(
        id=uuid.uuid4(), agent_run_id=uuid.uuid4(), goal_hash="h1", status="draft"
    )  # Cheap
    plan2 = AgentPlan(
        id=uuid.uuid4(), agent_run_id=uuid.uuid4(), goal_hash="h2", status="draft"
    )  # Expensive
    ...
    # Cheap tasks
    session.add(
        AgentTask(plan_id=plan1.id, title="Quick", task_type="model_call", description_hash="d1")
    )
