...
async def test_feature_gates_rag(admin_client: httpx.AsyncClient, admin_token_headers):
    ...
    client_payload = {"name": "no-rag-client", "billing_plan_id": plan_id}
    resp = await admin_client.post(
    ...
    key_payload = {"client_id": client_data["id"], "name": "test-key"}
...
async def test_feature_gates_responses(admin_client: httpx.AsyncClient, admin_token_headers):
    ...
    resp = await admin_client.post(
        "/admin/billing/plans", json=no_resp_plan, headers=admin_token_headers
    )
    plan_id = resp.json()["id"]
    ...
    resp = await admin_client.post(
        "/admin/clients",
        json={"name": "no-resp-client", "billing_plan_id": plan_id},
...
async def test_request_day_limit(admin_client: httpx.AsyncClient, admin_token_headers):
    ...
    resp = await admin_client.post(
        "/admin/billing/plans", json=limit_plan, headers=admin_token_headers
    )
    plan_id = resp.json()["id"]
    ...
    resp = await admin_client.post(
        "/admin/clients",
        json={"name": "limit-client", "billing_plan_id": plan_id},
        headers=admin_token_headers,
    )
    client_id = resp.json()["id"]
    resp = await admin_client.post(
    ...
    headers = {"Authorization": f"Bearer {api_key}"}
    chat_payload = {"model": "default", "messages": [{"role": "user", "content": "hi"}]}
