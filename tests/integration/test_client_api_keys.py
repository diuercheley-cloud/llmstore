...
async def test_manage_api_keys(admin_client: AsyncClient, portal_client_data):
    ...
    # List
    list_resp = await admin_client.get("/portal/api-keys", headers={"Authorization": f"Bearer {client_api_key}"})
    ...
    # Revoke
    revoke_resp = await admin_client.delete(f"/portal/api-keys/{new_key_id}", headers={"Authorization": f"Bearer {client_api_key}"})
    assert revoke_resp.status_code == 200

    # Verify
    me_resp_revoked = await admin_client.get("/portal/me", headers={"Authorization": f"Bearer {new_key}"})
