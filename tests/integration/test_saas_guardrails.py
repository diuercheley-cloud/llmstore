...
async def test_is_cloud_blocked_by_guardrails_global_limit():
    ...
    with patch("app.services.billing.guardrails.settings") as mock_settings:
        ...
        with patch(
            "app.services.billing.guardrails.get_global_provider_cost_today", return_value=150.0
        ):
            blocked, reason = await is_cloud_blocked_by_guardrails(session, client_id)
...
async def test_is_cloud_blocked_by_guardrails_client_limit():
    ...
    with patch("app.services.billing.guardrails.settings") as mock_settings:
        ...
        with patch(
            "app.services.billing.guardrails.get_global_provider_cost_today", return_value=50.0
        ):
            with patch(
                "app.services.billing.guardrails.get_client_provider_cost_today", return_value=15.0
            ):
                blocked, reason = await is_cloud_blocked_by_guardrails(session, client_id)
                assert blocked is True
