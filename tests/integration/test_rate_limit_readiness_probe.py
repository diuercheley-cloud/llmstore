...
async def test_rate_limit_readiness_probe_triggers_429(isolated_db_url, fake_redis):
    ...
    mock_result.response.body = json_lib.dumps(
    ...
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://testserver") as ac:
        ...
        async with testing_session_local() as session:
            ...
            client = Client(
                name="test-readiness-client", billing_plan_id=plan_id, billing_status="active"
            )
            session.add(client)
            await session.commit()
            ...
            backend = InferenceBackend(
                name="test-backend", provider="mock", backend_url="http://test", is_active=True
            )
            session.add(backend)
            await session.commit()
