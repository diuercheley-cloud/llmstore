...
def test_safe_release_passes(temp_release_dir):
    ...
    (temp_release_dir / "summary.md").write_text(
...
def test_detects_sk_token(temp_release_dir):
    ...
    (temp_release_dir / "summary.json").write_text(
        '{"api_key": "sk-abc123abc123abc123abc123abc123"}'
    )
...
def test_detects_bearer_token(temp_release_dir):
    ...
    (temp_release_dir / "manifest.json").write_text(
...
def test_unallowed_filename_in_checksum(temp_release_dir):
    ...
    (temp_release_dir / "bundle-checksums.sha256").write_text("a" * 64 + "  some-other-file.txt")
