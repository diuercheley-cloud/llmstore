...
async def test_detects_tenant_isolation_violation(session):
    ...
    source = await graph_service.add_node(session, "workflow", "Tenant A", {"tenant_id": "tenant-a"})
    target = await graph_service.add_node(session, "workflow", "Tenant B", {"tenant_id": "tenant-b"})
