"""Tests for Replicate provider sanitization — API key masking, prompt/response privacy."""
...
REPLICATE_MASK_KEY = "r8_abcdefghijklmnopqrstuvwxyz123456"
REPLICATE_REALISTIC_KEY = "r8_test_real_key_1234567890abcdef"


def test_mask_api_key_method():
    provider = ReplicateProvider()
    masked = provider.mask_api_key(REPLICATE_MASK_KEY)
    assert masked == "r8_a****3456"
...
KEY_LEAK_PATTERNS = [
    re.compile(r"r8_[a-zA-Z0-9][a-zA-Z0-9._-]{20,}"),
    re.compile(r"Bearer r8_[a-zA-Z0-9][a-zA-Z0-9._-]+"),
]


@pytest.mark.asyncio
async def test_no_key_in_health_check_output(monkeypatch):
    monkeypatch.setenv("REPLICATE_API_KEY", REPLICATE_REALISTIC_KEY)
    monkeypatch.setenv("CLOUD_PROVIDERS_ENABLED", "true")
    monkeypatch.setenv("REPLICATE_PROVIDER_ENABLED", "true")
...
def test_estimate_cost_sanitized():
    provider = ReplicateProvider()
    cost = provider.estimate_cost("meta/meta-llama-3-70b", 100, 50)
...
def test_provider_id_and_type():
    provider = ReplicateProvider()
    assert provider.provider_id == "replicate"
    assert provider.provider_type == ProviderType.REPLICATE
