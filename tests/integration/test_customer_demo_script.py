...
def test_script_help():
    ...
    assert "Uso:" in result.stdout or "./scripts/dev/customer-demo-local.sh" in result.stdout


def test_script_accepts_all_flags():
    result = subprocess.run(
        ["bash", SCRIPT_PATH, "--no-build", "--quick"], capture_output=True, text=True
    )
...
def test_script_handles_unknown_flag():
    result = subprocess.run(["bash", SCRIPT_PATH, "--unknown-flag"], capture_output=True, text=True)
    assert result.returncode != 0
