...
@pytest.mark.asyncio
async def test_audit_report_endpoint_exports_json(
    fastapi_app, async_client, session, admin_token_headers
):
    fastapi_app.include_router(compliance_router)
    ...
    response = await async_client.get(
        "/admin/compliance/audit-report?format=json", headers=admin_token_headers
    )
