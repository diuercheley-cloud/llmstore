...
async def test_eval_failure_on_prohibited_behavior(session):
    ...
    case_data = {
        ...
        "assertions": [{"type": "final_answer_contains", "value": "FAIL_ME"}],
    }
    await service.create_case(suite.id, case_data)
...
async def test_production_requires_eval_baseline(session):
    ...
    entry = await create_registry_entry(
        session,
        {"name": "Prod Agent", "owner": "admin", "instructions": "Go to prod", "risk_level": "low"},
    )
    ...
    try:
        ...
        await AgentEvalService(session).create_case(
            suite.id, {"name": "Test", "input_text": "test", "assertions": []}
        )
