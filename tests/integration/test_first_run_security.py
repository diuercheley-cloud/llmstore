...
def test_first_run_no_secrets_in_script():
    with open("scripts/deploy/first-run-local.sh") as f:
        content = f.read()
...
def test_validate_first_run_local_script_dry_run():
    result = subprocess.run(
        ["./scripts/legacy/validate-first-run-local.sh", "--dry-run"],
        capture_output=True,
        text=True,
    )
...
def test_validate_first_run_local_script_execution():
    result = subprocess.run(
        ["./scripts/legacy/validate-first-run-local.sh"], capture_output=True, text=True
    )
    assert result.returncode == 0
