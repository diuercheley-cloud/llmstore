...
async def setup_canary(session, agent_ids):
    ...
    canary = AgentDefinition(id=canary_id, name="Canary", version="2.0", model_id="m1", owner="o1", tenant_id="t1", instructions="i2", status="draft")
...
async def test_comparacao_gera_relatorio(session, setup_canary, agent_ids):
    ...
    record = AgentShadowRun(assignment_id=setup_canary.id, primary_run_id=primary.id, shadow_run_id=shadow.id)
    session.add(record)
    await session.commit()
