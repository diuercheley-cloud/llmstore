...
def test_backup_service_initialization_no_keys(monkeypatch):
    ...
    with pytest.raises(
        ValueError,
        match="BACKUP_ENCRYPTION_KEY and BACKUP_SIGNING_KEY environment variables must be defined",
    ):
        BackupService(None)
...
async def test_backup_service_key_id_warning(monkeypatch):
    ...
    with patch("pathlib.Path.read_bytes", return_value=b"encrypted_data"):
        ...
        with patch("tarfile.open") as mock_tar:
            ...
            assert (
                res.details["key_id_mismatch"]
                == "Expected 'different-key', current is 'current-key'"
            )
...
async def test_backup_restore_compatibility_full_scope(monkeypatch):
    ...
    service.verify_backup = AsyncMock(
        return_value=BackupVerificationResult(
            backup_id="legacy-backup",
            status="valid",
    ...
    service._extract_payload_parts = MagicMock(
        return_value={
