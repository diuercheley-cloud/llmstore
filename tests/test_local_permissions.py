...
def test_scripts_exist():
    ...
    assert os.path.exists("scripts/validate-local-permissions.sh")

def test_fix_permissions_help():
    result = subprocess.run(["bash", "scripts/fix-local-permissions.sh", "--help"], capture_output=True, text=True)
...
def test_fix_permissions_dry_run():
    ...
    result = subprocess.run(["bash", "scripts/fix-local-permissions.sh", "--dry-run"], capture_output=True, text=True)
    assert result.returncode == 0
    assert "[DRY-RUN]" in result.stdout

def test_no_dangerous_chmod_recursive():
    with open("scripts/fix-local-permissions.sh", "r") as f:
        content = f.read()
...
def test_validate_script_exists():
    assert os.access("scripts/validate-local-permissions.sh", os.F_OK)

@pytest.mark.parametrize("script", ["scripts/fix-local-permissions.sh", "scripts/validate-local-permissions.sh"])
