...
async def test_rag_flow(e2e_client, admin_headers):
    ...
    async def create_client(name):
        ...
        assert resp.status_code == 201
        c_id = resp.json()["id"]
        k_resp = await e2e_client.post(
            "/admin/api-keys", json={"client_id": c_id, "name": name}, headers=admin_headers
        )
    ...
    resp = await e2e_client.post(
        f"/v1/rag/documents?collection_id={col_a_id}", headers=headers_a, files=files
    )
    assert resp.status_code == 200
