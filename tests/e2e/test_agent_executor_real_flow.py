...
async def test_agent_executor_real_flow(e2e_client, monkeypatch):
    ...
    settings.agent_executor_mock_mode = (
        True  # Enable mock provider mode explicitly to avoid MockProviderError
    )
    ...
    client_resp = await e2e_client.post(
        "/admin/clients", json={"name": "E2E Agent Test Client"}, headers=admin_headers
    )
    ...
    monkeypatch.setattr(
        "app.services.agents.agent_executor.get_agent_llm_provider", lambda db, proxy=None: mock_llm
    )

    # 11. Start Agent Run via client E2E API
    headers = {"Authorization": f"Bearer {api_key_str}"}
    resp = await e2e_client.post(
        f"/v1/agents/{agent_id}/runs", json={"input_text": "Trigger e2e flow test"}, headers=headers
    )
    ...
    async with SessionLocal() as db:
        ...
        assert run.status == "waiting_approval", (
            f"Expected status 'waiting_approval', got {run.status}. Reason: {run.failure_reason}"
        )
    ...
    trace_resp = await e2e_client.get(
        f"/admin/agents/observability/traces/{run_id}", headers=admin_headers
    )
