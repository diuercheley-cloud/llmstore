...
async def test_multi_agent_e2e_research_code_review_deploy(session):
    ...
    run = AgentRun(id=run_id, agent_id=agent_id, tenant_id=tenant_id, status="running")
    session.add(run)
    ...
    interpreter_res = await interpreter.run_code(
        code="print('Hello Android')", agent_id=agent_id, run_id=run_id, tenant_id=tenant_id
    )
