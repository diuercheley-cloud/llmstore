...
async def test_attestation_plugin_flow(e2e_client, admin_headers):
    ...
    resp = await e2e_client.post(
        "/admin/security/attestation/verify", json=report, headers=admin_headers
    )
    assert resp.status_code == 200
    ...
    client_resp = await e2e_client.post(
        "/admin/clients", json={"name": "Plugin Client"}, headers=admin_headers
    )
    ...
    resp = await e2e_client.post(
        "/admin/operations/plugin-runtime/contracts", json=contract_payload, headers=admin_headers
    )
    assert resp.status_code == 200
