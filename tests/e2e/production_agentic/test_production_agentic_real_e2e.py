...
async def test_all_production_agentic_gates():
    ...
    server_process = subprocess.Popen(
        [sys.executable, "tests/e2e/production_agentic/fake_connector_server.py", str(port)]
    )
    time.sleep(0.5)
    ...
    try:
        ...
        call_req = {
            "method": "tools/call",
            "params": {"name": "fake_tool", "arguments": {"input": "test-data"}},
            "id": 2,
        }
        ...
        if "Fake tool called with: test-data" in call_res.get("result", {}).get("content", [{}])[
            0
        ].get("text", ""):
            mcp_ok = True
    except Exception as e:
