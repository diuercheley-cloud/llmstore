...
async def test_billing_tokenization_flow(e2e_client, admin_headers):
    ...
    client_resp = await e2e_client.post("/admin/clients", json={"name": "Billing Client"}, headers=admin_headers)
    assert client_resp.status_code == 201
    client_id = client_resp.json()["id"]
    
    key_resp = await e2e_client.post("/admin/api-keys", json={"client_id": client_id, "name": "Billing Key"}, headers=admin_headers)
    ...
    gen_resp = await e2e_client.post("/admin/billing/invoices/generate", json=gen_payload, headers=admin_headers)
