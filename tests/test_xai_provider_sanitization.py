...
def test_mask_api_key_method():
    ...
    masked = provider.mask_api_key(XAI_MASK_KEY)
    assert masked == "xai_****3456"
...
KEY_LEAK_PATTERNS = [
    re.compile(r"xai-[a-zA-Z0-9][a-zA-Z0-9._-]{20,}"),
    re.compile(r"Bearer xai-[a-zA-Z0-9][a-zA-Z0-9._-]+"),
]


@pytest.mark.asyncio
async def test_no_key_in_health_check_output(monkeypatch):
    monkeypatch.setenv("XAI_API_KEY", XAI_REALISTIC_KEY)
    monkeypatch.setenv("CLOUD_PROVIDERS_ENABLED", "true")
    monkeypatch.setenv("XAI_PROVIDER_ENABLED", "true")
...
def test_estimate_cost_sanitized():
    provider = XAIProvider()
    cost = provider.estimate_cost("grok-2", 100, 50)
...
def test_provider_id_and_type():
    ...
    assert provider.provider_type == ProviderType.XAI
