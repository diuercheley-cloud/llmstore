...
@pytest.mark.asyncio
async def test_model_capabilities_v1_models(admin_client: AsyncClient, admin_token_headers, models_dir: Path):
    """
    ...
    client_resp = await admin_client.post("/admin/clients", json=client_payload, headers=admin_token_headers)
    client_id = client_resp.json()["id"]
    
    key_payload = {"client_id": client_id, "name": "test-key"}
    key_resp = await admin_client.post("/admin/api-keys", json=key_payload, headers=admin_token_headers)
