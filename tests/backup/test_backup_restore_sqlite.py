...
def _rewrite_backup_for_path_traversal(service: BackupService, backup_store_root: Path, backup_id: str) -> None:
    ...
    parts["feature_flags.json"] = json.dumps(parts["feature_flags.json"], sort_keys=True).encode("utf-8")
...
async def test_sqlite_full_restore_recovers_all_required_domains(
    ...
    async with sqlite_session_factory() as session:
        manifest = await BackupService(session).create_backup(BackupCreateRequest(scope="full"), actor="sqlite-dr")
...
async def test_sqlite_restore_blocks_concurrent_restore_attempts(
    ...
    async with sqlite_session_factory() as session_one:
        task = asyncio.create_task(
            RestoreStagingService(session_one).restore_with_staging(backup_id, BackupRestoreRequest(dry_run=False))
        )
        await entered.wait()

        async with sqlite_session_factory() as session_two:
            with pytest.raises(HTTPException) as exc:
                await RestoreStagingService(session_two).restore_with_staging(backup_id, BackupRestoreRequest(dry_run=False))
...
async def test_sqlite_large_backup_round_trip(
    ...
    async with sqlite_session_factory() as session:
        ids = await seed_recovery_state(session, backup_repo_root, extra_agents=180, long_text_size=8192)
        expected = await collect_recovery_state(session, backup_repo_root, ids)
        backup_id = (await BackupService(session).create_backup(BackupCreateRequest(scope="full"))).backup_id
