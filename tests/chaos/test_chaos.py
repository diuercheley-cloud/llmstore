...
async def test_chaos_provider_timeout():
    ...
    with patch("httpx.AsyncClient.post") as mock_post:
        ...
        val = REGISTRY.get_sample_value(
            "llm_provider_failures_total", {"provider_name": "mock-timeout", "error_code": "504"}
        )
        assert val >= 1.0
...
async def test_chaos_queue_saturation():
    ...
    val = REGISTRY.get_sample_value("llm_queue_wait_seconds_count", {"plan": "free"})
    assert val >= 1.0
...
async def test_chaos_tokenizer_unavailable():
    ...
    with patch("app.services.tokenizer_service._load_tiktoken", return_value=None):
        ...
        with patch(
            "app.services.tokenizer_service.estimate_tokens_from_text",
            side_effect=Exception("estimation_failed"),
        ):
...
def test_chaos_hot_swap_failure():
    ...
    val = REGISTRY.get_sample_value(
        ...
        {"model_id": "failed-model", "reason": "health_check_failed"},
    )
    assert val >= 1.0
...
def test_chaos_attestation_failure():
    ...
    val = REGISTRY.get_sample_value(
        "llm_attestation_failures_total", {"node_id": "node-1", "reason": "invalid_signature"}
    )
    assert val >= 1.0
...
def test_chaos_plugin_invalid_signature():
    ...
    val = REGISTRY.get_sample_value(
        ...
        {"client_id": "attacker", "resource": "/admin/plugins/load", "action": "write"},
    )
    assert val >= 1.0
