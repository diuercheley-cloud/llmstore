...
async def test_backup_restore_operational_metrics(test_db, tmp_path, monkeypatch):
    ...
    with patch.object(
        BackupService, "_build_archive", side_effect=ValueError("Simulated archive error")
    ):
        async with session_factory() as db:
            service = BackupService(db)
    ...
    with patch.object(
        ...
        "_restore_with_staging_impl",
        side_effect=ValueError("Simulated restore error"),
    ):
        async with session_factory() as db:
            staging_svc = RestoreStagingService(db)
            with pytest.raises(ValueError):
                await staging_svc.restore_with_staging(
                    backup_id, BackupRestoreRequest(dry_run=False), actor="restore-operator"
                )
