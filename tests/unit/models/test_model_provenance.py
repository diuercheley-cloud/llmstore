...
async def test_model_provenance_api_flow(admin_client, session, admin_token_headers):
    ...
    # 2. Verify
    resp_v = await admin_client.post(f"/api/admin/models/provenance/verify/{record.id}", headers=headers)
...
async def test_file_hash_change_affects_verification(session):
    ...
    valid_hash = "a" * 64
    record = await service.register_model(model_id="hash-model", source="test", weights_hash=valid_hash)
    await session.commit()
