...
async def test_invoke_agent_sync_success(mock_db):
    ...
    tier = AgentServiceTier(
        id=uuid.uuid4(), name="pro", rate_limit_per_minute=10, features={"sync_mode": True}
    )
    api.tiers.get_tier = AsyncMock(return_value=tier)

    api.sync_invoker.invoke_sync = AsyncMock(
        return_value={"run_id": str(run_id), "status": "completed", "output": "The answer is 42"}
    )

    result = await api.invoke_agent(
        agent_id, tenant_id, "What is the answer?", mode="sync", tier_name="pro"
    )
...
async def test_callback_registration(mock_db):
    ...
    webhook = await service.register_webhook(
        agent_id, "tenant_1", "https://hooks.example.com/callback"
    )
