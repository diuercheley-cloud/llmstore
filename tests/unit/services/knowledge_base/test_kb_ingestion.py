...
async def test_ingest_url_disabled_by_default(mock_db):
    ...
    mock_db.execute = AsyncMock(
        return_value=MagicMock(scalar_one_or_none=lambda: KnowledgeBase(id=kb_id, tenant_id="t1"))
    )
...
async def test_ingest_file_success(mock_db):
    ...
    service.registry.create_document_version = AsyncMock(
        return_value=KBDocumentVersion(id=uuid.uuid4())
    )
