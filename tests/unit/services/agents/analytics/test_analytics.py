...
async def test_analytics_aggregator_success(mock_db):
    ...
    with MagicMock() as mock_queries:
        mock_queries.get_run_success_rate = AsyncMock(
            return_value={"total": 100, "success_rate": 0.9, "distribution": {}}
        )
        mock_queries.get_cost_and_tokens = AsyncMock(
            return_value={"total_cost_brl": 10.5, "total_tokens": 5000}
        )
        mock_queries.get_tool_metrics = AsyncMock(
            return_value={"total_tool_calls": 50, "tool_failure_rate": 0.05}
        )
...
async def test_analytics_no_data(mock_db):
    ...
    with MagicMock() as mock_queries:
        mock_queries.get_run_success_rate = AsyncMock(
            return_value={"total": 0, "success_rate": 0, "distribution": {}}
        )
        mock_queries.get_cost_and_tokens = AsyncMock(
            return_value={"total_cost_brl": 0.0, "total_tokens": 0}
        )
        mock_queries.get_tool_metrics = AsyncMock(
            return_value={"total_tool_calls": 0, "tool_failure_rate": 0.0}
        )
