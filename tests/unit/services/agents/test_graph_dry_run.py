...
async def test_agent_graph_dry_run_simple(session: AsyncSession):
    ...
    graph_json = {
        "nodes": [
            ...
            {"id": "n2", "node_type": "tool_call", "config": {"tool_name": "get_weather", "parameters": {"city": "Berlin"}}},
...
async def test_agent_graph_dry_run_memory_write_blocked(session: AsyncSession):
    ...
    graph_json = {
        "nodes": [
            {"id": "n1", "node_type": "memory_write", "config": {"key": "user_pref", "value": "dark-mode"}}
        ],
        "edges": []
    }
...
async def test_agent_graph_dry_run_node_failure(session: AsyncSession):
    ...
    assert result["trace"][0]["status"] == "completed" # node runner catches errors and returns status failed in trace
    ...
    assert result["trace"][0]["output"]["message"] == "Node type 'invalid_type' execution simulated."
