...
async def test_parallel_fanout_fanin(workflow_engine, mock_db):
    ...
    state_data = args[0]
    assert (
        "join" in state_data["active_nodes"]
    )  # Should be active but not ready if join checks predecessors
...
async def test_subworkflow_execution(workflow_engine, mock_db):
    ...
    nodes = [
        AgentWorkflowNode(
            ...
            config={"subworkflow_definition_id": str(sub_def_id)},
        )
    ]
    ...
    with MagicMock() as mock_runtime:
        ...
        with pytest.MonkeyPatch().context() as m:
            m.setattr(
                "app.services.agents.workflows.workflow_engine.SubworkflowRuntime",
                lambda db, run: mock_runtime,
            )
