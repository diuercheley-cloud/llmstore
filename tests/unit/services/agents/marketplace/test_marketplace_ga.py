...
async def test_marketplace_search_filters(mock_db):
    ...
    items = [
        MarketplaceItem(
            name="Sales Agent", category="sales", tags=["crm", "email"], is_public=True
        ),
...
async def test_marketplace_search_pagination(mock_db):
    service = MarketplaceSearchService(mock_db)
    items = [
        MarketplaceItem(name=f"Agent {i}", category="general", is_public=True) for i in range(5)
    ]
    mock_result = MagicMock()
    mock_result.scalars.return_value.all.return_value = items
...
async def test_marketplace_rating_update(mock_db):
    ...
    item_mock = MagicMock()
    item_mock.scalar_one_or_none.return_value = MarketplaceItem(
        id=item_id, avg_rating=0.0, total_ratings=0
    )
...
async def test_marketplace_search_by_tags(mock_db):
    ...
    items = [
        MarketplaceItem(
            name="Support Agent", category="support", tags=["chat", "ticket"], is_public=True
        ),
        MarketplaceItem(
            name="Sales Agent", category="sales", tags=["crm", "email"], is_public=True
        ),
...
async def test_publisher_registration(mock_db):
    ...
    publisher = await service.register_publisher(
        tenant_id="tenant-1", name="Acme Corp", description="Enterprise AI"
    )
