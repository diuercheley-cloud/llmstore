...
async def test_ab_comparison_winner(mock_db):
    ...
    results = [
        MagicMock(preference="A"),
        MagicMock(preference="A"),
        MagicMock(preference="B")
    ]
...
async def test_promotion_gate_logic(mock_db):
    ...
    with MagicMock() as mock_self:
        ...
        with pytest.MonkeyPatch().context() as m:
            ...
            mock_db.execute = AsyncMock(return_value=MagicMock(scalars=lambda: MagicMock(first=lambda: None)))
            
            ready, reasons = await manager.evaluate_promotion_readiness(agent_id, run_id, threshold=0.8)
    ...
    with MagicMock() as mock_self:
        ...
        with pytest.MonkeyPatch().context() as m:
            ...
            mock_db.execute = AsyncMock(return_value=MagicMock(scalars=lambda: MagicMock(first=lambda: MagicMock())))
            
            ready, reasons = await manager.evaluate_promotion_readiness(agent_id, run_id, allow_mock=False)
            assert ready is False
