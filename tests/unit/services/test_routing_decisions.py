...
async def test_inference_router_successful_routing(session):
    ...
    backend = InferenceBackend(
        name="gpu-vllm", provider="vllm", backend_url="http://localhost:8000", is_active=True
    )
    session.add(backend)
...
async def test_inference_router_failed_routing(session):
    ...
    router = InferenceRouter(session)
    adapter, reason = await router.get_best_backend(
        model_name="unsupported-model", capability=Capability.CHAT, request_id="req-failed-1"
    )
...
async def test_inference_router_retention_cleanup(session):
    ...
    session.expire_all()
    db_res = await session.execute(
        select(InferenceRoutingDecision).execution_options(populate_existing=True)
    )
