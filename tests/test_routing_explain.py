...
@pytest.mark.asyncio
async def test_routing_explain_endpoint(admin_client: AsyncClient, admin_token_headers: dict, session: AsyncSession):
    # 1. Setup Data
    plan = BillingPlan(
        code="premium-vllm",
    ...
    backend_llama = InferenceBackend(name="llama-backend", provider="llama.cpp", backend_url="http://llama")
    ...
    route1 = ModelBackendRoute(model_registry_id=model.id, inference_backend_id=backend_vllm.id, priority=100, weight=100)
    route2 = ModelBackendRoute(model_registry_id=model.id, inference_backend_id=backend_llama.id, priority=100, weight=100)
    session.add(route1)
    session.add(route2)
    await session.commit()
