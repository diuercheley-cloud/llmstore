...
@pytest.mark.asyncio
async def test_backup_restore_approval_workflow(
    ...
    create_req_res = await admin_client.post(
        "/admin/backup/restore-requests", headers=admin_token_headers, json=req_payload
    )
    ...
    approve_res = await admin_client.post(
        f"/admin/backup/restore-requests/{req_id}/approve", headers=admin_token_headers
    )
    ...
    approve_expired = await admin_client.post(
        f"/admin/backup/restore-requests/{req_id2}/approve", headers=admin_token_headers
    )
