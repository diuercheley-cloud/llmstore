...
def _prepare_config_tree(root: Path) -> None:
    ...
    (root / "config" / "feature-flags.yaml").write_text(
        "flags:\n  demo_enabled: true\n", encoding="utf-8"
    )
...
@pytest.mark.asyncio
async def test_backup_create_and_verify(
    admin_client, admin_token_headers, isolated_db_url, tmp_path, monkeypatch
):
    ...
    response = await admin_client.post(
        "/admin/backup", headers=admin_token_headers, json={"full": True}
    )
    ...
    verify = await admin_client.post(
        f"/admin/backup/{payload['backup_id']}/verify", headers=admin_token_headers
    )
...
@pytest.mark.asyncio
async def test_backup_restore_recovers_data_and_files(
    admin_client, admin_token_headers, isolated_db_url, tmp_path, monkeypatch
):
    ...
    assert "demo_enabled: true" in (repo_root / "config" / "feature-flags.yaml").read_text(
        encoding="utf-8"
    )
...
@pytest.mark.asyncio
async def test_backup_status_reports_integrity(
    admin_client, admin_token_headers, isolated_db_url, tmp_path, monkeypatch
):
    monkeypatch.setenv("LLMSTACK_BACKUP_SOURCE_ROOT", str(tmp_path / "repo"))
    _prepare_config_tree(tmp_path / "repo")
    ...
    status_response = await admin_client.get(
        f"/admin/backup/{backup_id}/status", headers=admin_token_headers
    )
...
@pytest.mark.asyncio
async def test_backup_detects_corruption_and_blocks_restore(
    admin_client, admin_token_headers, isolated_db_url, tmp_path, monkeypatch
):
    monkeypatch.setenv("LLMSTACK_BACKUP_SOURCE_ROOT", str(tmp_path / "repo"))
    _prepare_config_tree(tmp_path / "repo")
    ...
    verify_response = await admin_client.post(
        f"/admin/backup/{backup_id}/verify", headers=admin_token_headers
    )
...
@pytest.mark.asyncio
async def test_backup_restore_disabled_by_default(
    admin_client, admin_token_headers, isolated_db_url, tmp_path, monkeypatch
):
    monkeypatch.setenv("LLMSTACK_BACKUP_SOURCE_ROOT", str(tmp_path / "repo"))
    _prepare_config_tree(tmp_path / "repo")
...
@pytest.mark.asyncio
async def test_backup_verify_returns_manifest_error_code(
    admin_client, admin_token_headers, isolated_db_url, tmp_path, monkeypatch
):
    monkeypatch.setenv("LLMSTACK_BACKUP_SOURCE_ROOT", str(tmp_path / "repo"))
    _prepare_config_tree(tmp_path / "repo")
...
@pytest.mark.asyncio
async def test_backup_verify_returns_key_missing_code(
    admin_client, admin_token_headers, isolated_db_url, tmp_path, monkeypatch
):
    monkeypatch.setenv("LLMSTACK_BACKUP_SOURCE_ROOT", str(tmp_path / "repo"))
    _prepare_config_tree(tmp_path / "repo")
...
@pytest.mark.asyncio
async def test_backup_restore_returns_lock_error_code(
    admin_client, admin_token_headers, isolated_db_url, tmp_path, monkeypatch
):
    monkeypatch.setenv("LLMSTACK_BACKUP_SOURCE_ROOT", str(tmp_path / "repo"))
    _prepare_config_tree(tmp_path / "repo")
