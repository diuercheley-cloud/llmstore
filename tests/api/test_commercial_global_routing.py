...
async def test_commercial_global_routing_governance(admin_client, admin_token_headers, session):
    ...
    invalid_policy_json["commercial_global_routing_margin_weight"] = 0.10  # sum of weights is now 0.60
    ...
    activate_resp = await admin_client.post(
        f"/admin/routing/global-router/policies/{policy_id_1}/activate",
        headers=admin_token_headers
    )
    ...
    activate_resp_2 = await admin_client.post(
        f"/admin/routing/global-router/policies/{policy_id_2}/activate",
        headers=admin_token_headers
    )
    ...
    rollback_resp = await admin_client.post(
        "/admin/routing/global-router/policies/rollback",
        headers=admin_token_headers
    )
