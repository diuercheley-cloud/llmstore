...
async def test_list_agents_v1(admin_client: AsyncClient, auth_headers):
    ...
    await admin_client.post(
        ...
        json={"name": "Agent A", "version": "1", "instructions": "i", "model_id": "m"},
    )
...
@pytest.mark.asyncio
async def test_agent_v1_isolation(
    admin_client: AsyncClient, admin_token_headers, auth_headers, test_client_id
):
    ...
    resp_a = await admin_client.post(
        "/v1/agents",
        headers=auth_headers,
        json={"name": "Client 1 Agent", "version": "1", "instructions": "i", "model_id": "m"},
    )
...
async def test_agent_run_v1(admin_client: AsyncClient, auth_headers):
    ...
    # Start run
    resp_run = await admin_client.post(
        f"/v1/agents/{agent_id}/runs", headers=auth_headers, json={"input_text": "hello"}
    )
...
async def test_agent_sse_events_v1(admin_client: AsyncClient, auth_headers):
    ...
    async with admin_client.stream(
        "GET", f"/v1/agents/runs/{run_id}/events", headers=auth_headers
    ) as response:
