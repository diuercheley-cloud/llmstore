...
async def test_commercial_config_specificity_precedence(session: AsyncSession):
    ...
    await store.apply_config(scope_type="provider_model", provider="openai", model="gpt-4", cost_multiplier=1.4)
...
async def test_commercial_config_safety_validation(session: AsyncSession):
    ...
    with pytest.raises(ValueError, match="Weights must sum to 1.0"):
        await store.apply_config(scope_type="global", margin_weight=0.5, latency_weight=0.1, quality_weight=0.1)
...
async def test_commercial_config_deactivate_and_rollback(session: AsyncSession):
    ...
    res = await session.execute(select(CommercialRoutingConfig).where(CommercialRoutingConfig.id == c1.id))
    ...
    res = await session.execute(select(CommercialRoutingConfig).where(CommercialRoutingConfig.id == c2.id))
...
async def test_commercial_config_audit_logging(session: AsyncSession):
    store = CommercialConfigStore(session)
    config = await store.apply_config(scope_type="global", cost_multiplier=1.1, created_by="admin@test.com", notes="test audit")
    
    # In AdminActionLog, target_id is not a column, we use payload_json
    res = await session.execute(select(AdminActionLog).where(AdminActionLog.action == "apply_commercial_config"))
