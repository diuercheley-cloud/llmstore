...
def test_validate_first_run_local_script_dry_run():
    result = subprocess.run(["./scripts/validate-first-run-local.sh", "--dry-run"], capture_output=True, text=True)
    assert result.returncode == 0
    assert "Validating help output" in result.stdout

def test_validate_first_run_local_script_execution():
    result = subprocess.run(["./scripts/validate-first-run-local.sh"], capture_output=True, text=True)
    assert result.returncode == 0
