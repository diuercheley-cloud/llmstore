...
def test_check_allowlist_valid():
    ...
    is_allowlisted, reason = check_allowlist("artifacts/backups/test-backup-run/config/config.env", is_versioned=False)
...
def test_script_permission_warning(tmp_path):
    ...
    try:
        ...
        res = subprocess.run(
            ["./scripts/security-report-local.sh", "--output-dir", str(out_dir)],
            capture_output=True,
            text=True
        )
...
def test_secrets_allowlist_integration(tmp_path):
    ...
    try:
        ...
        res_1 = subprocess.run(
            ["./scripts/security-report-local.sh", "--output-dir", str(out_dir_1)],
            capture_output=True,
            text=True
        )
        ...
        sec_checks_1 = [c for c in report_data_1["checks"] if "test-secrets-temp" in c.get("details", "")]
        ...
        res_2 = subprocess.run(
            ["./scripts/security-report-local.sh", "--output-dir", str(out_dir_2)],
            capture_output=True,
            text=True
        )
        ...
        sec_checks_2 = [c for c in report_data_2["checks"] if "test-secrets-temp" in c.get("details", "")]
