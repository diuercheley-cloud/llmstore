...
async def test_models_v1_includes_enhanced_metadata(admin_client: AsyncClient, admin_token_headers):
    ...
    client_resp = await admin_client.post("/admin/clients", json={"name": "test-client"}, headers=admin_token_headers)
    client_id = client_resp.json()["id"]
    key_resp = await admin_client.post("/admin/api-keys", json={"client_id": client_id, "name": "test-key"}, headers=admin_token_headers)
...
async def test_embedding_model_capabilities(admin_client: AsyncClient, admin_token_headers):
    ...
    client_resp = await admin_client.post("/admin/clients", json={"name": "test-client-2"}, headers=admin_token_headers)
    client_id = client_resp.json()["id"]
    key_resp = await admin_client.post("/admin/api-keys", json={"client_id": client_id, "name": "test-key-2"}, headers=admin_token_headers)
    api_key = key_resp.json()["api_key"]
