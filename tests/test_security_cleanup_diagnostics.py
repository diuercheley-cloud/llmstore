...
def test_diagnostic_file_no_secrets():
    with open("docs/SECURITY_CLEANUP_v1.5.4.md", "r") as f:
        content = f.read()
...
def test_parser_script_exists_and_executable():
    assert os.path.exists("scripts/parse-security-report-local.sh")
    assert os.access("scripts/parse-security-report-local.sh", os.X_OK)
...
def test_parser_output_masking():
    ...
    result = subprocess.run(
        ["./scripts/parse-security-report-local.sh", report_json],
        capture_output=True,
        text=True
    )
