...
script_path = os.path.abspath(os.path.join(os.path.dirname(__file__), "../../scripts/validators/platform-freeze-check.py"))
...
def test_check_top_level_dirs():
    ...
    # No violation
    with patch("os.listdir", return_value=["control_plane", "config", ".venv"]), \
         patch("os.path.isdir", return_value=True):
        assert platform_freeze_check.check_top_level_dirs(rules) is True

    # Violation detected
    with patch("os.listdir", return_value=["control_plane", "config", "new_forbidden_dir"]), \
         patch("os.path.isdir", return_value=True):
...
def test_check_api_routers():
    ...
    with patch("os.path.exists", return_value=True), \
         ...
         patch("builtins.open", mock_open(read_data="""# Surface: admin\nrouter = APIRouter(prefix="/admin")""")):
    ...
    with patch("os.path.exists", return_value=True), \
         patch("os.listdir", return_value=["client.py", "admin.py", "new_admin.py"]), \
         patch("builtins.open", mock_open(read_data="""# Owner: Architecture\n# Surface: admin\nrouter = APIRouter(prefix="/admin")""")):
    ...
    with patch("os.path.exists", return_value=True), \
         ...
         patch("builtins.open", mock_open(read_data="""# Owner: Dev\nrouter = APIRouter(prefix="/foo")""")):
        assert platform_freeze_check.check_api_routers(rules_approved) is False

def test_check_new_services():
    rules = {
        "allowed_services": ["auth.py"],
        "approved_exceptions": ["new_service.py"]
    }

    # Case 1: New service with docstring -> Passes
    with patch("os.path.exists", return_value=True), \
         patch("os.walk", return_value=[("control_plane/app/services", [], ["auth.py", "new_service.py"])]), \
...
def test_check_models_and_migrations():
    rules = {
        "allowed_models": ["OldModel"],
        "approved_exceptions": ["NewModel"]
    }
    ...
    with patch("os.path.exists", return_value=True), \
        ...
        with patch("builtins.open", mock_open(read_data=model_content_ok)) as mock_file:
            ...
            mock_file.side_effect = [mock_open(read_data=migration_mock)(), mock_open(read_data=model_content_ok)()]
            assert platform_freeze_check.check_models_and_migrations(rules) is True

        # Fails when missing migration
        with patch("builtins.open", mock_open(read_data=model_content_ok)) as mock_file:
            mock_file.side_effect = [mock_open(read_data="")(), mock_open(read_data=model_content_ok)()]
            assert platform_freeze_check.check_models_and_migrations(rules) is False

def test_check_new_endpoints():
    rules = {
        "require_supported_surface_classification": True
    }
