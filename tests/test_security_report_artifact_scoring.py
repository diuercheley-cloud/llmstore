...
def test_security_report_artifact_scoring(tmp_path):
    ...
    scripts_dir.mkdir()
    shutil.copy2("scripts/check-secrets.sh", scripts_dir / "check-secrets.sh")
    shutil.copy2("scripts/security-report-local.sh", scripts_dir / "security-report-local.sh")
    shutil.copy2("scripts/redact-local-sensitive-artifacts.sh", scripts_dir / "redact-local-sensitive-artifacts.sh")
    ...
    subprocess.run(["chmod", "+x", "scripts/check-secrets.sh", "scripts/security-report-local.sh"], cwd=repo_root, check=True)

    output_dir = repo_root / "report_out"
    # Run first time with real secret in tracked file -> should FAIL
    result = subprocess.run(
        ["./scripts/security-report-local.sh", "--output-dir", str(output_dir)],
        cwd=repo_root,
        capture_output=True,
        text=True,
    )
