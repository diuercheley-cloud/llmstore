...
async def test_api_dry_run_permissions(admin_client, permission_graph):
    ...
    # Dry-run
    resp_dry = await admin_client.post(f"/admin/agents/studio/flows/{flow_id}/dry-run", json={}, headers=headers)
    ...
    resp_dry2 = await admin_client.post(f"/admin/agents/studio/flows/{flow_id2}/dry-run", json={}, headers=headers)
...
async def test_api_explain(admin_client, valid_graph):
    ...
    resp_exp = await admin_client.post(f"/admin/agents/studio/flows/{flow_id}/explain", headers=headers)
