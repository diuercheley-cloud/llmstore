...
async def test_portal_me_includes_tts_limits(mock_verify, tts_setup):
    ac = tts_setup["ac"]
    api_key = tts_setup["api_key"]
    response = await ac.get("/portal/me", headers={"Authorization": f"Bearer {api_key}"})
...
async def test_portal_usage_stats_includes_tts(mock_verify, tts_setup):
    ...
    assert response.status_code == 200
    data = response.json()
    assert data["tts_usage"]["usage"]["monthly_chars"] == 100
