...
def test_expected_libs_exist():
    ...
    for lib in expected:
        assert (SCRIPTS_LIB / lib).is_file(), f"Missing {lib} in scripts/lib/"
...
def test_no_duplicate_libs():
    for f in ["operator-errors.sh", "redaction.sh", "validation-logging.sh"]:
        ...
        assert not (in_root and in_scripts), f"DUPLICATE: {f} in both lib/ and scripts/lib/"
...
def test_no_root_lib_references():
    for sh in Path(ROOT_DIR / "scripts").rglob("*.sh"):
        ...
        for line in content.splitlines():
            line = line.strip()
            if line.startswith("source ") and "lib/" in line and "scripts/lib/" not in line:
                if '${SCRIPT_DIR}/lib/' in line or '${ROOT_DIR}/scripts/lib/' in line:
                    continue
                if '${ROOT_DIR}/lib/' in line:
                    pytest.fail(f"{sh} references root lib/ instead of scripts/lib/: {line}")
