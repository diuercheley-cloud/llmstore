...
def test_conflict_resolution_deterministic_and_blocking():
    ...
    source_bundle = protocol.export_bundle(session, {"bundle_name": "a", "bundle_type": "mixed", "payload": {"x": 1}})
    target_bundle = protocol.export_bundle(session, {"bundle_name": "a", "bundle_type": "mixed", "payload": {"x": 2}})
    ...
    merge = service.resolve_conflict({"conflict_type": "bundle_hash_conflict", "client_id": client_id, "session_id": session.id}, "deterministic_merge")
    manual = service.resolve_conflict({"conflict_type": "bundle_hash_conflict", "client_id": client_id, "session_id": session.id}, "manual_review_required")
    lineage = service.resolve_conflict({"conflict_type": "lineage_conflict", "client_id": client_id, "session_id": session.id}, "keep_source")
