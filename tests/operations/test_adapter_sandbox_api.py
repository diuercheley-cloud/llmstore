...
class TestAdapterSandboxAPI:
    async def test_register_manifest_endpoint(self, admin_client: AsyncClient, admin_token_headers: dict):
    ...
    async def test_simulate_run_flow(self, admin_client: AsyncClient, admin_token_headers: dict):
        ...
        m_resp = await admin_client.post("/admin/operations/adapter-sandbox/manifests", json=m_payload, headers=admin_token_headers)
        manifest_id = m_resp.json()["manifest"]["id"]
        
        # 2. Prepare run
        p_payload = {"client_id": client_id, "manifest_id": manifest_id, "sandbox_mode": "simulation"}
        p_resp = await admin_client.post("/admin/operations/adapter-sandbox/runs/prepare", json=p_payload, headers=admin_token_headers)
        ...
        s_resp = await admin_client.post("/admin/operations/adapter-sandbox/runs/simulate", json=s_payload, headers=admin_token_headers)
