...
async def test_federation_api_flow(session):
    ...
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://testserver") as ac:
        source = await ac.post(
            ...
            json={"client_id": str(client.id), "environment_name": "source", "environment_type": "airgap_node", "federation_scope": "ops", "trust_level": "trusted"},
        )
        target = await ac.post(
            "/admin/operations/federation/environments",
            json={"client_id": str(client.id), "environment_name": "target", "environment_type": "offline_staging", "federation_scope": "ops", "trust_level": "verified"},
        )
        ...
        sync = await ac.post(
            ...
            json={"client_id": str(client.id), "source_environment_id": source_id, "target_environment_id": target_id},
        )
        assert sync.status_code == 200
        session_id = sync.json()["session"]["id"]

        detail = await ac.get(f"/admin/operations/federation/sessions/{session_id}?client_id={client.id}")
        blocked_detail = await ac.get(f"/admin/operations/federation/sessions/{session_id}?client_id={other.id}")
        assert detail.status_code == 200
        assert blocked_detail.status_code == 404

        exported = await ac.post(
            "/admin/operations/federation/bundles/export",
            json={"client_id": str(client.id), "session_id": session_id, "bundle_name": "bundle", "bundle_type": "mixed", "payload": {"secret_token": "hidden"}},
        )
        ...
        imported = await ac.post(
            ...
            json={"client_id": str(client.id), "session_id": session_id, "bundle_payload": exported.json()["payload"]},
        )
        ...
        lineage = await ac.get(f"/admin/operations/federation/lineage/{bundle_id}?client_id={client.id}")
        ...
        cross_tenant_export = await ac.post(
            "/admin/operations/federation/bundles/export",
            json={"client_id": str(other.id), "session_id": session_id, "bundle_name": "bundle", "bundle_type": "mixed", "payload": {}},
        )
