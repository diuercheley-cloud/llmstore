...
class TestAdapterSandboxAuditEvents:
    async def test_log_manifest_registered(self, session: AsyncSession):
        ...
        stmt = select(AdminActionLog).where(AdminActionLog.action == "ops_adapter_sandbox:adapter_manifest_registered")
    ...
    async def test_log_run_completed(self, session: AsyncSession):
        ...
        stmt = select(AdminActionLog).where(AdminActionLog.action == "ops_adapter_sandbox:adapter_sandbox_run_completed")
        result = await session.execute(stmt)
        entry = result.scalars().one()
