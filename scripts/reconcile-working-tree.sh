...
git add \
  ...
  tests/operations/test_capability_readiness.py \
...
git add \
  ...
  control_plane/tests/test_agent_evals_gates.py \
...
git add \
  ...
  scripts/archive-deprecated-surface.sh \
  scripts/audit-feature-flags.py \
  scripts/bootstrap-agentic-production.sh \
  scripts/check-doc-consistency.py \
  scripts/check-feature-flags.py \
  scripts/check-runtime-prerequisites.sh \
  scripts/check-service-test-coverage.py \
  scripts/consolidate-platform-surface.py \
  scripts/platform-freeze-check.py \
  scripts/platform-freeze-check.sh \
  scripts/preflight-hermetic.sh \
  scripts/release-gate.sh \
  scripts/validate-agentic-production-profile.sh \
  scripts/validate_platform_documentation.py \
  2>/dev/null || echo "  (some already staged)"
...
git add \
  ...
  tests/docs/test_platform_documentation.py \
  2>/dev/null || echo "  (some already staged)"
...
git add \
  scripts/reconcile-working-tree.sh \
  scripts/check-working-tree-governance.sh \
