...
run_health_check_attempt() {
    ...
    echo "Running step: ${name}..."
    echo "Command: ./scripts/test-health.sh" >"${log_file}"
    if ./scripts/test-health.sh >>"${log_file}" 2>&1; then
        echo "OK"
        STEPS_STATUS["${name}"]="OK"
        return 0
    fi
...
if [[ "${RESET_FIRST}" == "true" ]]; then
    run_step "reset" "./scripts/reset-demo-local.sh --yes" || true
...
run_step "seed-data" "./scripts/seed-demo-local.sh"

run_step "validate-local" "./scripts/validate-demo-local.sh" || true
run_step "validate-portal" "./scripts/validate-demo-client-portal.sh" || true
run_step "validate-admin" "./scripts/validate-demo-admin-dashboard.sh" || true
run_step "validate-examples" "./scripts/validate-examples-local.sh" || true

if [[ "${SKIP_LMSTUDIO}" == "true" ]]; then
    skip_step "validate-lmstudio" "--skip-lmstudio requested"
else
    run_step "validate-lmstudio" "CONTROL_PLANE_URL=\"${BASE_URL}\" ./scripts/validate-lmstudio-backend.sh" || true
fi

if [[ "${SKIP_RAG}" == "true" ]]; then
    skip_step "validate-rag" "--skip-rag requested"
else
    run_step "validate-rag" "RAG_ENABLED=true ./scripts/validate-rag-local-multiclient.sh" || true
fi

run_step "validate-prod-full" "./scripts/validate-local-production-full.sh" || true
...
- Testar exemplos: \`./scripts/validate-examples-local.sh\`
- Resetar ambiente: \`./scripts/reset-demo-local.sh\`
...
if [[ "${KEEP_DATA}" == "false" && "${STEPS_STATUS[validate-local]:-ERROR}" == "OK" ]]; then
    echo
    echo "Validation passed. Run ./scripts/reset-demo-local.sh if you want to clean up."
fi
