...
if [[ "$RUN_DEEP_CHECKS" == "true" ]]; then
    if [[ -f ./scripts/check-secrets.sh ]]; then
        run_check "Check Secrets" "./scripts/check-secrets.sh --all"
    fi

    if [[ -f ./scripts/security-report-local.sh ]]; then
        run_check_warn "Security Report" "./scripts/security-report-local.sh"
    fi

    if [[ -f ./scripts/production-readiness-local.sh ]]; then
        run_check_warn "Production Readiness" "./scripts/production-readiness-local.sh"
    fi

    if [[ -f ./scripts/validate-local-production-full.sh ]]; then
        run_check_warn "Validate Production Full" "./scripts/validate-local-production-full.sh"
    fi

    if [[ "$MODE" == "demo" ]] && [[ -f ./scripts/demo-full-local.sh ]]; then
        run_check_warn "Demo Full Local" "./scripts/demo-full-local.sh --no-build"
    fi

    if [[ -f ./scripts/post-upgrade-smoke-local.sh ]]; then
        run_check_warn "Post Upgrade Smoke" "./scripts/post-upgrade-smoke-local.sh"
    fi

    if [[ -f ./scripts/validate-multitenant-isolation-full.sh ]]; then
        run_check_warn "Multitenant Isolation" "./scripts/validate-multitenant-isolation-full.sh"
    fi

    if [[ -f ./scripts/validate-abuse-protection-local.sh ]]; then
        run_check_warn "Abuse Protection" "./scripts/validate-abuse-protection-local.sh"
    fi

    if [[ -f ./scripts/validate-commercial-plans-local.sh ]]; then
        run_check_warn "Commercial Plans" "./scripts/validate-commercial-plans-local.sh"
    fi
fi
