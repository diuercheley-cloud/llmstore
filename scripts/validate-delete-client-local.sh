...
CREATE_OUTPUT=$("${SCRIPT_DIR}/create-client.sh" "${TEST_CLIENT_NAME}" "Temporary client for deletion validation")
...
"${SCRIPT_DIR}/delete-client-local.sh" --client-id "${CLIENT_ID}" --dry-run
...
"${SCRIPT_DIR}/delete-client-local.sh" --client-id "${CLIENT_ID}" --yes --require-export --delete-rag-files --delete-tts-files --delete-invoices
