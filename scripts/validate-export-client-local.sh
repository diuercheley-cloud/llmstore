...
"${SCRIPT_DIR}/export-client-local.sh" --client-id "${CLIENT_ID}" --dry-run

# 4. Run real export by Client ID
echo "Testing real export by Client ID..."
EXPORT_OUTPUT=$("${SCRIPT_DIR}/export-client-local.sh" --client-id "${CLIENT_ID}")
...
"${SCRIPT_DIR}/export-client-local.sh" --email "${TEST_EMAIL}"
