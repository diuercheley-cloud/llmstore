...
if [[ -f "${ROOT_DIR}/scripts/dev/lib/operator-errors.sh" ]]; then
...
WIZARD_SKIP_ENV_BACKUP=true "${SCRIPT_DIR}/../dev/configure-local-wizard.sh" "${WIZARD_ARGS[@]}"
...
if [ -x "${SCRIPT_DIR}/../validators/validate-migrations-local.sh" ]; then
  "${SCRIPT_DIR}/../validators/validate-migrations-local.sh" || warn "Migration validation reported issues."
fi
...
"${SCRIPT_DIR}/../validators/check-secrets.sh" --all || { warn "Secrets check failed."; VALIDATION_SUCCESS=false; }

log "Validating production setup..."
"${SCRIPT_DIR}/../validators/validate-local-production-full.sh" || { warn "Production validation failed."; VALIDATION_SUCCESS=false; }

log "Checking readiness..."
"${SCRIPT_DIR}/../dev/production-readiness-local.sh" || { warn "Readiness check failed."; VALIDATION_SUCCESS=false; }

log "Generating security report..."
"${SCRIPT_DIR}/../validators/security-report-local.sh" || warn "Security report generation failed."
...
"${SCRIPT_DIR}/../validators/validate-post-install-local.sh" "${POST_INSTALL_ARGS[@]}" || { warn "Post-installation validation failed."; VALIDATION_SUCCESS=false; }

DEMO_SEEDED=false
if [ "$WITH_DEMO" = true ]; then
  log "Seeding demo data..."
  "${SCRIPT_DIR}/../dev/demo-full-local.sh" --no-build || { warn "Demo seeding failed."; VALIDATION_SUCCESS=false; }
  DEMO_SEEDED=true
fi
