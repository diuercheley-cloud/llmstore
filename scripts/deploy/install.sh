...
if [[ ! -x "${ROOT_DIR}/scripts/deploy/install-wsl-deps.sh" ]]; then
  fail "missing scripts/deploy/install-wsl-deps.sh"
fi

log "installing base dependencies"
"${ROOT_DIR}/scripts/deploy/install-wsl-deps.sh"
...
printf '[install] next_step=%s\n' "${ROOT_DIR}/scripts/deploy/first-run.sh"
