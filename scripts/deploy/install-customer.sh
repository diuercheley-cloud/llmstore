...
# Load libraries
source "${SCRIPT_DIR}/../dev/lib/operator-errors.sh" 2>/dev/null || true
source "${SCRIPT_DIR}/../dev/lib/validation-logging.sh" 2>/dev/null || true
...
post_install() {
  ...
  # Use scripts/dev/create-plan.sh and scripts/dev/create-client.sh with correct positional args
  # Usage: ./scripts/dev/create-plan.sh CODE NAME [RPM] [DAILY] [WEEKLY] [MONTHLY] [MAX_TOKENS] [ALLOW_STREAMING] [DESCRIPTION]
  ./scripts/dev/create-plan.sh "standard-plan" "Standard Plan" 20 100000 500000 2000000 2048 true "Plano inicial do cliente"
  
  CLIENT_NAME="Initial Customer"
  # Usage: ./scripts/dev/create-client.sh CLIENT_NAME [DESCRIPTION]
  CLIENT_OUTPUT=$(./scripts/dev/create-client.sh "$CLIENT_NAME" "Cliente criado via instalador")
