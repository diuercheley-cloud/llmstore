...
# Script to DEACTIVATE an LLM model and free memory (VRAM)
# Usage: ./scripts/deploy/deactivate-model.sh <MODEL_ID_OR_ALIAS>
...
printf "[model] Desativando modelo: %s...\n" "${MODEL_INPUT}"
...
# 2. Atualizar Banco de Dados (Marcar como inativo)
printf "[db] Marcando backend %s como inativo...\n" "${BACKEND_NAME}"
docker compose exec -T postgres psql -U "${POSTGRES_USER}" -d "${POSTGRES_DB}" -c "
    UPDATE inference_backends SET is_active = false WHERE id = '${BACKEND_ID}';
    UPDATE model_backend_routes SET state = 'disabled' WHERE inference_backend_id = '${BACKEND_ID}';
" > /dev/null

# 3. Parar Container (Liberar VRAM)
if [[ -n "${DOCKER_SERVICE}" ]]; then
    printf "[docker] Parando serviço %s para liberar memória...\n" "${DOCKER_SERVICE}"
    docker compose stop "${DOCKER_SERVICE}"
else
    printf "[warn] Nenhum serviço docker específico mapeado para o backend %s.\n" "${BACKEND_NAME}"
fi

printf "[success] Modelo '%s' desativado e memória liberada.\n" "${MODEL_INPUT}"
