...
async def register_model():
    ...
    async with AsyncSession(engine) as session:
        ...
        result = await session.execute(
            text(
                "SELECT id FROM inference_backends WHERE provider = 'openrouter' AND is_active = true LIMIT 1"
            )
        )
        ...
        result = await session.execute(
            text("SELECT id FROM model_registry WHERE model_id = :mid"), {"mid": model_id}
        )
        if result.fetchone():
