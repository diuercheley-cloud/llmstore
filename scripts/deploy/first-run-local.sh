...
if [[ -f "${SCRIPT_DIR}/../dev/lib/operator-errors.sh" ]]; then
...
./scripts/dev/configure-local-wizard.sh "${WIZARD_ARGS[@]}"
...
if [ "$MODELS_DETECTED" -eq 0 ]; then
  ...
  echo "You will need to add a model manually (e.g., via ./scripts/deploy/download-model.sh)."
  if [ "$YES" = false ]; then
...
if ! ./scripts/validators/validate-local-production-full.sh; then
...
if [ "$WITH_DEMO" = true ]; then
  log "Running demo..."
  if ./scripts/dev/demo-full-local.sh --no-build; then
    DEMO_SEEDED=true
  else
