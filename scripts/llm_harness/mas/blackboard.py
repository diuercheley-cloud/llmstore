...
class Blackboard:
    ...
    def add_message(
        self, sender: str, recipient: str, content: str, metadata: dict[str, Any] | None = None
    ):
    ...
    def log_policy_block(self, agent_id: str, tool_name: str, reason: str):
        self._add_audit_event(
            "policy_blocked", {"agent_id": agent_id, "tool_name": tool_name, "reason": reason}
        )
    ...
    def generate_timeline(self) -> str:
        ...
        for event in self.audit_log:
            ...
            if etype == "message":
                ...
                lines.append(
                    f"[{ts}] **BLOCKED**: {data['agent_id']} tried `{data['tool_name']}`. Reason: {data['reason']}"
                )
            else:
