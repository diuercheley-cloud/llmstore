...
class TeamOrchestrator:
    ...
    async def _run_linear(self, team: Any, blackboard: Blackboard) -> ExecutionResult:
        ...
        for member in team.members:
            ...
            blackboard.add_message(
                "System",
                agent_id,
                f"Executed linear step: {member.role}",
                {"success": result.success},
            )
            blackboard.add_message(
                agent_id, "Blackboard", result.message, {"metrics": result.metrics}
            )
    ...
    async def _run_supervisor_team(self, team: Any, blackboard: Blackboard) -> ExecutionResult:
        ...
        supervisor_member = next(
            (m for m in team.members if m.role == "supervisor"), team.members[0]
        )
        ...
        for i in range(max_steps):
            ...
            if not decision:
                return ExecutionResult(
                    success=False, message="Supervisor failed to make a decision"
                )
            ...
            if decision.get("type") == "final" or next_agent_id == "final":
                return ExecutionResult(
                    ...
                    events=[
                        {"event": "team.timeline", "timeline": blackboard.generate_timeline()},
                        {
                            "event": "team.blackboard",
                            "state": blackboard.state.model_dump(mode="json"),
                        },
                        {
                            "event": "team.report",
                            "report": self._generate_team_report(team, blackboard),
                        },
                    ],
                )
            ...
            if not any(m.agent_id == next_agent_id for m in team.members):
                logger.warning(
                    "Supervisor chose agent '%s' not in team. Forcing finish.", next_agent_id
                )
                break
            ...
            blackboard.add_message(
                next_agent_id,
                supervisor_agent_id,
                result.message or "No message returned",
                {"success": result.success},
            )
    ...
    async def _call_team_supervisor(
        self, supervisor_agent_id: str, team: Any, blackboard: Blackboard
    ) -> dict:
        agent_def = self.registry.get_agent(supervisor_agent_id)
        members_info = [
            {"agent_id": m.agent_id, "role": m.role}
            for m in team.members
            if m.agent_id != supervisor_agent_id
        ]
        ...
        response = await self.router.chat_completion_with_fallback(
            messages, task_type="supervisor", profile_name=agent_def.model_profile, plain_chat=True
        )
