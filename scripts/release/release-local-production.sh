...
if [[ "${LOCAL_APPLIANCE_MODE}" == "true" && "${VALIDATION_METADATA_ONLY:-false}" != "true" ]]; then
    ...
    if ! "${SCRIPT_DIR}/../validators/check-secrets.sh" --all; then
    ...
    if ! "${SCRIPT_DIR}/../dev/production-readiness-local.sh"; then
...
if [[ "${SKIP_VALIDATION}" == "false" ]]; then
    ...
    if VALIDATION_VERSION="${VERSION_ARG}" "${SCRIPT_DIR}/../validators/validate-local-production-full.sh"; then
...
# Generate manifest
"${SCRIPT_DIR}/../legacy/generate-release-manifest.sh" \
    --version "${VERSION_ARG}" \
    --artifact-dir "${LATEST_ARTIFACT_DIR:-none}" \
    --validation-result "${VALIDATION_RESULT}"

if [[ "${VALIDATION_RESULT}" == "success" && -x "${SCRIPT_DIR}/../validators/validate-release-metadata.sh" ]]; then
    "${SCRIPT_DIR}/../validators/validate-release-metadata.sh" \
        --version "${VERSION_ARG}" \
        --release-dir "${RELEASE_DIR}"
fi

echo "Redacting sensitive information from release artifacts..."
"${SCRIPT_DIR}/../backup/redact-local-sensitive-artifacts.sh" --path "${RELEASE_DIR}" --in-place

echo "Validating release artifacts security..."
if ! "${SCRIPT_DIR}/../validators/validate-release-artifacts-security.sh" --release-dir "${RELEASE_DIR}"; then
