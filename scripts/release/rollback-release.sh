...
if [[ "${DRY_RUN}" == "true" ]]; then
  ...
  if [[ "${ROLLBACK_BACKUP}" != "none" && -d "${ROLLBACK_BACKUP}" ]]; then
    bash "${SCRIPT_DIR}/../backup/restore-local.sh" --dry-run "${ROLLBACK_BACKUP}"
  else
...
if [[ "${ROLLBACK_BACKUP}" != "none" && -d "${ROLLBACK_BACKUP}" ]]; then
  ...
  if ! bash "${SCRIPT_DIR}/../backup/restore-local.sh" --yes "${ROLLBACK_BACKUP}"; then
...
if [[ -f "${SCRIPT_DIR}/../validators/local-production-smoke.sh" ]]; then
  echo "--- Running Post-rollback Smoke Tests ---"
  if ! bash "${SCRIPT_DIR}/../validators/local-production-smoke.sh"; then
