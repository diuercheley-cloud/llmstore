...
# ---------------------------------------------------------------------------
run_gate "working-tree-certification" bash scripts/dev/working-tree-certification.sh

# ---------------------------------------------------------------------------
# 5. check-secrets
# ---------------------------------------------------------------------------
run_gate "check-secrets" bash scripts/validators/check-secrets.sh --all

# ---------------------------------------------------------------------------
# 6. check-alembic-integrity
# ---------------------------------------------------------------------------
run_gate "check-alembic-integrity" bash scripts/validators/check-alembic-integrity.sh

# ---------------------------------------------------------------------------
# 7. platform-freeze-check
# ---------------------------------------------------------------------------
run_gate "platform-freeze-check" bash scripts/validators/platform-freeze-check.sh

# ---------------------------------------------------------------------------
# 8. complexity-report
# ---------------------------------------------------------------------------
run_gate "complexity-report" bash scripts/validators/complexity-report.sh

# ---------------------------------------------------------------------------
# 9. security
# ---------------------------------------------------------------------------
run_gate "security" bash scripts/validators/security-report-local.sh
...
if VALIDATION_MODE=quick bash scripts/validators/validate-local-production-full.sh; then
...
READINESS_OUTPUT=$(bash scripts/dev/operational-readiness-pack.sh 2>&1)
READINESS_RC=$?
...
# ---------------------------------------------------------------------------
echo ""
echo "━━━ Gate: simulated-surface-audit ━━━"
SIMULATED_COUNT=$(grep -c "| \*\*Yes\*\*" docs/PRODUCT_SURFACE.md || echo "0")
if [ "$SIMULATED_COUNT" -gt 0 ]; then
    echo "[WARNING] Found $SIMULATED_COUNT simulated/mock capabilities in PRODUCT_SURFACE.md"
    grep "| \*\*Yes\*\*" docs/PRODUCT_SURFACE.md | awk -F'|' '{print "  - [SIMULATED] "$2}'
    GATE_RESULTS="${GATE_RESULTS}| simulated-surface-audit | WARN ($SIMULATED_COUNT simulated) |\n"
else
    echo "[PASS] No simulated capabilities found in PRODUCT_SURFACE.md"
    GATE_RESULTS="${GATE_RESULTS}| simulated-surface-audit | PASS |\n"
fi
...
# ---------------------------------------------------------------------------
if [ -f scripts/validators/compliance-check.sh ]; then
    run_gate "compliance-check" bash scripts/validators/compliance-check.sh
fi
