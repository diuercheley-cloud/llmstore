...
def main():
    ...
    required_files = [
        ...
        "scripts/release/generate_release_baseline.py",
        "scripts/validators/validate_release_engineering.py",
    ]
    ...
    prohibited_patterns = ["GITHUB_ACTIONS", "CI_PIPELINE", "DEPLOY_TO_CLOUD"]

    # Simple check in scripts for prohibited patterns
    with open("scripts/release/generate_release_baseline.py") as f:
        content = f.read()
        for p in prohibited_patterns:
            if p in content:
                print(
                    f"PROHIBITED PATTERN FOUND: {p} in scripts/release/generate_release_baseline.py"
                )
                success = False
