...
create_client() {
  local client_name="$1"
  local output
  output="$("${SCRIPT_DIR}/../dev/create-client.sh" "${client_name}")"
  local client_id
  local api_key
...
"${SCRIPT_DIR}/../dev/set-client-plan.sh" "${FREE_CLIENT_ID}" "free" >/dev/null
...
"${SCRIPT_DIR}/../dev/set-client-plan.sh" "${BASIC_CLIENT_ID}" "basic" >/dev/null
