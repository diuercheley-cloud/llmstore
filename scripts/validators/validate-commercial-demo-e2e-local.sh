...
if [ "${HTTP_CODE}" = "200" ]; then
    ...
    log "Cannot continue without a running stack. Start with ./scripts/deploy/up.sh"
...
if [ "${RESET_FIRST}" = true ]; then
    ...
    if [ -x "${ROOT_DIR}/scripts/dev/reset-commercial-demo-pack.sh" ]; then
        RESET_OUTPUT=$(bash "${ROOT_DIR}/scripts/dev/reset-commercial-demo-pack.sh" --yes 2>&1 || true)
...
if [ -x "${ROOT_DIR}/scripts/validators/validate-commercial-demo-pack.sh" ]; then
    if bash "${ROOT_DIR}/scripts/validators/validate-commercial-demo-pack.sh" >> "${LOGS_DIR}/demo-pack.log" 2>&1; then
...
if [ -x "${ROOT_DIR}/scripts/dev/generate-client-proposal.sh" ]; then
    PROP_OUTPUT=$(bash "${ROOT_DIR}/scripts/dev/generate-client-proposal.sh" --company-name "Cliente Demo E2E" --segment "tecnologia" --plan "pro" 2>&1 || true)
...
if [ -x "${ROOT_DIR}/scripts/dev/generate-local-quote.sh" ]; then
    QUOTE_OUTPUT=$(bash "${ROOT_DIR}/scripts/dev/generate-local-quote.sh" --company-name "Cliente Demo E2E" --plan "Pro" --rag --support-hours 4 2>&1 || true)
...
if [ -x "${ROOT_DIR}/scripts/dev/generate-sow-local.sh" ]; then
    SOW_OUTPUT=$(bash "${ROOT_DIR}/scripts/dev/generate-sow-local.sh" --company-name "Cliente Demo E2E" --project-name "Local AI Appliance Demo" 2>&1 || true)
...
if [ -x "${ROOT_DIR}/scripts/validators/generate-client-monthly-report.sh" ]; then
    MONTHLY_OUTPUT=$(bash "${ROOT_DIR}/scripts/validators/generate-client-monthly-report.sh" --email "demo@example.local" --month "2026-05" 2>&1 || true)
...
python3 "${ROOT_DIR}/scripts/legacy/validate_commercial_demo_e2e_helper.py" generate_json \
    "${REPORT_JSON}" \
    "${TIMESTAMP}" \
    "${VERSION}" \
    "${BASE_URL}" \
...
**Tool:** scripts/validators/validate-commercial-demo-e2e-local.sh
**Timestamp:** ${TIMESTAMP}
**Version:** ${VERSION}
**Base URL:** ${BASE_URL}
**Status:** ${STATUS}
...
## Scripts Used
- scripts/validators/validate-commercial-demo-e2e-local.sh
...
- scripts/dev/generate-local-quote.sh
