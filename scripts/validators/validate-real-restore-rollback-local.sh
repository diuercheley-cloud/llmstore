...
if grep -q "ROLLBACK LOCAL" "${ROOT_DIR}/scripts/dev/rollback-local.sh" 2>/dev/null; then
...
if grep -q "git diff-index" "${ROOT_DIR}/scripts/deploy/upgrade-local.sh" 2>/dev/null; then
...
if grep -q "SKIP_BACKUP" "${ROOT_DIR}/scripts/deploy/upgrade-local.sh" 2>/dev/null; then
...
# backup dry-run
BACKUP_DRY_OUTPUT=$(bash "${ROOT_DIR}/scripts/backup/backup-local.sh" --help 2>&1 || true)
log "${DRY_PREFIX}backup-local.sh --help: OK"

# upgrade dry-run
UPGRADE_OUTPUT=$(bash "${ROOT_DIR}/scripts/deploy/upgrade-local.sh" --to-version "${TO_VERSION}" --dry-run --skip-backup --yes --no-build 2>&1 || true)
...
ROLLBACK_OUTPUT=$(bash "${ROOT_DIR}/scripts/dev/rollback-local.sh" --to-version "${FROM_VERSION}" --backup-id "${LOGS_DIR}" --dry-run --yes --no-build 2>&1 || true)
...
if [ "${YES}" = true ] && [ "${DRY_RUN}" = false ]; then
    ...
    BACKUP_OUTPUT=$(bash "${ROOT_DIR}/scripts/backup/backup-local.sh" 2>&1 || true)
    ...
    UPGRADE_REAL_OUTPUT=$(bash "${ROOT_DIR}/scripts/deploy/upgrade-local.sh" --to-version "${TO_VERSION}" --no-build --yes 2>&1 || true)
    ...
    SMOKE_OUTPUT=$(bash "${ROOT_DIR}/scripts/validators/post-upgrade-smoke-local.sh" 2>&1 || true)
    ...
    if [ -n "${BACKUP_DIR}" ]; then
        ROLLBACK_REAL_OUTPUT=$(bash "${ROOT_DIR}/scripts/dev/rollback-local.sh" --to-version "${FROM_VERSION}" --backup-id "${ROOT_DIR}/${BACKUP_DIR}" --no-build --yes 2>&1 || true)
    ...
    SEC_OUTPUT=$(bash "${ROOT_DIR}/scripts/validators/check-secrets.sh" --all 2>&1 || true)
...
python3 "${ROOT_DIR}/scripts/legacy/validate_real_restore_rollback_helper.py" generate_json \
    "${REPORT_JSON}" \
    "${TIMESTAMP}" \
    "${VERSION}" \
    "${DRY_RUN}" \
...
**Tool:** scripts/validators/validate-real-restore-rollback-local.sh
**Timestamp:** ${TIMESTAMP}
**Version:** ${VERSION}
...
| Guarantee | Status |
|-----------|--------|
| Backup required before upgrade | $(grep -q "SKIP_BACKUP" "${ROOT_DIR}/scripts/deploy/upgrade-local.sh" 2>/dev/null && echo 'YES' || echo 'NO') |
| Rollback strong confirmation | $(grep -q "ROLLBACK LOCAL" "${ROOT_DIR}/scripts/dev/rollback-local.sh" 2>/dev/null && echo 'YES' || echo 'NO') |
| Git clean check | $(grep -q "git diff-index" "${ROOT_DIR}/scripts/deploy/upgrade-local.sh" 2>/dev/null && echo 'YES' || echo 'NO') |
...
| Rollback confirmation check | $(grep -q "ROLLBACK LOCAL" "${ROOT_DIR}/scripts/dev/rollback-local.sh" 2>/dev/null && echo 'PASS' || echo 'WARN') |
| Upgrade backup requirement | $(grep -q "SKIP_BACKUP" "${ROOT_DIR}/scripts/deploy/upgrade-local.sh" 2>/dev/null && echo 'PASS' || echo 'WARN') |
...
echo "  Backup required before upgrade: $(grep -q "SKIP_BACKUP" "${ROOT_DIR}/scripts/deploy/upgrade-local.sh" 2>/dev/null && echo 'YES' || echo 'NO')"
echo "  Rollback strong confirmation:  $(grep -q "ROLLBACK LOCAL" "${ROOT_DIR}/scripts/dev/rollback-local.sh" 2>/dev/null && echo 'YES' || echo 'NO')"
echo "  Git clean check:               $(grep -q "git diff-index" "${ROOT_DIR}/scripts/deploy/upgrade-local.sh" 2>/dev/null && echo 'YES' || echo 'NO')"
