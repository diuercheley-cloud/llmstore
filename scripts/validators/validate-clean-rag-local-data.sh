#!/usr/bin/env bash
# scripts/validators/validate-clean-rag-local-data.sh - Validate RAG cleanup script
set -euo pipefail
...
DRY_RUN_OUTPUT=$("${SCRIPT_DIR}/../backup/clean-rag-local-data.sh" --dry-run --client-id "${TEST_CLIENT_ID}")
...
"${SCRIPT_DIR}/../backup/clean-rag-local-data.sh" --yes --client-id "${TEST_CLIENT_ID}"
...
"${SCRIPT_DIR}/../backup/clean-rag-local-data.sh" --yes
