...
if [ -n "$ADMIN_TOKEN" ]; then
  ...
  if [ -z "$API_KEY" ]; then
    ...
    if [ -f "${SCRIPT_DIR}/smoke-client.sh" ]; then
      if ./scripts/validators/smoke-client.sh > /dev/null 2>&1; then log_pass; else log_fail "Chat completion smoke test failed"; fi
    else
...
if ./scripts/validators/check-secrets.sh --summary-only > /dev/null 2>&1; then
  log_pass
else
