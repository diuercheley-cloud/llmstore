...
run_step "check-secrets" ./scripts/validators/check-secrets.sh --all
run_step "docker-compose-ps" docker compose --env-file "${STACK_ENV_PATH}" -f "${ROOT_DIR}/docker-compose.yml" ps
run_step "validate-local-production-full" ./scripts/validators/validate-local-production-full.sh
...
if [[ -f "${ROOT_DIR}/scripts/dev/dr-test-local.sh" ]]; then
  run_step "dr-test-local" ./scripts/dev/dr-test-local.sh
else
