...
if [[ "${VALIDATION_METADATA_ONLY:-false}" != "true" ]]; then
  ...
  if [[ "${VALIDATION_MODE:-}" == "quick" ]]; then
    ...
    PYTEST_RUN_ARGS=("tests/" "tests/control_plane/" "-m" "release" "-n" "auto" "--timeout=300" "-q")
  elif [[ "${VALIDATION_MODE:-}" == "nightly" ]]; then
    # Comprehensive parallel run for nightly checks
    PYTEST_RUN_ARGS=("tests/" "tests/control_plane/" "-n" "auto" "--timeout=600" "-q")
  else
...
with open(summary_md, "w", encoding="utf-8") as handle:
    ...
    handle.write("\n## How to reproduce\n\n")
    handle.write("```bash\n")
    handle.write("./scripts/validators/validate-local-production-full.sh\n")
    handle.write("```\n\n")
...
"${SCRIPT_DIR}/../backup/redact-local-sensitive-artifacts.sh" --path "${OUTPUT_DIR}" --in-place
