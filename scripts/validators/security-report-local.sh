...
if [[ -f "${SCRIPT_DIR}/lib/operator-errors.sh" ]]; then
  source "${SCRIPT_DIR}/../dev/lib/operator-errors.sh"
fi
...
code, out, err = run_cmd("./scripts/validators/check-secrets.sh --all --verbose")
...
# 2. Staged files
code, out, err = run_cmd("./scripts/validators/check-secrets.sh --staged --verbose")
add_secret_scan_results("sec-secrets-staged", "staged files", out, "staged_files")

# 3. Releases (versioned)
_, out_rel_t, _ = run_cmd("git ls-files releases/ | xargs ./scripts/validators/check-secrets.sh --verbose --path")
...
_, out_rel_u, _ = run_cmd("git ls-files -o releases/ | xargs ./scripts/validators/check-secrets.sh --verbose --path")
add_secret_scan_results("sec-secrets-releases-untracked", "untracked releases", out_rel_u, "releases_untracked")

if not SKIP_ARTIFACTS:
    # 5. Artifacts (ignored)
    _, out_art_i, _ = run_cmd("git ls-files -o -i --exclude-standard artifacts/ | xargs ./scripts/validators/check-secrets.sh --verbose --path")
    ...
    _, out_art_r, _ = run_cmd("find artifacts/ -type f -mmin -1440 | xargs ./scripts/validators/check-secrets.sh --verbose --path")
    ...
    if temp_secrets_dir.exists():
        _, out_temp_secrets, _ = run_cmd(
            "find artifacts/test-secrets-temp -type f | xargs ./scripts/validators/check-secrets.sh --verbose --path"
        )
...
./scripts/backup/redact-local-sensitive-artifacts.sh --path "${OUTPUT_DIR}" --in-place
...
add_next_step "Execute ./scripts/backup/redact-local-sensitive-artifacts.sh se houver segredos expostos."
print_next_steps
