...
def main() -> int:
    ...
    metrics = {
        "max_api_router_files": {
            "value": count_files("control_plane/app/api"),
            "category": "routers",
            "suggestion": "Refactor APIs or consolidate controllers",
        },
        "max_service_files": {
            "value": count_files("control_plane/app/services"),
            "category": "models/services",
            "suggestion": "Service modularity is too high; consider consolidation",
        },
        "max_feature_flags": {
            "value": get_feature_flags(),
            "category": "config flags",
            "suggestion": "Remove stale feature flags",
        },
        "max_main_lines": {
            "value": line_count("control_plane/app/main.py"),
            "category": "core",
            "suggestion": "Main application file is too bloated; extract logic",
        },
        "max_settings_lines": {
            "value": line_count("control_plane/app/core/config.py"),
            "category": "core",
            "suggestion": "Config file is too large; use sub-configs",
        },
        "min_collected_tests": {
    ...
    for metric_name, data in metrics.items():
        ...
        if limit is None or (isinstance(limit, int) and (limit <= 0 or limit >= 1_000_000)):
            print(
                f"{metric_name:<30} | {data['value']:<8} | {'SKIP':<8} | {'SKIP':<10} | {data['suggestion']}"
            )
            continue
