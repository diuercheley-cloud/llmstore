...
def main():
    ...
    if not check_content("control_plane/app/services/operations/adapter_registry/policy_engine.py", policy_patterns):
        sys.exit(1)
    ...
    for df in determinism_files:
        with open(df, "r") as f:
            ...
            if "immutable_hash" in content and "utc_now()" in content:
                ...
                if 'immutable_hash=sha256_hex(f"{prefix}_" + utc_now().isoformat())' in content.replace(" ", ""):
    ...
    if not check_content("control_plane/app/api/operations_adapter_registry_admin.py", api_patterns):
        sys.exit(1)
    ...
    if not check_content("control_plane/app/services/operations/adapter_registry/registry_service.py", service_patterns):
        sys.exit(1)
