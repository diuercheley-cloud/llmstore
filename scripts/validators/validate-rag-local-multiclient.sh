...
# shellcheck source=/dev/null
source "${SCRIPT_DIR}/../dev/lib/validation-logging.sh"
init_stack_env
...
SUFFIX=$RANDOM
CLIENT_A_DATA=$(./scripts/dev/create-client.sh "Client-RAG-A-${SUFFIX}" "Client A for RAG validation")
CLIENT_A_ID=$(echo "$CLIENT_A_DATA" | grep client_id | cut -d'=' -f2)
KEY_A=$(echo "$CLIENT_A_DATA" | grep api_key | cut -d'=' -f2)

CLIENT_B_DATA=$(./scripts/dev/create-client.sh "Client-RAG-B-${SUFFIX}" "Client B for RAG validation")
...
CLIENT_C_DATA=$(./scripts/dev/create-client.sh "Client-RAG-C-${SUFFIX}" "Client C for Free Plan limit")
