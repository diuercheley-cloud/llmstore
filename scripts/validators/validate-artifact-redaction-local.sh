...
# scripts/validators/validate-artifact-redaction-local.sh
...
"${SCRIPT_DIR}/../backup/redact-local-sensitive-artifacts.sh" --path "${TEMP_DIR}" --dry-run

echo "Running redaction in-place..."
"${SCRIPT_DIR}/../backup/redact-local-sensitive-artifacts.sh" --path "${TEMP_DIR}" --in-place
