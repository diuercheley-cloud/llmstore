...
./scripts/deploy/upgrade-local.sh --help > /dev/null
./scripts/dev/rollback-local.sh --help > /dev/null
echo "[validate] OK"

# 2. Dry-run upgrade
CURRENT_VERSION="$(tr -d '\n' < "${ROOT_DIR}/VERSION" || echo "v1.0.0")"
echo "[validate] Testando dry-run upgrade para versão atual (${CURRENT_VERSION})..."
if ! ./scripts/deploy/upgrade-local.sh --to-version "${CURRENT_VERSION}" --dry-run; then
...
mkdir -p "${MOCK_BACKUP}"
if ! ./scripts/dev/rollback-local.sh --to-version "${CURRENT_VERSION}" --backup-id "${MOCK_BACKUP}" --dry-run --yes; then
...
if ./scripts/deploy/upgrade-local.sh > /dev/null 2>&1; then
  echo "[validate][error] upgrade-local.sh deveria ter falhado sem --to-version"
  exit 1
fi
if ./scripts/dev/rollback-local.sh > /dev/null 2>&1; then
...
if ! grep -q "git diff-index" scripts/deploy/upgrade-local.sh; then
  echo "[validate][error] upgrade-local.sh falta verificação de working tree clean"
  exit 1
fi
if ! grep -q "git diff-index" scripts/dev/rollback-local.sh; then
  echo "[validate][error] rollback-local.sh falta verificação de working tree clean"
  exit 1
fi
if ! grep -q "ROLLBACK LOCAL" scripts/dev/rollback-local.sh; then
