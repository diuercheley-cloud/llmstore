...
if [ -x "${INSTANCE_DIR}/scripts/deploy/install-local-appliance.sh" ]; then
    ...
    if [ "${DRY_RUN}" = false ] && [ "${YES}" = true ]; then
        ...
        INSTALL_OUTPUT=$(bash "./scripts/deploy/install-local-appliance.sh" "${INSTALL_ARGS[@]}" 2>&1 || true)
        ...
        if [ "${WITH_DEMO}" = true ]; then INSTALL_ARGS+=("--with-demo"); fi
        if [ "${CPU_ONLY}" = true ]; then INSTALL_ARGS+=("--cpu-only"); fi
        log "${DRY_RUN_PREFIX}Would run: ./scripts/deploy/install-local-appliance.sh ${INSTALL_ARGS[*]}"
    fi
    cd "${ROOT_DIR}"
else
    if [ "${DRY_RUN}" = false ] && [ "${YES}" = true ]; then
...
if [ "${YES}" = true ] && [ "${DRY_RUN}" = false ]; then
    cd "${INSTANCE_DIR}"
    VALIDATE_OUTPUT=$(bash "./scripts/validators/validate-post-install-local.sh" 2>&1 || true)
...
if [ "${YES}" = true ] && [ "${DRY_RUN}" = false ]; then
    cd "${INSTANCE_DIR}"
    SEC_OUTPUT=$(bash "./scripts/validators/check-secrets.sh" "--all" 2>&1 || true)
...
if [ "${YES}" = true ] && [ "${DRY_RUN}" = false ]; then
    cd "${INSTANCE_DIR}"
    SEC_REPORT=$(bash "./scripts/validators/security-report-local.sh" 2>&1 || true)
...
if [ "${YES}" = true ] && [ "${DRY_RUN}" = false ]; then
    cd "${INSTANCE_DIR}"
    READY_OUTPUT=$(bash "./scripts/dev/production-readiness-local.sh" 2>&1 || true)
...
cat <<EOF > "${REPORT_JSON}"
{
  "tool": "scripts/validators/validate-clean-install-local.sh",
  "timestamp": "${TIMESTAMP}",
  "version": "${VERSION}",
  "dry_run": ${DRY_RUN},
