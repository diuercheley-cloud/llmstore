...
def main():
    ...
    if status != "REAL_PROVIDER_E2E_SKIPPED":
        # Env check
        run_cmd(["./scripts/validators/validate-real-provider-env-local.sh"])
        ...
        if status != "REAL_PROVIDER_E2E_FAIL":
            if not args.skip_fallback:
                ret, out = run_cmd(["./scripts/validators/validate-real-fallback-local-to-cloud.sh", mode_flag], env=env)
                if ret != 0:
                    status = "REAL_PROVIDER_E2E_FAIL"
                    reason = "Fallback validation failed."
                    
            if not args.skip_billing:
                ret, out = run_cmd(["./scripts/validators/validate-real-billing-margin.sh", mode_flag, "--provider", "auto"], env=env)
                if ret != 0:
                    status = "REAL_PROVIDER_E2E_FAIL"
                    reason = "Billing margin validation failed."

            # Measure costs
            ret, out = run_cmd(["./scripts/dev/measure-real-provider-costs.sh", mode_flag], env=env)

            # Scanner
            ret, out = run_cmd(["./scripts/dev/scan-real-provider-artifacts.sh", "--path", "artifacts/real-provider-validation", "--redact", "--fail-on-findings"])
            if ret != 0:
                status = "REAL_PROVIDER_E2E_FAIL"
                reason = "Artifact sanitization failed."
                
            # Secrets check
            ret, out = run_cmd(["./scripts/validators/check-secrets.sh", "--all"])
            if ret != 0:
                status = "REAL_PROVIDER_E2E_FAIL"
