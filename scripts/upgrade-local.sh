...
if [[ "${DRY_RUN}" == "true" ]]; then
  ...
  if ! ./scripts/post-upgrade-smoke-local.sh; then
    SMOKE_STATUS="failed"
    operator_error "UPGRADE_FAILED" "O upgrade foi concluído, mas os testes de fumaça (smoke tests) falharam!" "Considere rodar ./scripts/rollback-local.sh --to-version ${FROM_VERSION} --backup-id ${BACKUP_PATH}" "post-upgrade-smoke-local.sh falhou."
...
if [[ "${BACKUP_PATH}" == "none" ]]; then
  ...
  ROLLBACK_COMMAND="./scripts/rollback-local.sh --to-version ${FROM_VERSION} --backup-id ${BACKUP_PATH}"
fi
