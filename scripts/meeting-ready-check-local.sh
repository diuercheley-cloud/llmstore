...
for arg in "$@"; do
  if [[ "$arg" == "--help" ]]; then
    cat <<'EOF'
Usage: ./scripts/meeting-ready-check-local.sh [options]
...
if [[ -f "${SCRIPT_DIR}/production-readiness-local.sh" ]]; then
  run_check_warn "production-readiness" "readiness" "Production Readiness Report" \
    "${SCRIPT_DIR}/production-readiness-local.sh --base-url ${BASE_URL} --output-dir ${REPORT_DIR}/readiness"
else
...
if [[ -f "${SCRIPT_DIR}/reset-commercial-demo-pack.sh" ]]; then
  reset_has_yes=$(grep -c "\-\-yes" "${SCRIPT_DIR}/reset-commercial-demo-pack.sh" || true)
...
| Situation | Command |
|-----------|---------|
| Restart stack | \`./scripts/up.sh\` |
| Stop stack | \`./scripts/down.sh\` |
| Check logs | \`make logs\` |
| Health check | \`make health\` |
| Reset demo data | \`./scripts/reset-commercial-demo-pack.sh --yes --include-rag --include-invoices --include-usage\` |
