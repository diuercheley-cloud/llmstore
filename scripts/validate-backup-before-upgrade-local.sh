...
OUT1=$(./scripts/upgrade-local.sh --to-version HEAD --dry-run 2>&1 || true)
...
if ! echo "$OUT1" | grep -q "scripts/backup-local.sh"; then
    echo "Fail: dry-run should reference backup-local.sh"
    exit 1
fi

# --skip-backup sem --yes falha
if ./scripts/upgrade-local.sh --to-version HEAD --skip-backup --dry-run > /dev/null 2>&1; then
    echo "Fail: --skip-backup without --yes should fail"
    exit 1
fi

# --skip-backup --yes registra risco
OUT2=$(./scripts/upgrade-local.sh --to-version HEAD --skip-backup --yes --dry-run 2>&1 || true)
...
OUT3=$(./scripts/rollback-local.sh --to-version HEAD --backup-id mybackup --yes --dry-run 2>&1 || true)
