...
PYTHONPATH=. "$PYTHON" -m py_compile scripts/agent_harness.py

# 2. Lint with ruff (if available)
if [ -f "$RUFF" ]; then
    echo "Running ruff lint..."
    RUFF_CACHE_DIR="/tmp/ruff-cache" "$RUFF" check scripts/llm_harness/ scripts/agent_harness.py
else
...
PYTHONPATH=. "$PYTHON" scripts/fix_imports.py

# 4. Run unit tests
echo "Running unit tests..."
PYTHONPATH=.:scripts "$PYTEST" tests/llm_harness/ -v --tb=short --cov=scripts/llm_harness --cov-report=term --cov-report=xml:coverage.xml --cov-fail-under=75
