...
if ! grep -q "\"models\"" scripts/create-release-bundle.sh; then
  error "models/ not excluded in scripts/create-release-bundle.sh"
fi
if ! grep -q "\"data/rag_uploads\"" scripts/create-release-bundle.sh; then
  error "data/rag_uploads/ not excluded in scripts/create-release-bundle.sh"
fi
log "Release bundle exclusions confirmed"

# 7. Check release guards
log "Checking release guards..."
if ! grep -q "LOCAL_APPLIANCE_MODE" scripts/release-local-production.sh; then
  error "LOCAL_APPLIANCE_MODE guards missing in scripts/release-local-production.sh"
fi
