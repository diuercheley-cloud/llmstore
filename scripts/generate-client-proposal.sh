...
if [[ "$FORMAT" == "pdf" ]]; then
    if [[ -f "${SCRIPT_DIR}/generate-proposal-pdf.sh" ]]; then
        echo "Generating PDF..."
        "${SCRIPT_DIR}/generate-proposal-pdf.sh" "$PROPOSAL_FILE" || echo "Warning: PDF generation failed."
    else
        echo "Skip: scripts/generate-proposal-pdf.sh not found. PDF not generated."
    fi
fi
