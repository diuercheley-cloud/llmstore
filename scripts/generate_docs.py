...
def generate_backup_capabilities():
    try:
        ...
        manifest_default = BackupManifest(
            archive_checksum="dummy", payload_file="dummy", payload_signature="dummy"
        )
        ...
        lines.append("| --- | --- | --- | --- |")
        lines.append(
            f"| {manifest_default.scope} | {', '.join(manifest_default.included)} | {', '.join(manifest_default.excluded)} | {manifest_default.encryption_algorithm} ({manifest_default.encryption_status}) |"
        )
...
def generate_product_surface():
    try:
        ...
        for cap in sorted(capabilities, key=lambda x: x.get("name", "")):
            lines.append(
                f"| {cap.get('name')} | `{cap.get('id')}` | {cap.get('status')} | {cap.get('support_level', '-')} | {cap.get('owner', '-')} |"
            )
