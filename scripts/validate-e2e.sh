...
[[ -f "models/${env_model_file}" ]] || fail "model file not found: models/${env_model_file}" "" "./scripts/download-model.sh"
...
curl_base_url "${BASE_URL}/v1/chat/completions" -fsS \
  ...
  || fail "chat completion failed" "control-plane" "./scripts/test-chat.sh"
...
curl_base_url "${BASE_URL}/v1/chat/completions/async" -fsS \
  ...
  || fail "async chat enqueue failed" "control-plane" "./scripts/async-chat.sh"
...
for _ in $(seq 1 60); do
  curl_base_url "${BASE_URL}/v1/jobs/${ASYNC_JOB_ID}" -fsS \
    ...
    || fail "async job status failed" "control-plane" "./scripts/job-status.sh ${ASYNC_JOB_ID}"
...
curl_base_url "${BASE_URL}/v1/chat/completions" -N -fsS \
  ...
  || fail "streaming chat failed" "control-plane" "./scripts/test-stream.sh"
...
curl_base_url "${BASE_URL}/admin/billing/invoices/generate" -fsS \
  ...
  || fail "invoice generation failed" "control-plane" "./scripts/generate-invoices.sh"
...
curl_base_url "${BASE_URL}/admin/billing/run-cycle" -fsS \
  ...
  || fail "billing cycle failed" "control-plane" "./scripts/run-billing-cycle.sh"
...
curl_base_url "${BASE_URL}/admin/security/events" -fsS \
  ...
  || fail "security events endpoint failed" "control-plane" "./scripts/security-events.sh"
...
curl_base_url "${BASE_URL}/admin/export/usage?format=json" -fsS \
  ...
  || fail "usage export json failed" "control-plane" "./scripts/export-usage.sh"
...
curl_base_url "${BASE_URL}/admin/reports/monthly" -fsS \
  ...
  || fail "monthly report endpoint failed" "control-plane" "./scripts/monthly-report.sh"
...
curl_base_url "${BASE_URL}/admin/security/clients/${DEMO_CLIENT_ID}/suspend" -fsS -X POST \
  ...
  || fail "suspend client endpoint failed" "control-plane" "./scripts/suspend-client.sh ${DEMO_CLIENT_ID}"
...
curl_base_url "${BASE_URL}/admin/security/clients/${DEMO_CLIENT_ID}/unsuspend" -fsS -X POST \
  ...
  || fail "unsuspend client endpoint failed" "control-plane" "./scripts/unsuspend-client.sh ${DEMO_CLIENT_ID}"
curl_base_url "${BASE_URL}/v1/models" -fsS \
  -H "Authorization: Bearer ${API_KEY}" \
