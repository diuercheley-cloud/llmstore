...
WIZARD="${SCRIPT_DIR}/configure-local-wizard.sh"
...
# So I'll put it in ${TEMP_DIR}/scripts/configure-local-wizard.sh

mkdir -p "${TEMP_DIR}/scripts"
cp "$WIZARD" "${TEMP_DIR}/scripts/configure-local-wizard.sh"
cd "$TEMP_DIR"

log "Running wizard in $TEMP_DIR..."
./scripts/configure-local-wizard.sh --non-interactive --yes --base-url "http://test.local" --gpu --enable-demo
...
./scripts/configure-local-wizard.sh --non-interactive --yes --base-url "http://test2.local"
