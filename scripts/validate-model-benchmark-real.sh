...
if [[ -f "${ROOT_DIR}/.venv/bin/python" ]]; then
    "${ROOT_DIR}/.venv/bin/python" "${ROOT_DIR}/scripts/benchmark_model_local_runner.py" --help | grep -q "stress"
else
    python3 "${ROOT_DIR}/scripts/benchmark_model_local_runner.py" --help | grep -q "stress" || echo "Aviso: Nao foi possivel verificar flags sem venv"
fi
