...
def get_recommendation(tps, latency_p95, error_rate):
    ...
    if tps >= 30 and latency_p95 < 2000:
        return (
            "safe_for_free",
            "High throughput and low latency, suitable for high volume/free tier.",
        )
    ...
    if tps >= 5 or latency_p95 < 15000:
        return (
            "safe_for_premium",
            "Low throughput or higher latency, recommended for premium/dedicated use.",
        )
...
async def run_benchmark(args):
    ...
    async with httpx.AsyncClient() as client:
        for r in range(args.runs):
            ...
            for _ in range(args.concurrency):
                tasks.append(
                    run_single_request(
                        client, url, headers, payload, args.streaming == "true", timeout
                    )
                )
    ...
    p95_ttft = (
        statistics.quantiles(ttfts, n=20)[18] if len(ttfts) >= 2 else (ttfts[0] if ttfts else 0)
    )
    ...
    avg_queue_wait = (
        statistics.mean([r["queue_wait_ms"] for r in successful_runs]) if successful_runs else 0
    )
    cache_hit_rate = (
        len([r for r in successful_runs if r["cache_hit"]]) / len(successful_runs)
        if successful_runs
        else 0
    )
    ...
    for i, r in enumerate(all_responses):
        tps = (
            (r["completion_tokens"] / (r["total_time_ms"] / 1000))
            if r["success"] and r["total_time_ms"] > 0
            else 0
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--runs", type=int, default=3, help="Number of runs (or batches of concurrency)"
    )
    parser.add_argument(
        "--streaming", choices=["true", "false"], default="false", help="Use streaming"
    )
    ...
    parser.add_argument(
        "--output-dir", default="artifacts/model-benchmarks", help="Output directory"
    )
    ...
    parser.add_argument(
        "--stress", action="store_true", help="Stress mode (Heavy load, high concurrency)"
    )
