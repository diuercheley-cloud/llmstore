#!/usr/bin/env bash
# scripts/dev/test-operations-dryrun.sh
...
if ! bash "${SCRIPT_DIR}/../backup/backup.sh" --dry-run; then
...
if [[ -d "${TEST_BACKUP}" ]]; then
  ...
  if ! bash "${SCRIPT_DIR}/../backup/restore-local.sh" --dry-run "${TEST_BACKUP}"; then
...
if ! bash "${SCRIPT_DIR}/../deploy/upgrade-release.sh" --dry-run --force; then
...
if ! bash "${SCRIPT_DIR}/../release/rollback-release.sh" --dry-run; then
