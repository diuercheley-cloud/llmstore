...
2. Run \`make install-local\` or \`./scripts/deploy/install-local-appliance.sh\`
...
add_next_step "Execute \`make install-local\` ou \`./scripts/deploy/install-local-appliance.sh\`"
...
if ask_confirm "Would you like to create a demo client now?" "n"; then
  ...
  echo "Command: ./scripts/dev/create-customer-demo.sh"
fi
