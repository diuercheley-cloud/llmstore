...
def generate():
    manifest_path = os.path.join(base_dir, "scripts/dev/manifest.yaml")
    records = []
    ...
    for root, dirs, files in os.walk(scripts_dir):
        ...
        for file in files:
            ...
            readme_scripts = [
                ...
                "scripts/legacy/generate-enterprise-pack.sh",
                "scripts/legacy/generate-customer-readiness-report.sh",
                "scripts/legacy/generate-acceptance-report.sh",
            ]
            ...
            try:
                ...
                if any(
                    kw in content_lower
                    for kw in ["> ", ">> ", "tee ", "mkdir ", "write_to_file", "open("]
                ):
                    writes_files = True
                ...
                if (
                    any(
                        kw in name_lower
                        for kw in ["reset-demo", "purge-data", "drop-db", "destroy-platform"]
                    )
                    or category not in ["validation", "testing"]
                    and any(
                        kw in content_lower
                        for kw in [
                            "drop database",
    ...
    print(f"Generated {len(records)} entries in scripts/dev/manifest.yaml")
