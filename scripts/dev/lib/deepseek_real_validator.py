#!/usr/bin/env python3
"""DeepSeek Real Provider Validator.

Validates DeepSeek provider connectivity, chat completion, billing BRL mapping,
and sanitization — all without leaking secrets.

Usage:
  python scripts/dev/lib/deepseek_real_validator.py --dry-run
  python scripts/dev/lib/deepseek_real_validator.py --real --model deepseek-chat
"""
...
def estimate_deepseek_cost_usd(model: str, prompt_tokens: int, completion_tokens: int) -> float:
    pricing = {
...
class DeepSeekRealValidator:
    ...
    def run(self) -> dict[str, Any]:
        ...
        if not api_key:
            self._check("env.api_key", "skip", "DEEPSEEK_API_KEY not set")
        ...
        if self.dry_run:
            ...
            self.report["status"] = "DEEPSEEK_REAL_PASS"
            return self.report
        ...
        self._check_chat(base_url, api_key)
        ...
        if self.report["summary"].get("fail", 0) > 0:
            self.report["status"] = "DEEPSEEK_REAL_FAIL"
    ...
    def _check_chat(self, base_url: str, api_key: str):
        ...
        try:
            ...
            if resp.is_success:
                ...
                cost_usd = estimate_deepseek_cost_usd(self.model, prompt_tokens, completion_tokens)
    ...
    def _check_billing(self):
        ...
        if not billing:
            self._check("billing", "skip", "no billing data — chat check did not complete")
            return
    ...
    def _check_sanitization(self):
        ...
        for pattern in ("DEEPSEEK_API_KEY=", "sk-"):
    ...
    def write_report(self):
        ...
        report_path = timestamp_dir / "deepseek-real-report.json"
        with open(report_path, "w") as f:
            json.dump(self.report, f, indent=2, default=str)
        self.report["artifacts"].append(str(report_path))
        md_path = timestamp_dir / "deepseek-real-report.md"
    ...
    def _format_md(self) -> str:
        ...
        for a in self.report["artifacts"]:
            lines.append(f"- {a}")
        lines += ["", "---", "", "*Report generated by deepseek_real_validator.py*"]
        return "\n".join(lines) + "\n"


def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Validate DeepSeek real provider")
...
def main(argv: list[str] | None = None) -> int:
    ...
    print(f"\nDeepSeek Real Provider Validation: {report['status']}")
    ...
    if report["status"] == "DEEPSEEK_REAL_FAIL":
        return 1
    return 0
