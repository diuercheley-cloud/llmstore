...
while IFS= read -r -d '' sh; do
  ...
  # Pattern: ROOT_DIR/lib/ (should be ROOT_DIR/scripts/dev/lib/ after migration)
  if grep -qP 'source\s+.*ROOT_DIR\}/lib/' "$sh" 2>/dev/null; then
    maybe_fix "$sh" '${ROOT_DIR}/lib/' '${ROOT_DIR}/scripts/dev/lib/' "lib migration to scripts/dev/lib/"
  fi

  # Pattern: ../dev/lib/ relative (should be lib/ since SCRIPT_DIR is scripts/)
  if grep -qP 'source\s+.*SCRIPT_DIR\}/\.\./lib/' "$sh" 2>/dev/null; then
    maybe_fix "$sh" '${SCRIPT_DIR}/../dev/lib/' '${SCRIPT_DIR}/lib/' "fragile ../dev/lib/ -> lib/"
  fi
