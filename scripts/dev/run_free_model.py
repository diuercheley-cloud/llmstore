...
def get_admin_token():
    ...
    cmd = ["docker", "exec", "-i", "llm-inference-stack-control-plane-1", "env"]
...
def get_admin_token_cached():
    ...
    if not token:
        print(
            "Admin Token not found in environment or .env.local. Attempting docker environment discovery..."
        )
        ...
        if token:
            ...
            print(
                "Warning: Could not dynamically discover ADMIN_TOKEN from control-plane container."
            )
...
def reload_gateway_cache(admin_token):
    ...
    req = urllib.request.Request(url, headers={"X-Admin-Token": admin_token}, method="POST")
    try:
        with urllib.request.urlopen(req, timeout=60) as response:
            res_data = json.loads(response.read().decode())
            print(
                f"Gateway cache reload status: {res_data.get('status', 'unknown')} - {res_data.get('detail', '')}"
            )
            return True
    except Exception as e:
...
def run_sql(query):
    ...
    cmd = [
        ...
        "exec",
        "-i",
        "llm-inference-stack-postgres-1",
        "psql",
        "-U",
        "llm_gateway",
        "-d",
...
def fetch_nvidia_models(api_key):
    ...
    req = urllib.request.Request(
        "https://integrate.api.nvidia.com/v1/models", headers={"Authorization": f"Bearer {api_key}"}
    )
    try:
        with urllib.request.urlopen(req, timeout=60) as response:
            data = json.loads(response.read().decode())
...
def register_model_if_needed(
    model_id, provider_choice, key, admin_token, custom_name=None, custom_url=None
):
    ...
    if provider_choice == "nvidia":
        ...
        backend_id, backend_slug = register_custom_backend_if_needed(
            custom_name, custom_url, key, admin_token
        )
        db_provider = "openai_compatible"
    else:
        ...
        if not backend_rows:
            print(
                "Error: Active OpenRouter backend not found in inference_backends database.",
                file=sys.stderr,
            )
            sys.exit(1)
    ...
    check_query = (
        f"SELECT id, inference_backend_id FROM model_registry WHERE model_id = '{model_id}';"
    )
    rows = run_sql(check_query)
    ...
    if rows:
        ...
        if not existing_backend_id or existing_backend_id != backend_id:
            print(
                f"Updating inference_backend_id in model_registry to active backend ({backend_id})..."
            )
            update_query = f"""
...
def main():
    ...
    if provider_choice == "openrouter":
        ...
        custom_name = input(
            f"Enter custom provider name (slug, e.g. deepseek) [default: {cached_name}]: "
        ).strip()
        if not custom_name:
        ...
        if cached_key:
            masked_key = (
                f"{cached_key[:10]}...{cached_key[-5:]}" if len(cached_key) > 15 else cached_key
            )
