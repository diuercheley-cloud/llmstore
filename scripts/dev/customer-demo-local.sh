...
# Uso: ./scripts/dev/customer-demo-local.sh [options]
...
show_help() {
    cat <<'EOF'
Uso: ./scripts/dev/customer-demo-local.sh [opcoes]
...
elif [ -x "${ROOT_DIR}/scripts/validators/security-report-local.sh" ]; then
    log "Gerando security report..."
    if bash "${ROOT_DIR}/scripts/validators/security-report-local.sh" >> "${LOGS_DIR}/security-report.log" 2>&1; then
...
if [ -x "${ROOT_DIR}/scripts/dev/production-readiness-local.sh" ]; then
    if bash "${ROOT_DIR}/scripts/dev/production-readiness-local.sh" >> "${LOGS_DIR}/production-readiness.log" 2>&1; then
...
if [ "${RESET_DEMO}" = true ] || [ "${FULL}" = true ]; then
    ...
    if [ -x "${ROOT_DIR}/scripts/dev/reset-commercial-demo-pack.sh" ]; then
        if [ "${YES}" = true ]; then
            if bash "${ROOT_DIR}/scripts/dev/reset-commercial-demo-pack.sh" --yes >> "${LOGS_DIR}/reset.log" 2>&1; then
            ...
            bash "${ROOT_DIR}/scripts/dev/reset-commercial-demo-pack.sh" --dry-run >> "${LOGS_DIR}/reset.log" 2>&1 || true
...
if [ "${FULL}" = true ]; then
    ...
    if [ -x "${ROOT_DIR}/scripts/dev/seed-commercial-demo-pack.sh" ]; then
        SEED_OUTPUT=$(bash "${ROOT_DIR}/scripts/dev/seed-commercial-demo-pack.sh" 2>&1 || true)
...
if [ -x "${ROOT_DIR}/scripts/validators/validate-fake-demo-data.sh" ]; then
    if bash "${ROOT_DIR}/scripts/validators/validate-fake-demo-data.sh" >> "${LOGS_DIR}/fake-data.log" 2>&1; then
...
if [ "${QUICK}" = false ]; then
    ...
    if [ -x "${ROOT_DIR}/scripts/dev/generate-client-proposal.sh" ]; then
        if bash "${ROOT_DIR}/scripts/dev/generate-client-proposal.sh" --company-name "Cliente Demo" --segment "tecnologia" --plan "pro" >> "${LOGS_DIR}/proposal.log" 2>&1; then
    ...
    if [ -x "${ROOT_DIR}/scripts/dev/generate-local-quote.sh" ]; then
        if bash "${ROOT_DIR}/scripts/dev/generate-local-quote.sh" --company-name "Cliente Demo" --plan "Pro" --rag --support-hours 4 >> "${LOGS_DIR}/quote.log" 2>&1; then
    ...
    if [ -x "${ROOT_DIR}/scripts/dev/generate-sow-local.sh" ]; then
        if bash "${ROOT_DIR}/scripts/dev/generate-sow-local.sh" --company-name "Cliente Demo" --project-name "Local AI Appliance" >> "${LOGS_DIR}/sow.log" 2>&1; then
    ...
    if [ -x "${ROOT_DIR}/scripts/validators/generate-client-monthly-report.sh" ]; then
        if bash "${ROOT_DIR}/scripts/validators/generate-client-monthly-report.sh" --email "demo@example.local" --month "$(date +%Y-%m)" >> "${LOGS_DIR}/monthly.log" 2>&1; then
...
if [ -x "${ROOT_DIR}/scripts/validators/meeting-ready-check-local.sh" ]; then
    MEETING_OUTPUT=$(bash "${ROOT_DIR}/scripts/validators/meeting-ready-check-local.sh" "${MEETING_ARGS[@]}" 2>&1 || true)
...
if [ "${SKIP_SCREENSHOTS}" = false ]; then
    ...
    if [ -x "${ROOT_DIR}/scripts/dev/prepare-demo-screenshots-local.sh" ]; then
        if bash "${ROOT_DIR}/scripts/dev/prepare-demo-screenshots-local.sh" --placeholders-only >> "${LOGS_DIR}/screenshots.log" 2>&1; then
...
# Generate MD report
{
    ...
    echo ""
    echo "---"
    echo "Gerado por \`scripts/dev/customer-demo-local.sh\`"
} > "${REPORT_MD}"
