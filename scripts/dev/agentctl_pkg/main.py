...
def validate_workflow(path: str, use_json: bool = False):
    ...
    errors = []
    if "name" not in workflow: errors.append("Missing 'name'")
    if "steps" not in workflow: errors.append("Missing 'steps'")
...
def run_workflow(path: str, watch: bool = False, debug: bool = False, trace: bool = False, use_json: bool = False):
    workflow = load_yaml(path)
    ...
    try:
        with get_client() as client:
            ...
            if resp.status_code != 200:
                print(f"{CLIColor.FAIL}Failed to start workflow: {resp.text}{CLIColor.ENDC}", file=sys.stderr)
                sys.exit(1)
        ...
        print(f"{CLIColor.FAIL}Error: Control Plane API at {API_URL} is unavailable.{CLIColor.ENDC}", file=sys.stderr)
        sys.exit(1)
...
def replay_run(run_id: str, dry_run: bool = False, use_json: bool = False):
    ...
    try:
        with get_client() as client:
            resp = client.post(f"/api/v1/admin/workflows/runs/{run_id}/replay", params={"dry_run": dry_run})
...
def show_costs(agent_id: Optional[str] = None, use_json: bool = False):
    try:
        with get_client() as client:
...
def main():
    parser = argparse.ArgumentParser(prog="agentctl", description="CLI for managing agents and workflows")
