...
def get_backend_endpoints():
    ...
    for py_file in BACKEND_DIR.glob("*.py"):
        ...
        route_matches = re.finditer(
            r'@router\.(get|post|put|patch|delete)\s*\(\s*[f]?["\'](.*?)["\']', content
        )
...
def get_api_client_coverage():
    ...
    for api_file in [FRONTEND_ADMIN_DIR / "lib/api.ts", FRONTEND_CLIENT_DIR / "lib/api.ts"]:
        if api_file.exists():
            ...
            helper_matches = re.finditer(
                r'this\.client\.(get|post|put|patch|delete)<.*?>\([`"\'](.*?)[`"\']', content
            )
...
def main():
    ...
    for ep in sorted(backend, key=lambda x: x["path"]):
        ...
        exception = next(
            (ex for ex in exceptions_list if normalize_path(ex["path"]) == ep["normalized"]), None
        )
        ...
        if exception:
            ...
            for i in range(2, len(parts)):
                ...
                parent_ex = next(
                    (ex for ex in exceptions_list if normalize_path(ex["path"]) == parent_path),
                    None,
                )
                if parent_ex:
