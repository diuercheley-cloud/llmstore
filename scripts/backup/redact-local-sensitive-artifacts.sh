...
source "${ROOT_DIR}/scripts/dev/lib/redaction.sh"
...
process_file() {
    ...
    if [[ "${extension}" == "json" ]]; then
        if [[ "${DRY_RUN}" == "true" ]]; then
            local diff_count
            diff_count=$("${ROOT_DIR}/scripts/backup/redact_json.py" "$file" | diff "$file" - | grep -c "^[<>]" || true)
            ...
            "${ROOT_DIR}/scripts/backup/redact_json.py" --inplace --file "$file"
            local new_md5
            ...
            "${ROOT_DIR}/scripts/backup/redact_json.py" "$file"
        fi
    else
        if [[ "${DRY_RUN}" == "true" ]]; then
            local diff_count
