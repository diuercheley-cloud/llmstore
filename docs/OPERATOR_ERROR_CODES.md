...
| **MODEL_NOT_FOUND** | O modelo de linguagem solicitado não foi encontrado localmente. | Execute o script `scripts/download-model.sh` para baixar o modelo. |
...
| **READY_DEGRADED** | O sistema está rodando, mas alguns componentes secundários falharam. | Verifique o status detalhado com `scripts/job-status.sh`. |
...
| **SECRET_DETECTED** | Um segredo (API key, senha) foi detectado em um local inseguro (ex: logs). | Execute o script de limpeza de segredos: `scripts/redact-local-sensitive-artifacts.sh`. |
