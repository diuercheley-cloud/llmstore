...
- **Escopo**: `scripts/llm_harness/`, `tests/llm_harness/`, `scripts/validate-llm-harness.sh`
...
A superfície atual de validação do harness coleta **164 testes** em `tests/llm_harness/`, superando o marco solicitado de **142+**.
...
O fluxo de validação do harness foi formalizado em `./scripts/validate-llm-harness.sh`.
...
- `ruff`
- `mypy`
- correção de imports via `scripts/fix_imports.py`
...
- Validação operacional: `./scripts/validate-llm-harness.sh`
- Checagem de compliance do core: `scripts/check-llm-harness-production-core.py`
