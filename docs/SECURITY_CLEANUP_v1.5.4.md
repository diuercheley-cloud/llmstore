...
## Items Fixed
- Aligned `scripts/validate-local-permissions.sh` with the key fixture policy so authorized fake `.pem` and `.key` fixtures no longer produce a false failure.
- Corrected `scripts/security-report-local.sh` to validate admin protection through `/admin/clients`, which exists and returns `401` without a token.
...
- Added `scripts/validate-security-cleanup-v1.5.4.sh` to run the full cleanup validation chain and fail on `FAIL` score or any `critical_failures`.
...
bash scripts/redact-local-sensitive-artifacts.sh
chmod +x scripts/redact-local-sensitive-artifacts.sh
chmod +x scripts/validate-security-cleanup-v1.5.4.sh
