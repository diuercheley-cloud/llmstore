...
async def run():
    _PROBE_CACHE.clear()
    agent = create_code_agent("local-openai-compatible", {"agent_id": "test", "base_url": "http://fake/v1", "model": "model-1"})
    ...
    mock_probe_response.json.return_value = {
        "choices": [{"message": {"tool_calls": [{"id": "call_1", "type": "function", "function": {"name": "final", "arguments": '{"message":"native_probe_ok"}'}}]}}]
    }
    
    async def fake_request(method, url, **kwargs):
        if method == "GET": return mock_models_response
